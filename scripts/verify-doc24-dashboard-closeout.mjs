...
const HORIZONTAL_SCROLL_ALLOWLIST = new Set([
  ...
  path.join(SCHOOL_DIR, "teaching-operations", "TeacherMicrocourseLibrary.tsx"), // 教师微课覆盖矩阵（宽表格）
...
// ---------------------------------------------------------------------------
// 3b：Dashboard 画布内的多栏判据不得回退成视口断点（docs/plan/27 §3 D4）
// ---------------------------------------------------------------------------

/**
 * 判据必须是"这块工作区有多宽"，不是"浏览器窗口有多宽"。固定侧栏与 gutter 先扣走
 * 240–304px，`xl:`（1280 视口）生效那一刻正文只有 976px：曾经出现过员工首页四列
 * 每列 226px、备课页 4:3 舞台 180×135px、机构设置三列最小和 732px 撑破 720px 正文。
 *
 * 只拦真正会挤爆的档位，不搞一刀切：
 *   - `xl:` / `2xl:` 的多栏：这两档在画布内永远判错，正文比视口至少窄 304px。
 *   - `md:` / `lg:` 的三列及以上、以及自定义轨道（`grid-cols-[...]`）。
 *   - `min-[NNNpx]:` 的任何多栏：这是写死的视口像素，比命名断点更难发现。
 * `sm:grid-cols-*` 与 `md/lg:grid-cols-2` 放行——两列表单在任何档位都放得下。
 *
 * 容器查询前缀（`@4xl/page:`、`@2xl:`）不在拦截范围内，它们正是这条规则要的写法；
 * 用不带名字的 `@Nxl:` 时记得祖先上有 `@container`，否则条件永远不成立。
 */
const VIEWPORT_MULTI_COLUMN = [
  /(?<!@)\b(?:xl|2xl):(?:grid-cols-(?!1\b)|columns-)/,
  /(?<!@)\b(?:md|lg):grid-cols-(?:[3-9]|1[0-2]|\[)/,
  /\bmin-\[\d+px\]:(?:grid-cols-(?!1\b)|columns-|flex-row)/,
];

eachLine((file, lineNo, line) => {
  for (const pattern of VIEWPORT_MULTI_COLUMN) {
    const guilty = line.match(pattern);
    if (!guilty) continue;
    fail(
      `${relative(file)}:${lineNo} 用视口断点决定多栏（${guilty[0]}）；` +
        `Dashboard 画布内的判据是容器宽度，改用 @Nxl/page: 或 @container + @Nxl:（doc 27 §3 D4）`,
    );
    return;
  }
});
