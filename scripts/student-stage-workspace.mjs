...
const root = path.resolve(assignment ? '.tmp/student-stage-assignment' : '.tmp/student-stage-workspace');
...
const migrations = (assignment ? ['20260908003000_student_stage_assignment'] : ['20260908000100_student_stage_workspace', '20260908000110_student_stage_workspace_index']).map(version => {
...
const tables = ['students', 'leads', 'lead_communications', 'lead_invitation_threads', 'activity_registrations',
  'assessment_results', 'course_enrollments', 'enrollments', 'student_follow_ups', ...(assignment ? ['lead_next_actions','domain_events'] : [])];
...
const signature = assignment ? 'public.assign_student_stage_subjects(jsonb,uuid)' : 'public.list_student_stage_workspace(text,text,text,integer,integer,text)';
...
sql(`begin;set local lock_timeout='5s';set local statement_timeout='90s';
  ...
  ${fs.readFileSync(assignment ? 'scripts/sql/student-stage-assignment-assertions.sql' : 'scripts/sql/student-stage-workspace-assertions.sql', 'utf8')}
