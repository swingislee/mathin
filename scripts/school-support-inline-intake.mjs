...
const root = path.resolve('.tmp/school-support-inline-intake');
...
const version = '20260909000100_school_support_inline_intake';
const file = `supabase/migrations/${version}.sql`, checksum = textFileSha256(file);
const assertionsFile = 'scripts/sql/school-support-inline-intake-assertions.sql';
...
const tables = ['students','leads','lead_communications','lead_next_actions','student_follow_ups','lead_invitation_threads',
  ...
  'history_import_records','profiles','enrollments','classroom_members','classrooms','orders','account_ledger'];
const snapshot = () => sql(`begin read only;select jsonb_build_object(${tables.map(table =>
  `'${table}',(select jsonb_build_object('count',count(*),'hash',md5(coalesce(string_agg(md5((to_jsonb(t)-'manual_entry'-'manual_identity_pending')::text),'' order by (to_jsonb(t)-'manual_entry'-'manual_identity_pending')::text),''))) from public.${table} t)`).join(',')});commit;`);
const definitions = () => sql(`begin read only;select jsonb_agg(jsonb_build_object('signature',p.oid::regprocedure::text,'definition',pg_get_functiondef(p.oid),'acl',p.proacl) order by p.proname)
  from pg_proc p where p.oid in ('public.add_school_support_work_item(uuid,jsonb)'::regprocedure);commit;`);
...
const statement = `begin;set local lock_timeout='5s';set local statement_timeout='90s';
  select pg_advisory_xact_lock(hashtextextended('school-support-inline-intake',0));
