...
const root = path.resolve('.tmp/school-support-manual-entry');
...
const version = '20260908160000_school_support_manual_entry';
const file = `supabase/migrations/${version}.sql`, checksum = textFileSha256(file);
const assertionsFile = 'scripts/sql/school-support-manual-entry-assertions.sql';
...
const tables = ['students','leads','lead_communications','lead_next_actions','student_follow_ups','lead_invitation_threads',
  ...
  'history_import_records','profiles'];
...
const definitions = () => sql(`begin read only;select jsonb_agg(jsonb_build_object('signature',p.oid::regprocedure::text,'definition',pg_get_functiondef(p.oid),'acl',p.proacl) order by p.proname)
  from pg_proc p where p.oid in ('public.ensure_lead_student_profile(uuid)'::regprocedure,'public.can_access_communication_row(text,boolean)'::regprocedure);commit;`);
...
const statement = `begin;set local lock_timeout='5s';set local statement_timeout='90s';
  select pg_advisory_xact_lock(hashtextextended('school-support-manual-entry',0));
