...
const increments = {
  ...
  'spatial-batch': {
    versions: ['20260921130000_solid_curved_nets', '20260921140000_capacity_displacement', '20260921150000_solid_geometry_exploration', '20260921160000_solid_revolution', '20260921170000_spatial_teaching_batch'],
    tests: ['prepared_tool_scenes_assertions', 'separate_net_tools_assertions', 'solid_net_pyramid_assertions', 'solid_curved_net_assertions', 'capacity_displacement_assertions', 'solid_geometry_exploration_assertions', 'solid_revolution_assertions', 'spatial_teaching_batch_assertions'],
  },
};
if (!Object.hasOwn(increments, increment)) throw new Error('Unknown Tools increment');
const { version, versions = [version], tests } = increments[increment];
...
const migrations = versions.map((version) => {
  ...
  if (recorded && recorded !== checksum) throw new Error(`TOOL_SCENES_MIGRATION_CHECKSUM_MISMATCH:${version}`);
  return { version, checksum, recorded, source: fs.readFileSync(file, 'utf8').replace(/^(?:begin|commit);\s*$/gm, '') };
});
const firstPending = migrations.findIndex(item => !item.recorded);
if (firstPending >= 0 && migrations.slice(firstPending).some(item => item.recorded)) throw new Error('TOOL_SCENES_MIGRATION_ORDER_MISMATCH');
const pending = migrations.filter(item => !item.recorded);
const checksum = migrations.length === 1 ? migrations[0].checksum : migrations.map(item => `${item.version}:${item.checksum}`).join(':');
const assertionsDigest = tests.map(test => textFileSha256(`supabase/tests/${test}.sql`)).join(':');
const migration = pending.map(item => item.source).join('\n');
const fingerprint = () => sql("begin read only; select md5(string_agg(pg_get_functiondef(p.oid), E'\\n' order by p.proname)) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and (p.proname in ('cw_courseware_composition_doc_is_valid','cw_spatial_teaching_tool_is_valid','cw_formal_cube_page_is_valid','cw_manual_composition_doc_is_valid','cube_drafts_account_ready','save_tool_scene_draft') or p.proname like 'tool_%'); select pg_get_constraintdef(oid) from pg_constraint where conrelid=to_regclass('public.tool_scene_drafts') order by conname; commit;");
...
if (mode === '--check') {
  ...
  if (!pending.length) throw new Error('TOOL_SCENES_MIGRATION_ALREADY_APPLIED');
