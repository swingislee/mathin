...
const output = path.resolve('.tmp/prepared-tool-scenes');
...
const version = '20260918000100_prepared_tool_scenes';
...
if (mode === '--check') {
  ...
  const assertions = fs.readFileSync('supabase/tests/prepared_tool_scenes_assertions.sql', 'utf8').replace(/^(?:begin|rollback);\s*$/gm, '');
  ...
  if (check.checksum !== checksum || check.host !== observed.host || check.rollback !== 'PASS' || Date.now() - Date.parse(check.checkedAt) > 3_600_000) throw new Error('TOOL_SCENES_FRESH_CHECK_REQUIRED');
