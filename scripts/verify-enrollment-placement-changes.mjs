...
const root=path.resolve('.tmp/enrollment-placement-startup');fs.mkdirSync(root,{recursive:true});
...
for(const role of ['admin','student']){
  ...
  try{
    ...
    if(transfers.error||!Array.isArray(transfers.data))throw new Error('SESSION_TRANSFERS_RPC_FAILED');
    ...
    for(const member of board.members.filter(m=>m.status==='active')){
      ...
      const target=origin&&board.options.classrooms.find(c=>c.id!==origin.id&&c.courseId===origin.courseId&&c.termId===origin.termId&&(c.capacity===null||c.capacity>0));
      if(!target)continue;
