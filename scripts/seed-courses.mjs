...
// 版本层出现后，同一 product_code 会有多条 plan（如秋季 2025 与 2026）。这份 seed 回放
// 在版本层建立之前，courses.product_code 仍是全局唯一，一个编码只能落一行——按 sortOrder
// 取最新版本，让回放结果等于目录当前在售的那一版，而不是数组顺序决定的任意一版。
function currentVersionPlans() {
  const byProductCode = new Map();
  for (const plan of plans) {
    const previous = byProductCode.get(plan.productCode);
    if (previous && catalogVersions.get(previous.catalogVersion).sortOrder >= catalogVersions.get(plan.catalogVersion).sortOrder) continue;
    byProductCode.set(plan.productCode, plan);
  }
  return plans.filter((plan) => byProductCode.get(plan.productCode) === plan);
}
...
function createPreFamilySeed() {
  ...
  for (const plan of currentVersionPlans()) {
    lines.push(
