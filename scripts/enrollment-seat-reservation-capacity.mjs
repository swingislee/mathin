...
const root = path.resolve('.tmp/enrollment-seat-reservation-capacity');
...
const version = '20260909000600_enrollment_reserved_seat_capacity';
const file = `supabase/migrations/${version}.sql`, checksum = textFileSha256(file);
const assertionsFile = 'scripts/sql/enrollment-seat-reservation-capacity-assertions.sql';
...
const definitions = () => sql(`begin read only;select jsonb_agg(jsonb_build_object('signature',p.oid::regprocedure::text,'definition',pg_get_functiondef(p.oid),'acl',p.proacl) order by p.proname)
  from pg_proc p where p.oid in ('public.current_session_roster_source(uuid)'::regprocedure,'public.can_insert_session_attendance_v2(uuid,uuid)'::regprocedure,'public.get_session_attendance_roster_v2(uuid)'::regprocedure,'public.is_session_member(uuid,uuid)'::regprocedure,'mathin_internal.student_merge_relations()'::regprocedure,'public.default_enrollment_placement_seat()'::regprocedure,'public.get_my_schedule(timestamptz,timestamptz)'::regprocedure);commit;`);
...
const statement = `begin;set local lock_timeout='5s';set local statement_timeout='90s';
  select pg_advisory_xact_lock(hashtextextended('enrollment-seat-reservation-capacity',0));
