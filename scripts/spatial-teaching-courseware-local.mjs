...
const output = path.resolve('.tmp/spatial-teaching-courseware');
...
const version = '20260916000100_spatial_teaching_courseware_content';
...
if (recorded && recorded !== checksum) throw new Error('SPATIAL_MIGRATION_CHECKSUM_MISMATCH');
const migration = fs.readFileSync(file, 'utf8').replace(/^(?:begin|commit);\s*$/gm, '');
const fingerprint = () => sql("begin read only; select md5(string_agg(pg_get_functiondef(p.oid), E'\\n' order by p.proname)) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in ('cw_courseware_composition_doc_is_valid','cw_spatial_teaching_tool_is_valid','cw_formal_cube_page_is_valid','cw_manual_composition_doc_is_valid'); commit;");
...
if (mode === '--check') {
  ...
  const assertions = fs.readFileSync('supabase/tests/spatial_teaching_courseware_assertions.sql', 'utf8').replace(/^(?:begin|rollback);\s*$/gm, '');
  sql(`begin; set local lock_timeout='5s'; set local statement_timeout='45s'; ${recorded ? '' : migration}\n${assertions}\nrollback;`);
  if (fingerprint() !== before || invariant() !== dataBefore) throw new Error('SPATIAL_ROLLBACK_FAILED');
  fs.writeFileSync(path.join(output, 'check.json'), JSON.stringify({ checksum, host: observed.host, checkedAt: new Date().toISOString(), contract: 'PASS', frozenPublication: 'PASS', rollback: 'PASS' }), 'utf8');
  console.log('Spatial courseware whitelist, saved revisions, frozen release and transaction rollback: PASS');
} else {
  if (recorded) throw new Error('SPATIAL_MIGRATION_ALREADY_APPLIED');
  const check = JSON.parse(fs.readFileSync(path.join(output, 'check.json'), 'utf8'));
  if (check.checksum !== checksum || check.host !== observed.host || check.rollback !== 'PASS' || Date.now() - Date.parse(check.checkedAt) > 3_600_000) throw new Error('SPATIAL_FRESH_CHECK_REQUIRED');
  ...
  if (invariant() !== before) throw new Error('SPATIAL_APPLY_DATA_DRIFT');
  console.log('Local spatial content validators enabled; existing pages, releases, sessions and events unchanged.');
}
