-- R1-Live: an operational status is an explicit operator choice, not a
-- readiness certificate. Quality signals stay visible, while authorization,
-- input shape, referential integrity and immutable history remain hard guards.
...
-- Compatibility for older clients: preparation artifacts, reviews and learning
-- checks remain queryable quality signals, but they no longer veto the teacher's
-- explicit decision to freeze this session's current (possibly blank) snapshot.
...
begin
  ...
  if not exists (
    ...
    where id = p_session_id and deleted_at is null and started_at is null
...
comment on function public.assert_session_preparation_complete(uuid) is
  'R1-Live compatibility guard: validates teacher/session scope only; preparation artifacts, reviews, checks and release readiness are advisory.';
