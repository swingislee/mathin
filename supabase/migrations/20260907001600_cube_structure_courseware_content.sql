-- 立方体结构固定副本。只新增内容版本，不重写旧 release 或私有草稿。
begin;
...
begin
  if not coalesce(
    ...
    and p_tool ->> 'contentVersion' = 'cube-structures-lesson-v1'
    and p_tool - array['toolId','contentVersion','payload'] = '{}'::jsonb
    and jsonb_typeof(payload) = 'object'
    and payload - array['title','history'] = '{}'::jsonb
...
comment on function public.cw_cube_structures_tool_is_valid(jsonb)
  is 'Envelope, version and capacity hard gate. Server schema additionally validates complete geometry and deterministic operation dependencies.';
