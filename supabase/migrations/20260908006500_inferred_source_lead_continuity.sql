...
begin
  ...
  registration_refs as materialized (
    ...
    union select l.key,r.id from public.business_activity_registrations r join public.history_import_records h on h.id=r.source_record_id
      join lead_refs l on l.id=h.lead_id where r.record_state='current'
  ), enrollment_facts as (
    ...
    union all select l.key,e.status='active' and (t.ends_on is null or t.ends_on>=today),false
      ...
      join lead_refs l on l.id=h.lead_id left join public.school_terms t on t.id=e.term_id where e.record_state='current'
    union all
...
begin
  ...
  select exists(select 1 from public.business_activity_registrations r
      where (r.student_id=student.id or r.lead_id=any(lead_ids) or exists(select 1 from public.history_import_records h where h.id=r.source_record_id and h.lead_id=any(lead_ids))) and r.record_state='current'
  ...
  select exists(select 1 from unnest(p_enrollments) e where (e.subject_key=coalesce('student:'||student.id,'lead:'||lead.id) or exists(select 1 from public.history_import_records h where h.id=e.source_record_id and h.lead_id=any(lead_ids))) and e.record_state='current')
    ...
    exists(select 1 from unnest(p_enrollments) e left join public.school_terms t on t.id=e.term_id
      where (e.subject_key=coalesce('student:'||student.id,'lead:'||lead.id) or exists(select 1 from public.history_import_records h where h.id=e.source_record_id and h.lead_id=any(lead_ids))) and e.status='active' and e.record_state='current'
      ...
      or exists(select 1 from public.enrollments e join public.classrooms c on c.id=e.classroom_id
        ...
        where e.student_id=student.id and e.status='active' and e.left_at is null
          and (t.ends_on is null or t.ends_on >= today))
      or (has_current_source_enrollment and not exists(select 1 from unnest(p_enrollments) e where (e.subject_key=coalesce('student:'||student.id,'lead:'||lead.id) or exists(select 1 from public.history_import_records h where h.id=e.source_record_id and h.lead_id=any(lead_ids))) and e.record_state='current'))
  ...
  select a.score,a.assessment_band,a.teacher_recommendation,a.source_record_id,r.id as registration_id,
    ...
    where (r.student_id=student.id or r.lead_id=any(lead_ids) or exists(select 1 from public.history_import_records h where h.id=r.source_record_id and h.lead_id=any(lead_ids))) and r.record_state='current' and a.record_state='current' and r.status not in ('no_show','cancelled')
  ...
  select r.id,r.status,r.assessment_started_at,a.scheduled_at into appointment
    ...
    where (r.student_id=student.id or r.lead_id=any(lead_ids) or exists(select 1 from public.history_import_records h where h.id=r.source_record_id and h.lead_id=any(lead_ids))) and r.record_state='current' and a.record_state='current' and a.deleted_at is null
  ...
  select e.id,e.status,c.title,t.name as term_name,e.course_id,e.term_id into enrollment
    ...
    where (e.subject_key=coalesce('student:'||student.id,'lead:'||lead.id) or exists(select 1 from public.history_import_records h where h.id=e.source_record_id and h.lead_id=any(lead_ids))) and e.record_state='current' order by (e.status='active' and e.record_state='current') desc,e.confirmed_at desc,e.id limit 1;
...
-- 统一明确的原测评等级，原备注与来源原文继续保留。
update public.assessment_results a set assessment_band='x_plus'
where a.assessment_band is null and a.result_source='legacy' and a.strengths ~ '(^|[\r\n])原测评等级：未达A([\r\n]|$)'
 and exists(select 1 from public.history_import_records h where h.id=a.source_record_id and h.source_data->>'format'='feishu-base'
   and exists(select 1 from jsonb_array_elements(h.record_data->'cells') c where c->>'fieldName'='思维测评等级' and btrim(c->>'text')='未达A'));
