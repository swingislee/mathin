...
当前处于 **R1-Live-1 · 可安全开始**。R1-Live 只保留两个结果门：先建立当前数据库/Storage 备份和生产防误写底线，再由正式教师在自己的 production 班级/课次完成一次整班点名，刷新和重登后仍可读取，管理员可见且无权限主体不可见。课程完整度、备课审核、点名时机、资源预载和无 release 只作提示；原 R1 暂停在 R1-9，P6-AIX-2 和 1305 讲来源合同结果保留，全量课件/公开模块/全站小王子视觉/E2E/恢复演练进入 R1-Live 后的 Production 1.0 队列。SML-0 独立并行，不阻塞首次真实使用。规划入口：
...
pnpm build
pnpm plan:audit
pnpm test             # 全量诊断；当前提交态 92 文件、618 项通过、1 项条件跳过
pnpm r1:test          # R1 定向合同；23 文件、178/178 通过
