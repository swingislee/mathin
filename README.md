...
版本与功能更新见 [更新说明](CHANGELOG.md)。当前本地版本为 `0.1.2`，各功能的生产部署与人工验收状态分别记录。

当前处于 **R1-Live-2 · 生产单老师试用**。Gate 1 已在当前数据库/Storage 同批次备份完成并独立通过可读性/SHA 复核后关闭；生产端现由 1 名正式教师小范围试用，Gate 2 仍要求其在自己的 production 班级/课次完成一次整班点名，刷新和重登后仍可读取，管理员可见且无权限主体不可见。开发端可并行预演产品负责人选中的新功能；完成相关机器检查和开发初验后，再以独立可回退提交经过生产 preflight、发布与 postflight 逐项晋级。课程完整度、备课审核、点名时机、资源预载和无 release 只作提示；原 R1 暂停在 R1-9，P6-AIX-2 和 1305 讲来源合同结果保留，全量课件/公开模块/全站小王子视觉/E2E/恢复演练进入 R1-Live 后的 Production 1.0 队列。SML-0 独立并行，不阻塞首次真实使用。规划入口：

- [00 总览与信息架构](docs/plan/00-overview.md)：产品宪章、文档状态和冲突裁决；
- [04 分期路线图](docs/plan/04-roadmap.md)：R1-Live Gate、生产单老师试用、开发→生产晋级和旧 R1 重新定位；
