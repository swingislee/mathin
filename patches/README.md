...
## React Three Fiber 9.6.1 的 Three 兼容
...
- `@react-three__fiber@9.6.1.patch` 修改 ESM、CJS 开发版和 CJS 生产版三个 `events` 构建中的时钟创建与阴影配置；Web 与 native 入口共用这些构建。
...
- 阴影布尔默认值、`soft` 别名、旧对象 `type` 均归一为 `PCFShadowMap`，兼容首次配置与后续重配；保留 basic、variance 和关闭阴影的原行为。全仓画布另外共用显式 `THREE_SHADOWS` 配置，覆盖范围见[全仓兼容说明](../docs/runbooks/three-compatibility.md)。
...
验证入口：`pnpm test -- tests/react-three-fiber-timer.test.ts tests/three-repository-compatibility.test.ts`。检查三个实际构建的真实 Fiber 根、启停及手动帧、共享 delta、独立画布、按需渲染、阴影参数矩阵，以及全仓源码与 Canvas 配置、依赖版本／实例。关联相机、立方体、展开图与骰子动画测试另行按受影响范围运行。
