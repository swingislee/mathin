...
- 视觉、版式、动效与交互手感由产品负责人在开发页面上人工验收。Agent 完成实现和代码逻辑复查后提供可直接访问的本地或局域网开发链接；除非产品负责人明确要求，Agent 不调用浏览器控制、Playwright、截图或其他视觉自动化代替人工验收。
- 交付给产品负责人的开发验收链接一律使用局域网入口 `http://192.168.5.213:3130/...`，不得使用 `localhost` 或 `127.0.0.1`；回环地址只允许用于 Agent 内部的非视觉健康检查，不得出现在验收交付中。
- 在产品负责人明确回复“视觉验证通过”之前，只运行保证页面可启动、类型/版本化合同与本次硬门成立所必需的最窄机器检查；不得追加全量回归、跨浏览器、截图对比、正式发布 Gate 或重复测试。视觉未验收不等于代码失败，应将状态准确写为“开发端已交付，待人工视觉验收”。
- 产品负责人提出视觉修改时，继续以同一开发链接迭代；每轮仍只做受影响代码的逻辑复查与必要窄检查。只有产品负责人明确宣布视觉通过、要求扩大测试或要求发布时，才进入相应批次 Gate。
...
- Dashboard 默认使用连续工作区语义：页面身份进入 `DashboardPage` / `ObjectBar`，筛选和业务操作进入 `DashboardCommandPanel`，正文依靠 section、分隔线、表格、局部底色和稳定留白组织。不得把标题、说明、筛选栏、导航、表格、表单、状态摘要、空状态或每个正文分组分别包成 Card；`Card` / `DashboardCard` 只允许产品负责人明确要求，或对象确实需要独立搬运、选择、并列比较时例外使用，并在代码附近说明理由。
