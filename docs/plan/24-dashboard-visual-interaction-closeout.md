...
§5.1「每页唯一主操作」实测已经成立：25 个页面的命令面板操作区与 ObjectBar 内，`bg-rose` 主按钮数量全部 ≤ 1，本轮无需改动。
...
- `lint` / `typecheck` / `build` / `messages:check`（2644 键 × 2 语言）全过；`doc21:audit` / `doc22:audit` / `doc23:audit` / `doc24:audit` 全过。
- 溢出与布局回归：26 页 × 7 档宽度（390 / 960 / 1024 / 1152 / 1440 / 1800 / 1920，其中 960/1152/1800 等价于 1440 窗口下的 150%/125%/80% 缩放）× 亮暗双主题 = 364 次检查，根节点与主画布横向溢出均为 0，无越界元素。
...
一次性 dev-server 500（暗色首次编译素材详情页）无法复现，连续 6 次重试均 200，判定为 Turbopack 冷编译竞态，非页面缺陷。
