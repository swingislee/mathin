...
2026-08-15 产品观察确认：学生 Dashboard 及其页面没有可发现的账号信息管理入口；其他身份虽可通过账户安全页或菜单进入，但现有信息架构与管理能力仍较简陋。产品负责人决定该项在 R1-Live Gate 0～4 全部通过后施工，不阻塞当前首个真实点名闭环。当前正式管理员候选的 MFA 继续使用已上线的 `/dashboard/account-security` 直达路径完成。

交付范围：

- student、parent、staff、admin，以及 learning/family/staff 各可用环境，在桌面和移动 Dashboard 都有一致、可发现的“账号与安全”入口；用户不需要手写 URL，切换环境后入口不会消失。
- 统一账号中心展示并允许管理显示名称、头像、已绑定邮箱/手机号的掩码与验证状态、密码、MFA、活动会话、恢复方式、隐私/儿童信息同意及账号支持状态；未启用的 OTP、手机号或 OAuth 能力显示明确“尚未启用”，不伪装成可用功能。
- 微信、QQ、邮箱和手机号绑定复用本文件的单 UUID 合同；绑定、冲突、解绑、恢复都不得新建第二个 profile。管理员的角色、岗位和权限管理继续留在管理端，不混入个人账号中心。
- 所有页面和错误状态维护 zh/en，支持键盘与移动布局；敏感变更要求近期凭据或 AAL2，并写入最小审计，不向客户端暴露 auth schema、完整标识、token 或恢复材料。

退出条件：四种顶层身份各至少 1 条入口可达旅程；可用环境切换后入口一致；账号资料、密码、MFA、会话和身份绑定按实际能力工作；跨账号读取/修改为 0；绑定前后 `auth.users.id` 与 `profiles.id` 不变；禁用能力不会发起 Auth 请求。
