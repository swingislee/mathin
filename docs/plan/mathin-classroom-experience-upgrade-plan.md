...
同日，下一增量登记独立视频与普通 `page-doc-v1`。Smart 下视频元素及其原生控件整体拥有 `native` 输入，播放、暂停和拖动进度不出墨；教师要在视频画面批注时明确切换书写锁。普通文档按实际交互把页级/节点级点击标为 `click`，其余已知区域保持 `ink`，内嵌音视频保持 `native`。文档尚未解析时只应用临时交互保护，不把加载态持久化成教师的锁选择；解析后若发现 H5、未知 adapter、未支持节点或非 `page-doc-v1`，继续 fail closed。开发验收夹具只呈现两个新对象：轻点普通文档出现步进结果且不出墨；使用视频原生播放/暂停后画面启停且不出墨。课堂工具覆盖层和 H5 bridge 仍不在本增量范围。
