...
原生输入 v1 已接入开发端共同舞台，生产组织开关 `teaching.classroom_input_v2` 仍默认 `false`。当前 capability registry 审计 `board`、`image` 与三个仓内原生游戏 `sudoku/kakuro/magic-square`：数独按钮/普通单元格输出 `click`，框选单元格模式动态输出 `drag`；数和、幻方的格子、数字与擦除按钮输出 `click`。文档、视频、课堂工具覆盖层与 H5 尚未审计，统一进入交互保护，不据此宣称 M3a/M3b 完成。
