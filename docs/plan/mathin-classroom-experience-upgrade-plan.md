...
2026-09-10 开发增量：教师可从底栏 Smart 旁的“显示设置”调整 4:3 课件占可用屏宽的比例，默认沿用现有分栏；横屏分栏保留右栏操作宽度，并按可用高度限制放大上限。“专注课件”隐藏课程信息和右栏，尺寸上限放开到可用屏宽的 100%。双指捏合缩放、双指上下拖动和右侧方形滑块共同调整课件与主板书，恢复默认回到完整显示；两只手各放一根手指使用同一触控手势。专注画面右侧提供可收起、可拖动的学生名单，按可用高度分列，保留点击加星及撤销，空间不足时在名单内部滚动。底栏悬浮于课件及主板书上方，退出专注或按 Esc 返回分栏。两种视图复用同一课件实例、同一主板书和同一页序。
