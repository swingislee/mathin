...
2026-09-10 开发增量：教师可从底栏 Smart 旁的“显示设置”调整 4:3 课件占可用屏宽的比例，默认沿用现有分栏；横屏分栏保留右栏操作宽度，并按可用高度限制放大上限。“专注课件”隐藏课程信息和右栏，尺寸上限放开到可用屏宽的 100%；超出屏幕高度时，通过“上下位置”同步移动课件与主板书，恢复默认回到完整显示。底栏悬浮于课件及主板书上方，退出专注或按 Esc 返回分栏。两种视图复用同一课件实例、同一主板书和同一页序，显示调整只改变等比尺寸和查看位置。尺寸偏好按当前设备、账号和角色保存，进入教室仍从分栏开始；展示设备可独立选择适合自己的尺寸。状态为开发端待人工视觉与设备验收。
