...
2026-08-25，产品负责人在本地开发试讲页确认 M4a 三项全部通过：未认领学生以稳定身份和座次保留；重复/逆序 v2 award 收敛为精确总数且撤销指向具体 award；名单来源变化只提示，教师确认后才生成新 revision。M4a 的产品结论限于开发环境 commits `4e2aa50`、`1c99684`、`ca8e262` 与 RPC cache follow-up `aeea93b`，不重复进入 M4b 人工验收，也不把提前进入 Xiaomi 的数据库 migration 记为生产发布。M4b 首轮布局尚未通过：产品负责人要求去除学生区可见抬头与座次编号、提高奖励颜色辨识度、改用星/月亮/太阳折算，并把右上信息区压缩为 40px 无外壳/分隔线薄条且复用老版下课按钮；底栏已从视觉分支中选择 A「贴底分区轨」。
...
4. 右列改为无卡片外壳的信息薄条 + 副板书 + 正式座次纵向 4×5 积分网格；积分区不显示可见抬头或座次编号；
