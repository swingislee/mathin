...
右侧学生卡同时作为全屏逐题学情控件的压缩投影：两者以同一 `students.id`、正式座次和 4 列 × 5 行基准对应。简卡只用与全屏卡相同的边框和底色表达当前检查状态，不显示状态词、图例或附加标记，星/月/日奖励保留完整宽度；状态名称仍进入可访问名称。全屏控件在姓名前用一盏灯承载四态出勤补登记，并继续承担逐生学情、批量登记和座次调整；出勤不再占用底栏独立入口。座次手柄只在教师开启“调整座次”后出现；拖动卡保持稳定 DOM 并跟随指针，其他卡按目标座实时让位，简卡同步预览，保存失败或取消时两处共同恢复。加星/撤销、出勤与学情状态是并列事实，不互相改写。试讲课若没有配置检查项，可以从当前课件页生成本页生命周期内的“课堂观察” fixture，出勤、学情和座次修改都只保留在本页并禁止调用数据库写路径。
