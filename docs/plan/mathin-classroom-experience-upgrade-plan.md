...
2026-08-25，产品负责人在本地开发试讲页确认 M4a 三项全部通过：未认领学生以稳定身份和座次保留；重复/逆序 v2 award 收敛为精确总数且撤销指向具体 award；名单来源变化只提示，教师确认后才生成新 revision。M4a 的产品结论限于开发环境 commits `4e2aa50`、`1c99684`、`ca8e262` 与 RPC cache follow-up `aeea93b`，不重复进入 M4b 人工验收，也不把提前进入 Xiaomi 的数据库 migration 记为生产发布。下一项只施工 control 端布局、学生卡十星章和角色/viewport 降级。
