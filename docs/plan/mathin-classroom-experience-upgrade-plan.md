...
2026-08-25，产品负责人在本地开发试讲页确认 M4a 三项全部通过：未认领学生以稳定身份和座次保留；重复/逆序 v2 award 收敛为精确总数且撤销指向具体 award；名单来源变化只提示，教师确认后才生成新 revision。M4a 的产品结论限于开发环境 commits `4e2aa50`、`1c99684`、`ca8e262` 与 RPC cache follow-up `aeea93b`，不重复进入 M4b 人工验收，也不把提前进入 Xiaomi 的数据库 migration 记为生产发布。M4b 首轮布局尚未通过：产品负责人要求去除学生区可见抬头与座次编号、提高奖励颜色辨识度、改用星/月亮/太阳折算并把右上信息区改为无卡片外壳的薄条；底栏外观先以多方案视觉对比人工选定，不直接修改正式组件。
...
5. 底部控制栏横跨页面，保留学情检查、点名、工具、发题、active board 与 pending 状态；外观在产品从视觉分支中选定后再实施；
...
- 上一页、下一页位于页面最右侧；
- 结束课程位于右上角；
- 右上信息区没有圆角卡片外壳，退出教室使用含义明确的 SVG 图标；
- 所有高频触控目标保持至少 44px；
