...
- `inputDigit + entryMode` 表示填数或候选印章；
- `highlightTool` 表示框选单元格、宫、行、列或同数突出；
- 数字印章与 `highlightTool` 必须互斥；切换操作时清除旧焦点，避免旧格看起来像会被新操作立即修改；
...
4. 选择任一突出工具，确认数字 `7` 取消；再点任一数字，确认突出工具取消。选择“突出同数”后点击题面里的 `7`，确认所有 `7` 突出，点击空格不改变结果。
