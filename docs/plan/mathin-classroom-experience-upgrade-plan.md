...
移动阈值用 CSS px 和累计位移/最大偏移计算，并带滞回；不能把单一 `5px` 写成所有设备的完成标准。M0 使用 `5/8/12px` 候选在一体机触控笔、手指、鼠标和“笔被上报为 mouse”的设备上采样轻点抖动与起笔距离，再冻结默认值。`pointerType`、压力和接触面积可以选择已校准阈值或进入诊断，但不得决定某个目标能否点击。

浏览器自动化验证事件传播和 click 恰好一次，实机验证误触率。每种设备至少执行 100 次按钮轻点、50 次按钮上起笔和 50 次普通区短划；记录错误点击、漏点击、漏笔和首点偏移，M0 冻结可接受上限后才能放量。
...
| 验收对象 | 已具备的行为 | 本批不证明 |
| --- | --- | --- |
| 输入所有权 | 共同舞台监听原生 Pointer Events；Smart + `pen` 下点击目标先暂存，阈值内释放保留原生 click，越阈值后转一笔并阻断 click；普通纸面立即起笔；Canvas `pointer-events:none`；可切换交互锁/书写锁 | 尚未接入 `LiveShell`，也未证明任一阈值适合真实一体机 |
...
| 实机手感 | `5/8/12px` 各自独立计数；快速协议为 10/5/5，正式冻结协议为每阈值 100 次按钮轻点、50 次按钮起笔、50 次纸面短笔；只汇总次数、误判、最大移动与 `pointerType`，不保存姓名、账号或原始坐标 | 快速协议不能关闭阈值 Gate；开发机鼠标结果不能替代真实授课设备 |
...
本轮不能冻结 8px：5px 轻点的平均/最大移动为 25.55/43.09px，而同一设备 8px 轮次只有 0.21/2.13px，差异远大于阈值本身，说明首轮操作理解、练习顺序或手势执行对结果有明显干扰；同时汇总无法区分实体鼠标与被设备误报为 `mouse` 的触控笔，也没有覆盖可确认的 `pen`/`touch` 样本。正式冻结仍须在真实授课设备上对三档完成 100/50/50，并至少用反向顺序 `12→8→5` 复测以抵消首轮 `5→8→12` 的学习效应。进入正式采样前的当前推荐值为 **8px provisional**，M0-B 阈值 Gate 保持打开。
