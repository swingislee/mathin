...
- 20 人名单以同屏直观扫视为目标；
- 积分不使用“单星 + 总数”的数字化缩写：0–9 直接排列星星，10 星折算 1 个月亮，10 个月亮折算 1 个太阳；业务总数和具体 award 不因视觉折算封顶；
...
2026-09-11 开发增量：常规课堂沿用原有自适应分栏、课程信息和副板书位置，读取设备偏好时忽略早期保存的分栏缩放比例；缩放仅作用于专注模式，退出后由原网格恢复布局。“专注课件”隐藏课程信息和右栏，尺寸上限放开到可用屏宽的 100%。专注模式的右下角使用两组磨砂图标工具：学生独立位于上方，下方是大小在上、位置在下的视图工具组，两组留出间距，入口只显示 SVG 图标，点击后向左展开竖向滑块或名单。贴边隐藏只控制大小和位置这一组，收起后边缘保留小箭头标签，隐藏状态按设备保存；学生入口与已展开的名单保持可见。双指捏合缩放和双指上下拖动继续调整课件与主板书，“显示设置”保留恢复默认、跟随视图及手掌校准入口。两只手各放一根手指使用同一触控手势。学生按钮可独立拖到左右边缘并记住位置，拖回默认位置附近可重新对齐；移动后上半区向下展开、下半区向上展开，展开与收起使用固定按钮锚点。名单按可用空间分列，浮窗高度最多 336px，长名单内部滚动，保留点击加星、撤销和学习状态提示。底栏悬浮于课件及主板书上方，退出专注或按 Esc 返回分栏。两种视图复用同一课件实例、同一主板书和同一页序。

教师视图使用 `viewport-v1`：缩放与课件中心位置通过现有 T0/T1/T2 发送，移动中间帧走临时通道，最终状态进入有 1 KB 上限的 `session_ctl/viewport` 快照。展示端和学生端默认跟随，手动调节后可重新选择“跟随老师视图”；刷新、重连及晚加入从已保存快照恢复。设备尺寸和名单位置偏好按账号、角色和本机保存。外层手势桥校验当前 iframe 与 token，第二触点接管前取消暂存笔迹，H5 内容原有同步／只读合同保持原样。状态为开发端待人工视觉与触屏、局域网设备验收。
...
信息区固定为 40px 高，不使用圆角卡片外壳或横向分隔线。结束课程按钮固定在最右侧，复用老版课堂的低矮文字按钮元素并使用醒目的实心玫红提示色，与其他操作留出明显间隔，并继续使用危险操作确认。
...
- 学生区不显示抬头、座次编号或额外的圆角/边框/底色容器，20 张学生卡直接组成网格；
- 20 人以内不滚动；
...
- 卡片第二层直接排列星星、月亮和太阳奖励符号；
- 点击学生卡继续加星；
- 长按可以继续作为快速撤销，但必须另有可聚焦、可键盘和屏幕阅读器操作的明确撤销入口。

奖励展示规则：

- 0–9 直接渲染对应数量、具有清晰金色填充的星星图标；
- 每满 10 星折算为 1 个无外圈、无底色的蓝紫色月亮图标，余星仍逐颗紧凑排列；每满 10 个月亮再折算为 1 个太阳徽章；
- 不显示 `10★`、`+17★` 或“单星 + 总数”一类数字化缩写；可访问名称始终包含精确总数；
- 符号可以换行但不得被裁切，奖励事件继续写入且撤销仍指向具体 award；
- 21–30 人按稳定座次进入学生区内部纵向滚动，稳定 position 只负责顺序和空座，不在学生卡上显示“1号/2号”等编号；
- 已开课名单默认冻结为进入课堂时的 session roster；重连发现报名变化时显示差异并由教师确认刷新，不在课中静默重排座位。

卡片需要覆盖 2–6 字中文、长英文名、空座、离线、举手、已作答、无账号、最高星数和错误/同步中状态。视觉截断不改变完整 `aria-label`；加星、撤销成功和失败通过短 `aria-live` 通知，不移动当前焦点。全屏学情详情卡沿用既有浅色卡面与状态色边框，顶部强调色只保留 4px 细线；卡内六个 44px 操作目标固定为 3 × 2 且不得超出 4 × 5 压缩座次的单卡高度。登记层使用完整 `100dvh` 并覆盖课堂底栏；20 个座位的五行均分标题区之外的全部剩余高度，背景座位与学生卡显式叠在同一 grid cell，禁止自动排布生成第二组不可见轨道；超过 20 个座位时继续以一屏五行为高度基准，在面板内部纵向滚动。右侧简卡只投影边框和浅色底，不复制状态线或状态词。

右侧学生卡同时作为全屏逐题学情控件的压缩投影：两者以同一 `students.id`、正式座次和 4 列 × 5 行基准对应。简卡只用与全屏卡相同的边框和底色表达当前检查状态，不显示状态词、图例或附加标记，星/月/日奖励保留完整宽度；状态名称仍进入可访问名称。全屏控件在姓名前用一盏灯承载四态出勤补登记，并继续承担逐生学情、批量登记和座次调整；出勤不再占用底栏独立入口。座次手柄只在教师开启“调整座次”后出现；拖动卡保持稳定 DOM 并跟随指针，其他卡按目标座实时让位，简卡同步预览，保存失败或取消时两处共同恢复。加星/撤销、出勤与学情状态是并列事实，不互相改写。试讲课没有配置检查项时，保留不绑定课件页的通用“课堂观察”入口；待检查提醒只跟随已保存的检查页。出勤、学情和座次修改都只保留在本页并禁止调用数据库写路径。

### 5.7 底部控制栏

底部控制栏横跨整个页面，但按上方两列对齐：

2026-08-25，产品负责人从三版视觉分支中选择 A「贴底分区轨」：底栏贴紧 viewport 底边，只有平面底色与顶部细分隔线，不使用整条圆角外壳、四周边框或悬浮阴影；课堂次级操作、书写工具和翻页以细竖线分区，书写工具自身的外层卡片 chrome 同步移除。高频书写与翻页目标仍保持至少 44px，safe area 只增加轨道底部内边距。

课堂轨道中的绘图工具固定展开，不显示收起或“展开绘图工具栏”按钮；也不显示“写在主板书/副板书”提示胶囊，教师通过直接点击目标板切换书写归属。横向空间不足时工具组仍可触摸横向滑动，但隐藏浏览器原生滚动条并禁止纵向溢出，不能让滚动条挤高控制轨或造成页面布局漂移。

底栏输入、绘图、页面和课堂操作统一采用中间绘图组的视觉语言：44px 圆形图标目标、浅色分组底、选中态月面色和无外描边；文字保留在可访问名称与 tooltip 中，不再混用描边文字胶囊、红色主按钮和描边圆钮。登记学情与更多固定组成最右操作组，页面列表和前后翻页位于其左侧；补登记出勤合并进学情面板，不再占底栏位置。页面列表使用带序号的列表图标，可访问名称同时声明当前页和总页数。危险动作仍使用独立提示色，不以“统一”削弱下课等不可逆操作。
...
一级界面只保留一个 Smart 开关；交互锁和画笔锁仍是 router 的内部有效状态，不再是教师需要额外选择的两个模式。白板 `tool`、笔色、粗细和主/副板书 `activeArea` 保持原有独立状态。教师底栏入口使用 `112×44px` 横向二态滑动条，显示小型 SVG、`Smart` 与开关轨道；外层透明且无边框/圆角底板，开启态的 SVG 与滑轨使用玫瑰强调色。不得恢复大圆形图标按钮或额外模式容器。不可用是禁用状态，不是第三个可选择模式。
...
尺规拖动由按下的单个指针持有，捕获阶段处理移动与松开；正常松开采用最后坐标并结束，鼠标或触控笔的按键已释放时保留最后按住的结果。取消、失去捕获、窗口失焦、页面隐藏、切换工具或卸载时撤销当前拖动并释放监听。尺子拉长按起点到当前位置在尺身方向的增量计算，点击手柄保持原长度。
...
数独所有高频动作统一为“先选操作，再点单元格应用”：

- 点击 `5` 只把 `5` 设为持续数字印章，不修改当前焦点格；
- 连续点击多个空格，可连续填入 `5`；
- 切换到 `7` 时，只把当前印章改为 `7`，不触发写入或答案错误；
- 取消“先点空格、再点数字立即写入”，数字写入只发生在随后点击目标格时；
- 候选模式使用同一合同：先选择候选数字，再点一个或多个格切换该候选；
- 选择任一讲解突出工具时清除数字印章；点击任一数字按钮时退出当前突出工具并改为数字印章；
- 候选/填数是独立输入方式，始终恰有一个保持选中；选择或切换讲解突出工具不改变该方式；
- 已产生的候选标记、填数和讲解突出结果互不清除，互斥的只是当前待应用操作。

### 7.2 焦点与输入目标

首版不新增镜像字段，通过现有状态区分当前待应用操作：

- `selected` 只表示视觉焦点，并为删除、显示答案等显式动作提供目标；
- `entryMode` 独立表示候选或填数方式；切换它只改变后续数字印章的写入方式；
- `inputDigit` 表示当前数字印章，应用时读取当时的 `entryMode`；
- `highlightTool` 表示框选单元格、宫、行、列或同数突出；
- 只有数字印章与 `highlightTool` 互斥；选择突出工具清除 `inputDigit`，选择数字清除 `highlightTool`，`entryMode` 不参与互斥；
- 切换待应用操作时清除旧焦点，避免旧格看起来像会被新操作立即修改；
- “突出同数”不复用数字输入区：先选突出同数工具，再点击题面或已填格中的具体数字，突出所有相同数字；点击空格不产生结果。

目标格点击的核心分派：

```ts
if (highlightTool) return applyHighlightTarget(index);
if (inputDigit !== null) return applyDigitAt(index, inputDigit, entryMode);
return focusCell(index);
```

### 7.3 错误提示触发条件

错误提示只在教师点击空格、明确应用当前填数印章时触发。点击数字按钮、候选/填数模式或讲解工具本身均不属于答题尝试。

单纯切换当前数字不属于答题尝试。

错误提交不得清空当前数字印章、候选数位图或输入模式。失败格可以保留错误反馈焦点；教师随后点击另一个数字时，该动作只切换印章并退出旧失败目标。下一次点击空格后再应用新印章。
...
Smart + pen 启用时，共同舞台必须同时设置标准与 WebKit `user-select: none`、关闭 WebKit touch callout、拦截 `selectstart`，并在真正进入书写时清理残留 selection；退出 Smart 接管后恢复原值。该约束用于阻止 iPad Safari/把笔上报为 mouse 的设备在 8px 判定前框选课件文字，不得扩散到键盘浏览、交互锁或非画笔工具。
...
2026-09-10 开发增量加入设备校准后的手掌擦除。入口位于课堂“显示设置”，分别采集单指与手掌的单个触点面积；两者可区分后将版本化阈值按账号、浏览器及屏幕参数保存，屏幕参数变化后重新校准。Smart 开启且目标板书选中笔时，较大的单个触点临时接管为橡皮，抬手后保留原画笔与颜色。主板书、副板书及 H5 内触点共用判定，H5 的瞬时坐标沿 viewport bridge 逐层换算；面积数据不写入课堂事件。

手掌候选先取消临时输入，继续移动后开始擦除；有效触控笔正在书写时忽略手掌，笔随后落下则取消尚未提交的掌擦。两根普通手指仍由专注视图接管缩放和移动，同一手势保持所有权到全部触点抬起，不使用双指间距推断手掌。掌擦沿既有 erase 笔迹、撤销和同步通道提交，取消、失焦、隐藏、切页及卸载丢弃临时擦除。设备仅上报默认接触面积或手指与手掌无法区分时保持未校准，并提示继续使用工具栏橡皮擦。定向机器合同与开发 H5 入口检查已通过；实际屏幕校准、误触率和擦除手感待产品负责人验收。
...
| 星星视觉折算 | 10 星换 1 月亮、10 月亮换 1 太阳；业务奖励不封顶 | 0/1/9/10/11/27/100/历史高分 fixture 与决定记录 |
...
| 星星超限 | 0–9 颗逐颗显示；10 星换 1 月亮、10 月亮换 1 太阳，余星继续逐颗排列；奖励继续写入且可访问名称包含精确总数 | M4 学生卡覆盖 0/1/9/10/11/27/100/历史高分；视觉折算与 v2 事件总数分离 |
...
2026-08-25，产品负责人在 M4b 视觉验收时以“10 星换月亮、10 月亮换太阳”替换原“十星章 + 数字余星”表达；该修订只改变 reader 的视觉折算，不改变 M0-A 已冻结的 v2 award/revoke、继续奖励和精确总数合同。
...
| 布局名单 | 严格 4:3 主板书、课程条、副板书、4 列稳定座次与全宽底栏；支持 1024×768、1280×720、1366×768、1920×1080 和 1/8/20/30 人；21–30 人只在学生区纵向滚动；未认领学生保留；奖励按星/月亮/太阳折算且可访问名称保留精确总数 | 尚未接正式 enrollment/seat、v2 award 或 control/display 角色分支 |
...
- 点击 `7` 时旧格不变化、不报错；
- 下一格使用 `7`；
- 先点空格再点数字不写入旧格，下一次点击目标格才应用数字；
- 候选数字与讲解突出工具互斥，选择一方会退出另一方；
- “突出同数”通过点击题面数字取值，不复用数字输入区。
...
同日，产品负责人确认上述四项全部通过，M3a 的共同舞台接管核心 Gate 关闭；该结论不覆盖尚未审计的文档、视频、其他原生游戏与工具覆盖层，因此 M3a 继续施工且仍不进入 M3b。验收同时将数独交互合同修订为 §7 的统一操作印章模型，后续不再保留“先格后数立即写入”或用数字输入区选择“突出同数”的旧分支。

后续数独复核确认先格后数、工具互斥、同数取值和目标格应用均符合预期，同时发现突出工具会让候选/填数在视觉上同时取消。§7 据此把 `entryMode` 明确为独立设置：突出工具只清除数字印章，不改变或隐藏当前输入方式；试讲页只保留这一项补验，不重复已通过对象。

同日，产品负责人确认该输入方式补验通过：突出工具与 `entryMode` 独立，候选/填数状态不再随突出工具取消。数独操作语义至此按当前合同关闭，后续 renderer 覆盖验收不得再机械重复数独对象。

随后，本增量把数和与幻方登记为已审计原生 renderer，并把所有会改变棋盘状态的格子、数字和擦除按钮显式标为 `click`；未登记游戏仍 fail closed。试讲夹具随之替换为可切换的“数和 / 幻方”双页，当前只等待两个新人工验收对象：Smart 下先点格再点数字能够原生填入，且不产生板书。文档、视频、课堂工具覆盖层和 H5 继续保持交互保护，待各自独立审计。

2026-08-25，产品负责人决定把数和、幻方及其他游戏的进一步体验优化放入后续队列，直接进入下一项 renderer。该决定只解除它们对当前施工顺序的阻塞，不补写人工验收结论；已有 capability 合同与机器检查保留，后续优化不得迫使文档/视频验收重复游戏对象。
...
### 12.4 M4：正式名单、积分语义与教师布局（M4a/M4b 已关闭）

2026-08-25，产品负责人在本地开发试讲页确认 M4a 三项全部通过：未认领学生以稳定身份和座次保留；重复/逆序 v2 award 收敛为精确总数且撤销指向具体 award；名单来源变化只提示，教师确认后才生成新 revision。M4a 的产品结论限于开发环境 commits `4e2aa50`、`1c99684`、`ca8e262` 与 RPC cache follow-up `aeea93b`，不重复进入 M4b 人工验收，也不把提前进入 Xiaomi 的数据库 migration 记为生产发布。M4b 首轮布局尚未通过：产品负责人要求学生卡直接排列、去除学生区抬头/座次编号/外层圆角容器，提高奖励颜色辨识度，改用星/月亮/太阳折算，并把右上信息区压缩为 40px 无外壳/分隔线薄条且复用老版下课按钮；底栏已从视觉分支中选择 A「贴底分区轨」。

同日，学情登记的批量方向冻结为“补齐未登记”，不再保留任意点选名单后批量覆盖：卡片继续以一次点击登记单个学生；右侧常驻 112px 工作条仅批量作用于当前题仍未登记且未明确缺席/请假的学生，提供完整状态名、剩余人数和单步撤销；卡片内部状态按钮只显示统一 SVG 与颜色，登记后在姓名后显示完整状态名。20 人及以内不出现学生区内部滚动，21–30 人才按稳定座次启用内部纵向滚动。该方向已落入本地开发实现，最终验收结论见下文。

同日，产品负责人为学情按钮选择 P16“灯泡内勾”表示提示完成、I6“虚线参照 + 描摹笔”表示模仿完成；姓名前出勤标识改为纯 LED 状态点，释放灯泡语义并保持三者在 18px 下具有不同轮廓。上述图标、卡片状态薄线、4 列 × 5 行座次、右侧补齐工作条及其撤销链路均纳入最终实机验收。

2026-08-25，产品负责人确认课堂部分整体验收通过，M4b 与整个 M4 在本地开发目标关闭。关闭基线为应用 commit `95ed9f1`，覆盖 40px 课程信息薄条、A「贴底分区轨」、直接排列的学生简卡、星/月亮/太阳折算、全屏学情与简卡共享 4 列 × 5 行座次、出勤 LED、P16/I6 状态图标、“补齐未登记”原子写入和单步撤销，以及补齐成功回执不遮挡撤销按钮。该结论只表示开发端产品验收，不表示 Xiaomi 生产应用已部署或开关已启用；M5 进入生产候选、一次性集成 Gate 与获批发布阶段。
...
4. 右列改为无卡片外壳的信息薄条 + 副板书 + 正式座次纵向 4×5 积分网格；积分区不显示可见抬头、座次编号或外层容器，学生卡直接排列；
5. 底部控制栏横跨页面，保留学情检查、工具、发题与 pending 状态，点名补登记合并进学情面板；采用 A「贴底分区轨」，移除整条圆角外壳、四周边框、悬浮阴影及内嵌书写工具外壳，左右控制统一为中间绘图组的圆形图标语言；
...
11. 全屏逐题学情与右侧学生简卡共享学生身份、4 × 5 座次和当前检查颜色；全屏姓名前的出勤灯承担补登记，座次开关开启后实时重排两处卡片；试讲缺少正式检查项时只创建不落库的页面级 fixture。
...
- 20 人无需滚动；
- 学生区只有直接排列的学生卡，不出现抬头、座次编号或额外圆角容器；
- 奖励使用清晰颜色，10 星换 1 月亮、10 月亮换 1 太阳，余星逐颗排列且精确总数可访问；
- 副板书获得连续书写空间；
- 课程标题与状态不压缩主板书高度；
- 三色笔、擦除、撤销靠近主板书右下角；
- 上一页、下一页位于页面最右侧；
- 结束课程位于右上角；
- 右上信息区为 40px、无圆角卡片外壳且无横向分隔线的薄条；退出教室使用含义明确的 SVG 图标，下课复用老版课堂按钮并使用醒目的实心提示色；
- 底栏为贴底平面分区轨，不出现整条圆角外壳或悬浮阴影，展开/收起开发浮窗时不移动；
- 所有高频触控目标保持至少 44px；
...
- 已填格只保留视觉焦点；
- 切换数字不触发错误；
- 先格后数不写入旧格，且切换数字会清除旧焦点；
- 候选数按“先数字、后目标格”连续输入；
- 候选/填数始终恰有一个选中，且与突出工具状态正交；
- 数字印章与五种突出工具双向互斥；
- “突出同数”从题面数字取值，空格无效，数字输入区只负责数字印章；
- 错误答案仍被拒绝；
...
- 奖励不以“图标 + 总数”数字化；
- 星/月亮/太阳在 0/1/9/10/11/27/100/历史高分下正确折算，并保留显式撤销；
- 控制栏分区；
- 右上信息区；
- 主板书 4:3；
...
3. 先点一个空格再点 `7`，确认旧格不变化、不报错；随后点击另一个空格才填入 `7`。
4. 选择任一突出工具，确认数字 `7` 取消但候选/填数仍保持原选择；再点任一数字，确认突出工具取消。选择“突出同数”后点击题面里的 `7`，确认所有 `7` 突出，点击空格不改变结果。
5. 在数字按钮上按下后拖出一笔，确认形成一条起点完整的笔迹且按钮没有 click；重复短轻点，确认每次恰好一次 click。
6. 切到数独拖选模式，确认格子拖拽保持原生语义，不被 Smart 抢走。
