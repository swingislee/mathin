...
2026-09-10 开发增量：常规课堂沿用原有自适应分栏、课程信息和副板书位置，读取设备偏好时忽略早期保存的分栏缩放比例；缩放仅作用于专注模式，退出后由原网格恢复布局。“专注课件”隐藏课程信息和右栏，尺寸上限放开到可用屏宽的 100%。大小和上下位置两条竖向滑块只在全屏专注模式显示，大小在上、位置在下；各自点击标签向右收起，边缘保留展开按钮，纵向位置保持固定。双指捏合缩放和双指上下拖动继续调整课件与主板书，“显示设置”保留恢复默认、跟随视图及手掌校准入口。两只手各放一根手指使用同一触控手势。学生名单独立浮在滑块上方，按钮可拖到左右边缘；上半区向下展开，下半区向上展开，展开与收起使用固定按钮锚点。名单按可用空间分列，浮窗高度最多 336px，长名单内部滚动，保留点击加星、撤销和学习状态提示。底栏悬浮于课件及主板书上方，退出专注或按 Esc 返回分栏。两种视图复用同一课件实例、同一主板书和同一页序。
