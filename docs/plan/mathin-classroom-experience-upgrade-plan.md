...
> **状态**：M0–M4a 已在开发端关闭；M4b 教师布局施工中，暂不进入 M5<br>
> **规划日期**：2026-08-24<br>
...
### 12.4 M4：正式名单、积分语义与教师布局（M4a 已关闭；M4b 施工中）
...
同日，学情登记的批量方向冻结为“补齐未登记”，不再保留任意点选名单后批量覆盖：卡片继续以一次点击登记单个学生；右侧常驻 112px 工作条仅批量作用于当前题仍未登记且未明确缺席/请假的学生，提供完整状态名、剩余人数和单步撤销；卡片内部状态按钮只显示统一 SVG 与颜色，登记后在姓名后显示完整状态名。20 人及以内不出现学生区内部滚动，21–30 人才按稳定座次启用内部纵向滚动。该方向已进入本地开发实现，仍待产品视觉与交互验收，不表示 M4b 关闭或生产可用。

同日，产品负责人为学情按钮选择 P16“灯泡内勾”表示提示完成、I6“虚线参照 + 描摹笔”表示模仿完成；姓名前出勤标识改为纯 LED 状态点，释放灯泡语义并保持三者在 18px 下具有不同轮廓。该选择只授权本地开发实现，仍待实机验收。
