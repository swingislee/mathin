...
右侧学生卡同时作为全屏逐题学情控件的压缩投影：两者以同一 `students.id` 和正式座次对应，简卡显示当前检查项的同色短状态与星/月/日奖励，全屏控件继续承担逐生状态、批量登记和座次调整。全屏控件内的状态或座次变化必须立即回映简卡；加星/撤销与学情状态是并列事实，不因增加学情摘要而改变既有奖励手势。试讲课若没有配置检查项，可以从当前课件页生成本页生命周期内的“课堂观察” fixture，但必须明确标示不保存，并禁止调用学情或座次数据库写路径。
...
底栏输入、绘图、页面和课堂操作统一采用中间绘图组的视觉语言：44px 圆形图标目标、浅色分组底、选中态月面色和无外描边；文字保留在可访问名称与 tooltip 中，不再混用描边文字胶囊、红色主按钮和描边圆钮。补登记考勤、登记学情与更多固定组成最右操作组，页面列表和前后翻页位于其左侧；页面列表使用带序号的列表图标，可访问名称同时声明当前页和总页数。危险动作仍使用独立提示色，不以“统一”削弱下课等不可逆操作。
...
5. 底部控制栏横跨页面，保留学情检查、点名、工具、发题与 pending 状态；采用 A「贴底分区轨」，移除整条圆角外壳、四周边框、悬浮阴影及内嵌书写工具外壳，左右控制统一为中间绘图组的圆形图标语言；
...
11. 全屏逐题学情与右侧学生简卡共享学生身份、座次和当前检查状态；试讲缺少正式检查项时只创建不落库的页面级 fixture。
