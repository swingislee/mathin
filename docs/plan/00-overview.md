...
- `DEV-CW-1` 当前为 `STEP 4B SINGLE-PAGE 4:3 DRAFT / DEV READY / AWAITING USER AUDIT`：产品负责人已于 2026-09-01 确认 Step 4A。Step 4B 把对照区策略物化为严格的 4:3 PageDoc，并只通过既有 `save_cw_track_page_draft` 写入当前页 `adapted-4x3` 草稿；策略标记保存在适配框节点内，刷新可恢复选择。既有魔法校 A/B→等高左侧、C/E→等宽居中、D→等宽顶部、F→背景等高·内容等宽，仅在没有新草稿标记时提供起始策略；人工保存后以新策略为准。未增加 schema/RPC；爱学习/source runtime 仍为会话只读，16:9、binding、release、整讲、批量、Storage 和生产均未进入本批。
