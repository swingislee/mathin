...
- `DEV-CW-1` 当前为 `STEP 7A REVIEW OBJECT SEPARATION / READY FOR USER AUDIT`：产品负责人已人工确认 Step 0～6；爱学习来源页继续由原 Viewer 渲染，并通过共享编辑桥获得与 PageDoc 相同的选择、文字、几何、图层、网格和历史控件。Step 7A 已把正式课 4:3 适配审核与教师微课目录审核拆成独立工作区和侧栏入口；旧 `?tab=microcourses` 与旧详情 URL 保留兼容跳转，不删除研发任务、适配校对、公共资源或其他旧入口，不修改两套审核状态机，也不开放新的游戏/H5/工具能力。本批只涉及应用路由、共享队列组件与双语导航，无数据库、Storage、release 或生产写入，等待产品负责人人工验收。
