...
- `DEV-SCHOOL-OPS-1` 当前为 `PHASE 1C / DEVELOPMENT READY / AWAITING PRODUCT OWNER UI AND INTERACTION ACCEPTANCE`：两组真实小地推脱敏 `.xlsx` 仍只建立未确认身份的 `leads`、来源记录、意向标签和复核决定；第一组 1071 条验收种子保持未分配。`/dashboard/leads` 已增加当前页批量分配与“我的初次电联”：分配自动生成首联任务，老师只填接通、加 V、诺访、A/B/C、摘要与可选下次时间，沟通时间／人员／月份／周次、池内状态和下一动作由系统生成。没有建立 Student、到访、销售机会、订单或报名，也没有 Xiaomi／生产迁移或发布。
