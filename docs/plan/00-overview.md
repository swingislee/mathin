...
- `DEV-CW-1` 当前为 `STEP 3B SHARED IMAGE / READY FOR USER AUDIT`：产品负责人已于 2026-09-01 验收同一 `CoursewareWorkbench` 的预览／正式课／微课三种模式、共用目录与文字编辑、正式 PageDoc 自动保存、标题保存及旧样机删除。当前小增量又让正式课和微课直接复用唯一图片元素面板，统一填充方式、位置尺寸、透明度、层级和显隐；正式课仍走原有 PageDoc 草稿自动保存，未接文件上传或共享资源替换。4:3 A～F、爱学习写态、整讲回填、跨页资源替换、批量任务、schema/RPC 和生产动作继续关闭。
