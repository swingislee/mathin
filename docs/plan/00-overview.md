...
- `DEV-CW-1` 当前为 `STEP 5A PAGE-CONTEXT REPLACEMENT DRY-RUN / IN PROGRESS`：产品负责人已于 2026-09-01 确认 Step 4B 的旧 A～F 映射、单页 4:3 草稿保存、刷新重读和轨道隔离；首轮 binding 缺失问题已通过“4:3 显式 binding 优先、缺项只读继承 16:9”修复并复验通过。Step 5A 只在当前页上下文读取一个背景／图片资源的页、讲、课程版本、课程族、全部引用五档影响清单与冻结引用提示；未获再次人工确认前不上传、不替换、不回滚、不写数据库/Storage/release，也不进入整讲或批量任务。
