...
>
> **核对日期**：2026-09-01；依据代码、迁移、内容目录、CI、R1-Live 差距审阅、产品负责人阶段指令与 doc 00～30。`DEV-SCHOOL-OPS-1` 已按产品负责人纠正从内部 Phase 0 核对进入 Phase 1 真实业务施工，不改变生产事实或当前施工阶段。
...
| `30-mathin_school_ops_architecture_plan.md` | active | `DEV-SCHOOL-OPS-1` 的 Phase 0～6 里程碑；Phase 0 只留规划核对，当前施工 Phase 1 数据收件箱与线索工作台，业务页面人工验收后才进入 Phase 2 |
...
- `DEV-SCHOOL-OPS-1` 当前为 `PHASE 1 / FIRST BUSINESS SLICE DEVELOPMENT READY / AWAITING PRODUCT OWNER UI AND INTERACTION ACCEPTANCE`：规划型 `/dashboard/school-ops` 已删除；真实 `/dashboard/students/import` 数据收件箱和 `/dashboard/followups` 线索工作台已接入现有批次、导入、分配、沟通与下一联系时间合同。当前仅等待页面设计、交互手感与数据填写流程的第一轮人工检阅；二进制 `.xlsx`、逐行合并决定与 Family／Contact／Lead 分表尚未施工，Phase 1 未关闭且不得进入 Phase 2。
