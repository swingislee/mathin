...
- `DEV-CW-1` 当前为 `STEP 5A PAGE-CONTEXT REPLACEMENT DRY-RUN / DEV READY / AWAITING USER AUDIT`：产品负责人已于 2026-09-01 确认 Step 4B。Step 5A 已在正式 PageDoc 的“替换”右栏复用公共资源使用树的同一读模型；选择背景／图片后可切换页、讲、课程版本、课程族、全部引用五档范围并查看命中位置、冻结次数和独立钉版提示。固定样本公共背景的只读基线为 1/1/13/26/26 个位置。首轮人工审计要求移除编辑右栏不参与操作的页面 ID／来源／轨道／草稿基线摘要、让引用清单占满剩余高度，并把五档范围与 4:3 五档粗调统一为共享的 SVG＋数字紧凑选择器；开发端已按该口径更新，仍待产品负责人复审。当前没有上传、执行或回滚入口，也不写数据库/Storage/release。
