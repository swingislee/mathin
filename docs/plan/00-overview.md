...
- `DEV-CW-1` 当前为 `STEP 7B REVIEW SEMANTICS AND LEGACY RETIREMENT / USER ACCEPTED`：提交 `a4f051e` 已把正式课修改审核与教师微课入库审核收敛到唯一教研审核工作区，备课资料审核回到课次对象并由工作项进入；旧 Studio 和旧适配校对／发布界面已经从仓库删除。提交 `de72449` 新增退役 migration，移除公开直发 `publish_cw_track_release` 与批量适配发布 RPC，同时保留历史审核、release 和适配审计数据。TypeScript、受影响 ESLint、9 个定向测试文件 58/58 及发布退休单测 3/3 通过；产品负责人于 2026-09-02 明确回复“完成”。迁移尚未应用到生产，本批没有生产数据库、Storage、release 或服务写入；Step 8 扩量与生产候选尚未获执行授权。
