...
- `DEV-CW-1` 当前为 `STEP 2A REVISION READY FOR USER AUDIT`：Step 1/2 已获产品负责人确认；产品随后明确正式课件与教师微课“本质上应该是同一个组件”。首轮 Step 2A 审计要求微课只读方案复用成熟 `CoursewarePreviewWorkspace`、正式工作区压缩底部留白，并恢复历史提交 `487b60d` 已实现的“原标题栏插入、左侧当前页重命名”；第二轮要求正式课件也复用同一顶栏插入工具，并把微调／4:3／替换移到右栏标题行；第三轮确认不能只共享原语或槽位，统一编辑器应以微课功能为基线，正式课只额外开启 4:3 适配。开发端现由一个 `CoursewareEditorWorkbench` 直接承担页面目录、插入顶栏、画布/翻页、右栏标题和右栏滚动区，两条编辑链共用 `CoursewareInsertionToolbar`；正式轨道内重复的“16:9 原生版”等标题行已退出，`adapt4x3` 是正式 adapter 相对微课 adapter 的显式增量。composition 的 800ms 自动保存和既有插入 action 不变，正式 PageDoc/source-runtime 仍无写入。当前没有新增 editor action、schema、数据库/Storage 或生产变更；Step 2A 修订尚未获确认，Step 3 继续关闭。
