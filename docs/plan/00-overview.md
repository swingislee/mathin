...
- R1-Live 已建立只绑定 `127.0.0.1` 且关闭自助注册的本机隔离开发目标；11 个固定开发身份已从 gitignored manifest 初始化并通过密码登录。Xiaomi 上首名真实教师已通过邮箱绑定的一次性员工邀请完成注册，账号为 active `staff`；正式管理员已分配 `research` 与 `teacher` 两个岗位。active manifest 仍为 8 条 protected、0 条 purge；日常真实写入无需逐条 replacement，任何未来清理必须先显式生成和批准准删 test 根。数据库现为 head `20260823000100_r1_live_school_year_periods`，应用 current/previous 为 `20260822-181331` / `b899942…` 与 `20260822-162416` / `6dfb3af…`；同批次 PostgreSQL+Storage 备份已在 Xiaomi 外置盘独立校验，Gate 1 `PASS`。当前阶段为 Gate 2：生产已有 1 个 production 班级、15 个课次和 1 条 active 报名，春季边界已修正为 `2026-06-29`，班级/课次/报名均归入 `2026–2027` 秋季，2026 学年仍为 planning，学生仍为五年级；只剩正式教师点名持久再读和权限对照。2026-08-15 产品负责人给出 standing execution direction：当前 Gate 已明确规划且不扩张范围的步骤由 Agent 每轮自检目标、写态、可逆性和漂移后直接推进，不重复索取“允许”；需要真实教师/班级等产品输入、人工操作或验收，发现计划外差异，或进入清理、不可逆动作和范围扩张时才停下。生产写入仍须保留精确目标、只读 preflight、fail-closed 断言和证据登记。
