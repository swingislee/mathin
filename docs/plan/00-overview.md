...
- `DEV-CW-1` 当前为 `STEP 2A REVISION READY FOR USER AUDIT`：Step 1/2 已获产品负责人确认；产品随后明确正式课件与教师微课“本质上应该是同一个组件”。首轮 Step 2A 审计要求微课只读方案复用成熟 `CoursewarePreviewWorkspace`、正式工作区压缩底部留白，并恢复历史提交 `487b60d` 已实现的“原标题栏插入、左侧当前页重命名”；第二轮要求正式课件也复用同一顶栏插入工具，并把微调／4:3／替换移到右栏标题行。开发端修订已完成：两类编辑器共用 `CoursewareEditorToolbar` 七枚图标，正式右栏正文不再承载插入页签；预览同步获得方向键／PageUp／PageDown／空格翻页，composition adapter 保留 800ms 自动保存和后续工具组件，正式 PageDoc/source-runtime 仍无写入。当前没有新增 editor action、schema、数据库/Storage 或生产变更；Step 2A 修订尚未获确认，Step 3 继续关闭。
...
- `HOTFIX-20260831-COURSEWARE-SESSION-FIXES` 当前为 `DEPLOYED / MACHINE POSTFLIGHT PASSED / PENDING PRODUCTION USER ACCEPTANCE`：隔离候选 `05a1027…` 以真实生产 `6e0b5bd…` 为基线，集中发布爱学习/Mofaxiao 来源运行时与预览加载根修、X+ 动画体积门及正式课件/教师微课共用编辑工作台；未重放已上线课程修复，未执行导入、migration 或数据库/Storage 写入。发布 current/previous=`20260831-052938` / `05a1027…` 与 `20260830-084610` / `6e0b5bd…`，健康、鉴权、bundle、错误与业务不变量 postflight 通过，生产登录态手感和编辑工作面仍待产品验收。
...
- R1-Live 已建立只绑定 `127.0.0.1` 且关闭自助注册的本机隔离开发目标；11 个固定开发身份已从 gitignored manifest 初始化并通过密码登录。Xiaomi 上首名真实教师已通过邮箱绑定的一次性员工邀请完成注册并持有工作岗位；正式管理员 verified MFA=`1`；active manifest 仍为 8 条 protected、0 条 purge。2026-08-31 课件 session 修复集中发布后，生产数据库 ledger/head=`236 / 20260830000700_teacher_microcourse_editor_unification`，应用 current/previous=`20260831-052938` / `05a1027…` 与 `20260830-084610` / `6e0b5bd…`。最近 PostgreSQL 写前备份为 `mathin-db-prechange-20260830T042220Z-tmc-unification-8b9b195`，最近 PostgreSQL+Storage 同批次全量备份仍为 `mathin-20260828T052842Z-teacher-microcourse-variant-087b497`；本轮 app-only 发布不改 schema、业务、课程 release、审核快照或 Storage。Gate 1 `PASS`；生产共有 4 个班级、19 个课次和 1 条 active 报名，教师/教研岗位成员=`6/4`，Storage objects/bytes=`125917/51524182412`，正式点名仍为 0。Gate 2 继续 `BLOCKED`，退出差距仍是正式教师点名持久再读和权限对照；各项生产页面、真实设备与性能手感验收单独保持 pending。生产写入继续保留精确目标、只读 preflight、fail-closed 断言和证据登记。
