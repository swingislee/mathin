...
- `DEV-ORG-1` 已完成本机实现与机器检查，当前为 `LOCAL MACHINE PASS / PENDING USER ACCEPTANCE`：机构资料、校区/教室目录、课表学年/日历/排课默认和系统能力发布已按工作对象拆分，固定管理员完整旅程 1/1 通过。兼容迁移继续保留旧字段与 RPC 并双写一个回退窗口；开发人工验收和旧版本退休后才能另行删除兼容合同。该工作不修改 R1-Live Gate 2，也不授权任何生产迁移或数据写入。
- `DEV-DASH-1` 已完成本机实现与机器检查，当前为 `LOCAL MACHINE PASS / PENDING USER ACCEPTANCE`：后台侧栏改为总览、学科运营、教学、教研、组织管理和系统管理的职能结构；学年、教学日历和唯一一项排课默认合并到 `/dashboard/academic-years`，旧设置与旧排课子路径直接退役且不重定向；个人班级默认优先、全部班级使用分页表格，课件资源库每页只读取并签名 10 条。定向合同 22/22 与固定账号 Playwright 2/2 通过；该开发结果不改变 R1-Live Gate 2，也未授权生产发布。
- `DEV-DASH-2` 已完成本机实现与机器检查，当前为 `LOCAL MACHINE PASS / PENDING USER ACCEPTANCE`：Dashboard 顶层数据表统一使用完整圆角、完整描边和卡片底色的 shadcn `Table` 外壳；页面只在标题之后保留一条页头结构线，横向状态导航与正文保持同一语义区，不在二者之间重复画线；普通设置字段依靠留白、标签和局部底色组织，逐项分割线退出。表格表头和数据行仍保留行列辨识所需的内部边界。定向合同 14/14、TypeScript、全量 ESLint、Dashboard 设计审计与固定管理员 zh/en/390px Playwright 1/1 通过；该开发结果不改变 R1-Live Gate 2，也未授权生产发布。
