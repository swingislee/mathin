...
- `DEV-CW-1` 当前为 `STEP 3A UNIFIED HOST / READY FOR USER RE-AUDIT`：预览、正式课编辑和微课编辑现由同一 `CoursewareWorkbench` 面板树装配，不再由模式 dispatcher 回调旧预览实现，也不再用 DOM 槽位分别维护编辑控件。三种模式共用页面目录、画布适配、底部翻页与快捷键；预览从同一树裁掉中间编辑顶栏和右栏，正式课与微课共用左目录／中画布／右属性三栏，中间顶栏固定为左侧插入、右侧保存，正式课只额外开启 4:3 适配。当前待产品负责人对三种模式进行同轮人工复审；正式课写态仍只开放 Step 3 固定讲次第 5 页样本，未扩大到其他页面。四年级秋季标题偏移已确认为源站全系列特例，不再作为编辑器缺陷施工；爱学习写入、整讲回填、跨页资源替换、批量任务、schema/RPC 和生产动作继续关闭。
