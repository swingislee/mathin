...
- `DEV-CW-1` 当前为 `STEP 5B REPLACEMENT + EDIT HISTORY / DEV READY / AWAITING USER STAGING PREVIEW`：产品负责人已确认 Step 5A，并指定本机素材 `docs/test_material/white-board.png`（1024×768，SHA-256 `704b500e80c4ab26728a85b9561ca4f06d08b796819d783bee09f7ee3fd227fb`）与“全部引用”范围。页面上下文现与公共资源页共用 staging/apply/rollback 状态编排、上传控件、前后对比和 replacement ledger；暂存图会直接进入当前画布预览，全部引用只选择未独立钉版的 binding。正式课与微课同时复用一份最多 20 个合并步骤的会话撤销/重做内核及顶栏按钮，支持 Ctrl/Cmd+Z、Ctrl/Cmd+Shift+Z 和 Ctrl/Cmd+Y，撤销结果继续自动保存。当前仅代码与本机写前核对完成，尚未上传素材、执行替换或回滚，也未写数据库/Storage/release。
