...
- `DEV-CW-1` 当前为 `STEP 4A SHARED 4:3 PREVIEW / DEV READY / AWAITING USER AUDIT`：产品负责人已确认 Step 3B 的唯一元素检查器、可折叠完整图层列表和所有 PageDoc 元素公共层级设置，并要求进入下一项。Step 4A 已把正式 PageDoc 与爱学习 source runtime 接入同一份 4:3 策略状态、分类面板、对照画布和撤销／重置逻辑；旧 A～F／自定义已按截图审计收敛为“等宽顶部、等宽居中、等高左侧、等高居中”四种整页视口变换，以及 PageDoc 可用的“背景等高·内容等宽”分层模式。粗调面板只出现在对照视图，单独 16:9／4:3 进入对应轨道的页面微调。可由统一适配器生成的魔法校 PageDoc 与爱学习 source runtime 即使没有独立 4:3 release，也直接显示来源默认策略，不再回退并提示版本缺失；爱学习 iframe 固定以来源逻辑 viewport 渲染，再由共享宿主整体缩放，避免大画布暴露源 Viewer 的白色审阅余边。当前不保存 4:3 草稿，不新增 schema/RPC，不写数据库/Storage/release，不扩到整讲、批量或生产。
