...
- `DEV-CW-1` 当前为 `STEP 3A UNIFIED HOST / IN PROGRESS`：产品负责人已确认从成熟预览基础升级统一工作台，预览、正式课编辑和微课编辑是同一 `CoursewareWorkbench` 的三种模式；三者共用 renderer adapter、资源加载和完整适配画布，编辑模式再加载顶栏、右栏与选择覆盖层，正式课只额外加载 4:3 适配，微课默认 4:3。首个无写入小增量只收敛模式合同和 `CoursewareStageViewport`，仍待人工核对三端表现。E 系列 PageDoc 来源 CSS／字体／行为 runtime 与稳定选择桥继续属于 Step 3A 未完成项；既有单页保存复审降为 Step 3B。爱学习写入、整讲回填、跨页资源替换、批量任务、schema/RPC 和生产动作继续关闭。
