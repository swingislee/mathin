...
- R1-Live 已建立只绑定 `127.0.0.1` 且关闭自助注册的本机隔离开发目标；11 个固定开发身份已从 gitignored manifest 独立初始化并通过密码登录。Xiaomi 上首名真实教师已通过邮箱绑定的一次性员工邀请完成注册，账号为 active `staff`；正式管理员已按产品安排分配 `research` 与 `teacher` 两个岗位，替代 active manifest 已保护该教师的 auth/profile/两条岗位成员关系，旧 manifest 已转为 `retired`。班级、课次、报名和点名仍待建立。2026-08-15 产品负责人给出 standing execution direction：当前 Gate 已明确规划且不扩张范围的步骤由 Agent 每轮自检目标、写态、可逆性和漂移后直接推进，不重复索取“允许”；需要真实教师/班级等产品输入、人工操作或验收，发现计划外差异，或进入清理、不可逆动作和范围扩张时才停下。生产写入仍须保留精确目标、只读 preflight、fail-closed 断言和证据登记。
