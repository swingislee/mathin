...
- `DEV-CW-1` 当前为 `STEP 4A SHARED 4:3 PREVIEW / DEV READY / AWAITING USER AUDIT`：产品负责人已确认 Step 3B 的唯一元素检查器、可折叠完整图层列表和所有 PageDoc 元素公共层级设置，并要求进入下一项。Step 4A 已把正式 PageDoc 与爱学习 source runtime 接入同一份 4:3 策略状态、分类面板、对照画布和撤销／重置逻辑；PageDoc 使用 A～F／自定义，爱学习原生 4:3 母版默认新增“来源原版”分类，需要播放器兼容的页面默认 E。当前不保存 4:3 草稿，不新增 schema/RPC，不写数据库/Storage/release，不扩到整讲、批量或生产。
