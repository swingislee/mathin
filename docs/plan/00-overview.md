...
>
> **核对日期**：2026-09-02；依据代码、迁移、内容目录、CI、R1-Live 差距审阅、产品负责人阶段指令与 doc 00～30。产品负责人已明确要求 `DEV-SCHOOL-OPS-1` 从 Phase 1 第一业务切片继续进入 Phase 2，不改变生产事实或当前施工阶段。
...
| `30-mathin_school_ops_architecture_plan.md` | active | `DEV-SCHOOL-OPS-1` 的 Phase 0～6 里程碑；当前回补 Phase 1B 真实小地推 `.xlsx` → Leads 种子池，Phase 2 活动参与／本场测评登记保留既有开发成果；交付物是可操作业务页面而非规划页面 |
...
- `DEV-CW-1` 当前为 `STEP 8A SOURCE-RUNTIME DRAFT PERSISTENCE / DEVELOPMENT READY`：Step 7C 的预览／正式课／微课三端视觉已由产品负责人确认；代码复核确认三端直接复用同一 `CoursewareWorkbench`、目录、翻页、编辑 chrome、元素检查器、图层、文字编辑器与 4:3 控制器，文档 schema、renderer 和 typed patch translator 按 adapter 分离。Step 8A 已让爱学习 `source-runtime-page-v1` 从 release 临时副本改为读取双轨 draft head，节点文字／几何／显示修改写入当前轨草稿，4:3 粗排只写 adapted 草稿；数据库只允许稳定 `sourcePath` 节点的已声明字段和一个宿主 4:3 策略变化，来源包、fingerprint、binding、节点身份／顺序、current head 和 release 不变。本机 migration `20260902000900_courseware_source_runtime_drafts` 及 begin/rollback 事务断言通过；尚未进入正式课新增组件、5,508 页批量改写、生产 migration、Storage 或 release 写入。
- `DEV-SCHOOL-OPS-1` 当前为 `PHASE 1B / DEVELOPMENT READY / AWAITING PRODUCT OWNER UI AND INTERACTION ACCEPTANCE`：两组真实小地推脱敏 `.xlsx` 已接入数据收件箱，导入只建立 `leads` 种子、来源记录、意向标签和逐行复核决定，不建立 Family、Contact、Student、到访、销售机会、订单或报名。第一组 1071 行已在本机形成 1071 个未分配种子；第二组 157 行保持 dry-run，11 行复用、8 行待人工判断、0 行格式错误；学生档案前后均为 7 条。`/dashboard/leads` 承载未确认身份池，`/dashboard/followups` 明确为已建立学生身份后的学员跟进。Phase 2 的“名单与到访 / 本场测评登记”开发成果继续保留，但在本轮种子池人工验收前不进入 Phase 3；没有生产迁移或发布。
- `DEV-STAFF-ONBOARD-1` 当前为 `DEVELOPMENT READY / AWAITING PRODUCT OWNER UI AND INTERACTION ACCEPTANCE`：主管及以上可从员工页粘贴 Mathin 三列或魔法校 Tab 四列名单，预检后直接建立 Auth 账号、员工档案和岗位；档案立即可用于排课。首次登录密码只在本次交接清单出现，Auth 建档触发完成后同事务清除临时元数据键；员工登录后必须输入并二次确认新密码，完成前本人 staff 权限与 staff-wide RLS fail closed，完成后再自行处理必要同意。`staff.invite` 与完整 `staff.manage` 分离；本机迁移、数据库回滚断言、相关 Vitest、ESLint、TypeScript、双语键和类型摘要已通过，固定测试管理员权限及既有身份/业务计数无漂移。该功能未进入 Xiaomi／生产，仍待产品负责人页面、首次登录和交接验收。
