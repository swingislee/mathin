...
- `DEV-SCHOOL-OPS-1` 当前为 `PHASE 2 / IN DEVELOPMENT`：Phase 1 已交付真实 `/dashboard/students/import` 数据收件箱和 `/dashboard/followups` 线索工作台；产品负责人于 2026-09-02 明确要求继续下一阶段，以该指令关闭第一业务切片的阶段推进门。二进制 `.xlsx`、逐行合并决定与 Family／Contact／Lead 分表仍作为显式后续项保留，不据此宣称 Phase 1 全量能力完备；Phase 2 只施工 Activity／Participation／AssessmentResult／Opportunity 的实际业务页、交互和数据合同。
