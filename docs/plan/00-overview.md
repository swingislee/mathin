...
- `DEV-CW-1` 当前为 `STEP 8D PRODUCTION READ-ONLY INVENTORY / READY FOR USER AUDIT`：Step 0～8C 已由产品负责人确认；正式 PageDoc 与爱学习复用同一插入与自动保存链，来源 producer 节点继续不可变，游戏／工具保持 fail closed。获明确授权后，候选 `71b008dd` 使用同一聚合 SQL 经 `BatchMode` SSH 在 Xiaomi 的 `REPEATABLE READ READ ONLY` 事务完成生产盘点并回滚：生产有 77,224 页、77,060 个正式可编辑页、154,120 个可插入轨道 head、559,865 个 binding、3,770 个 release 和 4 个冻结课次；164 个微课 composition 由独立 adapter 处理，没有未知／旧文档版本。既有页面、binding、Storage、release 与冻结会话均无需回填。生产 head 仍为 `20260830000700_teacher_microcourse_editor_unification`，两个候选 migration/function 尚未部署；当前只证明 inventory 可兼容，不代表获准部署或生产通过。
