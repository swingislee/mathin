...
- `DEV-CW-1` 当前为 `STEP 3A RENDERER ROOT REVIEW / PLANNED`：产品负责人已确认 Step 0～2A，统一编辑器以微课能力为基线、正式课只增加 4:3 适配的组件合同继续成立；双端最终视觉仍在 Step 7 后、Step 8 前定稿。Step 3 已实现显式单页入口、共享保存控件、800ms 自动保存和乐观锁，但连续审计确认画面偏移不只来自 `slideInLeft`：E 系列 PageDoc 富文本仍依赖来源 CSS／字体语义，而 Mathin `DocStage` 只移植节点外层 renderer。产品负责人否决按 `data-align-v` 或页面坐标逐项追补，要求把 E 系列与爱学习都纳入来源渲染兼容边界。当前只规划无写入 Step 3A：用内容寻址来源 runtime、版本化宿主协议和语义签名 inventory 证明代表页一致；既有单页保存复审降为 Step 3B。爱学习写入、整讲回填、跨页资源替换、批量任务、schema/RPC 和生产动作继续关闭。
