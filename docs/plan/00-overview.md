...
- `DEV-CW-1` 当前为 `STEP 2A READY FOR USER AUDIT`：Step 1/2 已获产品负责人确认；产品随后明确正式课件与教师微课“本质上应该是同一个组件”。两条编辑链现共用 `CoursewareEditorWorkbench` 圆角工作面、编辑头、画布框和动作网格，由 PageDoc/source-runtime/composition adapter 保留各自能力与写态。当前没有新增 editor action、schema、数据库/Storage 或生产变更；Step 2A 尚未获确认，Step 3 继续关闭。
