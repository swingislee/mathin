...
- `DEV-CW-1` 当前为 `STEP 8C ROLLOUT INVENTORY / READY FOR USER AUDIT`：Step 0～8B 已由产品负责人确认；正式 PageDoc 与爱学习现复用同一插入工具、节点工厂、资源选择、文字／公式／形状／图片／H5 插入和自动保存链，来源 producer 节点继续不可变，游戏／工具因无版本化持久化合同保持 fail closed。本机只读盘点固定候选 `71b008dd`，覆盖 77,170 页、82,571 个可插入轨道 head、344,946 个 binding、2,350 个 release 和 3 个冻结课次；结论为既有页面改写 0、binding 改写 0、Storage 预上传 0、release／冻结会话改写 0，插入资源只在实际使用时按需登记。108 个微课 composition 由独立已部署 adapter 处理，1 个旧文档版本 fail closed。尚未采集生产只读 inventory，也未获生产候选授权；未触碰 Xiaomi/生产。
