...
- `DEV-CW-1` 当前为 `STEP 3A UNIFIED HOST / IN PROGRESS`：产品负责人已确认从成熟预览基础升级统一工作台，预览、正式课编辑和微课编辑是同一 `CoursewareWorkbench` 的三种模式；三者共用 renderer adapter、资源加载和完整适配画布，编辑模式再加载顶栏、右栏与选择覆盖层，正式课只额外加载 4:3 适配，微课默认 4:3。首轮人工审计判定只有模式 dispatcher 与画布共享仍是假统一，并发现正式课保存入口消失、微课保存回退右栏；当前小增量新增唯一编辑适配表面并统一顶栏保存位置，待人工复核。四年级秋季标题偏移已确认为源站全系列特例，不再作为编辑器缺陷施工。爱学习写入、整讲回填、跨页资源替换、批量任务、schema/RPC 和生产动作继续关闭。
