...
- `DEV-CW-1` 当前为 `STEP 3B SHARED ELEMENT SYSTEM / READY FOR USER AUDIT`：产品负责人已于 2026-09-01 验收同一 `CoursewareWorkbench` 的预览／正式课／微课三种模式、共用目录、正式 PageDoc 自动保存、标题保存及旧样机删除。Step 3B 首轮只共用了图片表单，产品复审指出两端父组件仍分别分发文字／图片／形状并拼装元素集合。返修后，唯一 `CoursewarePageElementInspector` 负责全部 PageDoc 节点的类型分发，文字与图片表单只由该入口调用；唯一可折叠 `CoursewareLayerPanel` 在正式课与微课展示完整元素列表，公共透明度、层级和显隐不再归属图片。正式课仍走原有 PageDoc 草稿自动保存，未接文件上传或共享资源替换。4:3 A～F、爱学习写态、整讲回填、跨页资源替换、批量任务、schema/RPC 和生产动作继续关闭。
