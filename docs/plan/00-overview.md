...
- `DEV-CW-1` 当前为 `STEP 4B SINGLE-PAGE 4:3 DRAFT / AUTHORIZED / IN PROGRESS`：产品负责人已于 2026-09-01 确认 Step 4A。正式 PageDoc 与爱学习 source runtime 共用的一份 4:3 策略、对照画布、默认版式和来源逻辑 viewport 已通过人工验收；四种通用模式与 PageDoc“背景等高·内容等宽”分层模式保持不变。下一批只把一个本机 PageDoc 样本页的策略接入既有草稿保存、刷新重读和自动保存反馈，不开放爱学习来源写态，不新增 schema/RPC，不改 binding/release，不扩到整讲、批量、Storage 或生产。
