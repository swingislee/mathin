...
- `DEV-CW-1` 当前为 `STEP 8A SOURCE-RUNTIME DRAFT PERSISTENCE / USER ACCEPTED`：Step 7C 的预览／正式课／微课三端视觉已由产品负责人确认；代码复核确认三端直接复用同一 `CoursewareWorkbench`、目录、翻页、编辑 chrome、元素检查器、图层、文字编辑器与 4:3 控制器，文档 schema、renderer 和 typed patch translator 按 adapter 分离。Step 8A 已让爱学习 `source-runtime-page-v1` 从 release 临时副本改为读取双轨 draft head，节点文字／几何／显示修改写入当前轨草稿，4:3 粗排只写 adapted 草稿；数据库只允许稳定 `sourcePath` 节点的已声明字段和一个宿主 4:3 策略变化，来源包、fingerprint、binding、节点身份／顺序、current head 和 release 不变。验收后的历史控件复审又把粗调／微调统一到顶栏唯一一组撤销／重做，自动保存只推进保存基线而不清空本地历史，并支持 `Ctrl/Cmd+Z`、`Ctrl/Cmd+Shift+Z` 与 `Ctrl+Y`。本机 migration `20260902000900_courseware_source_runtime_drafts` 及 begin/rollback 事务断言通过；尚未进入正式课新增组件、5,508 页批量改写、生产 migration、Storage 或 release 写入。
