...
- R1-Live 已建立只绑定 `127.0.0.1` 且关闭自助注册的本机隔离开发目标；11 个固定开发身份已从 gitignored manifest 初始化并通过密码登录。Xiaomi 上首名真实教师已通过邮箱绑定的一次性员工邀请完成注册，账号为 active `staff`；正式管理员已分配 `research` 与 `teacher` 两个岗位。active manifest 仍为 8 条 protected、0 条 purge；日常真实写入无需逐条 replacement，任何未来清理必须先显式生成和批准准删 test 根。生产数据库现为 head `20260825000500_classroom_roster_rpc_schema_reload`，应用 current/previous 仍为 `20260823-123746` / `9bc9ff3…` 与 `20260823-122633` / `ba5c991…`；2026-08-25 因执行者误把 Xiaomi 判断为开发库，课堂输入/H5 开关及 M4a roster/star v2 六个 migration 在未获生产授权时提前进入生产数据库，应用未部署，三项开关均为 false，v2 星星事件与 production 名单 revision 均为 0，唯一回填对象是 `purpose=test` 的开放课次且名单条目为 0。该事件不构成 M3/M4 生产验收或发布通过，处置见 R1-Live 目标审计。同批次 PostgreSQL+Storage 备份仍是 2026-08-22 已独立校验的最近登记备份，Gate 1 `PASS`。生产已有 1 个 production 班级、15 个课次和 1 条 active 报名，春季边界已修正为 `2026-06-29`，班级/课次/报名均归入 `2026–2027` 秋季，2026 学年仍为 planning，学生仍为五年级。单老师生产试用已启动，但正式点名仍为 0，Gate 2 继续 `BLOCKED`，退出差距仍只有正式教师点名持久再读和权限对照。2026-08-15 产品负责人给出 standing execution direction：当前 Gate 已明确规划且不扩张范围的步骤由 Agent 每轮自检目标、写态、可逆性和漂移后直接推进，不重复索取“允许”；需要真实教师/班级等产品输入、人工操作或验收，发现计划外差异，或进入清理、不可逆动作和范围扩张时才停下。生产写入仍须保留精确目标、只读 preflight、fail-closed 断言和证据登记。
