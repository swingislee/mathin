...
- `DEV-SCHOOL-OPS-1` 当前为 `PHASE 0 / IN DEVELOPMENT`：产品负责人于 2026-09-01 选入 doc 30 的学辅运营与教学履约主干。Phase 0 只审阅现有 Student、Followup、Activity、Enrollment、Class、Session、Attendance、导入批次、学年与工作项实现，建立复用/重构/新增映射和真实入口审阅页；不新增 schema，不写生产数据库/Storage，也不越过局域网人工验收进入 Phase 1。
