...
- R1-Live 已建立只绑定 `127.0.0.1` 且关闭自助注册的本机隔离开发目标；11 个固定开发身份已从 gitignored manifest 独立初始化并通过密码登录，学生、班级、课次、报名和点名业务表仍为空。这不授权创建真实账号、写入真实数据或部署 Xiaomi。执行 Gate 1～4 的生产环境动作前需产品负责人明确目标、真实教师/班级输入和授权。
