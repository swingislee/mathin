...
| `29-teacher-microcourse-browser-redesign.md` | active | DEV-TMC-4 本机开发端已交付、待产品验收：766 场景树、学术维度、适用范围、课程表格与按需预览、维护方向及默认版本；未获生产授权 |
...
- `DEV-TMC-4` 当前为 `LOCAL DEVELOPMENT DELIVERED / AWAITING PRODUCT ACCEPTANCE / NOT DEPLOYED`：教师微课课程族已在本机按 doc 29 重构为使用场景树、课程表格和当前默认版本按需预览，并补齐机构学术维度、课程适用范围、维护方向与默认版本管理。本机开关已启用供产品负责人浏览；生产 schema、数据归并、开关和发布均未获授权。
