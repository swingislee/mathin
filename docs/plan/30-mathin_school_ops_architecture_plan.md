...
>
> **当前里程碑**：`Phase 2 · 活动参与 / 测评录入 / 后续归类`；节点化表格工作台已重做并在本机开发库接通，等待产品负责人检阅页面设计、批量填写和节点切换手感。
>
...
每一个活动打开后先选择业务节点，再进入该节点的动态表格：

| 节点 | 主要可编辑列 | 背后事实 |
|---|---|---|
| 名单与到访 | 学生/联系人、报名、到场、未到、现场备注 | Participation |
| 测评录入 | 等级、分数、学情、家长关注、建议班型、老师判断 | AssessmentResult |
| 后续归类 | 未报名继续沟通、等待产品投放、本轮结束 | ActivityRoute / 后续池成员关系 |

节点之间可以展示少量只读上下文，但不得把所有字段合并为一张永久宽表。表格是录入交互，业务事实仍按对象分别保存。
...
## Phase 2：活动参与 / 测评录入 / 后续归类

### 节点化表格重做（2026-09-02）

产品负责人否决了“选中单个学生 → 填测评表单 → 创建通用销售机会”的第一版。飞书宽表只作为批量录入手感参考，不作为 Mathin 的整体数据结构；前台先选择业务节点，再在该节点的窄表格中批量填写，后台分别保存 Participation、AssessmentResult 和后续归类事实。

- `/[locale]/dashboard/activities` 展示有效名单、已到场、已填测评与待归类数量，不再展示通用销售机会／成交漏斗；
- `/[locale]/dashboard/activities/[activityId]` 提供“名单与到访 / 测评录入 / 后续归类”三个互斥节点。每个节点仍是一行一学生的高密度表格，但只显示当前节点字段；
- 测评节点批量填写实际样本中出现的等级、分数、优势、待提升点、家长关注点、建议班型和老师判断；字段自动保存，开始填写会把本次 Participation 标为已到场；
- 后续归类仅记录“未报名·继续沟通 / 待产品投放 / 本轮结束”，不要求负责人和下一动作；具体产品／运营期存在后才建立相应投放或成交上下文；
...
- 受影响 ESLint 与定向 Vitest 通过；本机 SQL 在事务内验证“测评自动到场、字段保存、全局学生阶段不变、归类保存、不创建旧销售机会”后 `ROLLBACK`。全仓 TypeScript 当前被另一条未提交课件工作的既有缺页／字段错误阻断，本次活动文件不再产生类型错误。这些机器结果不替代产品负责人实际操作页面。
...
局域网验收页：`/[locale]/dashboard/activities/[activityId]`。用户可切换三个业务节点，在不同窄表中修改参与状态、连续填写多名学生测评，并把已到场学生归入后续工作去向。

人工验收：确认“先选节点、再按表格批量录入”的信息密度、横向滚动、自动保存和角色交接符合真实习惯后，才继续 Enrollment／待分班施工。
