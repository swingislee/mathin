...
- `/[locale]/dashboard/leads` 的未分配池增加当前页勾选、负责人选择和显式“批量分配”；分配 RPC 在同一事务内把 `unassigned` 改为 `uncontacted` 并生成 `initial_contact` 任务，目标员工必须具备 `followup.write`；
- 同一路由增加“我的初次电联”任务页签。老师只填写接通结果、加微信事实、诺访事实、A/B/C 初步意向、沟通摘要和可选下次时间；池内状态与 `retry / wechat_followup / visit_confirmation / nurture` 下一动作由系统推导；
