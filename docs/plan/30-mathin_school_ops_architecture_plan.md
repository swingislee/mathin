...
本项目的每一个 Phase / 里程碑都必须以**用户可在局域网中直接打开并操作的真实页面**作为阶段交付物，不能以 Agent 自行运行测试、截图、日志或“代码已完成”作为阶段验收。
...
- 如阶段功能尚不足以形成完整业务页面，可提供临时验收入口，但该入口仍应调用真实 schema / service / action；
...
> **代码完成 + 自动检查通过 + 局域网验收页面可访问 + 用户人工验证通过。**
...
9. **每阶段必须提供局域网人工验收页。** 开发服务固定绑定 `192.168.5.213`，交付形如 `http://192.168.5.213:<port>/...` 的实际 LAN URL；不能只报告 localhost、自动测试、截图或日志。
