...
> **规划状态**：`reference`
>
> **2026-09-18 产品方向更正**：当前建设目标是可备课、可嵌入、可现场操作的独立数学教具，权威范围见[可备课数学教具](prepared-teaching-tools.md)。下文 SML-0～8、题型库、学生作答、判题与 checkpoint 是历史系统化设想，不再作为当前教具施工或验收的默认要求。已落地内核与历史空间文档继续兼容；不删除已有课件与课堂记录。
>
...
2026-09-07 独立 Tools 验收增量：五条体素教学支线合并为“立方体结构”，统一备课与本机演示的操作工作台。范围、分批验收及本地草稿边界见 [立方体结构工作台](cube-structures-workbench.md)；该增量不改变本专题的 SML-0 暂停状态或正式课堂接入边界。

2026-09-14 独立 Tools 下一场景：按产品负责人要求，将正方体展开图改为老师逐面控制折痕、角度及判断时机。首批范围与待人工验收清单见 [逐面折叠教学工作台](cube-net-teaching-workbench.md)；本地教学操作不改变冻结课件或正式课堂同步合同。
...
2. `spatial-scene-v1`、4:3 原生优先的 `spatial-page-v1`、`spatial-runtime-state-v1`、`spatial-command-v1`、学生私有 `spatial-attempt-v1`、多面体系列严格 schema、单写者 reducer、确定性重放与 pinned-kernel 作答判定已经存在；`spatial-page-v1` 已进入生产 `CoursewareDoc`、docVersion-aware 数据库/RPC、一次审核原子双 release、成对 rollback、selected-release freeze/reconnect 与 Studio/课堂只读分发。Studio 已能从服务端可信分层计数模板创建、跨类型重排和删除空间页，但尚未接入完整模型/教学编排编辑器。20 道体素和 11 种正方体展开图仍只是工程金标候选，尚未经教研签名与跨领域复核；可复用题型模板库、课堂语义 transport、学生 attempt 持久化与完整浏览器纵向验收仍未落地。
...
纵向合同测试必须覆盖：Studio 创建/重排一份 4:3 空间页 → 两条兼容 head 指向同一 revision → 一次审核原子发布两条 release → 课次备课 → freeze → 既有 4:3 live 舞台 → 学生重连回放 → 回退后两条 head 仍指向同一历史 revision。当前开发库事务断言已覆盖 create/copy/save/revert、双 head、一次审核、双 release、freeze/reconnect、成对 rollback 和替课教师精确课次 membership；TypeScript 合同已覆盖严格 `CoursewareDoc` 解析与按 layout 分发。真实 Chromium 已在同一 QA 讲次覆盖可信模板创建/混排、审核发布、备课、freeze、4:3 live 与教师刷新重连，并确认 `native-16x9` 兼容选择冻结的实际空间文档仍为 1200×900 `standard-4x3`；自动化无法操作 Canvas 的 solution annotation 由相同正式 RPC 在开发库准备，因此不计为浏览器 Canvas 证据。学生身份重连与成对 release 回退尚未在该纵向浏览器旅程完成，测试仍是 SML-0 退出项，不得用数据库断言或已完成的教师段替代剩余证据。
...
正式启动 SML-0 后的第九个工程增量修复按课次替课教师在备课成功后进入 live 仍返回 404 的权限断层。migration `20260813000400_sml0_substitute_session_membership` 让 `is_session_member` 同时识别班级成员与该课次 active `teacher_override`，并以相同谓词开放目标 `class_sessions`、classroom shell、roster 和 member-scoped release/page/assets 读取；未关联 staff、其他课次和 override 清除后的 actor 均继续 fail closed，不增加班级管理或邀请权限。服务端 `getClassroom(classId, sessionId)` 只在精确 session override 成立时合成教师课堂角色，live 路由显式传入 sessionId，普通班级入口仍只认长期 membership。真实 Chromium 使用固定开发 staff 作为指定替课教师完成备课、审核、freeze、进入等候室、开课和刷新重连；冻结快照为 release 1、1 个空间页，平台选择键是 `native-16x9`，实际 revision 仍为 1200×900 `standard-4x3` 且 `shared-standard-4x3`。开发库事务断言另以无全局权限、无课程责任的临时 staff 证明精确授予与撤销；迁移幂等重放后，`pnpm sml:test` 为 37 个文件/279 项，`pnpm test` 为 88 个文件/569 项，`pnpm sml:db-audit`、`pnpm typecheck` 与 `pnpm lint` 通过，数据库类型与初始化 manifest 同步。本增量关闭替课 live 阻断并完成纵向链的教师段；学生身份重连、成对 release 回退、20 道题教研签名与跨端 hash 向量仍未完成，SML-0 保持当前阶段。
