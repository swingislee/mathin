...
1. 2026-08-11 已建立无生产依赖的体素、正方体展开图和多面体拓扑 spike：体素覆盖整数坐标、分层、六向投影、隐藏块、连通分量、封闭空腔、内外表面和染色分类；展开图覆盖单位方格网规范化、自由多连方枚举和正方体 90° 精确折叠；通用拓扑覆盖显式顶点/棱/有序面、闭合可定向球面壳诊断、面邻接、相对面候选和铰链生成树。带几何位置的任意面二面角、平面展开布局、自相交、连续折叠、截面、完整单位检查和参数/函数求值内核仍未落地。
2. 已实现预生产 `spatial-scene-v1`、4:3 原生优先的 `spatial-page-v1`、`spatial-runtime-state-v1`、`spatial-command-v1`、学生私有 `spatial-attempt-v1`、`polyhedron-topology-v1` 和 `polyhedron-hinge-graph-v1` 严格 schema，以及单写者 reducer、reset epoch、命令指纹、学生本地分支、确定性重放与 pinned-kernel 作答判定，并录入 20 道体素和 11 种正方体展开图工程金标候选；候选题尚未经教研签名与跨领域复核，新合同也尚未加入生产 `CoursewareDoc`、数据库/RPC/RLS、课堂 transport 或发布链冻结，教研编辑器和题型模板仍未落地。
...
当前预生产 spike 已完成上述 35/11 离散合同、通用闭壳面/棱邻接和铰链生成树，并保留 11 个教研待签名候选。拓扑层的“相对面候选”只表示两面无公共顶点；只有结合位置、法向与平行性后才能成为几何相对面答案。SML-4 后续仍需建立带几何约束的平面展开布局、目标二面角、自相交检测和连续折叠，接入 `spatial-scene-v1`、相邻/相对面题型、4:3 renderer、课堂交互和触控证据；不得用组合拓扑结论替代几何合法性。
