...
2026-09-08 开发端补齐统一编辑器顶部“审核与发布”入口，复用讲次现有审核面板与提交／撤回／通过／退回／发布动作。入口按当前实际保存轨道读取最新状态（并排适配使用 4:3），保存未完成时先置灰；发布面板显示当前正式版本及草稿状态，每次操作成功后刷新。发布仍以本讲指定比例的完整页面为范围，页面或资源不齐时显示明确提示；课程产品继续只读正式 release。本增量待产品人工验收，不推进生产或关闭阶段 Gate。
