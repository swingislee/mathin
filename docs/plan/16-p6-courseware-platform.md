...
>
> **剩余项**：P6-9 全局量化验收和正式生产 release-1 重建见 doc 25 R1-9/15/18；§14 的 `DEV-CW-1` Step 0～6 已获产品负责人确认。Step 6 已按产品负责人复审从“宿主叠加／遮罩／裁切”改为来源 runtime 编辑桥：爱学习与魔法校继续由各自权威 renderer 出图，但共用选择、文字／元素属性、图层、网格、几何与历史控件；修改只作用会话副本，来源快照保持不可变。下一开发项尚未授权，不进入草稿 RPC、整讲/批量数据或生产变更，也未授权任何生产清理。
>
...
> **当前状态**：`STEP 7A REVIEW OBJECT SEPARATION / READY FOR USER AUDIT`
>
> **施工授权**：产品负责人已确认 Step 0～6，并明确回复“继续下一项”。Step 7A 审核对象分流已完成机器检查，等待人工验收：正式课 4:3 适配审核与教师微课目录审核使用独立列表、详情路径和侧栏入口；旧 URL 只做兼容跳转。不得删除研发任务、适配校对、公共资源或其他旧入口，不修改审核状态机，不开放新的游戏/H5/工具能力，不新增 migration/RPC，不触碰生产。
>
...
| 7A 正式适配与教师微课审核分流 | `READY FOR USER AUDIT` | 新增独立教师微课审核列表／详情 canonical 路径与侧栏入口；正式课审核页只保留背景、返工、页面、release 与历史，旧 `?tab=microcourses` 和旧详情 URL 保留兼容跳转。两套审核继续使用各自原有数据 loader、action 与状态机。机器检查：TypeScript、双语 5,603×2、规划审计与 3 个定向 Vitest 文件 15/15 通过 | 2026-09-02 产品负责人在 Step 6 验收后回复“继续下一项”；本批等待人工验收 | 本批人工通过前不开展共享插入能力、旧入口删除或 Step 8；无 migration/RPC、数据库、Storage、release 或生产写入 |
