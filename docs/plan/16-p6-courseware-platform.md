...
>
> **剩余项**：P6-9 全局量化验收和正式生产 release-1 重建见 doc 25 R1-9/15/18；§14 的 `DEV-CW-1` Step 0～2A 已获产品负责人确认，Step 3 本机 E 系列单页 PageDoc 纵切已交付开发端并待人工审计；未授权爱学习写入、整讲/批量数据、跨页替换或生产变更，本阶段也未授权任何生产清理。
>
...
| 3 PageDoc 单页纵切 | `AWAITING USER AUDIT` | 本机 E 系列“进位制初步”第 5 页；显式 `edit=page-doc` 且具备 `courseware.page.edit` 才读取 draft head。文字／字体和当前 16:9 轨几何分别追踪，保存复用 `save_cw_track_page_draft` 乐观锁；普通入口仍读 immutable release | 人工修改文字与位置，核对两类脏态不串扰；保存后刷新重读并核对 revision 递增；只读查询确认 release 与历史课次未变 | 禁止整讲、爱学习、跨页替换、批量回填和生产动作 |
...
Step 3 开发端变更边界：写态由显式 `edit=page-doc`、固定讲次 `ff04ee0e-2112-43c5-b4fd-27f0d853d3c3`、固定第 5 页 `b58bbb24-e11a-418a-b269-92dde7009fbe` 和 `courseware.page.edit` 四重条件限制；改变页面参数会回到 release 只读模式。PageDoc adapter 在共享 `CoursewareEditorWorkbench` 内直接承担画布点选、文字／HTML、字号／行高、位置／尺寸、透明度、层级、显隐和保存反馈；共享内容与当前 16:9 轨版式分别计算脏态，但当前样本没有独立 4:3 草稿，因此界面明确不伪造跨轨同步。保存继续调用既有 `saveCoursewareDraftAction` → `save_cw_track_page_draft`，只推进该页 draft revision；插入、4:3 A～F、自定义和上下文资源替换仍禁用并指向后续步骤。交付前没有执行草稿、release、Storage 或课次写入；TypeScript、受影响 ESLint和定向 Vitest 2/2 通过，普通未登录路由按预期 307 到登录页，状态为待产品负责人在本机样本页实际保存与刷新审计。
