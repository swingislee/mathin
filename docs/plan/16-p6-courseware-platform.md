...
| 2A 共享课件编辑框架 | `READY FOR USER AUDIT` | 正式课件与教师微课都以 `CoursewareEditorWorkbench` 为根；共享 Header、Body、CanvasFrame、ActionGrid，分别以 PageDoc/source-runtime/composition adapter 注入能力 | 产品负责人明确“两者共通的不止矩形外壳，本质上二者应该是同一个组件”。开发端已迁移为一个圆角矩形工作面和同一组编辑原语；等待核对整体 Card、微课布局回归和两边功能归属 | 无；确认前不开始 Step 3 |
...
Step 2A 变更边界：正式课程统一工作区和教师微课 composition 编辑器不再各自拥有独立外壳。`CoursewareEditorWorkbench` 统一圆角 Card、编辑头、正文、画布框和动作网格；正式 PageDoc／source-runtime 与微课 composition 通过 `data-courseware-editor-adapter` 和各自状态逻辑注入页面目录、画布、属性与动作。微课页面切换、800ms 自动保存、审核和插入 action 保持原实现；正式课程仍只有无写入样机，共享组件不导入 action、不调用 `fetch`。本批不统一不同文档 schema，不把 A～F 强加给微课 12×9 网格，也不新增持久化。机器证据为 TypeScript、受影响 ESLint、共享 Workbench／Step 1/2／微课编辑链定向 Vitest 18/18；这些结果不代替产品负责人对圆角工作面和微课回归的人工审计。
