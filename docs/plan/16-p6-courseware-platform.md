...
| 2A 共享课件编辑框架 | `REVISION READY FOR USER AUDIT` | 正式课件与教师微课都以 `CoursewareEditorWorkbench` 为根；只读微课方案另复用 `CoursewarePreviewWorkspace`，composition 按 `487b60d` 恢复原标题栏插入和左侧当前页重命名；正式工作区压缩底部安全区 | 首轮审计确认共享方向正确，并要求：成熟预览统一承担目录、全屏、完整适配和快捷键翻页；底部留白尽量少；不得用旧编辑版覆盖已实现的新布局。开发端已按 Git 历史恢复，等待第二轮核对 | 无；确认前不开始 Step 3 |
...
Step 2A 变更边界：正式课程统一工作区和教师微课 composition 编辑器不再各自拥有独立外壳。`CoursewareEditorWorkbench` 统一圆角 Card、编辑头、正文与画布框；正式 PageDoc／source-runtime 与微课 composition 通过 `data-courseware-editor-adapter` 和各自状态逻辑注入页面目录、画布、属性与动作。首轮人工审计后，`MicrocourseVariantPreview` 删除自建目录/画布，直接接入成熟 `CoursewarePreviewWorkspace`，因而同步获得可拖拽目录、全屏、可用宽高内完整适配、上一页/下一页和方向键／PageUp／PageDown／空格翻页。Git 历史确认 `487b60d` 已实现标题栏插入和左栏重命名，`0356de5` 后的布局恢复造成回归；本轮以 `0356de5^` 为 UI 基线恢复这两个位置，同时保留其后的工具组件、多互动、800ms 自动保存和共享 Workbench。正式工作区以局部负 margin 抵消 Dashboard panel 的 20/24px 底部 padding，只保留约 4px 安全区。正式课程仍只有无写入样机，共享组件不导入 action、不调用 `fetch`；本批不统一文档 schema，不新增持久化。机器证据为 TypeScript、受影响 ESLint、共享 Workbench／预览快捷键／微课编辑链定向 Vitest 28/28；这些结果不代替产品负责人第二轮人工审计。
