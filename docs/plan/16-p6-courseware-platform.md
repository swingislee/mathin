...
| 4A 共享 4:3 策略与单页会话预览 | `DEV READY / AWAITING USER AUDIT` | 一份 `CoursewareFourByThreeAdapter` 同时供正式 PageDoc 与爱学习 source runtime 使用，并共同拥有策略状态、分类面板、对照画布和撤销／重置。2026-09-01 产品负责人以 `docs/screenshots/Step 4A` 审计后，A～F／自定义被收敛为“等宽顶部、等宽居中、等高左侧、等高居中”四种带 SVG 几何图示的整页视口变换；背景、内容和互动层共同复用既有 renderer，不再分别派生。原 F 改名为“背景等高·内容等宽”，只对可拆分的 PageDoc 开放；爱学习 iframe 支持前四种，默认“等高居中”，E 系列 PageDoc 默认“等宽顶部”。4:3 粗调面板只在对照视图注册，单独 16:9／4:3 使用相应轨道的元素微调。策略只在当前浏览器会话切换，不调用保存 action。随后修复了 RSC 检查器标题的 key、source runtime 单轨二次拟合和大画布白边；缺独立 4:3 release、但统一适配器可处理的页面直接生成默认版式，不再显示不存在提示。爱学习 iframe 固定以文档逻辑 viewport 渲染并由共享宿主整体缩放，未重导或改写来源包。最新定向 Vitest 40 项、typecheck 与受影响 ESLint 已通过 | 待产品负责人复核爱学习 16:9／4:3 无白边、魔法校缺 release 时的默认等宽顶部，以及 key 告警不再出现；确认前不新增数据库合同 | 未确认前不进入 Step 4B 持久化、整讲或批量 |
