...
> **当前状态**：`STEP 3A UNIFIED HOST / READY FOR USER RE-AUDIT`
>
...
| 3A 来源渲染一致性 Gate | `READY FOR USER RE-AUDIT` | “进位制初步”标题偏移已由产品负责人核对为源站 2026 年四年级秋季全系列特例，预览与编辑器一致复现；该页退出编辑器回归证据，不再逐页修补。统一工作台、共同 renderer adapter、三栏几何、共用翻页和保存反馈已完成当前小增量 | 产品负责人需同轮复审预览、正式课与微课：三者必须来自同一面板树，正式课只比微课多 4:3；机器检查不代替该结论 | 未确认前不进入来源 runtime、Step 4、schema/RPC、重导、批量和生产动作 |
| 3A-1 统一工作台模式、三栏几何与翻页 | `READY FOR USER RE-AUDIT` | `CoursewareWorkbench` 自身拥有唯一可拖拽面板树；预览兼容入口只转入其 `preview` 模式，不再由 Workbench 反向委托旧预览组件。编辑态固定为左目录／中间“插入＋保存”顶栏与画布及翻页／右属性栏，顶栏只占中栏并与右栏标题等高；预览态从同一树裁掉编辑顶栏和右栏。三种模式共用 `CoursewareStageViewport`、`CoursewareWorkbenchPageRail`、`CoursewareWorkbenchPager` 和快捷键逻辑。2026-08-31 复审发现微课／正式课顶栏仍有轻微高度差，已将左目录头、中顶栏和右栏头收敛到同一个固定 44px 行高合同，不再允许 adapter padding 撑高 | 待产品负责人对照同一页核对：预览只有左／中且保留下翻页；正式课与微课均为真正三栏；三列顶栏高度一致；中间顶栏没有跨入右栏；微课目录内重命名和正式课轨道点仍留在共用目录行内 | 未确认前不进入来源 runtime 或扩大写入页面 |
| 3A-2 共享编辑注册与保存顶栏 | `READY FOR USER RE-AUDIT` | `CoursewareEditorAdapterSurface` 不再查询 DOM ID 或 `createPortal`；正式 PageDoc 与微课 composition 都通过同一 React context 向 Workbench 注册插入、保存状态和右栏控件。保存状态／立即保存由中间顶栏右端唯一承载；右栏只承载属性／4:3 适配。PageDoc 继续调用 `StagePreview`；正式页面选择、拖动和缩放经 `DocStage` 可选编辑桥写回右栏使用的同一份 PageDoc，再进入同一 800ms 自动保存链。composition grid 继续以 `CoursewareCompositionStage` 为来源基底并只叠加选择／拖拽层。本机固定样例原缺少课程责任关系，数据库按 SML-0 返回 `RELATION_REQUIRED`；已仅为本机固定管理员和固定讲次补 lecture 级 editor 审计关系，复核 `page.edit allowed=true`，未改 release。2026-09-01 固定第 5 页隐藏门和无写入旧样机已按授权删除；PageDoc 写态按当前页面加载，来源 runtime 无 patch 协议时明确只读 | 待产品负责人核对：微课插入不会回到右栏；正式 PageDoc 在右栏输入及画布拖动／缩放后，几何元数据同步变化，并依次出现等待保存／保存中／已自动保存；刷新后保存结果仍在；切页后不再出现旧样机 | 未确认前不进入来源 patch、Step 4、schema/RPC、整讲或任何数据扩量 |
...
| 3A-4 共享页面目录 | `READY FOR USER RE-AUDIT` | 预览、正式课编辑器与微课编辑器继续调用唯一的 `CoursewareWorkbenchPageRail`；目录正文拥有统一的受限高度与内部纵向滚动。翻页、目录点击或程序切页后，共享目录把当前行滚入可视区。当前页标题输入由共享目录直接渲染：预览保持只读，微课沿用既有标题自动保存，正式课标题编辑不再依赖当前文档是否恰好进入 PageDoc 画布写态，并通过 `courseware.page.edit` 能力控制的 `rename_cw_page` RPC 保存；本增量没有开放新增、删除、重排或其他页面写态 | 待产品负责人核对：三个工作区的长目录都可滚动；跳到目录下方页面后再前后翻页，当前行持续可见且高亮；预览目录无编辑框；正式课与微课当前页使用相同的行内标题输入，提交并刷新后标题仍在 | 未确认前不开放目录新增、删除、排序，也不进入 Step 4 |
| 3A-5 正式编辑态单一来源 | `READY FOR USER RE-AUDIT` | 2026-09-01 对 `aaa2564` 与当前 `ac07234` 的 Git blob 复核显示五个正式编辑核心文件没有回滚；实际偏差来自同一路由长期并存“真实 PageDoc 编辑器”和“无写入旧样机”两条运行分支，且前者被 `edit=page-doc`、固定讲次和固定第 5 页隐藏门限制，翻页或普通入口会静默回落旧样机。按产品负责人授权，`CoursewareCapabilityPrototype`、固定样本 gate 及其旧合同已删除；正式工作区现在统一要求 `courseware.page.edit`，按当前页和当前轨加载真实 PageDoc 草稿，来源 runtime 暂不具备 patch 协议时只显示明确只读说明，不再模拟编辑按钮。正式课与微课还共用目录标题组件，避免服务端跨边界传入松散多子节点；正式课标题编辑与画布文档类型解耦 | 待产品负责人核对：不带 `edit=page-doc` 的正式入口直接出现真实顶栏与保存状态；跨页后不再恢复旧样机；可写 PageDoc 修改后保存并刷新仍在；来源只读页没有假编辑控件；正式课标题可编辑且控制台不再出现 `CoursewareWorkbenchFrame` child key 警告 | 未确认前不扩展来源 patch、图片／公式编辑、4:3 策略、批量或生产动作 |
| 3B PageDoc 单页纵切复审 | `CHANGES REQUESTED` | 800ms 自动保存、立即保存与离页保护保留；固定页面显式入口已退出，但当前开发端写态仍需完成 3A-5 人工复审，不能把机器合同当成来源一致性或保存体验结论 | 等 3A 通过后再复审修改、保存、刷新和 release 不变；来源 runtime 写态仍等待独立 patch 协议 | 3A 未通过不得继续 |
