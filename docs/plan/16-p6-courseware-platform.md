...
>
> **剩余项**：P6-9 全局量化验收和正式生产 release-1 重建见 doc 25 R1-9/15/18；§14 的 `DEV-CW-1` Step 0～5A 已获产品负责人确认。Step 5B 已接通页面上下文 staging/apply/rollback、共享编辑撤销/重做和“对照粗适配保存 → 4:3 草稿替换／微调”的单页状态链，当前等待产品负责人沿固定样本完整复核，再执行 `white-board.png` 的“全部引用”暂存、应用与回滚。当前不开放来源节点写态，不进入整讲/批量数据或生产变更，也未授权任何生产清理。
>
...
| 5B 单本机样本真实替换与回滚 | `DEV READY / AWAITING USER WORKFLOW AUDIT` | 页面上下文与公共资源页共用 `useAssetReplacementFlow`、`AssetReplacementControls`、前后对比和 batch history；正式课与微课共用 `useCoursewareEditHistory` 与顶栏撤销／重做。针对固定第 8 页无适配页头的真实条件，统一 loader 现在把 native revision 作为 4:3 虚拟乐观锁基线；首次保存经版本化 RPC 原子建立 `adapted-4x3` 页头、复制资源 binding 并写入严格 960×720 草稿。进入 4:3 标签后装载该真实草稿，暂存 URL、元素移动／缩放和 800ms 自动保存均作用于最终 4:3 画布。2026-09-02 本机已应用管理员对象级 capability 豁免；固定管理员在该讲次 0 条责任关系时解析为 `admin/platform`，第 8 页真实首次保存成功创建 revision 2 与 adapted head，事务随后回滚且页面仍无 adapted 草稿，证明保存链不再依赖逐讲次测试授权。尚未上传／应用素材 | 固定工作流：刷新第 8 页，在对照区选择粗适配并保存；切换 4:3，确认顶栏编辑／保存与右栏微调／替换均可用；在这里上传 `white-board.png` 并先看暂存画面，移动内容验证自动保存，再决定是否 apply“全部引用” | 暂存画面确认前不得 apply；草稿定稿继续走既有审校/release，不新增旁路发布；不扩到 source-runtime、整讲、批量或生产 |
