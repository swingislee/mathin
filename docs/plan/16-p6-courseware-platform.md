...
| 2A 共享课件编辑框架 | `REVISION READY FOR USER AUDIT` | 一个 `CoursewareEditorWorkbench` 直接渲染页面目录、插入顶栏、画布/翻页、右栏标题和右栏正文，两条链共用 `CoursewareInsertionToolbar`；微课 adapter 是共同功能基线，正式 adapter 只额外开启 4:3 适配；正式轨道内重复标题行退出 | 首轮审计要求成熟预览、压缩底部留白并按 Git 历史恢复编辑布局；第二轮要求统一插入顶栏和右栏模式；第三轮否决仅共享外壳、原语或槽位，确认正式课理论上只应比微课多 4:3。开发端已完成第三轮修订，等待继续核对 | 无；确认前不开始 Step 3 |
