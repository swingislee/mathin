...
>
> **剩余项**：P6-9 全局量化验收和正式生产 release-1 重建见 doc 25 R1-9/15/18；§14 的 `DEV-CW-1` Step 0～6 已获产品负责人确认。Step 7A 已完成人工审计但“仅分流、旧入口继续并存”的结果被退回；Step 7B 已在开发端完成审核语义收口与旧链删除，等待产品负责人验收。历史对象与审计记录继续保留。不得由此进入生产清理、整讲/批量内容改写或新的游戏/H5/工具能力。
>
...
> **当前状态**：`STEP 7B REVIEW SEMANTICS AND LEGACY RETIREMENT / READY FOR USER AUDIT`
>
> **施工授权**：产品负责人已确认 Step 0～6，并完成 Step 7A 人工审计。审计结论是原“正式适配／教师微课仅拆页、旧入口继续保留”没有恢复审核的业务语义，现授权执行 Step 7B：教研审核统一承载正式课修改审核与教师微课入库审核；备课资料审核归入课次教学履约，并通过“我的待办”投影给审核人；删除尚未正式投产的旧 `/studio/courseware/[lectureId]` 编辑器、旧适配校对工作台、公开 `publish_cw_track_release` 及其批量适配发布链。旧审核与 release 数据保留用于审计，公共资源保持资源治理。只在本机开发目标新增退役 migration 并验证，不触碰生产。
>
...
| 7B 审核语义收口与旧链退役 | `READY FOR USER AUDIT` | `a4f051e`：唯一 `/dashboard/courseware/review` 以“正式课／教师微课”投影两类教研审核；微课详情保留 canonical 对象路由，独立列表和旧兼容详情退出；备课资料审核由工作项进入课次对象，指定审核人在课次内批准／退回；旧 Studio 与旧适配校对／发布组件删除。`de72449`：migration `20260902000500_courseware_legacy_publish_retirement` 移除直发与批量适配发布 RPC，历史审核、release 和适配审计数据不删除。机器检查为 TypeScript、受影响 ESLint、9 个定向测试文件 58/58 及退休单测 3/3 | 2026-09-02 产品负责人明确“旧编辑器直接从仓库移除”“备课资料审核作为教学环节”，并在工作树／热修复核对后回复“开始执行”；当前等待页面与完整流程人工验收 | 未获确认前不进入 Step 8，不应用生产 migration，不执行生产数据库、Storage 或 release 写入 |
