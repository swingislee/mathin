...
>
> **剩余项**：P6-9 全局量化验收和正式生产 release-1 重建见 doc 25 R1-9/15/18；§14 的 `DEV-CW-1` Step 0～2A 已获产品负责人确认，Step 3 本机 E 系列单页 PageDoc 纵切已按首轮意见修订并待人工复审；未授权爱学习写入、整讲/批量数据、跨页替换或生产变更，本阶段也未授权任何生产清理。
>
...
| **3 PageDoc 单页纵切** | 只在本机开发库、一个已指定 E 系列讲次的一页实现草稿保存/重载；先完成共享内容与当前轨布局，不做全讲回填 | 修改文字/图片/位置后刷新可重读；确认共享内容与分轨几何没有互相污染；确认历史 release 未变 | 适配整讲、爱学习写入、公共资源跨页替换、生产部署 |
...
| 3 PageDoc 单页纵切 | `READY FOR USER AUDIT` | 本机 E 系列“进位制初步”第 5 页；显式 `edit=page-doc` 且具备 `courseware.page.edit` 才读取 draft head。首轮审计发现来源 `slideInLeft` 在编辑态误播放、保存反馈不可见；第二轮确认直接跳过动画会隐藏标题。现由 `settleAuto` 立即落到自动步骤结束态，两端共用保存状态控件，PageDoc 以 800ms 自动保存并保留立即保存 | 复审标题直接显示在结束位置且没有进入位移；修改文字与位置时看到自动保存状态；出现“已自动保存”后刷新重读，核对两类脏态不串扰、revision 递增且 release 未变 | 禁止整讲、爱学习、跨页替换、批量回填和生产动作 |
...
Step 3 开发端变更边界：写态由显式 `edit=page-doc`、固定讲次 `ff04ee0e-2112-43c5-b4fd-27f0d853d3c3`、固定第 5 页 `b58bbb24-e11a-418a-b269-92dde7009fbe` 和 `courseware.page.edit` 四重条件限制；改变页面参数会回到 release 只读模式。PageDoc adapter 在共享 `CoursewareEditorWorkbench` 内直接承担画布点选、文字／HTML、字号／行高、位置／尺寸、透明度、层级、显隐和保存反馈；共享内容与当前 16:9 轨版式分别计算脏态，但当前样本没有独立 4:3 草稿，因此界面明确不伪造跨轨同步。首轮人工审计确认两项缺口：样本 revision 自身含自动 `slideInLeft`，编辑画布误执行后出现进入位移；保存按钮位于右栏底部且没有微课式自动保存。第一次修订直接跳过自动步骤，产品负责人复审确认标题因初始 `visible=false` 而完全不出现；当前修订给 interaction runtime 增加 `settleAuto`，按正常 auto 流直到首个 click 门立即应用结束态，不创建动画、不解析音频，因此标题显示在基础 transform 位置且不会经历位移。正式课与微课共同使用 `CoursewareEditorSaveControls`；PageDoc 每次修改在 800ms 后调用既有 `saveCoursewareDraftAction` → `save_cw_track_page_draft`，顶栏提供同一即时保存按钮，离页时对未落库修改触发保存与浏览器保护。保存只推进该页 draft revision；插入、4:3 A～F、自定义和上下文资源替换仍禁用。修订期间没有执行草稿、release、Storage 或课次写入；新增 runtime 定向合同确认 auto-enter 结束态会显示、click 后续仍隐藏且动画/音频调用均为 0，TypeScript、受影响 ESLint和定向 Vitest 3 文件 10/10 通过，状态为待产品负责人实际保存与刷新复审。
