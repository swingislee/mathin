...
>
> **剩余项**：P6-9 全局量化验收和正式生产 release-1 重建见 doc 25 R1-9/15/18；§14 的 `DEV-CW-1` Step 1 已获产品负责人确认，Step 2 无写入交互样机当前为 `READY FOR USER AUDIT`；未授权 Step 3、schema、数据写入、批量数据或生产变更，本阶段也未授权任何生产清理。
>
...
| 2 功能交互样机 | `READY FOR USER AUDIT` | 在已确认的右侧“页面与能力”栏提供四个页签；仍使用 Step 1 的 E 系列与爱学习局域网入口 | 已实现微调、A～F/自定义、五档替换范围、插入、选中反馈和会话级撤销。爱学习只开放 E 与宿主级自定义，文字/节点/插入因来源 patch 尚未决定而 fail closed；等待产品负责人审计名称、位置、联动和禁用理由 | 无；确认前不开始 Step 3 |
