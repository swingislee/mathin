...
| 3 PageDoc 单页纵切 | `READY FOR USER AUDIT` | 本机 E 系列“进位制初步”第 5 页；显式 `edit=page-doc` 且具备 `courseware.page.edit` 才读取 draft head。首轮审计发现来源 `slideInLeft` 在编辑态误播放、保存反馈不可见；现已关闭编辑态自动动画与默认选中，两端共用保存状态控件，PageDoc 以 800ms 自动保存并保留立即保存 | 复审初始画面不再发生动画位移；修改文字与位置时看到自动保存状态；出现“已自动保存”后刷新重读，核对两类脏态不串扰、revision 递增且 release 未变 | 禁止整讲、爱学习、跨页替换、批量回填和生产动作 |
...
Step 3 开发端变更边界：写态由显式 `edit=page-doc`、固定讲次 `ff04ee0e-2112-43c5-b4fd-27f0d853d3c3`、固定第 5 页 `b58bbb24-e11a-418a-b269-92dde7009fbe` 和 `courseware.page.edit` 四重条件限制；改变页面参数会回到 release 只读模式。PageDoc adapter 在共享 `CoursewareEditorWorkbench` 内直接承担画布点选、文字／HTML、字号／行高、位置／尺寸、透明度、层级、显隐和保存反馈；共享内容与当前 16:9 轨版式分别计算脏态，但当前样本没有独立 4:3 草稿，因此界面明确不伪造跨轨同步。首轮人工审计确认两项缺口：样本 revision 自身含自动 `slideInLeft`，编辑画布误执行后出现进入位移；保存按钮位于右栏底部且没有微课式自动保存。修订后编辑态以 `playAutoInteractions=false` 保持权威排版静止并默认不选节点，正式课与微课共同使用 `CoursewareEditorSaveControls`；PageDoc 每次修改在 800ms 后调用既有 `saveCoursewareDraftAction` → `save_cw_track_page_draft`，顶栏提供同一即时保存按钮，离页时对未落库修改触发保存与浏览器保护。保存只推进该页 draft revision；插入、4:3 A～F、自定义和上下文资源替换仍禁用。修订期间只读确认该页仍为 current r1、draft 为空，没有执行草稿、release、Storage 或课次写入；TypeScript、受影响 ESLint、双语键 5,360×2和定向 Vitest 4 文件 11/11 通过，状态为待产品负责人实际保存与刷新复审。
