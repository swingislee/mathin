...
| 3A-3 共享文字节点编辑 | `READY FOR USER RE-AUDIT` | 正式 PageDoc 与微课 composition 的文字／富文本节点都由同一个 `DocStage` 编辑桥承担选中、框内直接改字、移动和缩放；微课旧的 node 网格框不再重复绘制文字节点。两端右栏都调用唯一的 `CoursewareTextElementInspector`，只显示内容、字号、颜色与对齐等普通编辑字段，不再分别维护原始 HTML／节点路径／坐标式文字面板。2026-08-31 复审发现来源 rich-text 内部行内样式覆盖节点编辑值，现改为“未修改沿用来源、明确修改后由节点样式覆盖所有历史子标签”；唯一的 `CoursewareGridSnapToggle` 已从右栏迁到中间顶栏，关闭后保留自由像素几何；右栏重复的“已放置组件”正文标题与文字类型药丸已删除。文字框获得输入焦点时使用红色虚线，移动／缩放时使用红色实线，contenteditable 自带黑色焦点框被中和 | 待产品负责人在正式课固定第 5 页和微课各选一个文字框核对：框内可直接输入；正式课既有文字的字号、颜色、左中右对齐实时生效；两端顶栏吸附图标位置和开关反馈一致；右栏不再重复标题／类型；输入为虚线框且变换为实线框；修改触发各自既有自动保存且刷新可重读 | 本小增量只统一文字节点；未确认前不扩到图片、公式专项、H5、4:3、批量资源替换或新的写入页面 |
