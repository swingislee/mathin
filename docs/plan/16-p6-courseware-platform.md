...
| 3A-3 共享文字节点编辑 | `READY FOR USER RE-AUDIT` | 正式 PageDoc 与微课 composition 的文字／富文本节点都由同一个 `DocStage` 编辑桥承担选中、框内直接改字、移动和缩放；微课旧的 node 网格框不再重复绘制文字节点。两端右栏都调用唯一的 `CoursewareTextElementInspector`，只显示内容、字号、颜色与对齐等普通编辑字段，不再分别维护原始 HTML／节点路径／坐标式文字面板；唯一的 `CoursewareGridSnapControl` 控制 12×9 吸附，关闭后保留自由像素几何 | 待产品负责人在正式课固定第 5 页和微课各选一个文字框核对：框内可直接输入；移动／缩放手柄、吸附网格与关闭吸附后的自由拖动表现一致；右栏字段与视觉一致；修改触发各自既有自动保存且刷新可重读 | 本小增量只统一文字节点；未确认前不扩到图片、公式专项、H5、4:3、批量资源替换或新的写入页面 |
