...
>
> **剩余项**：P6-9 全局量化验收和正式生产 release-1 重建见 doc 25 R1-9/15/18；§14 的 `DEV-CW-1` Step 0～3B 已获产品负责人确认。当前只施工 Step 4A：正式 PageDoc 与爱学习 source runtime 共用一份 4:3 策略组件和单页会话预览；爱学习原生 4:3 母版默认“来源原版”，播放器兼容页默认 E。当前不保存适配草稿、不开放来源节点写态，不进入跨页替换、整讲/批量数据或生产变更，本阶段也未授权任何生产清理。
>
...
> **当前状态**：`STEP 4A SHARED 4:3 PREVIEW / DEV READY / AWAITING USER AUDIT`
>
> **施工授权**：产品负责人已确认 Step 0～3B；2026-09-01 在 Step 3B 返修交付后回复“下一开发项，记得遵循前面的原则”，并补充爱学习必须使用同一 4:3 组件，只允许默认分类不同。当前只允许 Step 4A：一份策略组件同时接入 PageDoc 与 source runtime，提供单页会话态 A～F／自定义／来源原版选择和即时预览。不得持久化 4:3 草稿、修改 schema/RPC、重导数据、开放爱学习节点写态、跨页替换、整讲回填、批量任务或触碰生产。
>
...
| 4A 共享 4:3 策略与单页会话预览 | `DEV READY / AWAITING USER AUDIT` | 一份 `CoursewareFourByThreeAdapter` 同时供正式 PageDoc 与爱学习 source runtime 使用，并共同拥有策略状态、分类面板、对照画布和撤销／重置。2026-09-01 产品负责人以 `docs/screenshots/Step 4A` 审计后，A～F／自定义被收敛为“等宽顶部、等宽居中、等高左侧、等高居中”四种带 SVG 几何图示的整页视口变换；背景、内容和互动层共同复用既有 renderer，不再分别派生。原 F 改名为“背景等高·内容等宽”，只对可拆分的 PageDoc 开放；爱学习 iframe 支持前四种，默认“等高居中”，E 系列 PageDoc 默认“等宽顶部”。4:3 粗调面板只在对照视图注册，单独 16:9／4:3 使用相应轨道的元素微调。策略只在当前浏览器会话切换，不调用保存 action。首次交付的跨子树 Context 与条件检查器 key 回归已改为显式共享 controller 和稳定 key；本轮定向 Vitest 18 项、typecheck 与受影响 ESLint 已通过 | 待产品负责人核对五个图示、四种整页几何、分层模式、爱学习默认值，以及对照／单轨的控件边界；确认前不新增数据库合同 | 未确认前不进入 Step 4B 持久化、整讲或批量 |
