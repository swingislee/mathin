...
>
> **剩余项**：P6-9 全局量化验收和正式生产 release-1 重建见 doc 25 R1-9/15/18；§14 的 `DEV-CW-1` Step 0～6 已获产品负责人确认。Step 7A 已完成人工审计但“仅分流、旧入口继续并存”的结果被退回，当前授权 Step 7B 收敛审核语义并删除尚未投产的旧 Studio／适配发布链；历史对象与审计记录继续保留。不得由此进入生产清理、整讲/批量内容改写或新的游戏/H5/工具能力。
>
...
> **当前状态**：`STEP 7B REVIEW SEMANTICS AND LEGACY RETIREMENT / IN PROGRESS`
>
...
| 7B 审核语义收口与旧链退役 | `IN PROGRESS` | 教研审核收敛为正式课修改审核＋教师微课入库审核；备课资料审核回到课次教学履约并向“我的待办”投影；旧 Studio、旧适配工作台、公开直发 RPC 与批量适配发布链退出，历史数据保留 | 2026-09-02 产品负责人明确“旧编辑器直接从仓库移除”“备课资料审核作为教学环节”，并在工作树／热修复核对后回复“开始执行” | 只在本机开发目标施工；完成窄检查后交付页面人工验收，不触碰生产，不开展 Step 8 |
