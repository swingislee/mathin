...
>
> **剩余项**：P6-9 全局量化验收和正式生产 release-1 重建见 doc 25 R1-9/15/18；§14 的 `DEV-CW-1` Step 0～5B 已获产品负责人确认。Step 5B 已完成“对照粗适配保存 → 独立 4:3 草稿替换／微调 → `white-board.png` 全部引用 apply → replacement ledger rollback”的本机单页闭环。当前只进入 Step 6 爱学习/source-runtime 决策样机；不开放来源节点写态，不进入整讲/批量数据或生产变更，也未授权任何生产清理。
>
