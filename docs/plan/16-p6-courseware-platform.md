...
>
> **已落地**：P6-1～P6-8 主体与 P6-10；开发数据已有 E 系列 1135 讲与爱学习 G+/X+/A+ 秋季 170 讲双轨资源，另有1年级 A+ 全国版暑期 2 讲/66 页开发增量和 13 个空占位，P6-5 有课堂集成证据。旧 v11 导入仅作为 §12 历史记录；projection v31 来源事实、生产兼容边界和 source-runtime 开发导入合同见 §13。
>
> **当前施工**：P6-AIX-2 已于 2026-08-13 关闭；当前由 R1-9/P6-9 采集 1305 讲正式来源 inventory、Storage/H5 对象审计与非执行者复核。
>
> **剩余项**：P6-9 全局量化验收和正式生产 release-1 重建见 doc 25 R1-9/15/18；§14 的 `DEV-CW-1` Step 0～8D 已获产品负责人确认，Step 8E 的窄范围 schema/app 已发布生产并通过机器 postflight，等待产品负责人在真实工作流验收。历史对象与审计记录继续保留；不得由此自动进入生产清理、整讲/批量内容改写或新的游戏/工具能力。
>
> **最后核对**：2026-09-03；§13 以前的来源与生产基线结论沿用原证据；§14 Step 8E 依据 Xiaomi 生产写前备份、migration 回滚／正式事务、应用原子发布和机器 postflight，结果为已部署、待产品验收。
...
| --- | ---: | ---: | ---: | ---: |
| `2026-gplus-sujiao-math` | 4 | 56 | 1641 | 5、6年级各 2 讲，共 4 讲 |
| `2026-xplus-sujiao-math` | 6 | 84 | 2767 | 2、5、6年级各 2 讲，共 6 讲 |
...
当前来源命中结果：G+ 1641 页全部 `source-master`；X+ 1年级 208/665、2年级 173/517 页进入兼容轨，3至6年级为 0；A+ 1年级 10/497、2年级 31/537 页进入兼容轨。该结果是来源快照审计值，不得编码成 grade/package 白名单。
...
- **范围裁决**：G+/X+/A+ 170 条源站讲次进入 1.0；G+ 5/6年级与 X+ 2/5/6年级的第 7/15 讲共 10 条由教学计划补充占位，均无 release、准备状态为“未发布”。正式总基线仍为 1305 讲/2610 条 release-1，执行仍受 R1-9/15/18 约束。
...
> **当前状态**：`STEP 8E PRODUCTION DEPLOYED / MACHINE POSTFLIGHT PASSED / PENDING PRODUCT ACCEPTANCE`
>
> **施工授权**：产品负责人已确认 Step 0～8D，先授权准备窄范围候选并明确本批不是全量部署、不做大范围备份或测试，随后明确回复“授权生产发布”。Step 8E 已按该边界完成 PostgreSQL-only 写前备份、6 条 migration 回滚／零残留演练与正式提交、应用原子发布和机器 postflight；没有 Storage 写入、存量页面回填、release 推进或冻结会话改写。生产产品验收仍待负责人完成。
>
...
2026-09-08 开发端统一编辑器在完整工作区上方提供独立“发布版本”操作行，复用讲次现有审核面板与提交／撤回／通过／退回／发布动作。入口按当前实际保存轨道读取最新状态（并排适配使用 4:3），保存未完成时先置灰；发布面板显示当前正式版本及草稿状态，每次操作成功后刷新。首次进入讲次自动按既有匹配策略分批补齐缺失的 4:3 粗版，完成后直接浏览整讲并只微调需要调整的页面，用户无需逐页生成或保存默认粗版。自动准备只补缺，保留已有草稿、发布版本与冻结快照；失败可续跑，来源并发变化则重读后重试。发布仍是显式操作，课程产品继续只读正式 release。本增量待产品人工验收，不推进生产或关闭阶段 Gate。
...
2026-08-31 继续抽查后，产品负责人确认“进位制初步”的标题偏移是源站 2026 年4年级秋季全系列特例，其他年级没有同类现象，预览与编辑器均复现，因此该页不再作为 Mathin 编辑器偏移证据，也不做逐页或 `data-align-v` 特判。本阶段保留来源 renderer 边界审计，但不再以修复该源站内容问题阻塞统一工作台。
...
| 3A 来源渲染一致性 Gate | `USER ACCEPTED` | “进位制初步”标题偏移已由产品负责人核对为源站 2026 年4年级秋季全系列特例，预览与编辑器一致复现；该页退出编辑器回归证据，不再逐页修补。统一工作台、共同 renderer adapter、三栏几何、共用翻页和保存反馈已完成当前小增量 | 2026-09-01 产品负责人在旧样机删除、标题保存和正式编辑链复审后回复“验收通过，进入下一个开发项” | 只授权 Step 3B 单页草稿闭环 |
| 3A-1 统一工作台模式、三栏几何与翻页 | `USER ACCEPTED` | `CoursewareWorkbench` 自身拥有唯一可拖拽面板树；预览兼容入口只转入其 `preview` 模式，不再由 Workbench 反向委托旧预览组件。编辑态固定为左目录／中间“插入＋保存”顶栏与画布及翻页／右属性栏，顶栏只占中栏并与右栏标题等高；预览态从同一树裁掉编辑顶栏和右栏。三种模式共用 `CoursewareStageViewport`、`CoursewareWorkbenchPageRail`、`CoursewareWorkbenchPager` 和快捷键逻辑。2026-08-31 复审发现微课／正式课顶栏仍有轻微高度差，已将左目录头、中顶栏和右栏头收敛到同一个固定 44px 行高合同，不再允许 adapter padding 撑高 | 2026-09-01 产品负责人确认当前整体表现通过 | 只授权 Step 3B |
| 3A-2 共享编辑注册与保存顶栏 | `USER ACCEPTED` | `CoursewareEditorAdapterSurface` 不再查询 DOM ID 或 `createPortal`；正式 PageDoc 与微课 composition 都通过同一 React context 向 Workbench 注册插入、保存状态和右栏控件。保存状态／立即保存由中间顶栏右端唯一承载；右栏只承载属性／4:3 适配。PageDoc 继续调用 `StagePreview`；正式页面选择、拖动和缩放经 `DocStage` 可选编辑桥写回右栏使用的同一份 PageDoc，再进入同一 800ms 自动保存链。composition grid 继续以 `CoursewareCompositionStage` 为来源基底并只叠加选择／拖拽层。2026-09-02 权限复审确认旧 SML-0 把 active admin 也错误要求为每个课程对象分配 owner/editor；统一 capability 已改为管理员通过全局 permission 后跳过对象责任关系，普通 staff 继续要求有效责任，双方仍受账号有效性与对象生命周期约束。2026-09-01 固定第 5 页隐藏门和无写入旧样机已按授权删除；PageDoc 写态按当前页面加载，来源 runtime 无 patch 协议时明确只读 | 2026-09-01 产品负责人确认保存顶栏、真实编辑链和切页表现通过 | 只授权 Step 3B |
| 3A-3 共享文字节点编辑 | `USER ACCEPTED` | 正式 PageDoc 与微课 composition 的文字／富文本节点都由同一个 `DocStage` 编辑桥承担选中、框内直接改字、移动和缩放；微课旧的 node 网格框不再重复绘制文字节点。两端右栏都调用唯一的 `CoursewareTextElementInspector`，只显示内容、字号、颜色与对齐等普通编辑字段，不再分别维护原始 HTML／节点路径／坐标式文字面板。2026-08-31 复审发现来源 rich-text 内部行内样式覆盖节点编辑值，现改为“未修改沿用来源、明确修改后由节点样式覆盖所有历史子标签”；唯一的 `CoursewareGridSnapToggle` 已从右栏迁到中间顶栏，关闭后保留自由像素几何；右栏重复的“已放置组件”正文标题与文字类型药丸已删除。文字框获得输入焦点时使用红色虚线，移动／缩放时使用红色实线，contenteditable 自带黑色焦点框被中和 | 2026-08-31 产品负责人回复“上面改动通过” | 本小增量只统一文字节点；后续图片、公式专项、H5、4:3 和批量资源替换仍按各自人工门推进 |
| 3A-4 共享页面目录 | `USER ACCEPTED` | 预览、正式课编辑器与微课编辑器继续调用唯一的 `CoursewareWorkbenchPageRail`；目录正文拥有统一的受限高度与内部纵向滚动。翻页、目录点击或程序切页后，共享目录把当前行滚入可视区。当前页标题输入由共享目录直接渲染：预览保持只读，微课沿用既有标题自动保存，正式课标题编辑不再依赖当前文档是否恰好进入 PageDoc 画布写态，并通过 `courseware.page.edit` 能力控制的 `rename_cw_page` RPC 保存；本增量没有开放新增、删除、重排或其他页面写态 | 2026-09-01 产品负责人确认目录、标题保存和切页表现通过 | 只授权 Step 3B；不开放目录新增、删除或排序 |
...
| 6 爱学习/source-runtime 编辑桥纵切 | `USER ACCEPTED` | 上一版“宿主叠加／遮罩／裁切”样机已被产品负责人否决并删除。固定样本《10的认识和加减法（上）》继续由来源 Viewer 渲染；既有 `sourcePath` adapter 已把来源节点接入与 PageDoc 相同的顶栏、元素检查器、图层、网格、历史、框内文字及几何操作，iframe 只注入选择／编辑桥，不重写来源渲染树。修改仅进入 React 会话副本并重新发送 render payload，来源快照、release 和数据库不变。机器检查：TypeScript、双语 5,601×2、编辑桥 JavaScript 语法解析及 3 个定向 Vitest 文件 21/21 通过 | 2026-09-02 产品负责人在固定页检查真实选中、框内文字、样式、图层、几何、网格和撤销表现后明确回复“验收通过”，随后回复“继续下一项” | 只授权 Step 7A 审核对象分流；不自动进入插入持久化、整讲或批量，不升级 5,508 页，不触碰生产 |
...
| 8E 窄范围生产发布 | `PRODUCTION DEPLOYED / MACHINE POSTFLIGHT PASSED / PENDING PRODUCT ACCEPTANCE` | 最终独立候选 `8c50b48…` 从实际生产基线 `750bd607…` 建立，恢复已验收的共享课堂工作台并排除并行业务。20 个受影响 Vitest 文件 118/118、TypeScript、数据库类型摘要、受影响 ESLint、候选 production build 通过；发布器再次完成全量 ESLint、TypeScript、本地与远端 production build。6 条 migration 包含页面重命名、4:3 草稿启动、管理员对象能力、旧发布链退休、source-runtime 草稿与页面插入 | 产品负责人明确“授权生产发布”后，先建立 PostgreSQL-only 写前备份，再以真实 owner 完成 `SERIALIZABLE` 回滚／零残留演练和正式提交；ledger/head=`242 / 20260903000700_courseware_page_insertions`。应用 current/previous=`20260903-115645` / `8c50b48…` 与 `20260903-100016` / `750bd607…`；健康、鉴权、canonical 路由、错误增量和备份 postflight 通过，11 项课件／资源／release／冻结／Storage 计数不变 | 生产产品验收 pending；应用问题可切 previous，数据库默认 forward-fix。不得据此批量改写页面、推进 release、清理历史对象或开放未版本化游戏／工具能力 |
...
Step 2A 变更边界：正式课程统一工作区和教师微课 composition 编辑器不再各自拼装独立外壳。后续 3A 复审进一步把预览也收回同一 `CoursewareWorkbench`：组件直接拥有可拖拽面板树、页面目录头/正文/页脚、中间插入与保存顶栏、完整适配画布、共用翻页页脚、右栏标题和右栏正文；`CoursewarePreviewWorkspace` 只保留兼容调用名。`CoursewareInsertionToolbar` 统一七枚插入动作的顺序与按钮语义，`CoursewareWorkbenchPageRail` 统一选中行／序号／标题／状态，`CoursewareWorkbenchPager` 统一按钮、链接和方向键/PageUp/PageDown/空格；微课当前页重命名作为共用目录行内的编辑内容。正式 PageDoc／source-runtime 与微课 composition 只提供数据、动作、renderer 和能力内容，并通过 React context 注册编辑控件，不再维护 portal 目标。Git 历史确认 `487b60d` 已实现标题栏插入和左栏重命名，当前 composition 保留该布局及后续工具组件、多互动和 800ms 自动保存。正式 adapter 只额外声明 `adapt4x3`，不借此把两种文档 schema 或写态强并；2026-09-01 又删除正式路由内残留的无写入旧样机和固定页面隐藏门，PageDoc 写态按当前页面能力加载，来源 runtime 无 patch 协议时明确只读。本批不新增 schema、RPC 或持久化范围。机器证据只证明类型、双语键和源码结构合同，不代替产品负责人后续人工审计。

Step 3 当前边界：产品负责人已确认“进位制初步”的标题偏移是源站 2026 年4年级秋季全系列特例，其他年级未复现，因此该页退出编辑器通用缺陷，不再对 `slideInLeft`、富文本垂直对齐或节点坐标继续打特例补丁。随后对“正式课似乎回到 `aaa2564` 前”的偏差做了 Git 与运行分支双重核对：当前提交中的编辑核心 blob 没有回滚，真正原因是正式路由仍保留两个互斥实现——仅固定样本隐藏入口装载真实 PageDoc 编辑器，其他入口和翻页装载旧的无写入 `CoursewareCapabilityPrototype`；标题输入也错误地复用了该隐藏门。产品负责人明确授权删除旧样机后，固定样本常量、`edit=page-doc` 查询门、旧样机组件及其专属测试已退出；正式工作区按当前页／轨道装载真实 PageDoc 草稿，800ms 自动保存、立即保存、乐观锁和离页保护继续使用同一持久化链，来源 runtime 暂无 patch 协议时明确只读。该修复仍处于开发端待人工复审，不等于 Step 3、来源写态或 4:3 功能已经验收；插入扩展、4:3 A～F、自定义、上下文资源替换、爱学习写态、整讲和生产动作继续关闭。
