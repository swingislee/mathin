...
| 3A-1 统一工作台模式、三栏几何与翻页 | `READY FOR USER RE-AUDIT` | `CoursewareWorkbench` 自身拥有唯一可拖拽面板树；预览兼容入口只转入其 `preview` 模式，不再由 Workbench 反向委托旧预览组件。编辑态固定为左目录／中间“插入＋保存”顶栏与画布及翻页／右属性栏，顶栏只占中栏并与右栏标题等高；预览态从同一树裁掉编辑顶栏和右栏。三种模式共用 `CoursewareStageViewport`、`CoursewareWorkbenchPageRail`、`CoursewareWorkbenchPager` 和快捷键逻辑 | 待产品负责人对照同一页核对：预览只有左／中且保留下翻页；正式课与微课均为真正三栏；中间顶栏没有跨入右栏；微课目录内重命名和正式课轨道点仍留在共用目录行内 | 未确认前不进入来源 runtime 或扩大写入页面 |
| 3A-2 共享编辑注册与保存顶栏 | `READY FOR USER RE-AUDIT` | `CoursewareEditorAdapterSurface` 不再查询 DOM ID 或 `createPortal`；正式 PageDoc、正式无写入能力样机与微课 composition 都通过同一 React context 向 Workbench 注册插入、保存状态和右栏控件。保存状态／立即保存由中间顶栏右端唯一承载；右栏只承载属性／4:3 适配。PageDoc 继续调用 `StagePreview`，composition grid 继续以 `CoursewareCompositionStage` 为来源基底并只叠加选择／拖拽层 | 待产品负责人核对：微课插入不会回到右栏；正式已授权样本出现等待保存／保存中／已自动保存与立即保存；刷新后保存结果仍在。当前正式写态仍只限 Step 3 固定讲次第 5 页，不把其他页面的无写入样机伪装成已开放保存 | 未确认前不进入来源 runtime、Step 4、schema/RPC、整讲或任何数据扩量 |
