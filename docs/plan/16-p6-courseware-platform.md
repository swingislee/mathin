...
>
> **剩余项**：P6-9 全局量化验收和正式生产 release-1 重建见 doc 25 R1-9/15/18；§14 的 `DEV-CW-1` Step 0～3A 已获产品负责人确认。当前只施工 Step 3B：在同一共享编辑链内复审正式课 PageDoc 的单页内容／几何草稿保存，并补齐与微课共用的图片元素编辑面板；不进入 4:3 A～F、爱学习来源写态、跨页资源替换、整讲/批量数据或生产变更，本阶段也未授权任何生产清理。
>
...
> **当前状态**：`STEP 3B SHARED ELEMENT SYSTEM / READY FOR USER AUDIT`
>
> **施工授权**：产品负责人已确认 Step 0～3A；2026-09-01 对统一宿主、共用目录／文字编辑、正式 PageDoc 自动保存、旧样机删除和标题保存回复“验收通过，进入下一个开发项”。当前只允许 Step 3B 单页草稿闭环：正式课与微课共用图片元素局部编辑组件，正式 PageDoc 继续通过既有 800ms 自动保存链持久化内容与几何并核对历史 release 不变。不得修改 schema、重导数据、扩到爱学习写态、进入 4:3 A～F、跨页替换、批量回填或触碰生产。
>
...
| 3B PageDoc 单页纵切复审 | `READY FOR USER AUDIT` | 首轮唯一 `CoursewareImageElementInspector` 只统一图片表单；产品复审指出正式课和微课父组件仍分别判断文字／图片／形状并拼装元素集合。返修新增唯一 `CoursewarePageElementInspector` 负责全部 PageDoc 节点的类型分发，文字与图片检查器只由此入口调用；唯一 `CoursewareLayerPanel` 默认折叠，展开后以列表展示正式 PageDoc 全部递归节点和微课全部 composition blocks，支持选择、逐层上移／下移、直接输入层级和节点显隐。透明度、层级与显隐已成为所有 PageDoc 节点的公共属性，退出图片专属面板。两端仍通过各自 adapter 写回同一共享控件；正式课继续使用 800ms 自动保存、立即保存、乐观锁与离页保护。本批没有文件上传、binding 改写、schema/RPC 或共享资源替换；数据库合同仍只推进 `draft_revision_id`，不写 lecture release/head | 待产品负责人核对：两端选中文字、图片和形状时使用同一右栏组件；图层默认收起、展开为完整列表，选择、上移／下移、输入层级和显隐生效；正式课修改后自动保存并刷新重读。机器证据：TypeScript、受影响 ESLint、双语键 `5424×2`、规划审计以及定向 Vitest `4 文件 28/28` 均通过；不代替人工结论 | 未确认前不进入 Step 4、来源 patch、资源替换、整讲或生产动作 |
