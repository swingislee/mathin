...
| 5A 页面上下文资源替换影响预览 | `DEV READY / AWAITING USER AUDIT` | 正式 PageDoc 的“替换”右栏复用既有公共资源使用树的同一 `loadCoursewareSharedAssetDetail` 读模型；当前页在画布中选择背景／图片后，按页、讲、课程版本、课程族、全部引用五档筛选同一组 usage，并显示命中页面、当前轨、冻结次数和独立钉版。首轮审计指出右栏元数据无操作价值、长条按钮密度低且清单高度受限；现移除页面 ID／来源／轨道／草稿基线摘要，以同一 `CoursewareCompactChoiceGroup` 承载资源范围与 4:3 粗调的 SVG＋数字五项选择，并让引用清单独占余下滚动高度。固定样本是合法的纯背景 PageDoc（`nodes=[]`），首次选择第 5 种分层版式暴露旧内容框错误依赖首节点；共享 `derive43PageDoc` 现直接构造严格 schema 的合成 group，空节点页与有节点页均可预览并物化全部五种策略。Server Action 只有 `courseware.asset.manage` 读权限，不调用上传、替换或回滚 RPC；界面没有文件输入和执行按钮。固定样本《定义新运算进阶》第 1 页公共背景 `a7b6d47e-e5d1-4a62-9ed9-c206e8d742cf` 的本机只读计数为 1/1/13/26/26 | 打开固定样本的 16:9 原生版，点击背景进入“替换”；确认右栏无冗余元数据、五项图标同排、默认当前页、计数 1/1/13/26/26，长清单占满并独立滚动；进入对照区依次点击五种版式，确认无 `CONTENT_FRAME_REQUIRES_NODE`、五项同排且完整名称／说明随选择显示；确认页面没有上传或执行按钮。机器检查：定向 Vitest 15/15、TypeScript、受影响 ESLint、diff check 与规划审计通过 | 未确认前不得上传、替换、回滚、写 binding/Storage/release 或扩到 source-runtime、整讲、批量 |
