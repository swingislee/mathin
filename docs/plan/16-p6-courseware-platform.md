...
> **当前状态**：`STEP 3B SHARED IMAGE / READY FOR USER AUDIT`
>
...
| 3B PageDoc 单页纵切复审 | `READY FOR USER AUDIT` | 新增唯一的 `CoursewareImageElementInspector`，正式 PageDoc 与微课 composition 都直接调用它，不再分别维护图片属性面板；同一组件提供完整显示／裁切填满／拉伸填满／原始尺寸、位置尺寸、透明度、层级与显隐。几何分别进入两端既有 transform adapter，正式课继续使用 800ms 自动保存、立即保存、乐观锁与离页保护；本批没有文件上传、binding 改写或共享资源替换。数据库合同复核草稿函数只新增 page revision 并推进 `draft_revision_id`，不写 lecture release/head | 待产品负责人核对：两端选中图片出现同一属性面板；填充方式、位置尺寸、透明度、层级和显隐修改后表现一致；正式课自动保存并刷新重读。机器证据为 TypeScript、受影响 ESLint、双语键 `5403×2`、规划审计与定向 Vitest `5 文件 23/23`，不代替人工结论 | 未确认前不进入 Step 4、来源 patch、资源替换、整讲或生产动作 |
