...
| 4A 共享 4:3 策略与单页会话预览 | `DEV READY / AWAITING USER AUDIT` | 一份 `CoursewareFourByThreeAdapter` 同时供正式 PageDoc 与爱学习 source runtime 使用，并共同拥有策略状态、分类面板、对照画布和撤销／重置；PageDoc 默认 C，爱学习 `source-master` 默认新增“来源原版”，`source-player-compat` 默认 E。PageDoc 与来源 runtime 只提供文档／能力 adapter。策略只在当前浏览器会话切换，不调用保存 action。首次交付发现注册到工作台右栏的 ReactNode 已离开画布 Context；现已改为显式共享 controller，同时为条件检查器面板补稳定 key，避免跨子树 Context 与子节点 key 回归；定向 Vitest 23 项、typecheck、受影响 ESLint、双语 key 和规划审计已通过 | 待产品负责人核对策略分类、爱学习默认值、控件位置、对照画面和撤销／重置；确认前不新增数据库合同 | 未确认前不进入 Step 4B 持久化、整讲或批量 |
