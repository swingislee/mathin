...
- **P6-3 导入 CLI + 样本讲导入**（mathin）：`scripts/cw-import.mjs` 幂等导入样本讲（对象上传、H5 包上传、docs/revisions/bindings/shared_assets 入库、`courseware_template` 生成、release 1 发布）。验收：对账报告平；重复执行零新增；教研改过的页（手工造一条 edit revision）重导入不被覆盖。
