...
| 3A-2 共享编辑注册与保存顶栏 | `READY FOR USER RE-AUDIT` | `CoursewareEditorAdapterSurface` 不再查询 DOM ID 或 `createPortal`；正式 PageDoc、正式无写入能力样机与微课 composition 都通过同一 React context 向 Workbench 注册插入、保存状态和右栏控件。保存状态／立即保存由中间顶栏右端唯一承载；右栏只承载属性／4:3 适配。PageDoc 继续调用 `StagePreview`；正式单页选择、拖动和缩放现在经 `DocStage` 可选编辑桥写回右栏使用的同一份 PageDoc，再进入同一 800ms 自动保存链。composition grid 继续以 `CoursewareCompositionStage` 为来源基底并只叠加选择／拖拽层。本机固定样例原缺少课程责任关系，数据库按 SML-0 返回 `RELATION_REQUIRED`；已仅为本机固定管理员和固定讲次补 lecture 级 editor 审计关系，复核 `page.edit allowed=true`，未改 release | 待产品负责人核对：微课插入不会回到右栏；正式样例在右栏输入及画布拖动／缩放后，几何元数据同步变化，并依次出现等待保存／保存中／已自动保存；刷新后保存结果仍在。当前正式写态仍只限 Step 3 固定讲次第 5 页，不把其他页面的无写入样机伪装成已开放保存 | 未确认前不进入来源 runtime、Step 4、schema/RPC、整讲或任何数据扩量 |
...
| 3A-4 共享页面目录 | `READY FOR USER RE-AUDIT` | 预览、正式课编辑器与微课编辑器继续调用唯一的 `CoursewareWorkbenchPageRail`；目录正文现在拥有统一的受限高度与内部纵向滚动。翻页、目录点击或程序切页后，共享目录把当前行滚入可视区。当前页标题输入也由共享目录直接渲染：预览保持只读，微课沿用既有标题自动保存，正式课仅在 Step 3 固定可写页面开放相同的行内输入，并通过 `courseware.page.edit` 能力控制的 `rename_cw_page` RPC 保存；本增量没有开放新增、删除、重排或其他页面写态 | 待产品负责人核对：三个工作区的长目录都可滚动；跳到目录下方页面后再前后翻页，当前行持续可见且高亮；预览目录无编辑框；正式课固定第 5 页与微课当前页使用相同的行内标题输入，提交并刷新后标题仍在 | 未确认前不扩大正式课可写页面，不开放目录新增、删除、排序，也不进入 Step 4 |
