...
课程族页采用同页目录—详情工作区。默认目录展示全部可见微课，不要求先确定年级、班型或季节；顶部用微课总数、单次课/短系列/系列课数量和完整度提供宏观视图，标题、讲次名称与目标、主题、关键词、来源年级/班型/学期和完整度作为可组合筛选面。点击目录条目只切换同一工作区中的详情，保留目录和筛选上下文；详情显示来源标签、场次数量、完成度、教学计划与逐讲课件预览。来源维度只说明这门微课从哪个真实班级产生，不宣称它只能用于该维度。
