...
课次内制作、派生、选择、试讲和上课不依赖审核。任一方案需要进入校内“教师微课”目录时，才以该方案作为独立作品进入 DEV-TMC-1 的提交、审核、发布、撤回和 immutable release 流程；发布不会改变课次当前选用，也不会把自由班转换成正式课程班。
