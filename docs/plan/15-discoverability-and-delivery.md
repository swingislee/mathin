...
> **规划状态**：`partial`
>
> **当前用途**：P4G SEO、双语、缓存、CI 与性能治理参考。
>
> **已落地**：P4G-0～5/7/9 及多项 CI 检查。
>
...
>
> **最后核对**：2026-08-30；`POST-LIVE-PERF-01` 当前只有用户反馈与静态代码初筛，尚无浏览器／API／数据库时延采样，不得把候选原因写成已证实瓶颈。
...
### 6.5 `POST-LIVE-PERF-01` · 全站导航与局部切换手感（R1 收集期）

2026-08-30 产品负责人将“多个页面点击后有明显卡顿感”登记为独立体验优化事项。它不表示路由、权限或业务功能缺失，不改变 R1-Live Gate 2；R1 期间先按真实使用持续记样本，Gate 2 后或产品负责人明确提前选择时再集中实现。若后续发现卡顿实际导致核心动作无法完成、重复写入或错误状态，则按 P0／核心 P1 另行提升，不继续留在体验池。

本事项不把 History API 当成通用修法。只有目标数据已经在客户端、动作只是切换本地选择或页码时，才允许只更新 URL；需要服务端鉴权、RLS 查询或真实对象切换的导航仍然经过 App Router。优化目标是消除重复工作、缩短关键路径并立即反馈，不是绕过数据和权限边界。

#### 首轮静态筛查

以下均为 E1 候选，不是性能采样结论。`await` 数量、`Link`／`router.push` 存在、Client Component 或 renderer `key` 单独都不能证明问题；每项先取得冷／热路径时间分解再决定是否改造。

| 编号 | 用户动作与静态证据 | 初步判断 | 集中优化前必须取得的证据／候选方向 |
| --- | --- | --- | --- |
...
| `PERF-UX-03` | `/studio/courseware/[lectureId]` 的普通、来源和空间课件仍用 `?page=` Link／`router.push` 翻页；每次会重新执行鉴权、workbench context、页 revision／binding 解析，来源或空间舞台也随路由替换 | 与讲次只读预览修复前同类，且编辑器还必须保留未保存离开保护 | 复用提交 `50a1648` 的“页级读取＋缓存＋相邻预取”思路，但先记录不同 docVersion 的 RSC、签名和 renderer 初始化时间，并为 dirty editor 单独设计切页边界 |
...
讲次只读预览已先完成一个独立开发增量：提交 `50a1648` 把页切换改为页级按需读取、生命周期缓存、相邻页预取和同 runtime iframe 复用，并批量解析教师微课资源 URL。该提交的机器检查只证明编码合同；它不证明全站卡顿已解决，也不替代产品负责人的实际手感验收。

#### R1 问题记录格式与集中优化触发点

每个新增样本记录：日期、commit／环境、角色、路由族、具体点击动作、冷／热状态、可见数据规模、是否立即出现选中／loading、RSC／API／DB／renderer 时间、缓存命中情况、影响程度和可复现次数。共享文档不记录本机地址、账号、姓名、token 或业务 PII。

收集期只接受三类小改动：让等待状态可见、修复已确认的重复请求／无限扇出、以及产品负责人明确点名的局部性能增量。集中批次先按用户频率 × 等待时长 × 共享杠杆排序；同一原因覆盖三个以上路由时建立共享原语，否则保留局部实现。缓存命中时应在同一交互帧更新选择且不发重复请求；未命中时先立即反馈，再以本节采样结果和 doc 25 `PERF-03` 决定具体时延门槛。
