...
> **提出日期**：2026-07-20。  
> **适用范围**：员工工作台、课程研发、课程产品、讲次课件、班级、课次、课表、备课、课后闭环及其与学服/家庭摘要的数据接缝。
...
- 员工首页将学生、课表、财务、课程、课评、视频和学服任务全部放进可自由排布的磁贴池；
...
- 未来 7 天使用但无 release；
- 4:3 适配待校对。
- 4:3 人工退回待修：仅当前 binding 仍选中退回候选时生成讲次工作项，主动作进入 4:3 修复/重提；
- 4:3 系统已替代：只进历史审计，不生成工作项、不阻塞 release。

> **P6 实施脚注（2026-07-26）**：上述两类已在 `/dashboard/adapt-review` 的“退回待修 / 历史审计”页签落地；裁切修复以新 CAS revision + successor 候选重提，原审核决定不可改。当前真实数据仅有 21 条系统已替代历史，人工退回待修为 0。
...
建班与正式班准备度以每讲存在 16:9 原生 release 为基础条件；4:3 是可选增强轨，没有 4:3 release 不得阻止选课、创建 planning 班或使用 16:9 默认轨。
...
主管、教务和学服可以审阅必要状态，不获得实时频道或课堂资产权限。
...
`session_family_briefs` 是家庭、学生和学服共同可见的信息边界：
...
扩展 `classroom_staff_assignments` 以表达主责学服，或增加独立唯一约束。
