...
通用表格强调（2026-09-15 产品反馈）：通用表格与学服工作台共享浅黄悬停、选中及暖金轮廓。`--table-hover`、`--table-selected` 分别复用 `--followup-hover`、`--followup-checked`，选中行侧边使用 `--followup-outline`；浅色、显式深色和系统深色三个通道保持同一语义。蓝色继续服务已定义的业务色与画布内容，不用于通用表格选中态。
