...
数据表表面与连续登记（2026-09-06 产品反馈）：浅色模式的表格、表头及普通行统一使用不透明白色 `--card`；深色模式使用星夜 `--card`。选中／展开行及其详情区统一使用 `--table-selected`，固定列继承整行颜色，保持连续高亮；悬停使用更轻的 `--table-hover`。两种主题分别从 `--blue` 与 `--card` 派生反馈色。Dashboard 数据工作区关闭全页纸纹覆盖，公开页面保留纸感。登记按钮与选择器完整显示标签，空间不足时换行；详情按实际容器宽度安排主操作与备注栏，笔记本宽度优先横排类型及圆点进度，宽工作区再展开左侧类型栏。
