...
> **规划状态**：`partial`
>
> **当前用途**：P4F 导航、动作反馈、会话与层级体验规范。
>
> **已落地**：P4F-0/0b/2/3/4/9 等已有代码/提交证据。
>
> **剩余项**：发布必要的配置、反馈与一致性缺口并入 doc 25；§6.5 仅供相关历史 UI 债务任务读取。
>
> **最后核对**：2026-07-28。
...
**现状证据**：全站搜 `unread/badge/lastSeen/resume/未读/待办徽标` 实质命中 **0**。没有红点、没有"距上次登录 N 条新消息"、没有"2 份作业待批改"角标、没有"继续上次"。用户被迫成为轮询循环：家长要自己翻档案/财务/作业页打捞"孩子这周有无新课评、有无该交的费"；学服要挨页查"有无新分派、哪条跟进到期"。看板本应"打开就知道有什么需要我"，现在只呈现静态现状。
...
**验收**：登录后铃铛显示"我不在时发生的、与我有关的变化"，逐条可直达；已读后角标清零；家长/学服无需逐页打捞即知"要我处理什么"。
...
**验收**：键盘可一步跳过导航到正文；所有弹层焦点受困且 Esc 可关；读屏文案随 locale 切换；基线清单全过（清单见 `docs/a11y-baseline.md`）。
