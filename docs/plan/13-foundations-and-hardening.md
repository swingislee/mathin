...
> **规划状态**：`complete`
>
> **当前用途**：P4E 横切地基、安全、审计、通知与可验证性历史记录。
>
> **权威边界**：地基阶段已关闭；旧 roadmap 的“通知延后”不再有效。
>
> **剩余项**：R1-2 已关闭 Job、站内通知、文件治理与外部集成安全关闭合同；R1-3 已关闭版本化同意、权利请求、账户门控、员工邀请和管理员支持合同。生产供应商、容量/指标、正式管理员、告警和恢复演练继续由 doc 25 的 R1-13/14/16/17/18 承接。
>
> **最后核对**：2026-07-28。
...
3. **学生码与家长码分离**：家长绑定不应与学生本人认领共用码。家长绑定改为"教师/学服在后台对该学生签发一次性家长邀请码 + 关系"，或家长自助申请后**教师点确认**才生效（见 §7.3 多监护人差异权限，两者合并设计）。
...
**论证——为什么通知被误伤进暂缓清单**：`04-roadmap` 长期暂缓"消息通知系统"，但那指的是社交向的评论/私信。学校运营的每个瞬间都是"某人需要被告知"：课次改时间（家长）、作业批改完（学生）、缴费到期（学服）、跟进超时（主管）、试听到场（销售）。现状 `grep notif` 零结果，全靠人肉刷后台。更致命的是架构后果：等要补通知/微信推送时，需翻改**所有**已写的 action 去插桩。
...
**修法**：教师/学服代建账号（手机号/用户名）+ 家长手机验证码登录。自托管 Supabase 接国内短信网关（GoTrue phone auth + SMS hook）。存量 email 账号双轨过渡——越晚做迁移越难受。**这是独立小期，工程量不小，建议排在 P4D 与 P5 之间。**
