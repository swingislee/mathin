...
>
> 继续保留、doc 19 直接复用、不重做的部分：§5 生命周期状态机（版本/讲次/班级/课次状态与转换 RPC）、§6 capability 模型（权限 = 岗位权限 × 对象关系 × 当前状态）、`course_staff_assignments`/assignment 表结构与继承规则、§2.3 安全不变量（禁止物理级联删除、学服不进 `classroom_members`、403≠404 等）、P4H-0～10 已落地的数据库 schema 与 RPC。
...
5. **禁止权限偷渡**：主管、教研、学服不能为了“能打开页面”被写进 `classroom_members`。该表继续只表达真实课堂成员。
...
| F8 | 教师、教研、学服、主管共享同一张数据库式列表，默认内容不随任务变化 | `courses/page.tsx`、`classes/page.tsx` | 用户第一屏看不到自己的下一步工作 |
...
- **班级（`classrooms`）**：学生、教师、学服、排课和进度的运营容器，引用一个课程版本。
...
4. 主管管理、教研研发、教师授课、学服客勤四种职责清晰分离；多人兼岗时用视角切换，不把所有动作混在一起。
...
- 不开发短信、微信、邮件实际发送集成；P4H 只提供学服通知任务与状态记录。
- 不做永久删除正式历史数据的普通 UI。
- 不重做学生、财务、活动模块。
- 不把所有后台详情改成多层嵌套路由。
...
| `support` | 本人为 learning_support | 学服 |
| `all` | 有 `class.view.all` 的全部班级 | 主管/教务 |
| `test` | 可管理且 `purpose='test'` | 主管/管理员 |

有合法 `scope` query 时使用该值；没有 query 时：仅管理者默认 `all`，仅学服默认 `support`，具备 teaching 关系者默认 `teaching`，其他情况按 `all` → `support` 回落。不得为“记住视角”新增数据库字段或 localStorage。
...
- 学服兼教师时可在 `support` 与 `teaching` 间切换。
...
| 状态 | 文案 | 允许的核心操作 |
| --- | --- | --- |
| `planning` | 筹备中 | 配课程、配教师/学服、排课、检查资源、启用 |
...
- 教师只有课程读取权时看“课程”，不看“制作工作台”；学服默认两者都不看。
...
每行固定字段：班级 + 正式/测试 Badge、课程产品 + 版本、主讲 + 学服、报名/容量、已上/总课次、下一课、readiness、异常徽章、与当前视角匹配的一个主动作。禁止用同一个“打开”链接承担所有角色。
...
- 主管默认 all，教师默认 teaching，学服默认 support；
- 多岗位可切换且 query 可分享；
- 主管课次打开管理抽屉，不进入课堂；
- 教师本课次出现备课/上课；
- support 只见负责班；
...
| 查看本人负责班 | 若有 support 关系 | 若有 support 关系 | 若兼学服 | 是 |
