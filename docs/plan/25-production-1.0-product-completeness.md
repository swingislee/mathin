...
>
> **基线日期**：2026-09-02；每次验收另记 commit、migration head、环境和数据 manifest。产品负责人已明确要求 `DEV-SCHOOL-OPS-1` 从 Phase 1 第一业务切片继续进入 Phase 2；生产运行基线仍来自 2026-08-30 的已核对证据。
>
...
- `DEV-CW-1` 当前为 `STEP 8A SOURCE-RUNTIME DRAFT PERSISTENCE / USER ACCEPTED`：Step 7C 三端视觉已确认，源码审计确认三端直接复用工作台、目录、翻页、编辑 chrome、元素检查器、图层、文字编辑器与 4:3 控制器；adapter 只隔离文档 schema、renderer 与 typed patch translator。Step 8A 已让爱学习编辑器读取并保存 native/adapted draft head，4:3 策略只进入 adapted 草稿；数据库拒绝来源包、fingerprint、binding、节点身份／顺序和未知 producer 字段变化，current/release 不推进。验收后的历史复审又统一了粗调／微调顶栏撤销与重做，自动保存不再清空本地历史。本机 migration 与事务回滚断言通过；尚未批量改写 5,508 页，也没有生产数据库、Storage 或 release 写入。
...
| 课程产品统一课件工作区 | M1（Step 0～8A 已确认） | M4 | doc 16 §14 已记录人工确认门；预览、正式课、微课共用工作台，4:3 草稿／资源替换／回滚及来源 runtime 编辑桥已通过人工验收。Step 7C 已完成三端视觉定稿与源码复用审计；Step 8A 已接通并验收爱学习双轨 typed-patch 草稿，粗调／微调历史统一由顶栏撤销／重做承载且自动保存不清空历史。扩量前仍需新增组件能力门、真实 inventory、全量 dry-run、失败隔离与回退证据 | DEV-CW-1/9 |
