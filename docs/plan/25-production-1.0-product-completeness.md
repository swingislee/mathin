...
- `DEV-CW-1` 当前为 `STEP 5B REPLACEMENT + EDIT HISTORY / DEV READY / AWAITING USER STAGING PREVIEW`：Step 5A 已获人工验收；产品负责人已指定本机 `white-board.png` 与“全部引用”。页面上下文与公共资源页复用同一 staging/apply/rollback 流程，暂存图进入当前画布预览；正式课与微课复用最多 20 个合并步骤的会话撤销/重做和快捷键，资源替换继续由持久化 batch ledger 回滚。当前尚未上传、应用或回滚素材，没有生产数据库/Storage/release、source-runtime、整讲或批量写入。
...
| 课程产品统一课件工作区 | M1（Step 0～5A 已确认；Step 5B 暂存预览待人工确认） | M4 | doc 16 §14 已记录人工确认门；三个工作区共用 `CoursewareWorkbench`，正式课与微课新增同一 20 步合并式撤销/重做内核。页面上下文替换与公共资源页复用 staging/apply/rollback、对比与 ledger；指定素材和“全部引用”尚未上传或应用，须先人工看暂存画面再执行和回滚。整体重构完成后、扩量前仍需双端定稿 | DEV-CW-1/9 |
