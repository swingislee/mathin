...
- `DEV-STAFF-ONBOARD-1` 当前为 `IN PROGRESS / LOCAL DEVELOPMENT ONLY`：员工页将复用现有 ImportBatch 与 email/phone 一次性员工邀请，提供版本化模板、逐行预检、整批原子应用、幂等重试、错误下载和仅一次展示的邀请凭据；预授岗位只在受邀人注册后落到同一 `auth.users.id`，且 `permission.configure` 岗位继续由顶层 admin 独占授权。该增量不直接创建 Auth 用户、不设置密码、不静默转换已有顾客身份，当前没有生产 migration、身份、邀请或业务写入。
