...
>
> **基线日期**：2026-09-01；每次验收另记 commit、migration head、环境和数据 manifest。2026-09-01 登记 `DEV-SCHOOL-OPS-1` Phase 0 开发端审阅门；生产运行基线仍来自 2026-08-30 的已核对证据。
>
...
- `DEV-SCHOOL-OPS-1` 当前为 `PHASE 0 / DEVELOPMENT READY / AWAITING PRODUCT OWNER LAN ACCEPTANCE`：只读运营主流程审阅页已接入现有导入、学生、跟进、活动、班级、课表与考勤工作面，并显示当前账号经 RLS 可见的业务数量；当前没有 migration、业务数据写入、Storage 或生产动作，产品负责人局域网确认前不进入 Phase 1。
- `DEV-CW-1` 当前为 `STEP 5B SINGLE LOCAL REPLACEMENT + ROLLBACK / TARGET INPUT REQUIRED`：产品负责人已于 2026-09-01 明确完成人工验收 Step 5A；正式 PageDoc 当前元素上下文中的页、讲、课程版本、课程族、全部引用五档只读影响清单、冻结／独立钉版提示、紧凑选择器、滚动边界与共享翻页键均已确认。Step 5B 只允许一个本机 PageDoc 样本的真实替换和回滚，须等待产品负责人指定替换图片与精确范围；当前没有新增文件上传、真实替换、rollback 写入、source-runtime patch、整讲回填、生产数据库/Storage/release 或批量任务。
...
| 课程产品统一课件工作区 | M1（Step 0～5A 已确认；Step 5B 等待单样本替换素材） | M4 | doc 16 §14 已记录人工确认门；预览、正式课编辑与微课编辑使用同一 `CoursewareWorkbench` 面板树、目录、舞台、翻页、元素检查器和图层面板。Step 5A 的页面上下文五档影响预览已获人工验收；Step 5B 只允许一个本机 PageDoc 样本的 staging/apply/rollback，素材和范围明确前不写入。整体重构完成后、扩量前仍需双端定稿组件表现 | DEV-CW-1/9 |
