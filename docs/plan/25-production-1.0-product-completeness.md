...
- `DEV-CW-1` 当前为 `STEP 2A REVISION READY FOR USER AUDIT`：Step 1/2 已获产品负责人确认；第三轮 Step 2A 审计进一步确认不能只共享外壳、按钮原语或 Portal 槽。开发端现由一个 `CoursewareEditorWorkbench` 直接负责页面目录、插入顶栏、画布/翻页、右栏标题和右栏滚动区，两条编辑链共用 `CoursewareInsertionToolbar`；正式轨道内重复标题已退出，正式 adapter 只通过 `capabilities.adapt4x3` 比微课 adapter 多开 4:3 适配。只读微课继续复用成熟预览与键盘翻页，composition 继续保留实际插入 action、左栏重命名和 800ms 自动保存，正式 PageDoc/source-runtime 仍无写入。该结果只有开发端 TypeScript、ESLint 和定向合同证据，尚无 Step 2A 视觉与微课回归确认；当前没有新增 editor action、schema、开发库/Storage 或生产写入授权，Step 3 继续关闭。
...
| 课程产品统一课件工作区 | M1（Step 1/2 已确认；Step 2A 修订待审） | M4 | doc 16 §14 已记录人工确认门；正式课件与教师微课由同一个 `CoursewareEditorWorkbench` 渲染五个完整工作区区域，并共用 `CoursewareInsertionToolbar`，只读微课另复用成熟 `CoursewarePreviewWorkspace`。微课 adapter 是共同功能基线，正式 adapter 只额外开启 4:3 适配；PageDoc/source-runtime/composition 仍分别承载来源限制、12×9 网格和写态。尚未新增 editor action、schema、持久化或来源 patch；Step 3 在共享框架人工确认前保持关闭 | DEV-CW-1/9 |
