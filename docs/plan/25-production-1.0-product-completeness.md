...
| Jobs/通知/文件/集成 | M3 | M4 | R1-2 已完成 durable job/dead-letter/重放、第一方通知、TUS/文件策略、webhook 防重放与开发环境验证；`DEV-WEB-PUSH-1 / PUSH-P5` 已完成生产关闭态底座和独立 postflight。2026-09-09 全体在职员工测试已获产品授权，实际启用仍待剩余技术检查，生产开关与 Worker 保持关闭。权威授权、安全与 Gate 见 [`employee-desktop-web-push.md`](employee-desktop-web-push.md)。M4 仍需 `PUSH-G5`、生产 Worker 激活、选中并验收的外部渠道、容量、告警恢复和 14 天 RC | 2/14/16/17/DEV-WEB-PUSH-1 |
...
| 通知 | 站内通知为硬门；员工 Web Push 的 `DEV-WEB-PUSH-1 / PUSH-P5` 生产暗部署已完成。2026-09-09 已获全体在职员工测试授权；专题 `PUSH-G5` 剩余技术检查与受控启用完成前，生产继续保持关闭。独立告警 SMTP 的选定不启用业务邮件渠道；未验收的邮件、SMS、微信和 Webhook 业务渠道继续关闭 | 平台+运维负责人 | R1-2/DEV-WEB-PUSH-1 | P5 deployed, fail-closed；employee test authorized, activation pending |
