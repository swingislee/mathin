...
- `DEV-CW-1` 当前为 `STEP 5A PAGE-CONTEXT REPLACEMENT DRY-RUN / IN PROGRESS`：产品负责人已于 2026-09-01 确认 Step 4B 的旧 A～F 映射、一个 PageDoc 样本页的 4:3 草稿保存、刷新重读、资源显示和轨道隔离。当前只继续读取一个背景／图片资源在页、讲、课程版本、课程族和全部引用五种范围的命中清单与冻结引用提示；没有授权文件上传、真实替换、rollback 写入、source-runtime patch、整讲回填、生产数据库/Storage/release 或批量任务。
...
| 课程产品统一课件工作区 | M1（Step 0～4B 已确认；Step 5A 只读 dry-run 施工中） | M4 | doc 16 §14 已记录人工确认门；预览、正式课编辑与微课编辑使用同一 `CoursewareWorkbench` 面板树、目录、舞台、翻页、元素检查器和图层面板。Step 4B 已确认旧分类映射、单页 `adapted-4x3` 草稿保存、刷新重读与轨道隔离；Step 5A 只展示页面上下文资源替换的五档影响清单和冻结提示，不执行写入；整体重构完成后、扩量前仍需双端定稿组件表现 | DEV-CW-1/9 |
