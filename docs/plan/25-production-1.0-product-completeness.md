...
- Gate 1 当前 `PASS`：Xiaomi 目标指纹、危险写拒绝、唯一正式 admin MFA、首名真实教师及双岗位、active 8 条 protected/0 条 purge manifest、current/previous、健康探针和错误查询位置均已确认。生产数据库 ledger 为 220 条、head=`20260829000200_admin_self_staff_roles`；应用 current/previous=`20260829-031327` / `bf81aa4…` 与 `20260828-195733` / `ba98a8e…`。产品负责人已确认 Stage B1/B2，生产 checkpoint version/chunk/head=`2/2/2`；Stage B3 共同 bridge、权威档案表与单一 Smart 开关已部署，但 H5 保持 version 3 / false，空档案表不授权任何现有 package。board/input/layout 为 version 2 / true，star/learning result 仍为 0。最新 PostgreSQL+Storage 同批次全量备份 `mathin-20260828T052842Z-teacher-microcourse-variant-087b497` 与最近 PostgreSQL 写前备份 `mathin-db-prechange-20260828T195236Z-admin-self-role-ba98a8e` 均已验证；最新教研课件入口 hotfix 是 app-only 发布，没有 schema、业务或 Storage 写入，未执行 restore 或 prune。
...
| 部署/备份/恢复 | M2（生产发布/当前备份） | M4 | commit `35b9f60` 与[部署子门](../evidence/r1/r1-16-deployment-preflight.md)已固定 fail-closed 合同；仓库/历史 secret scan 已关闭。2026-08-29 教研课件入口 app-only hotfix 后 current/previous=`20260829-031327` / `bf81aa4…` 与 `20260828-195733` / `ba98a8e…`，原子切换及健康门通过；数据库 ledger=`220`、head=`20260829000200_admin_self_staff_roles`。最近 PostgreSQL-only 写前备份绑定上一个 schema 候选，既有 PostgreSQL+Storage 同批次全量备份继续作为 Storage 回退基线；本轮没有 schema、业务或 Storage 写入，previous 是应用回退点。恢复演练、异机/静态加密备份和错误 release 标签属于 M4 | Live/16 |
