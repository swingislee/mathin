...
- `DEV-CW-1` 当前为 `STEP 1 READY FOR USER RE-AUDIT`：首轮人工审计要求课程主操作与筛选同排、4:3 缺失时保留 16:9 并提示、各轨课件完整适配画布，开发端已完成三项修订。该结果只有开发端 TypeScript、ESLint、双语和定向合同证据，尚无产品布局确认；当前没有 editor action、schema、开发库/Storage 或生产写入授权，Step 2 继续关闭。
...
| 课程产品统一课件工作区 | M1（Step 1 只读路由/布局已按首轮意见修订，产品复核 pending） | M4 | doc 16 §14 已记录 Step 0～8 人工确认门；课程主操作进入版本筛选行，4:3 缺失时保留 16:9 并提示，单轨舞台在可用宽高内完整显示。尚未接 editor action、schema、持久化或来源 patch；Step 2 在产品确认布局前保持关闭 | DEV-CW-1/9 |
