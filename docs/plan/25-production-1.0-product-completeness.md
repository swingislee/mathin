...
- `DEV-CW-1` 当前为 `STEP 5B COARSE-TO-FINE 4:3 / DEV READY / AWAITING USER WORKFLOW AUDIT`：Step 5A 已获人工验收；正式课与微课共用最多 20 个合并步骤的会话撤销/重做，页面上下文与公共资源页共用 staging/apply/rollback。没有独立适配页头时，对照区现在可从 native 基线首次保存并原子建立 4:3 页头和 binding；4:3 标签随后装载真实草稿，在最终画布暂存背景并微调元素。固定第 8 页已确认命中原缺口，本机 migration 与事务回滚集成断言已通过；尚未替真实页创建草稿或上传、应用、回滚素材，没有生产数据库/Storage/release、source-runtime、整讲或批量写入。
...
| 课程产品统一课件工作区 | M1（Step 0～5A 已确认；Step 5B 工作流待人工复核） | M4 | doc 16 §14 已记录人工确认门；三个工作区共用 `CoursewareWorkbench`，正式课与微课共用 20 步撤销/重做，页面上下文替换与公共资源页共用 staging/apply/rollback。对照区首次粗适配保存可建立 4:3 草稿，4:3 画布可替换／微调；当前等待产品负责人沿完整单页链复核后再执行指定素材的 apply 与 ledger rollback。整体重构完成后、扩量前仍需双端定稿 | DEV-CW-1/9 |
