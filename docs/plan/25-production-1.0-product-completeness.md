...
- Gate 1 当前 `PASS`：Xiaomi 目标指纹、危险写拒绝、唯一正式 admin MFA、首名真实教师及双岗位、active 8 条 protected/0 条 purge manifest、current/previous、健康探针和错误查询位置均已确认。生产数据库 ledger 为 219 条、head=`20260829000100_classroom_personal_scope_default`；应用 current/previous=`20260828-174731` / `34f07e8…` 与 `20260828-075322` / `c7c8219…`。产品负责人已确认 Stage B1/B2，生产 checkpoint version/chunk/head=`2/2/2`；Stage B3 共同 bridge、权威档案表与单一 Smart 开关已部署，但 H5 保持 version 3 / false，空档案表不授权任何现有 package。board/input/layout 为 version 2 / true，star/learning result 仍为 0。最新 PostgreSQL+Storage 同批次全量备份 `mathin-20260828T052842Z-teacher-microcourse-variant-087b497` 与本轮 PostgreSQL 写前备份 `mathin-db-prechange-20260828T173725Z-dev-org-34f07e8` 均已验证；未执行 restore 或 prune。
...
| 部署/备份/恢复 | M2（生产发布/当前备份） | M4 | commit `35b9f60` 与[部署子门](../evidence/r1/r1-16-deployment-preflight.md)已固定 fail-closed 合同；仓库/历史 secret scan 已关闭。2026-08-29 DEV-ORG/DEV-DASH 发布后 current/previous=`20260828-174731` / `34f07e8…` 与 `20260828-075322` / `c7c8219…`，原子切换及健康门通过；数据库 ledger=`219`、head=`20260829000100_classroom_personal_scope_default`。PostgreSQL+Storage 同批次全量备份继续作为 Storage 回退基线，本轮 PostgreSQL 写前备份已通过 dump TOC、前后计数与 SHA-256；7 个 migration 已完成事务回滚/零残留演练，旧字段和 RPC 保证 previous 兼容。恢复演练、异机/静态加密备份和错误 release 标签属于 M4 | Live/16 |
