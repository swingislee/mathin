...
- `DEV-CW-1` 当前为 `STEP 7B REVIEW SEMANTICS AND LEGACY RETIREMENT / USER ACCEPTED`：提交 `a4f051e` 已交付唯一教研审核入口，分别投影正式课修改与教师微课入库；备课资料审核从独立教研页退出并回到课次对象，由“我的待办”打开该对象。旧 Studio、旧适配校对／发布界面和孤立微课列表已删除。提交 `de72449` 新增退役 migration，移除公开直发与批量适配发布 RPC，历史对象与审计数据保留。此前 4:3 单页草稿、共享资源替换、回滚和 source runtime 编辑桥的验收结论继续有效；产品负责人于 2026-09-02 明确回复“完成”。本批没有生产数据库、Storage、release 或服务写入，Step 8 扩量与生产候选仍待单独授权。
...
| 课程产品统一课件工作区 | M1（Step 0～7B 已确认） | M4 | doc 16 §14 已记录人工确认门；预览、正式课、微课共用工作台，4:3 草稿／资源替换／回滚及来源 runtime 编辑桥已通过人工验收。Step 7B 已删除未投产旧 Studio 与适配发布链，并将教研审核收敛为正式课修改＋教师微课入库；备课资料审核回归课次教学履约。扩量前仍需双端组件定稿、真实 inventory、全量 dry-run、失败隔离与回退证据 | DEV-CW-1/9 |
