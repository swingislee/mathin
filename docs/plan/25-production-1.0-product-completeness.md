...
- Gate 1 当前 `PASS`：Xiaomi 目标指纹、危险写拒绝、唯一正式 admin MFA、首名真实教师及双岗位、active 8 条 protected/0 条 purge manifest、current/previous、健康探针和错误查询位置均已确认。生产数据库 ledger 为 206 条、head=`20260827000700_teacher_microcourse_course_catalog_access`；应用 current/previous=`20260827-094025` / `bc76f68…` 与 `20260826-125052` / `964ca5e…`。产品负责人已确认 Stage B1/B2，生产 checkpoint version/chunk/head=`2/2/2`；Stage B3 共同 bridge、权威档案表与单一 Smart 开关已部署，但 H5 保持 version 3 / false，空档案表不授权任何现有 package。board/input/layout 为 version 2 / true，star/learning result 仍为 0。教师微课 PostgreSQL-only pre-change 备份 `mathin-db-prechange-20260827T092348Z-teacher-microcourse-bc76f68` 已核验；最近 PostgreSQL+Storage 同批次备份仍为 `mathin-20260822T093529Z`，两者证据边界不互相替代。
...
- 普通教师短期微课开发轨 `DEV-TMC-1` 已在本机实现、完成机器检查并获准部署生产：一份自由课次惰性孵化一门单讲 `microcourse`，作者可用草稿试讲，校内共享须经现有 Courseware review/release 机制审核发布；其他具备建班权限的校内 staff 只从已启用且存在正式 release 的目录搜索建班。来源只接受正式 `curriculum` 当前 release；入口复用课程产品 `CoursePicker`，先按课程族/版本、年级、课程季节、班型和历史版本选课程，再只列课次名称与页数，一次事务插入本讲全部页，编辑器内再删选与排序，不加载页面预览。通用 `game-page-v1`、游戏内容注册表、服务端校验凭证与来源 capability predicate 接入四宫/六宫/九宫数独，后续游戏内容版本不修改微课审核、冻结或来源选择器。元数据、页面、资源、游戏校验事实与 H5 hash 在提交时共同冻结，原自由课次和历史课次保持原 release。本机 CI 16/16、全量 Vitest 106 文件 742 通过/1 条件跳过、微课 Vitest 4 文件 21/21、固定账号 Playwright 1/1 和 production build 通过；生产 12 个 migration 经 backup/rollback/零残留/formal 推进到 head=`20260827000700_teacher_microcourse_course_catalog_access`，release=`20260827-094025` / `bc76f68…`，`teaching.teacher_microcourses_v1`=version 2 / true。HTTP、真实作者只读目录权限、服务、错误和既有业务/Storage 计数 postflight 通过；产品负责人生产写态验收仍为 `UNKNOWN`。
- `DEV-TMC-2` 已在开发端实现：自由课次允许任课教师与教研直接建立并行课件方案，编辑他人方案时派生新 head，主讲在开课前选择本节使用方案；课次使用不经过审核，校内共享发布仍沿用 DEV-TMC-1 审核/release。本机两条 migration、回滚数据库断言、TypeScript、双语键、全库 ESLint、定向 Vitest 和固定账号 Playwright 新旧旅程通过；产品负责人页面验收仍为 `UNKNOWN`，且没有新的生产 schema、应用或开关授权，不改变上条 DEV-TMC-1 的生产事实。
...
| 部署/备份/恢复 | M2（生产发布/当前备份） | M4 | commit `35b9f60` 与[部署子门](../evidence/r1/r1-16-deployment-preflight.md)已固定 fail-closed 合同；仓库/历史 secret scan 已关闭。2026-08-27 教师微课授权发布后 current/previous=`20260827-094025` / `bc76f68…` 与 `20260826-125052` / `964ca5e…`，原子切换及内外健康门通过；数据库 ledger=`206`、head=`20260827000700_teacher_microcourse_course_catalog_access`。当前 PostgreSQL+Storage 同批次备份已独立通过 TOC、文件数、源前后清单与 SHA-256，课堂 Stage B3 与教师微课发布各有新鲜 PostgreSQL-only pre-change 备份；previous 兼容回退、恢复演练、异机/静态加密备份和错误 release 标签属于 M4 | Live/16 |
