...
| Vitest | 当前为 88 个测试文件、567/567 全绿，其中非 spatial 基线 51 个文件、290/290，空间数学/SML-0 专项 37 个文件、277/277；`pnpm r1:test` 既有基线为 18 个文件、121/121 | 单元/合同套件保持全绿；SML-0 与 R1-14 后续实现均须维持 100%，并补发布目标、并发、文件和竞争矩阵 |
...
| Vitest 基线需在最终 build 复验 | 当前为 88 个测试文件、567/567 全绿，非 spatial 290/290，空间数学/SML-0 专项 277/277；R1 定向既有基线 121/121 | 后续实现可能重新引入合同回归 | 技术+QA | SML-0 与 R1-14 保持全量 100% |
