...
- `DEV-SCHOOL-OPS-1` 当前为 `PHASE 1 / IN DEVELOPMENT`：Phase 0 模型核对只保留在规划与 Agent 对话；错误新增的规划型 `/dashboard/school-ops` 页面、导航与快照已经删除。首个开发验收门是可真实导入、填写、分配和跟进的数据收件箱与线索工作台；当前没有生产 migration、业务数据写入、Storage 或发布动作。
