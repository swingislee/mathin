...
- `DEV-CW-1` 当前为 `STEP 1 READY FOR USER RE-AUDIT`：首轮的主操作同排、缺轨降级提示和画布完整适配已保留；第二轮又下移三点菜单，修正“对照”缺轨空态，恢复爱学习 `source-master` 直接 4:3／兼容页顶置，并移除两处冗余栏位。本机开发库 5,508 页旧模式只读比对为 0 差异。该结果只有开发端 TypeScript、ESLint、双语和定向合同证据，尚无产品布局确认；当前没有 editor action、schema、开发库/Storage 或生产写入授权，Step 2 继续关闭。
...
| 课程产品统一课件工作区 | M1（Step 1 只读路由/布局已按两轮意见修订，产品复核 pending） | M4 | doc 16 §14 已记录 Step 0～8 人工确认门；主操作与三点菜单进入版本筛选命令行，“对照”缺轨保留双栏空态，单轨舞台完整适配可用宽高；爱学习普通源母版恢复直接 4:3，动画/H5/原生题型/宽画布继续顶部兼容。尚未接 editor action、schema、持久化或来源 patch；Step 2 在产品确认布局前保持关闭 | DEV-CW-1/9 |
