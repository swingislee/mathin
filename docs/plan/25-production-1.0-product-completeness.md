...
正式课堂逐生检查面板在全屏模式把题目、进度、批量入口收进首行，第二行只保留横向滚动的检查项，正文按最小 11rem 卡宽自适应为 iPad 横屏 5×4、竖屏 4×5 的 20 人首屏目标。学生卡采用座位状态键盘：手柄与座位号共用身份区，移除重复“已登记”徽标，六个登记状态以两字短标签和颜色编码呈现，完整文案保留为无障碍名称，每个触控区继续保持至少 44px 高；选中状态同时使用实色按键与卡顶色条，未检查卡保持虚线灰态。学生卡按从左到右、从上到下显示座位序号，教师只从姓名旁手柄拖拽，排序写入班级级 `classroom_student_seat_order` 并沿用到后续课次；新入班学生追加在既有顺序之后，保存时名单变化则整体拒绝并要求重新打开面板。
