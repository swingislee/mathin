...
- `DEV-TMC-4` 已由产品负责人选入开发轨，当前为 `LOCAL IMPLEMENTATION IN PROGRESS / NOT DEPLOYED`。doc 29 固定教师微课课程族的新合同：一门课可以关联多个 766 使用场景，年级/学期/班型三个维度零选择分别表示通用，主页面使用目录树＋每课一行表格＋当前默认版本轻量预览，维护方向与提交历史下钻展示。现有课次方案、review/release 和课堂冻结继续复用；本轮不执行生产 schema、历史归并、开关或发布，也不以代码检查替代产品负责人本地页面验收。
