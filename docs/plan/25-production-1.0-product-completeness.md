...
- `DEV-CW-1` 当前为 `STEP 5A PAGE-CONTEXT REPLACEMENT DRY-RUN / DEV READY / AWAITING USER AUDIT`：产品负责人已于 2026-09-01 确认 Step 4B。正式 PageDoc 的“替换”右栏现复用公共资源使用树的同一只读数据，展示页、讲、课程版本、课程族和全部引用五档命中清单、冻结次数与独立钉版；固定样本公共背景计数为 1/1/13/26/26。没有文件上传、真实替换、rollback 写入、source-runtime patch、整讲回填、生产数据库/Storage/release 或批量任务。
...
| 课程产品统一课件工作区 | M1（Step 0～4B 已确认；Step 5A 开发端待人工审计） | M4 | doc 16 §14 已记录人工确认门；预览、正式课编辑与微课编辑使用同一 `CoursewareWorkbench` 面板树、目录、舞台、翻页、元素检查器和图层面板。Step 5A 已把公共资源同一读模型接到正式 PageDoc 当前元素上下文，只展示五档影响清单和冻结提示，不执行写入；整体重构完成后、扩量前仍需双端定稿组件表现 | DEV-CW-1/9 |
