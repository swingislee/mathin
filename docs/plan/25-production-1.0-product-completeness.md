...
- `DEV-SCHOOL-OPS-1` 当前为 `PHASE 2 / DEVELOPMENT READY / AWAITING PRODUCT OWNER UI AND INTERACTION ACCEPTANCE`：提交 `207d6e5` 已交付真实活动列表／漏斗、单次活动参与与测评工作台、销售机会队列；本机 migration `20260902000200_school_ops_phase2_activity_funnel` 新增 AssessmentResult／Opportunity、RLS、到场前置、测评前置和下一动作投影合同。本地使用明确标记的“Phase 2 验收 · 1v1 测评”及“Phase 2 验收学生”作为可删除验收数据；这不改变 R1-Live 生产基线，当前没有生产 migration、业务数据写入、Storage 或发布动作。二进制 `.xlsx`、逐行合并决定与 Family／Contact／Lead 分表仍未施工并保留为后续项。
