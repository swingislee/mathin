...
- `DEV-CW-1` 当前为 `STEP 7B REVIEW SEMANTICS AND LEGACY RETIREMENT / IN PROGRESS`：Step 0～6 已获人工验收，Step 7A 的“仅拆分两个审核页面并保留全部旧入口”经审计退回。当前在隔离开发目标把正式课修改审核与教师微课入库审核收敛为教研审核工作区，把备课资料审核归回课次教学履约并投影到“我的待办”；尚未正式投产的旧 Studio、旧适配校对工作台、公开直发与批量适配发布 RPC 退出，历史对象与审计数据保留。此前 4:3 单页草稿、共享资源替换、回滚和 source runtime 编辑桥的验收结论继续有效；本批没有生产数据库、Storage、release 或服务写入。
...
| 课程产品统一课件工作区 | M1（Step 0～6 已确认；Step 7B 施工中） | M4 | doc 16 §14 已记录人工确认门；预览、正式课、微课共用工作台，4:3 草稿／资源替换／回滚及来源 runtime 编辑桥已通过人工验收。当前删除未投产旧 Studio 与适配发布链，并将教研审核收敛为正式课修改＋教师微课入库；备课资料审核回归课次教学履约。完成后、扩量前仍需双端组件定稿 | DEV-CW-1/9 |
