...
- `DEV-TMC-4` 当前为 `LOCAL DEVELOPMENT DELIVERED / AWAITING PRODUCT ACCEPTANCE / NOT DEPLOYED`。doc 29 的五个 Phase 已在本机交付：一门课可关联多个 766 使用场景，年级/学期/班型三个维度零选择分别表示通用，主页面使用目录树＋每课一行表格＋当前默认版本按需预览，维护方向与提交历史下钻展示；主浏览、课程详情、课次编辑、审核队列/详情和场景设置已统一为连续 Dashboard 工作区，具备审核权限时审核根路由默认进入教师微课队列。本机 v2 开关已启用供产品负责人验收。教师创作边界只保留 `courseware-composition-v1` 和一个 composition 编辑器；本地先将 27 条旧 Sudoku revision 升级为 composition 内嵌 Sudoku v2，再将 72 条旧 composition envelope 原位升级，当前教师 revision 为有效 composition `163/163`、旧 envelope 为 0。生产迁移未获授权。现有课次方案、review/release 和课堂冻结继续复用；本轮不执行生产 schema、历史归并、开关或发布，机器检查不替代产品负责人本地页面验收。
