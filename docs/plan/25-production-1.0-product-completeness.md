...
- `DEV-STAFF-ONBOARD-1` 当前为 `DEVELOPMENT READY / AWAITING PRODUCT OWNER UI AND INTERACTION ACCEPTANCE`：本机 migration `20260902000700_staff_bulk_onboarding` 已让员工页复用现有 ImportBatch 与 email/phone 一次性员工邀请，提供版本化模板、逐行预检、整批原子应用、幂等重试、错误下载，以及应用后自动下载且只显示一次的邀请凭据；预授岗位只在受邀人注册后落到同一 `auth.users.id`，且 `permission.configure` 岗位继续由顶层 admin 独占授权。定向 Vitest 4/4、受影响 ESLint、TypeScript、双语键审计、本机 migration+事务回滚断言和零业务数据 postflight 通过。该增量不直接创建 Auth 用户、不设置密码、不静默转换已有顾客身份；当前没有生产 migration、身份、邀请或业务写入，页面与交接操作仍待产品负责人人工验收。
