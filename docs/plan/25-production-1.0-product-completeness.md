...
- `DEV-CW-1` 当前为 `STEP 1 READY FOR USER AUDIT`：课程产品预览可从当前页直接进入只读统一课件工作区，页面提供目录、16:9/4:3/并排画布、release/页面身份和后续能力位置。该结果只有开发端 TypeScript、ESLint、双语和定向合同证据，尚无产品布局确认；当前没有 editor action、schema、开发库/Storage 或生产写入授权，Step 2 继续关闭。
...
| 课程产品统一课件工作区 | M1（Step 1 只读路由/布局已开发，产品验收 pending） | M4 | doc 16 §14 已记录 Step 0～8 人工确认门；当前课程预览可从准确页面直达连续三栏工作区，并读取两条 immutable release 做 16:9/4:3/并排预览。尚未接 editor action、schema、持久化或来源 patch；Step 2 在产品确认布局前保持关闭 | DEV-CW-1/9 |
