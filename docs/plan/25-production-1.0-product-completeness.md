...
- 课堂可扩展互动在开发轨使用版本化 capability provider 和统一 conformance；普通 renderer 复用既有输入原语时不逐个重复人工 Gate。新增原语、输入路由/所有权算法、跨 iframe 协议和真实设备回归仍单独人工验收；未声明或不匹配 provider 的 renderer 继续 fail closed。M3a/M3b 已在 commit `00f4766` / `61c6c70` 完成开发端产品初验，M4a 的稳定身份、星星 v2 set/undo 和确认式名单 revision 三项随后通过；2026-08-25 产品负责人又以 `95ed9f1` 为应用基线确认 M4b 教师布局与逐题学情通过，因此 M0–M4 开发端整体关闭并进入 M5 生产候选 Gate。相关数据库 migration 曾被未授权提前应用且所有开关关闭，生产应用仍未包含本轮完整实现；开发验收不得记为课堂输入、H5 pointer、M4 或 Gate 2 已在生产通过。
