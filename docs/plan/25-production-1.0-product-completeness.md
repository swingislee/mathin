...
- `DEV-CW-1` 当前为 `STEP 3 READY FOR USER RE-AUDIT`：产品负责人于 2026-08-31 确认 Step 2A 通过；一个 `CoursewareEditorWorkbench` 和 `CoursewareInsertionToolbar` 承担两条编辑链，微课为共同能力基线，正式课只增加 `adapt4x3`。统一组件的双端最终表现推迟到 Step 7 完成整体工作区重构后、Step 8 扩量前再审。Step 3 首轮人工审计发现编辑画布误播放来源自动动画且没有顶栏自动保存反馈，第二轮确认完全跳过动画会隐藏标题；开发端现以 `settleAuto` 直接落到自动步骤结束态，保留标题但不播放位移/音频，另抽出两端共用的 `CoursewareEditorSaveControls`，并为固定 PageDoc 样本接入 800ms 自动保存、立即保存和脏态离页保护。仍待产品负责人刷新重读和核对 release；爱学习、整讲、跨页替换、批量回填、生产数据库/Storage/release 均不在授权范围。
...
| 课程产品统一课件工作区 | M1（Step 0～2A 已确认；Step 3 待复审） | M4 | doc 16 §14 已记录人工确认门；正式课件与教师微课共用完整 `CoursewareEditorWorkbench`、`CoursewareInsertionToolbar` 和保存状态控件，正式 adapter 只额外开启 4:3。Step 3 只为本机 E 系列第 5 页交付 PageDoc 草稿纵切；PageDoc/source-runtime/composition 仍分别承载来源限制、12×9 网格和写态。整体重构完成后、扩量前再双端定稿组件表现 | DEV-CW-1/9 |
