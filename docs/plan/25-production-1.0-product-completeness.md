...
- `DEV-CW-1` 当前为 `STEP 3B SHARED ELEMENT SYSTEM / READY FOR USER AUDIT`：产品负责人已于 2026-09-01 验收统一 `CoursewareWorkbench` 三种模式、共用目录、正式 PageDoc 自动保存、标题保存和旧样机删除。Step 3B 首轮图片表单之后的返修又把文字／图片／形状节点分发、公共透明度／层级／显隐和可折叠图层列表下沉为唯一共享组件；正式课与微课只提供各自文档 adapter。正式课仍只推进 PageDoc draft revision，未接文件上传、binding 改写或共享资源替换。4:3 A～F、爱学习写入、整讲、跨页替换、schema/RPC、批量回填、生产数据库/Storage/release 均不在授权范围。
...
| 课程产品统一课件工作区 | M1（Step 0～3A 已确认；Step 3B 共享元素系统待人工验收） | M4 | doc 16 §14 已记录人工确认门；预览、正式课编辑与微课编辑使用同一 `CoursewareWorkbench` 面板树、目录、舞台、翻页和保存反馈，正式模式只额外开启 4:3。当前文字／图片／形状由唯一共享元素检查器分发，完整元素列表使用同一可折叠图层面板，正式 PageDoc 单页保存仍只推进 draft revision；整体重构完成后、扩量前仍需双端定稿组件表现 | DEV-CW-1/9 |
