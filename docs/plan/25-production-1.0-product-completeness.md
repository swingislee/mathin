...
- Gate 1 当前 `PASS`：Xiaomi 目标指纹、危险写拒绝、唯一正式 admin MFA、首名真实教师及双岗位、active 8 条 protected/0 条 purge manifest、current/previous、健康探针和错误查询位置均已确认。生产数据库 head 现为 `20260825000500_classroom_roster_rpc_schema_reload`：2026-08-25 执行者误把 Xiaomi 当作开发库，未经生产授权提前应用课堂输入/H5 开关及 M4a roster/star v2 六个 migration；应用仍为 current `20260823-123746` / `9bc9ff3…`、previous `20260823-122633` / `ba5c991…`。只读审计确认三项新开关均为 false、v2 星星事件与 production 名单 revision 均为 0，唯一回填对象为 `purpose=test` 开放课次且名单条目为 0；该事件不构成 M3/M4 生产发布或验收。当前最近登记的 PostgreSQL+Storage 同批次备份仍为 `mathin-20260822T093529Z`，其数据库目录、Storage 文件数、前后清单和全部 SHA-256 已独立复核通过。
...
- 课堂可扩展互动在开发轨使用版本化 capability provider 和统一 conformance；普通 renderer 复用既有输入原语时不逐个重复人工 Gate。新增原语、输入路由/所有权算法、跨 iframe 协议和真实设备回归仍单独人工验收；未声明或不匹配 provider 的 renderer 继续 fail closed。M3a/M3b 已在 commit `00f4766` / `61c6c70` 完成开发端产品初验；M4a 的稳定身份、星星 v2 set/undo 和确认式名单 revision 三项也已于 2026-08-25 完成本地产品初验，下一项为 M4b control 端教师布局。只有相关数据库 migration 被未授权提前应用且所有开关关闭，生产应用并未包含这些实现，因此不得记为课堂输入、H5 pointer 或 M4a 已部署生产。
...
| 部署/备份/恢复 | M2（生产发布/当前备份） | M4 | commit `35b9f60` 与[部署子门](../evidence/r1/r1-16-deployment-preflight.md)已固定 fail-closed 合同；仓库/历史 secret scan 已关闭。2026-08-23 current 已发布为 `20260823-123746` / `9bc9ff3…`，previous 为 `20260823-122633` / `ba5c991…`，原子切换及内外健康门通过。数据库 head 于 2026-08-25 因未授权提前迁移变为 `20260825000500_classroom_roster_rpc_schema_reload`，应用未同步发布、开关均关闭、production 业务未回填；该事件已单列证据并冻结后续生产写入。当前 PostgreSQL+Storage 同批次备份已独立通过 TOC、文件数、源前后清单与 SHA-256 校验，因此 R1-Live Gate 1 仍为通过；previous 兼容回退、恢复演练、异机/静态加密备份和错误 release 标签属于 M4 | Live/16 |
