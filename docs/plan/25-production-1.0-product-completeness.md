...
- `DEV-CW-1` 当前为 `STEP 3A UNIFIED HOST / READY FOR USER RE-AUDIT`：预览、正式课编辑和微课编辑现由同一 `CoursewareWorkbench` 面板树装配并共用页面目录、画布适配、底部翻页与快捷键，不再以 dispatcher／DOM 槽位维护三份结构。预览从同一树裁掉编辑顶栏和右栏；正式课与微课共用左目录／中画布／右属性三栏，中间顶栏固定左插入、右保存且不跨入右栏，正式课只增加 4:3 适配。正式 PageDoc、无写入样机与微课 composition 通过 React context 向同一工作台注册控件。当前三种模式待产品负责人同轮人工复审，正式写态仍只开放 Step 3 固定讲次第 5 页；四年级秋季标题偏移已确认为源站特例。爱学习写入、整讲、跨页替换、schema/RPC、批量回填、生产数据库/Storage/release 均不在授权范围。
...
| 课程产品统一课件工作区 | M1（Step 0～2A 已确认；Step 3A 统一宿主待同轮人工复审） | M4 | doc 16 §14 已记录人工确认门；预览、正式课编辑与微课编辑现由同一 `CoursewareWorkbench` 面板树装配并共用页面目录、完整适配舞台与翻页，正式模式只额外开启 4:3。当前小增量待产品负责人核对三种模式的真实布局、插入和保存；固定 E 系列单页写态没有扩量。通过后再决定来源 runtime 与 Step 3B 保存复审，整体重构完成后、扩量前仍需双端定稿组件表现 | DEV-CW-1/9 |
