...
- `DEV-TMC-4` 当前为 `LOCAL DEVELOPMENT DELIVERED / AWAITING PRODUCT ACCEPTANCE / NOT DEPLOYED`。doc 29 的五个 Phase 已在本机交付：一门课可关联多个 766 使用场景，年级/学期/班型三个维度零选择分别表示通用，主页面使用目录树＋每课一行表格＋当前默认版本按需预览，维护方向与提交历史下钻展示；本机 v2 开关已启用供产品负责人验收。现有课次方案、review/release 和课堂冻结继续复用；本轮不执行生产 schema、历史归并、开关或发布，机器检查不替代产品负责人本地页面验收。
