...
- `DEV-CW-1` 当前为 `STEP 3 AWAITING USER AUDIT`：产品负责人于 2026-08-31 确认 Step 2A 通过；一个 `CoursewareEditorWorkbench` 和 `CoursewareInsertionToolbar` 承担两条编辑链，微课为共同能力基线，正式课只增加 `adapt4x3`。统一组件的双端最终表现推迟到 Step 7 完成整体工作区重构后、Step 8 扩量前再审。开发端现只为本机 E 系列“进位制初步”第 5 页开放显式 PageDoc 草稿入口并复用既有 revision RPC，尚待产品负责人实际保存、刷新和核对 release；爱学习、整讲、跨页替换、批量回填、生产数据库/Storage/release 均不在授权范围。
...
| 课程产品统一课件工作区 | M1（Step 0～2A 已确认；Step 3 待人工审计） | M4 | doc 16 §14 已记录人工确认门；正式课件与教师微课共用完整 `CoursewareEditorWorkbench` 和 `CoursewareInsertionToolbar`，正式 adapter 只额外开启 4:3。Step 3 只为本机 E 系列第 5 页交付 PageDoc 草稿纵切；PageDoc/source-runtime/composition 仍分别承载来源限制、12×9 网格和写态。整体重构完成后、扩量前再双端定稿组件表现 | DEV-CW-1/9 |
