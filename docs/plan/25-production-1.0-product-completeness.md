...
当前等级截至 2026-07-31，并按各行最近关闭阶段的仓库证据更新；后续 R1 阶段仍须生成对应 E2/E3/E4 才能提升至 M4。
...
| R1-7D 修复 | 学生合并、课件 replacement rollback、测试数据清理 | 领域修复计划、影响计数、恢复点、执行/回滚审计 | 先预览后执行；目标集合 hash 不变才允许写；失败不留半成品；可恢复动作实际回滚 |
