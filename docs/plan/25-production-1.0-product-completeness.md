...
- `DEV-CW-1` 当前为 `STEP 3A RENDERER ROOT REVIEW / PLANNED`：产品负责人已确认 Step 2A；一个 `CoursewareEditorWorkbench` 和 `CoursewareInsertionToolbar` 承担两条编辑链，微课为共同能力基线，正式课只增加 `adapt4x3`，双端最终表现仍在 Step 7 后、Step 8 前复审。Step 3 已接入固定 PageDoc 样本的 800ms 自动保存、立即保存与脏态保护，但后续人工审计确认布局偏移来自 Mathin renderer 缺失来源 CSS／字体／行为语义，不能用 `data-align-v` 或页面坐标逐项修复。当前先审内容寻址来源 runtime、统一宿主／选择协议和语义签名 inventory；现有单页保存降为 Step 3B，只有 Step 3A 通过后复审。爱学习写入、整讲、跨页替换、schema/RPC、批量回填、生产数据库/Storage/release 均不在授权范围。
...
| 课程产品统一课件工作区 | M1（Step 0～2A 已确认；Step 3A 待方案审计） | M4 | doc 16 §14 已记录人工确认门；正式课件与教师微课共用完整 `CoursewareEditorWorkbench`、`CoursewareInsertionToolbar` 和保存状态控件，正式 adapter 只额外开启 4:3。固定 E 系列单页的写态已实现但因来源 renderer 一致性缺口暂停验收；Step 3A 先用内容寻址来源 runtime 与语义签名 Gate 覆盖 E 系列和爱学习宿主边界，Step 3B 再复审保存。整体重构完成后、扩量前再双端定稿组件表现 | DEV-CW-1/9 |
