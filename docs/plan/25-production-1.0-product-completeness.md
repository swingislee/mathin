...
| 机构配置/场地/Feature Flag | M3 | M4 | R1-1 已完成旧设置中心与版本化开关；`DEV-ORG-1` 已在本机完成机构资料、校区/教室目录、机构级学年/日历/排课默认和系统能力发布拆分，`DEV-DASH-1` 已按职能重组导航、合并单一排课默认、建立个人优先/全部表格班级视图并把课件资源缩至每页 10 条。兼容双写及固定账号 Playwright 2/2 通过。M4 仍需开发人工验收、生产初始化、正式角色旅程和发布门 | 1/DEV-ORG-1/DEV-DASH-1/15/18 |
...
R1-1 的 `20260728000100_r1_organization_settings.sql` 与 `20260728000200_r1_public_publish_guard.sql` 是旧合同和回退基线。`DEV-ORG-1` 已新增 `organization.profile.manage`、`location.manage`、不含代码字段的 V2 DTO/RPC、结构化场地引用和兼容双写；旧设置 RPC、`classrooms.room`、默认校区与学年校区列至少保留一个生产回退窗口。迁移前必须输出自由文本教室候选、学年/周期重复差异、有效校区级规则或开关覆盖和正式业务影响；多校区无法唯一匹配、学年日期/状态冲突或有效覆盖存在时直接失败，不猜测也不静默丢弃。开发验收并明确退休 previous 后，才以独立授权增量删除旧合同。本机 SQL/Vitest/TypeScript/ESLint 与固定管理员 Playwright 旅程已通过，产品负责人开发端验收仍为 `UNKNOWN`；该工作继续只在开发预演轨，不授权生产写入。

`DEV-DASH-1` 规定班级默认 scope 为“本人任教 → 本人负责 → 两者为空时全部”，个人 scope 使用信息卡，全部与测试 scope 使用同一套 shadcn `Table` 圆角完整描边表格壳；普通列表每页 20 条。课件资源库每页只读取 11 条用于判断下一页，最多展示并签名 10 条预览。机构资料、校区目录和教学日历采用线性设置区或表格。本机 migration `20260829000100_classroom_personal_scope_default`、定向合同 22/22 与固定账号 Playwright 2/2 已通过，产品负责人页面验收仍为 `UNKNOWN`，且没有执行生产写入。

`DEV-DASH-2` 已以本机提交 `e1e8c87` 统一 Dashboard 的表格与分隔语义，当前为 `LOCAL MACHINE PASS / PENDING USER ACCEPTANCE`。顶层数据表使用共享的 shadcn `Table` 圆角完整描边外壳，表头和数据行保留内部边界；页面标题后只有一条结构分割线，横向状态导航、筛选、操作与正文处在该线下的同一工作区，不再额外分割。普通设置字段依靠间距、标签层级和局部底色成组，禁止逐项 `border-y`/`divide-y`；独立的大内容板块可以在起点保留一条分割线。定向合同 14/14、TypeScript、全量 ESLint、Dashboard 设计审计及固定管理员 zh/en/390px Playwright 1/1 已通过，产品负责人页面验收仍为 `UNKNOWN`，且没有执行生产写入。
