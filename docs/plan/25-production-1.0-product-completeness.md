...
- Gate 1 当前 `PASS`：Xiaomi 目标指纹、危险写拒绝、唯一正式 admin MFA、首名真实教师及双岗位、active 8 条 protected/0 条 purge manifest、current/previous、健康探针和错误查询位置均已确认。手机号与账号中心两次获批发布后，生产数据库 ledger 为 192 条、head=`20260825000800_account_center_profile`，应用 current/previous=`20260825-072801` / `72d8127…` 与 `20260825-041101` / `8ec0ba0…`。此前误应用的课堂 schema 继续保持前向兼容，board/input/layout/H5 四个开关均为 false，v2 checkpoint/star/learning result 均为 0，唯一 roster revision 仍为 1 且 entry 为 0；课堂 bulk migration `20260825000700` 尚未登记。最近已核验 PostgreSQL+Storage 同批次备份仍为 `mathin-20260822T093529Z`，早于后两次授权发布，不能冒充课堂候选的新鲜 pre-change 备份。
...
- 课堂可扩展互动在开发轨使用版本化 capability provider 和统一 conformance；普通 renderer 复用既有输入原语时不逐个重复人工 Gate。新增原语、输入路由/所有权算法、跨 iframe 协议和真实设备回归仍单独人工验收；未声明或不匹配 provider 的 renderer 继续 fail closed。M3a/M3b 已在 commit `00f4766` / `61c6c70` 完成开发端产品初验，M4a 的稳定身份、星星 v2 set/undo 和确认式名单 revision 三项随后通过；2026-08-25 产品负责人又以 `95ed9f1` 为应用基线确认 M4b 教师布局与逐题学情通过，因此 M0–M4 开发端整体关闭。M5 候选 `8c303a2` 已通过一次工程、隔离数据库与本地浏览器 Gate，Xiaomi 只读 preflight 也已完成；生产仍缺 `20260825000700`、新鲜数据库 pre-change 备份、候选暗发布和分段开关授权，不能记为课堂输入、H5 pointer、M4/M5 或 Gate 2 已在生产通过。
