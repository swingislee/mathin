...
>
> **基线日期**：2026-09-03；每次验收另记 commit、migration head、环境和数据 manifest。`DEV-CW-1` 窄范围 schema/app 已发布生产并通过机器 postflight；产品人工验收仍 pending。其他开发轨不因本次发布改变状态。
>
...
| 爱学习 G+/X+/A+ 秋季 | G+ 苏教版 56 讲、X+ 苏教版 84 讲、A+ 全国版 30 讲；16:9/4:3 双轨资源和 release 机制 | 保留 170×2 源资源；正式基线包含 340 条 `release_no=1`；另有 10 条教学计划第 7/15 讲补充占位（G+ 5/6年级、X+ 2/5/6年级），占位无 release 且准备状态为“未发布” |
...
- Gate 1 当前 `PASS`：Xiaomi 目标指纹、危险写拒绝、唯一正式 admin MFA、首名真实教师及双岗位、active 8 条 protected/0 条 purge manifest、current/previous、健康探针和错误查询位置均已确认。2026-09-03 统一课件工作区窄范围发布后，生产数据库 ledger/head=`242 / 20260903000700_courseware_page_insertions`；应用 current/previous=`20260903-115645` / `8c50b48…` 与 `20260903-100016` / `750bd607…`。最近 PostgreSQL 写前备份为 `mathin-db-prechange-20260903T114653Z-courseware-workspace-8c50b48a8c4d`，既有 PostgreSQL+Storage 同批次全量备份 `mathin-20260828T052842Z-teacher-microcourse-variant-087b497` 继续作为 Storage 回退基线；本轮迁移与应用发布没有改写存量页面、binding、课程 release、审核快照、冻结会话或 Storage，未执行 restore 或 prune。
- Gate 2 当前仍为 `BLOCKED`，但生产单老师试用已经启动：本机隔离固定账号 Golden Path 已完成未完整课程建班、自动课次、`lead` 报名、教师点名保存和换页再读。生产现有 1 个长期正式班/15 个课次/1 条 active 报名和 1 个短期专题班/3 个课次/0 报名；另有 2 个 test 长期班合计 1 个课次，当前总数为班级/课次=`4/19`。学年迁移把春季边界修正为 `2026-06-29`，并只把原长期正式班/课次/报名改挂到 `2026–2027` 秋季，2026 学年仍为 planning，学生仍为5年级。正式点名仍为 0；退出前仍由该教师完成点名保存、刷新/重登再读，再做管理员读取和无权限拒绝。
...
- `DEV-SCHOOL-OPS-1` 当前为 `PHASE 2 / DEVELOPMENT READY / AWAITING PRODUCT OWNER UI AND INTERACTION ACCEPTANCE`：提交 `207d6e5` 已交付真实活动列表／漏斗、单次活动参与与测评工作台、课程意向列表；本机 migration `20260902000200_school_ops_phase2_activity_funnel` 新增 AssessmentResult／Opportunity、RLS、到场前置、测评前置和下一动作投影合同。本地使用明确标记的“Phase 2 验收 · 1v1 测评”及“Phase 2 验收学生”作为可删除验收数据；这不改变 R1-Live 生产基线，当前没有生产 migration、业务数据写入、Storage 或发布动作。二进制 `.xlsx`、逐行合并决定与 Family／Contact／Lead 分表仍未施工并保留为后续项。
- `DEV-STAFF-ONBOARD-1` 当前为 `DEVELOPMENT READY / AWAITING PRODUCT OWNER UI AND INTERACTION ACCEPTANCE`：本机 migrations `20260902000700_staff_bulk_onboarding`、`20260902001000_staff_direct_provisioning`、`20260902001100_staff_initial_password_hardening` 与 `20260902001200_staff_initial_password_reissue` 让员工页接受中文三列模板及魔法校 Tab 四列名单，逐行预检后由主管及以上直接创建 Auth 账号、员工档案和岗位；成功档案立即可用于排课，首次登录密码只在本次下载响应出现，建档触发完成后同事务清除 Auth 临时元数据键。新员工本人首次登录必须输入并二次确认不同的新密码，完成前 staff 权限/RLS fail closed；此期间主管可在员工行重新生成一次性显示的新密码，旧密码立即失效，完成改密后重签入口关闭。系统不伪造联系方式验证或隐私同意。`staff.invite` 只开放建档和待首次改密密码重签，停用、改岗和既有顾客身份提升仍要求 `staff.manage`，含 `permission.configure` 的岗位仍只允许顶层 admin 预授。失败行可在同一批次重试，数据库与 Auth 元数据均不保存明文密码。迁移回滚演练、正式本机应用、数据库回滚行为断言、相关 Vitest 17/17、受影响 ESLint、TypeScript、双语键与类型摘要检查通过；固定测试管理员角色与既有权限保留，新重签账本为空。当前没有 Xiaomi／生产 migration、身份创建或业务写入，页面、首次登录、密码重签和交接操作仍待产品负责人人工验收。
- `DEV-CW-1` 当前为 `STEP 8E PRODUCTION DEPLOYED / MACHINE POSTFLIGHT PASSED / PENDING PRODUCT ACCEPTANCE`：Step 7C 三端视觉与源码复用、Step 8A 双轨 typed-patch 草稿及 Step 8B 共享插入均已获开发端人工确认。最终窄范围候选 `8c50b48…` 已完成 6 条 production migration 和应用原子发布；ledger/head=`242 / 20260903000700_courseware_page_insertions`，current/previous=`20260903-115645` / `8c50b48…` 与 `20260903-100016` / `750bd607…`。77,224 个页面及资源、release、冻结会话和 Storage 计数不变；机器 postflight 已通过，真实生产工作流验收仍 pending。
...
| Jobs/通知/文件/集成 | M3 | M4 | R1-2 已完成 durable job/dead-letter/重放、第一方通知、TUS/文件策略、webhook 防重放与开发环境验证；2026-09-09 员工 Windows Edge Web Push 已按明确生产指令进入 P6，当前 36 个在职账号可自主开启，专用 Worker、生产 WNS 出站和独立告警恢复 postflight 通过。权威状态与证据见 [`employee-desktop-web-push.md`](employee-desktop-web-push.md)。M4 仍需完整 G5/P7、真实员工体验、容量/SLO、支持与隐私签收及 14 天 RC | 2/14/16/17/DEV-WEB-PUSH-1 |
...
| 课程研发/双轨 release | M3 | M4 | P6-AIX-2 已固定 G+/X+/A+ 秋季 170 讲/5442 页；2026-08-27 开发库另导入1年级 A+ 暑期 2 讲/66 页并保留 13 个空占位，开发库爱学习现为 13 门课程、195 个教学计划讲次（172 个 source-backed）/5508 页。开发库已在保留 revision/release 1 的前提下把 5508 页两轨 current head 升级到 `source-runtime-page-v1` revision 2 / release 2；该适配器和暑期增量均未进入 Production 1.0 基线。[来源 manifest 子门](../evidence/r1/r1-9-courseware-source-manifest.md)已同步 102 门/1305 讲/2610 条 release-1 的 v4 确定性合同、只读导出/对象校验核心与受控 runner；批准副本真实 inventory、Storage/H5 审计、隔离演练和正式 release-1 仍缺 | 9/15/18 |
| 课程产品统一课件工作区 | M2（schema/app 已部署，待生产产品验收） | M4 | doc 16 §14 已记录 Step 0～8D 人工确认门；预览、正式课、微课共用工作台，4:3 草稿／资源替换／回滚、来源 runtime 编辑桥和共享插入已实现。Step 8E 以窄范围候选 `8c50b48…` 完成 PostgreSQL 写前备份、6 migration 回滚／正式提交、应用原子发布与机器 postflight；存量页面、binding、资源、release、冻结会话和 Storage 零漂移。生产框内编辑、插入、保存／刷新、历史和 4:3 粗调→微调仍待产品验收；Production 1.0 的全量来源审计、恢复与 RC 门不因此关闭 | DEV-CW-1/9 |
...
| 部署/备份/恢复 | M2（生产发布/当前备份） | M4 | commit `35b9f60` 与[部署子门](../evidence/r1/r1-16-deployment-preflight.md)已固定 fail-closed 合同；仓库/历史 secret scan 已关闭。2026-09-03 统一课件工作区发布后 current/previous=`20260903-115645` / `8c50b48…` 与 `20260903-100016` / `750bd607…`，数据库 ledger/head=`242 / 20260903000700_courseware_page_insertions`。最新 PostgreSQL 写前备份为 `mathin-db-prechange-20260903T114653Z-courseware-workspace-8c50b48a8c4d`，既有 PostgreSQL+Storage 同批次全量备份继续作为 Storage 回退基线；6 migration 回滚／零残留演练、正式提交、原子切换及健康／鉴权／路由／业务计数门通过，存量页面、release、冻结会话和 Storage 不变。恢复演练、异机/静态加密备份和错误 release 标签属于 M4 | Live/16 |
...
开发增量 `DEV-STAFF-ONBOARD-1` 已把单人 claim 邀请替换为员工页的版本化 ImportBatch 直接建档，并为魔法校四列员工名单建立显式适配：Tab 是表格列分隔符，岗位单元格内的逗号只拆来源岗位，性别不进入员工档案；来源岗位必须映射到现有 Mathin staff role 或被操作者明确忽略。待首次改密员工遗失密码时，主管可重签一次性显示的新首次密码；该能力受 `staff.invite`、员工在职状态和 `password_change_required` 三重约束，完成改密后不能作为通用密码重置入口。该直接建档、首次强制改密与重签合同仍只在本机开发验收，不表示生产 migration、员工身份创建或业务写入已经获准；生产现有 claim 邀请与正式教师登录事实不被本增量改写。
...
学年与运营周期采用机构级唯一的两层合同。`school_years` 是学生年级的年度归属，创建时只登记起始年并建立四个周期，不要求预先知道放假或开学日期，也不改学生年级；`school_terms` 固定为 `暑期 → 秋季 → 寒假 → 春季`，日期成对可空，临近后补，周期重叠只形成运营提示。暑期属于新学年，因此“2升3”的暑假按3年级归属。启用下一学年必须先显示1至11年级晋级数和12年级保留数，再填写该起始自然年内的生效日并显式确认；切换当前运营周期不触发晋级。学年、四个周期、当前周期和升年级均不保存或查询校区作用域，保证一次升年级只执行一次。产品负责人确认 `2025–2026` 学年春季结束日为 `2026-06-29`；此前挂在旧春季、且全部课次晚于该日的 production 班级只迁移到 `2026–2027` 秋季归属，不改班级状态、教师、课次时间或学生年级。
...
- 教师课堂旅程包括进入课次、点名、读取已发布课件、课堂记录、布置作业和课后动作；教务/学服从异常 work item 进入对应领域页处理。
...
爱学习课程与 E 系列共享课程族/课程/讲次、CAS、revision/release、双轨 head 和课次冻结，不共享页面文档接口。1.0 范围固定为 2026 秋季数学：G+ 苏教版3至6年级 56 条源站讲次、X+ 苏教版1至6年级 84 条源站讲次、A+ 全国版1至2年级 30 条源站讲次；当前批准的生产 release-1 页面仍使用 projection v31 的 `aixuexi-page-doc-v1`。2026-08-27 开发导入器已改为 `source-runtime-page-v1`，复用来源 Viewer 而不在 Mathin 重造 DOM/CSS；生产切换仍需 R1-9/15/18 的批准 manifest、隔离演练和正式授权。G+ 5/6年级与 X+ 2/5/6年级缺少第 7/15 讲源站课件，教学计划补充 10 条占位，不创建 release，课件准备状态为“未发布”；其他缺失讲次、年级、季节和难度不生成空课程或伪页面。

课程产品的编辑主路径按 doc 16 §14 收敛为“课程/版本 → 讲次 → 指定页面 → 统一课件工作区”。共享内容修订与 16:9/4:3 分轨版式分别保存；4:3 起始布局只保留等宽顶部、等宽居中、等高左侧、等高居中四种整页变换，以及背景等高／内容等宽的分层页面模式，不再保留抽象 A～F 或自由宿主偏移。对照区负责粗调，单独比例页面负责元素微调。图片/背景替换从当前页面发起，执行前显式选择页、讲、课程版本、课程族或全部引用范围并展示影响，已发布 release 和课次冻结不回写。正式 PageDoc、来源 runtime 和教师 composition 通过能力 adapter 共用工作区动作，不强行共用页面 schema。该合同仍处于规划人工审计期：先确认路径、布局和功能样机，再允许单页/样本讲次持久化，最后才讨论全量回填和生产候选。
...
| 学服 | 学生/家庭上下文 → 跟进/续费 → 跨角色协同 → 领域动作关闭 |
...
| 通知 | 站内通知为硬门；2026-09-09 已按产品指令启用全部当前在职员工的 Windows Edge 自主测试，实际范围及生产证据以专题为准。独立告警 SMTP 不启用业务邮件渠道；未验收的邮件、SMS、微信和 Webhook 业务渠道继续关闭 | 平台+运维负责人 | R1-2/DEV-WEB-PUSH-1 | Employee test active；pending user acceptance；M4 未关闭 |
...
| 平台运行内核尚无完整生产验收 | R1-2 开发合同及 Web Push P5 底座保留；2026-09-09 P6 员工测试已启用，生产 WNS 201、专用 Worker、独立告警恢复、备份/迁移与业务不变量通过，详见专题证据 | Windows 人工体验、完整共享电脑旅程、真实容量/成功率、最终保留期与支持签收和持续观察尚未完成；继续保留完整 G5/P7 与 M4 缺口，不把运行状态当作正式验收 | 平台+运维+安全+QA | DEV-WEB-PUSH-1/R1-14/16/17 |
