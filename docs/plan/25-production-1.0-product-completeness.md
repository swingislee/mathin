...
- `DEV-CW-1` 当前为 `STEP 3B SHARED IMAGE / READY FOR USER AUDIT`：产品负责人已于 2026-09-01 验收统一 `CoursewareWorkbench` 三种模式、共用目录／文字编辑、正式 PageDoc 自动保存、标题保存和旧样机删除；当前小增量又让正式课与微课直接复用唯一图片元素面板，统一填充方式、位置尺寸、透明度、层级和显隐。正式课仍只推进 PageDoc draft revision，未接文件上传、binding 改写或共享资源替换。4:3 A～F、爱学习写入、整讲、跨页替换、schema/RPC、批量回填、生产数据库/Storage/release 均不在授权范围。
...
| 课程产品统一课件工作区 | M1（Step 0～3A 已确认；Step 3B 共享图片待人工验收） | M4 | doc 16 §14 已记录人工确认门；预览、正式课编辑与微课编辑使用同一 `CoursewareWorkbench` 面板树、目录、舞台、翻页、文字编辑和保存反馈，正式模式只额外开启 4:3。当前已共用图片元素局部编辑面板，正式 PageDoc 单页保存仍只推进 draft revision；整体重构完成后、扩量前仍需双端定稿组件表现 | DEV-CW-1/9 |
