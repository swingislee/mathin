...
- 普通教师短期微课开发轨 `DEV-TMC-1` 已在本机实现并完成机器检查：一份自由课次惰性孵化一门单讲 `microcourse`，作者可用草稿试讲，校内共享须经现有 Courseware review/release 机制审核发布；其他具备建班权限的校内 staff 只从已启用且存在正式 release 的目录搜索建班。来源只接受正式 `curriculum` 当前 release，并由 commit `dd9a755` 收敛为“按课程族/课程/课次检索 → 一次事务插入本讲全部页 → 编辑器内删选与排序”；元数据、页面、资源与 H5 hash 在提交时共同冻结，原自由课次和历史课次保持原 release。本机 migration head=`20260827000100_teacher_microcourse_lecture_source_picker`；基础闭环的 SQL/Vitest/固定账号 Playwright/CI/R1-Live 合同及整讲增量的回滚式内容 SQL、4 文件 19/19 Vitest、typecheck/审计与固定账号 Playwright 1/1 均通过。产品负责人对该增量的开发初验仍为 `UNKNOWN`。该能力由 `teaching.teacher_microcourses_v1` 默认关闭，本任务未连接 Xiaomi，生产迁移和启用尚未发生。
