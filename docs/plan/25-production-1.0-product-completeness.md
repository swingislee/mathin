...
- `DEV-CW-1` 当前为 `STEP 2 READY FOR USER AUDIT`：Step 1 已获产品负责人确认；右侧栏现提供无写入的微调、A～F/自定义、五档替换范围、插入和会话级撤销样机。内容默认双版联动，几何只作用 4:3；爱学习未获准的节点编辑与插入 fail closed。该结果只有开发端 TypeScript、ESLint、双语和定向合同证据，尚无 Step 2 产品确认；当前没有 editor action、schema、开发库/Storage 或生产写入授权，Step 3 继续关闭。
...
| 课程产品统一课件工作区 | M1（Step 1 已确认；Step 2 无写入交互样机待审） | M4 | doc 16 §14 已记录 Step 0～8 人工确认门；已确认的入口、三栏、画布与爱学习 4:3 投影保持不变，右侧新增微调、A～F/自定义、替换范围、插入及会话级撤销样机。尚未接 editor action、schema、持久化或来源 patch；Step 3 在产品确认交互前保持关闭 | DEV-CW-1/9 |
