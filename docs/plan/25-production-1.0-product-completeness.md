...
- `DEV-CW-1` 当前为 `STEP 4B SINGLE-PAGE 4:3 DRAFT / DEV READY / AWAITING USER AUDIT`：产品负责人已于 2026-09-01 确认 Step 4A。当前实现只把一个 PageDoc 页的对照策略物化到 `adapted-4x3` 草稿轨，复用现有自动保存 RPC，并从文档内适配框恢复策略；旧 A/B→等高左侧、C/E→等宽居中、D→等宽顶部、F→分层模式，只有无新草稿标记时才作为起点。没有新增 schema/RPC，也不写 16:9、binding、release 或来源课件。当前只审计一个本机样本页的旧分类映射、保存反馈、刷新重读和轨道隔离；source-runtime patch、文件上传、共享资源替换、整讲、跨页、批量回填、生产数据库/Storage/release 均不在授权范围。
...
| 课程产品统一课件工作区 | M1（Step 0～4A 已确认；Step 4B 开发端待人工审计） | M4 | doc 16 §14 已记录人工确认门；预览、正式课编辑与微课编辑使用同一 `CoursewareWorkbench` 面板树、目录、舞台、翻页、元素检查器和图层面板。Step 4A 已确认 PageDoc／爱学习共用的 4:3 视口策略与单页会话预览；Step 4B 已接一个 PageDoc 样本页的 `adapted-4x3` 草稿保存与刷新重读，尚未获产品验收；整体重构完成后、扩量前仍需双端定稿组件表现 | DEV-CW-1/9 |
