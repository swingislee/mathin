...
- `DEV-CW-1` 当前为 `STEP 2A REVISION READY FOR USER AUDIT`：Step 1/2 已获产品负责人确认；正式课件与教师微课现共用 `CoursewareEditorWorkbench`。首轮审计后的开发修订让只读微课复用 `CoursewarePreviewWorkspace` 及方向键／PageUp／PageDown／空格翻页，按 `487b60d` 恢复 composition 标题栏插入与左栏重命名，并把正式工作区底部安全区压缩到约 4px；PageDoc/source-runtime/composition adapter 仍保留不同能力与写态。该结果只有开发端 TypeScript、ESLint 和定向合同证据，尚无 Step 2A 视觉与微课回归确认；当前没有新增 editor action、schema、开发库/Storage 或生产写入授权，Step 3 继续关闭。
...
| 课程产品统一课件工作区 | M1（Step 1/2 已确认；Step 2A 修订待审） | M4 | doc 16 §14 已记录人工确认门；正式课件与教师微课共用 `CoursewareEditorWorkbench`，只读微课另复用成熟 `CoursewarePreviewWorkspace`。composition 已按历史正确版本恢复标题栏插入和左栏重命名；PageDoc/source-runtime/composition adapter 继续区分 A～F、12×9 网格、来源限制与写态。尚未新增 editor action、schema、持久化或来源 patch；Step 3 在共享框架人工确认前保持关闭 | DEV-CW-1/9 |
