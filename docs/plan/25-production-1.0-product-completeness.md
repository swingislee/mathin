...
- `DEV-CW-1` 当前为 `PLANNED / AWAITING PRODUCT PLAN CONFIRMATION`：课程产品统一课件工作区按 doc 16 §14 拆为规划确认、只读布局、无持久化功能样机、PageDoc 单页、4:3 样本讲次、上下文资源替换、source-runtime 决策、入口收敛和扩量九个 Step。每一步必须先交付可人工审计的文档或局域网页面，并获得产品负责人对本步布局/功能的明确确认；确认前不跨步，不做大范围或长时间变更。当前没有页面、schema、开发库或生产写入授权。
...
| 课程产品统一课件工作区 | M0（规划待确认；既有入口与分散能力不构成统一闭环） | M4 | doc 16 §14 已记录当前仓库/本机开发库差距、统一工作区对象分层和 Step 0～8 人工确认门。目标为从课程产品指定页面直接编辑共享内容与 16:9/4:3 分轨版式，在页面上下文预览资源替换影响，并沿 draft/review/release 发布；`source-runtime-page-v1` 的真编辑需另行确认 patch/投影协议。当前只允许审计规划，未授权实现或数据变更 | DEV-CW-1/9 |
