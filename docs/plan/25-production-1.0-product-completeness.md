...
- Gate 1 当前 `PASS`：Xiaomi 目标指纹、危险写拒绝、唯一正式 admin MFA、首名真实教师及双岗位、active 8 条 protected/0 条 purge manifest、current/previous、健康探针和错误查询位置均已确认。生产数据库 ledger 为 194 条、head=`20260826000100_classroom_h5_input_profiles`；应用 current/previous=`20260826-125052` / `964ca5e…` 与 `20260825-085754` / `8c303a2…`。产品负责人已确认 Stage B1/B2，生产 checkpoint version/chunk/head=`2/2/2`；Stage B3 共同 bridge、权威档案表与单一 Smart 开关已部署，但 H5 保持 version 3 / false，空档案表不授权任何现有 package。board/input/layout 为 version 2 / true，star/learning result 仍为 0。新的 PostgreSQL-only pre-change 备份已核验；最近 PostgreSQL+Storage 同批次备份仍为 `mathin-20260822T093529Z`，两者证据边界不互相替代。
...
- 普通教师短期微课开发轨 `DEV-TMC-1` 已在本机实现并完成机器检查：一份自由课次惰性孵化一门单讲 `microcourse`，作者可用草稿试讲，校内共享须经现有 Courseware review/release 机制审核发布；其他具备建班权限的校内 staff 只从已启用且存在正式 release 的目录搜索建班。来源只接受正式 `curriculum` 当前 release，并由 commit `dd9a755` 收敛为“按课程族/课程/课次检索 → 一次事务插入本讲全部页 → 编辑器内删选与排序”；commit `1135099` 再用通用 `game-page-v1`、游戏内容注册表、服务端校验凭证与来源 capability predicate 接入四宫/六宫/九宫数独，后续游戏内容版本不修改微课审核、冻结或来源选择器。元数据、页面、资源、游戏校验事实与 H5 hash 在提交时共同冻结，原自由课次和历史课次保持原 release。本机 migration head=`20260827000400_teacher_microcourse_game_source_adapter`；新增回滚式内容 SQL、定向 Vitest 6 文件 47/47、固定账号 Playwright 1/1 及最终 CI 16/16 均通过，全量 Vitest 为 106 文件、742 通过/1 条件跳过，production build 成功。产品负责人对该增量的开发初验仍为 `UNKNOWN`。该能力由 `teaching.teacher_microcourses_v1` 默认关闭，本任务未连接 Xiaomi，生产迁移和启用尚未发生。
...
| 部署/备份/恢复 | M2（生产发布/当前备份） | M4 | commit `35b9f60` 与[部署子门](../evidence/r1/r1-16-deployment-preflight.md)已固定 fail-closed 合同；仓库/历史 secret scan 已关闭。2026-08-26 课堂 Stage B3 修复底座发布后 current/previous=`20260826-125052` / `964ca5e…` 与 `20260825-085754` / `8c303a2…`，原子切换及内外健康门通过；数据库 ledger=`194`、head=`20260826000100_classroom_h5_input_profiles`。当前 PostgreSQL+Storage 同批次备份已独立通过 TOC、文件数、源前后清单与 SHA-256，课堂 Stage B3 发布另有新鲜 PostgreSQL-only pre-change 备份；previous 兼容回退、恢复演练、异机/静态加密备份和错误 release 标签属于 M4 | Live/16 |
