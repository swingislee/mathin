...
- `DEV-CW-1` 当前为 `STEP 4A SHARED 4:3 PREVIEW / DEV READY / AWAITING USER AUDIT`：产品负责人已确认 Step 3B 的唯一元素检查器、可折叠完整图层列表和公共层级设置。正式 PageDoc 与爱学习 source runtime 已接入同一份 4:3 策略状态、分类面板、对照画布和撤销／重置逻辑；当前待审模型为四种整页视口变换与一个 PageDoc 分层模式，爱学习默认“等高居中”，E 系列 PageDoc 默认“等宽顶部”。缺独立 4:3 release 但适配器可处理的页面直接显示来源默认版式；爱学习 iframe 使用固定逻辑 viewport 与宿主整体缩放，避免二次拟合及源 Viewer 白边。粗调只在对照视图呈现，单轨进入页面微调。此批不推进 `draft_revision_id`，不接文件上传、binding 改写或共享资源替换；整讲、跨页替换、schema/RPC、批量回填、生产数据库/Storage/release 均不在授权范围。
...
| 课程产品统一课件工作区 | M1（Step 0～3B 已确认；Step 4A 开发端待人工审计） | M4 | doc 16 §14 已记录人工确认门；预览、正式课编辑与微课编辑使用同一 `CoursewareWorkbench` 面板树、目录、舞台、翻页、元素检查器和图层面板。Step 4A 已增加一份 PageDoc／爱学习共用的 4:3 视口策略组件和单页会话预览；四种通用模式整体复用既有 renderer，PageDoc 另有一个分层模式；整体重构完成后、扩量前仍需双端定稿组件表现 | DEV-CW-1/9 |
