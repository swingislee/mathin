...
- `DEV-CW-1` 当前为 `STEP 3A UNIFIED HOST / IN PROGRESS`：产品负责人确认从成熟预览组件升级，预览、正式课编辑和微课编辑统一为 `CoursewareWorkbench` 三种模式；共用 renderer、资源加载和完整适配画布，预览裁掉编辑顶栏/右栏，微课默认 4:3，正式课只增加 4:3 适配。首个无写入小增量收敛模式合同和共享 `CoursewareStageViewport`，仍待人工验收。来源 CSS／字体／行为 runtime、稳定选择桥和语义签名 inventory 继续属于 Step 3A；现有单页保存降为 Step 3B。爱学习写入、整讲、跨页替换、schema/RPC、批量回填、生产数据库/Storage/release 均不在授权范围。
