...
- `HOTFIX-20260829-CLASSROOM-INTERACTION-SYNC` 当前仅在本机修复，尚未部署：新式 `game-page-v1` 数独适配链补齐 `GameMirrorState` 的采集、100ms 合并广播与 viewer replay；统一 `ClassroomInteractionSyncProvider` 对每个 Mathin 自研 docVersion、微课 mode 和游戏 registry 项做穷尽审计。自研 H5 与空间页在版本化状态/语义命令协议接入前保持课堂只读。本项不含 schema、数据库、Storage 或生产写入，开发设备验收与生产晋级仍 pending。
