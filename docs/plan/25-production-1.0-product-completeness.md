...
- `DEV-CW-1` 当前为 `STEP 4B SINGLE-PAGE 4:3 DRAFT / AUTHORIZED / IN PROGRESS`：产品负责人已于 2026-09-01 确认 Step 4A 的共享策略、来源默认版式、缺独立 release 的会话预览以及爱学习固定逻辑 viewport。下一批只复用现有 PageDoc 草稿合同，让一个本机样本页的 4:3 策略可保存、刷新重读并进入同一自动保存反馈；不推进 source-runtime patch，不新增 schema/RPC，不改 binding/release，不接文件上传或共享资源替换，整讲、跨页、批量回填、生产数据库/Storage/release 均不在授权范围。
...
| 课程产品统一课件工作区 | M1（Step 0～4A 已确认；Step 4B 单页草稿施工中） | M4 | doc 16 §14 已记录人工确认门；预览、正式课编辑与微课编辑使用同一 `CoursewareWorkbench` 面板树、目录、舞台、翻页、元素检查器和图层面板。Step 4A 已确认 PageDoc／爱学习共用的 4:3 视口策略与单页会话预览；Step 4B 只接一个 PageDoc 样本页的草稿保存与刷新重读；整体重构完成后、扩量前仍需双端定稿组件表现 | DEV-CW-1/9 |
