...
- Gate 1 当前 `PASS`：Xiaomi 目标指纹、危险写拒绝、唯一正式 admin MFA、首名真实教师及双岗位、active 8 条 protected/0 条 purge manifest、current/previous、健康探针和错误查询位置均已确认。课堂 Stage A/B1 后，生产数据库 ledger 为 193 条、head 仍为较晚的 `20260825000800_account_center_profile`，`20260825000700_classroom_learning_fill_bulk` 已按规范化 checksum 登记；应用 current/previous=`20260825-085754` / `8c303a2…` 与 `20260825-072801` / `72d8127…`。board checkpoint/input 为 version 2 / true，layout/H5 为 version 1 / false；启用瞬间 v2 checkpoint/star/learning result 均为 0，唯一 roster revision 仍为 1 且 entry 为 0。新的 PostgreSQL-only pre-change 备份已核验；最近 PostgreSQL+Storage 同批次备份仍为 `mathin-20260822T093529Z`，两者证据边界不互相替代。
...
- 课堂可扩展互动在开发轨使用版本化 capability provider 和统一 conformance；普通 renderer 复用既有输入原语时不逐个重复人工 Gate。新增原语、输入路由/所有权算法、跨 iframe 协议和真实设备回归仍单独人工验收；未声明或不匹配 provider 的 renderer 继续 fail closed。M3a/M3b 已在 commit `00f4766` / `61c6c70` 完成开发端产品初验，M4a 的稳定身份、星星 v2 set/undo 和确认式名单 revision 三项随后通过；2026-08-25 产品负责人又以 `95ed9f1` 为应用基线确认 M4b 教师布局与逐题学情通过，因此 M0–M4 开发端整体关闭。M5 候选 `8c303a2` 已完成 Stage A，并以 board checkpoint/input 两开关进入 Stage B1；layout/H5 尚未启用，B1 尚待真实测试课堂手感验收，因此仍不能记为 H5 pointer、M4/M5 或 Gate 2 已在生产通过。
