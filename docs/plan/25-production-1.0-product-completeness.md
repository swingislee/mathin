...
- `DEV-CW-1` 当前为 `STEP 4A SHARED 4:3 PREVIEW / DEV READY / AWAITING USER AUDIT`：产品负责人已确认 Step 3B 的唯一元素检查器、可折叠完整图层列表和公共层级设置。正式 PageDoc 与爱学习 source runtime 已接入同一份 4:3 策略状态、分类面板、对照画布和撤销／重置逻辑；当前待审模型为四种整页视口变换与一个 PageDoc 分层模式，爱学习默认“等高居中”，E 系列 PageDoc 默认“等宽顶部”。粗调只在对照视图呈现，单轨进入页面微调。此批不推进 `draft_revision_id`，不接文件上传、binding 改写或共享资源替换；整讲、跨页替换、schema/RPC、批量回填、生产数据库/Storage/release 均不在授权范围。
