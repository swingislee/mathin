...
>
> **基线日期**：2026-08-23；每次验收另记 commit、migration head、环境和数据 manifest。
>
...
- Gate 1 当前 `PASS`：Xiaomi 目标指纹、危险写拒绝、唯一正式 admin MFA、首名真实教师及双岗位、active 8 条 protected/0 条 purge manifest、current/previous、健康探针和错误查询位置均已确认。生产数据库 ledger 为 193 条、head=`20260825000800_account_center_profile`，`20260825000700_classroom_learning_fill_bulk` 已登记；应用 current/previous=`20260825-085754` / `8c303a2…` 与 `20260825-072801` / `72d8127…`。产品负责人已确认 Stage B1/B2，生产 checkpoint version/chunk/head=`2/2/2`；Stage B3 核对到当前课次/发布引用的 399 条 H5-kind 页面记录、409 条 binding 均走未接 bridge 的 Aixuexi 舞台后已安全回退，这些记录不是 399 个实际 iframe。board/input/layout 为 version 2 / true，H5 为 version 3 / false，star/learning result 仍为 0。新的 PostgreSQL-only pre-change 备份已核验；最近 PostgreSQL+Storage 同批次备份仍为 `mathin-20260822T093529Z`，两者证据边界不互相替代。
...
- 普通教师短期微课开发轨 `DEV-TMC-1` 已在本机实现并完成机器检查：一份自由课次惰性孵化一门单讲 `microcourse`，作者可用草稿试讲，校内共享须经现有 Courseware review/release 机制审核发布；其他具备建班权限的校内 staff 只从已启用且存在正式 release 的目录搜索建班。首版的选页来源只接受正式 `curriculum` 当前 release，元数据、页面、资源与 H5 hash 在提交时共同冻结；原自由课次和历史课次保持原 release。仓库 head=`f418dd9`、本机 migration head=`20260826000600_teacher_microcourse_runtime_fixes`，SQL/Vitest/固定账号 Playwright/CI/R1-Live 合同均通过；产品负责人开发初验仍为 `UNKNOWN`。该能力由 `teaching.teacher_microcourses_v1` 默认关闭，本任务未连接 Xiaomi，生产迁移和启用尚未发生。
- 课堂可扩展互动在开发轨使用版本化 capability provider 和统一 conformance；普通 renderer 复用既有输入原语时不逐个重复人工 Gate。新增原语、输入路由/所有权算法、跨 iframe 协议和真实设备回归仍单独人工验收；未声明或不匹配 provider 的 renderer 继续 fail closed。M0–M4 开发端已整体关闭，M5 候选 `8c303a2` 已完成 Stage A；产品负责人于 2026-08-26 确认 Stage B1/B2 通过。Stage B3 的生产 H5 类型核对揭示开发 fixture 与实际 `aixuexi-page-doc-v1` renderer 错位，开关已关闭。开发候选 `f1f8d98` 新增按不可变 package SHA-256 登记的 active 能力档案，并让通用魔法校混合页和爱学习嵌入页复用同一 frame 注册与 pointer bridge；H5 原始包自带声明会被剥离，缺档案、档案不匹配或握手失败均锁住 Smart 接管。`e2ff273` 把一级 UI 收敛为一个 Smart 开关：关闭或不可用时，指针工具把输入交给课件，笔、颜色、橡皮及其他绘图工具把输入交给板书，不再要求教师另选交互锁/书写锁。`c03c382` 把该入口收成 `112×44px` 的横向二态滑动条，与底栏触控目标等高并移除圆形星光按钮；`aa6b75b` 去掉开关外围边框和圆角底板，以玫瑰强调色 SVG 与滑轨表达开启态。该候选只在本机开发库应用 migration，等待产品负责人开发初验，因此仍不能记为 H5 pointer、M5 或 Gate 2 已在生产通过。
...
| 部署/备份/恢复 | M2（生产发布/当前备份） | M4 | commit `35b9f60` 与[部署子门](../evidence/r1/r1-16-deployment-preflight.md)已固定 fail-closed 合同；仓库/历史 secret scan 已关闭。2026-08-25 课堂 Stage A 后 current/previous=`20260825-085754` / `8c303a2…` 与 `20260825-072801` / `72d8127…`，原子切换及内外健康门通过；数据库 ledger=`193`、head=`20260825000800`。当前 PostgreSQL+Storage 同批次备份已独立通过 TOC、文件数、源前后清单与 SHA-256，课堂发布另有新鲜 PostgreSQL-only pre-change 备份；previous 兼容回退、恢复演练、异机/静态加密备份和错误 release 标签属于 M4 | Live/16 |
...
学年与运营周期采用两层合同。`school_years` 是学生年级的年度归属，创建时只登记起始年并建立四个周期，不要求预先知道放假或开学日期，也不改学生年级；`school_terms` 固定为 `暑期 → 秋季 → 寒假 → 春季`，日期成对可空，临近后补，周期重叠只形成运营提示。暑期属于新学年，因此“二升三”的暑假按三年级归属。启用下一学年必须先显示一至十一年级晋级数和十二年级保留数，再填写该起始自然年内的生效日并显式确认；切换当前运营周期不触发晋级。课表页是唯一学年管理入口，机构设置只维护节假日，避免两处合同漂移。产品负责人确认 `2025–2026` 学年春季结束日为 `2026-06-29`；此前挂在旧春季、且全部课次晚于该日的 production 班级只迁移到 `2026–2027` 秋季归属，不改班级状态、教师、课次时间或学生年级。migration `20260823000100_r1_live_school_year_periods` 已通过最终空库重建、旧库升级、回滚式晋级演练、生产 preflight、完整回滚演练和独立 postflight，并随应用 `b899942…` 上线；当前 2026 学年仍为 planning、promotion=0。
