...
- Gate 1 当前 `PASS`：Xiaomi 目标指纹、危险写拒绝、唯一正式 admin MFA、首名真实教师及双岗位、active 8 条 protected/0 条 purge manifest、current/previous、健康探针和错误查询位置均已确认。生产数据库 ledger 为 211 条、head=`20260828000200_courseware_summer_a_plus_catalog`；应用 current/previous=`20260828-071313` / `087b497…` 与 `20260828-071024` / `087b497…`。产品负责人已确认 Stage B1/B2，生产 checkpoint version/chunk/head=`2/2/2`；Stage B3 共同 bridge、权威档案表与单一 Smart 开关已部署，但 H5 保持 version 3 / false，空档案表不授权任何现有 package。board/input/layout 为 version 2 / true，star/learning result 仍为 0。最新 PostgreSQL+Storage 同批次全量备份 `mathin-20260828T052842Z-teacher-microcourse-variant-087b497` 已通过数据库 TOC、Storage 全量清单与全部 SHA-256 独立复核；未执行 restore 或 prune。
...
| 部署/备份/恢复 | M2（生产发布/当前备份） | M4 | commit `35b9f60` 与[部署子门](../evidence/r1/r1-16-deployment-preflight.md)已固定 fail-closed 合同；仓库/历史 secret scan 已关闭。2026-08-28 DEV-TMC-2 授权发布后 current/previous=`20260828-071313` / `087b497…` 与 `20260828-071024` / `087b497…`，原子切换及内外健康门通过；数据库 ledger=`211`、head=`20260828000200_courseware_summer_a_plus_catalog`。当前 PostgreSQL+Storage 同批次全量备份 `mathin-20260828T052842Z-teacher-microcourse-variant-087b497` 已独立通过 dump TOC、Storage 文件清单、源前后计数与 SHA-256；previous 兼容当前 schema。恢复演练、异机/静态加密备份和错误 release 标签属于 M4 | Live/16 |
