...
`DEV-DASH-1` 规定班级默认 scope 为“本人任教 → 本人负责 → 两者为空时全部”，个人 scope 使用信息卡，全部与测试 scope 使用分页表格；普通列表每页 20 条。课件资源库每页只读取 11 条用于判断下一页，最多展示并签名 10 条预览。机构资料、校区目录和教学日历采用线性设置区或表格。本机 migration `20260829000100_classroom_personal_scope_default`、定向合同 22/22 与固定账号 Playwright 2/2 已通过，产品负责人页面验收仍为 `UNKNOWN`，且没有执行生产写入。
