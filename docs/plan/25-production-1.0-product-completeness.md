...
>
> **基线日期**：2026-08-28；每次验收另记 commit、migration head、环境和数据 manifest。
>
...
- Gate 1 当前 `PASS`：Xiaomi 目标指纹、危险写拒绝、唯一正式 admin MFA、首名真实教师及双岗位、active 8 条 protected/0 条 purge manifest、current/previous、健康探针和错误查询位置均已确认。生产数据库 ledger 为 212 条、head=`20260828000210_classroom_offering_and_activity_kind`；应用 current/previous=`20260828-075322` / `c7c8219…` 与 `20260828-071313` / `087b497…`。产品负责人已确认 Stage B1/B2，生产 checkpoint version/chunk/head=`2/2/2`；Stage B3 共同 bridge、权威档案表与单一 Smart 开关已部署，但 H5 保持 version 3 / false，空档案表不授权任何现有 package。board/input/layout 为 version 2 / true，star/learning result 仍为 0。最新 PostgreSQL+Storage 同批次全量备份 `mathin-20260828T052842Z-teacher-microcourse-variant-087b497` 与本轮当前 PostgreSQL 写前备份 `mathin-db-prechange-20260828T074643Z-classroom-offering-c7c8219` 均已验证；未执行 restore 或 prune。
...
- 普通教师短期微课开发轨 `DEV-TMC-1` 已在本机实现、完成机器检查并获准部署生产：一份作品映射一门单讲 `microcourse`，作者可用草稿试讲，校内共享须经现有 Courseware review/release 机制审核发布；其他具备建班权限的校内 staff 只从已启用且存在正式 release 的目录搜索建班。来源只接受正式 `curriculum` 当前 release；入口复用课程产品 `CoursePicker`，先按课程族/版本、年级、课程季节、班型和历史版本选课程，再只列课次名称与页数，一次事务插入本讲全部页，编辑器内再删选与排序，不加载页面预览。通用 `game-page-v1`、游戏内容注册表、服务端校验凭证与来源 capability predicate 接入四宫/六宫/九宫数独。DEV-TMC-1 首轮本机 CI 16/16、全量 Vitest 106 文件 742 通过/1 条件跳过、固定账号 Playwright 1/1 和 production build 通过；12 个 migration 经 backup/rollback/零残留/formal 于 2026-08-27 上线，`teaching.teacher_microcourses_v1`=version 2 / true。当前生产 app 已由后续 DEV-TMC-2 候选推进到 `087b497…`；产品负责人生产写态验收仍为 `UNKNOWN`。
- `DEV-TMC-2` 已获明确授权并部署生产：自由课次允许任课教师与教研直接建立并行课件方案，编辑他人方案时派生新 head，主讲在开课前选择本节使用方案；课次使用不经过审核，校内共享发布仍沿用 DEV-TMC-1 审核/release。候选同时纳入已结课班级重新启用入口，明确排除未获授权的班级课型/活动类型增量。本机两份 SQL、定向 Vitest 8 文件 64/64、固定账号 Playwright 新旧旅程 2/2、CI 16/16、全量 Vitest 108 文件 761 通过/1 条件跳过与 production build 通过；生产三 migration 经同批次全量 backup、rollback/零残留/formal 后把 ledger 推进到 `211`，双 release current/previous=`20260828-071313` / `20260828-071024`、commit=`087b497…`。数据库/ACL、HTTP、服务、错误、业务不变量和登录态审核页只读 smoke 通过，生产方案/选用记录仍为 0；产品负责人生产写态验收仍为 `UNKNOWN`。
...
| 机构配置/场地/Feature Flag | M3 | M4 | R1-1 已完成旧设置中心与版本化开关；`DEV-ORG-1` 正在开发端把机构资料、校区/教室目录、机构级学年/日历/排课默认和系统能力发布拆分，并以兼容双写保留一个回退窗口。M4 仍需开发人工验收、生产初始化、正式角色旅程和发布门 | 1/DEV-ORG-1/15/18 |
...
| 部署/备份/恢复 | M2（生产发布/当前备份） | M4 | commit `35b9f60` 与[部署子门](../evidence/r1/r1-16-deployment-preflight.md)已固定 fail-closed 合同；仓库/历史 secret scan 已关闭。2026-08-28 `POST-LIVE-OPS-02` 发布后 current/previous=`20260828-075322` / `c7c8219…` 与 `20260828-071313` / `087b497…`，原子切换及内外健康门通过；数据库 ledger=`212`、head=`20260828000210_classroom_offering_and_activity_kind`。同日 PostgreSQL+Storage 全量备份继续作为 Storage 回退基线，本轮当前 PostgreSQL 写前备份已通过 dump TOC、前后计数与 SHA-256；新 RPC 末尾参数带默认值，previous 兼容当前 schema。恢复演练、异机/静态加密备份和错误 release 标签属于 M4 | Live/16 |
...
机构后台按工作对象组织，不再保留“大而全”的设置中心。`/dashboard/organization` 只保存机构名称与统一 IANA 时区；中文是产品固定默认语言，不形成无效设置项。`/dashboard/campuses` 维护校区目录及其教室，校区字段只有名称、可空地址和状态，教室字段只有名称、可空容量和状态。内部代码由数据库生成，页面、表单和公开 DTO 均不可见。校区只作为教室的上一级目录，不承担班级、员工、权限、学年、规则或 Feature Flag 作用域；所有员工继续按既有岗位或本人班级权限访问全部场地，不建立全局校区切换。
...
教学日历支持全机构记录和“某校区全部教室”的局部例外。停课支持日期范围；教学日与补课日是单日，可按指定周几映射排课，也可仅手动开放。自动排课跳过停课范围，并按日期顺序加入映射日；校区记录覆盖全机构记录，同一作用范围内禁止重叠。课次地点待定时只应用全机构日历并提示不能判断校区例外；人工安排到有效停课日时必须确认并填写原因，追加领域事件。排课默认首版只保存新班级默认时长，初始为 90 分钟；冲突策略固定为警告。原始 JSON 业务规则停止新写入并只保留历史，Feature Flag 进入“运行与错误 → 能力发布”，修改要求 `system.operations.manage`，`audit.view` 只读历史，财务发布门继续固定关闭。

R1-1 的 `20260728000100_r1_organization_settings.sql` 与 `20260728000200_r1_public_publish_guard.sql` 是旧合同和回退基线。`DEV-ORG-1` 先新增 `organization.profile.manage`、`location.manage`、不含代码字段的 V2 DTO/RPC、结构化场地引用和兼容双写；旧设置 RPC、`classrooms.room`、默认校区与学年校区列至少保留一个生产回退窗口。迁移前必须输出自由文本教室候选、学年/周期重复差异、有效校区级规则或开关覆盖和正式业务影响；多校区无法唯一匹配、学年日期/状态冲突或有效覆盖存在时直接失败，不猜测也不静默丢弃。开发验收并明确退休 previous 后，才以独立授权增量删除旧合同。该工作当前只在开发预演轨，不授权生产写入。
...
| --- | --- | --- | --- | --- |
| R1-Live 正式目标 Golden Path 未完成 | 本机隔离 Golden Path 1/1 已通过并关闭 `lead → enrolled` 合同冲突；生产已有 1 个班级、15 个课次和 1 条 active 报名，学年/秋季归属修复已上线；仍缺正式教师点名保存与刷新或重登再读、管理员对照和越权对照 | 班级、课次和报名存在只能证明闭环前半段；尚不能证明正式教师能完成持久点名及权限闭环 | 教学运营+QA/发布 | R1-Live Gate 2 |
