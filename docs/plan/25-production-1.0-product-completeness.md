...
- Gate 1 当前 `PASS`：Xiaomi 目标指纹、危险写拒绝、唯一正式 admin MFA、首名真实教师及双岗位、active 8 条 protected/0 条 purge manifest、current/previous、健康探针和错误查询位置均已确认。2026-08-31 课件 session 修复集中发布 postflight 后，生产数据库 ledger/head=`236 / 20260830000700_teacher_microcourse_editor_unification`；应用 current/previous=`20260831-052938` / `05a1027…` 与 `20260830-084610` / `6e0b5bd…`。产品负责人已确认 Stage B1/B2；Stage B3 共同 bridge、权威档案表与单一 Smart 开关已部署，H5 仍按既有 provider 合同 fail closed。最近 PostgreSQL 写前备份 `mathin-db-prechange-20260830T042220Z-tmc-unification-8b9b195` 与 PostgreSQL+Storage 同批次全量备份 `mathin-20260828T052842Z-teacher-microcourse-variant-087b497` 已在 preflight 中核对；本轮 app-only 发布没有导入、schema、业务、课程 release、审核快照或 Storage 写入，未执行 restore 或 prune。
...
- `DEV-CW-1` 当前为 `STEP 2A REVISION READY FOR USER AUDIT`：Step 1/2 已获产品负责人确认；正式课件与教师微课现共用 `CoursewareEditorWorkbench`。两轮审计后的开发修订让只读微课复用 `CoursewarePreviewWorkspace` 及方向键／PageUp／PageDown／空格翻页，按 `487b60d` 恢复 composition 顶栏插入与左栏重命名，把正式工作区底部安全区压缩到约 4px，并让正式课件复用同一 `CoursewareEditorToolbar`；正式右栏标题行只切换微调／4:3／替换，插入退出右栏正文。PageDoc/source-runtime/composition adapter 仍保留不同能力与写态。该结果只有开发端 TypeScript、ESLint 和定向合同证据，尚无 Step 2A 视觉与微课回归确认；当前没有新增 editor action、schema、开发库/Storage 或生产写入授权，Step 3 继续关闭。
...
| 课程产品统一课件工作区 | M1（Step 1/2 已确认；Step 2A 修订待审） | M4 | doc 16 §14 已记录人工确认门；正式课件与教师微课共用 `CoursewareEditorWorkbench` 与 `CoursewareEditorToolbar`，只读微课另复用成熟 `CoursewarePreviewWorkspace`。composition 已按历史正确版本恢复标题栏插入和左栏重命名；正式右栏只保留微调、4:3、替换三种检查面。PageDoc/source-runtime/composition adapter 继续区分 A～F、12×9 网格、来源限制与写态。尚未新增 editor action、schema、持久化或来源 patch；Step 3 在共享框架人工确认前保持关闭 | DEV-CW-1/9 |
...
| 部署/备份/恢复 | M2（生产发布/当前备份） | M4 | commit `35b9f60` 与[部署子门](../evidence/r1/r1-16-deployment-preflight.md)已固定 fail-closed 合同；仓库/历史 secret scan 已关闭。2026-08-31 课件 session app-only 集中发布后 current/previous=`20260831-052938` / `05a1027…` 与 `20260830-084610` / `6e0b5bd…`，原子切换及健康门通过；数据库 ledger/head=`236 / 20260830000700_teacher_microcourse_editor_unification`。最近 PostgreSQL 写前备份为 `mathin-db-prechange-20260830T042220Z-tmc-unification-8b9b195`，既有 PostgreSQL+Storage 同批次全量备份继续作为 Storage 回退基线；本轮没有导入、schema、业务、课程 release、审核快照或 Storage 写入，previous 是应用回退点。恢复演练、异机/静态加密备份和错误 release 标签属于 M4 | Live/16 |
