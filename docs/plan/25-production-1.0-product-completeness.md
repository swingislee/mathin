...
- `DEV-STAFF-ONBOARD-1` 当前为 `DEVELOPMENT READY / AWAITING PRODUCT OWNER UI AND INTERACTION ACCEPTANCE`：本机 migrations `20260902000700_staff_bulk_onboarding`、`20260902001000_staff_direct_provisioning` 与 `20260902001100_staff_initial_password_hardening` 让员工页接受中文三列模板及魔法校 Tab 四列名单，逐行预检后由主管及以上直接创建 Auth 账号、员工档案和岗位；成功档案立即可用于排课，首次登录密码只在本次下载响应出现，建档触发完成后同事务清除 Auth 临时元数据键。新员工本人首次登录必须输入并二次确认不同的新密码，完成前 staff 权限/RLS fail closed，完成后再自行处理必要同意；系统不伪造联系方式验证或隐私同意。`staff.invite` 只开放建档，停用、改岗和既有顾客身份提升仍要求 `staff.manage`，含 `permission.configure` 的岗位仍只允许顶层 admin 预授。失败行可在同一批次重试，数据库不保存明文密码。迁移回滚演练、正式本机应用、数据库回滚断言、相关 Vitest 16/16、受影响 ESLint、TypeScript、双语键与类型摘要检查通过；固定测试管理员权限和既有账号／profile／邀请／批次数量无漂移。当前没有 Xiaomi／生产 migration、身份创建或业务写入，页面、首次登录和交接操作仍待产品负责人人工验收。
...
开发增量 `DEV-STAFF-ONBOARD-1` 已把单人 claim 邀请替换为员工页的版本化 ImportBatch 直接建档，并为魔法校四列员工名单建立显式适配：Tab 是表格列分隔符，岗位单元格内的逗号只拆来源岗位，性别不进入员工档案；来源岗位必须映射到现有 Mathin staff role 或被操作者明确忽略。该直接建档与首次强制改密合同仍只在本机开发验收，不表示生产 migration、员工身份创建或业务写入已经获准；生产现有 claim 邀请与正式教师登录事实不被本增量改写。
