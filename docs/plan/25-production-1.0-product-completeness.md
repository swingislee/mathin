...
- 普通教师短期微课已登记为开发轨 `DEV-TMC-1`：一份自由课次惰性孵化一门单讲 `microcourse`，作者可用草稿试讲，校内共享须经现有 Courseware review/release 机制审核发布；其他教师只从已启用且存在正式 release 的目录搜索建班。首版的选页来源只接受正式 `curriculum` 当前 release，元数据、页面、资源与 H5 hash 在提交时共同冻结；原自由课次和历史课次保持原 release。该能力由 `teaching.teacher_microcourses_v1` 默认关闭，开发验收不等于生产授权。
