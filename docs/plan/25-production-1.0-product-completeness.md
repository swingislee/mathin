...
| --- | --- | --- | --- | --- |
| R1-Live 正式身份/数据和防误清未成立 | 目标组合指纹与匿名计数已登记；危险写入口、两个 R1-Live migration 和 active protected-only manifest 已生效；正式 admin auth/profile 与 2 个 production 课程族根受保护，4 个 protected、0 个 purge 条目，候选列表为空且数据零漂移。production role RPC 尚未以真实授权动作验收，正式教师/真实闭环对象和课次内容保护仍缺 | 正式管理员身份、migration 与当前正式对象保护已成立；首名真实教师和业务数据尚未建立，Gate 1 仍不能通过；双人恢复不加入 R1-Live 当前执行链 | 产品+教学运营+运维+安全 | R1-Live Gate 1 |
