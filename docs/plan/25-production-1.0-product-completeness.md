...
| 语言 | `messages/zh.json` 与 `messages/en.json` 各 4028 个 key；`content/en` 仅 README | UI 永久 zh/en；英文课程/文章可延期，缺失内容显示明确回退或“尚未发布”状态 |
...
| 对象 | 最近可核对事实（截至 2026-08-12） | 尚不能支持的结论 |
| --- | --- | --- |
...
| Vitest | commit `cbb2a0f` 已将本轮修复前 19 项失败清零；当前为 84 个测试文件、544/544 全绿，其中非 spatial 基线 51 个文件、290/290，空间数学/SML-0 专项 33 个文件、254/254，`pnpm r1:test` 为 18 个文件、121/121 | 单元/合同套件已恢复全绿；SML-0 与 R1-14 后续实现均须维持 100%，并补发布目标、并发、文件和竞争矩阵 |
...
当前等级截至 2026-08-12，并按已登记的阶段或子门证据更新；后续 R1 阶段仍须生成对应 E2/E3/E4 才能提升至 M4。
...
| 部署/备份/恢复 | M2（只读 preflight） | M4 | commit `35b9f60` 与[部署子门](../evidence/r1/r1-16-deployment-preflight.md)已固定 fail-closed 合同；commits `3077fee`、`82c0920`、`8e5c076` 与[仓库扫描子门](../evidence/r1/r1-16-repository-secret-scan.md)已关闭当前树/历史 high-confidence secret scan。独立环境、运行时 secret、监控探针、数据库/Storage 恢复、应用回滚与非执行者复核仍缺 | 16 |
...
| Vitest 基线需在最终 build 复验 | commit `cbb2a0f` 已清零本轮修复前 19 项失败；当前为 84 个测试文件、544/544 全绿，非 spatial 290/290，空间数学/SML-0 专项 254/254，R1 定向 121/121 | 后续实现可能重新引入合同回归 | 技术+QA | SML-0 与 R1-14 保持全量 100% |
