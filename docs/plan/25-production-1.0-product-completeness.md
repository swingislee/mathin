...
- `DEV-CW-1` 当前为 `STEP 3A UNIFIED HOST / IN PROGRESS`：产品负责人确认从成熟预览组件升级，预览、正式课编辑和微课编辑统一为 `CoursewareWorkbench` 三种模式；共用 renderer、资源加载和完整适配画布，预览裁掉编辑顶栏/右栏，微课默认 4:3，正式课只增加 4:3 适配。首轮审计否决只共享 dispatcher／槽位的实现，并发现正式课保存入口消失、微课保存回到右栏；当前无数据写入增量已让正式 PageDoc 与微课 composition 共用唯一编辑适配表面，顶栏统一承载插入和保存，待人工验收。四年级秋季标题偏移已确认为源站特例。爱学习写入、整讲、跨页替换、schema/RPC、批量回填、生产数据库/Storage/release 均不在授权范围。
...
| 课程产品统一课件工作区 | M1（Step 0～2A 已确认；Step 3A 统一宿主施工中） | M4 | doc 16 §14 已记录人工确认门；预览、正式课编辑与微课编辑统一为 `CoursewareWorkbench` 三种模式并共用完整适配舞台，正式模式只额外开启 4:3。固定 E 系列单页的写态已实现但因来源 renderer 一致性缺口暂停验收；Step 3A 继续用内容寻址来源 runtime 与语义签名 Gate 覆盖 E 系列和爱学习宿主边界，Step 3B 再复审保存。整体重构完成后、扩量前再双端定稿组件表现 | DEV-CW-1/9 |
