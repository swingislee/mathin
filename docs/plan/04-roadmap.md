...
> **规划状态**：`active`
>
> **当前施工阶段**：`R1-7 · 初始化、导入、质量、修复与导出`
>
> **当前未结项**：R1-6 已关闭。R1-7A 学生 CSV 导入、R1-7B 初始化 manifest 与 R1-7C 版本化数据质量扫描已完成；当前进入 R1-7D，先为每个领域修复冻结影响计数、目标集合 hash、恢复点和执行/回滚审计，再进入 R1-7E 按角色裁剪的自动过期导出。
>
...
| **R1-7 初始化、导入、质量、修复与导出（当前）** | R1-1～6 | 版本化初始化 manifest；CSV dry-run、逐行错误、去重和幂等重跑；检测孤儿、重复主体、非法状态、金额不平、缺失内容对象；区分运营导出和用户权利导出 | 干净库可初始化；错误批次不留半成品；修复有影响计数、审计和恢复点；导出按角色裁剪字段并自动过期 |
