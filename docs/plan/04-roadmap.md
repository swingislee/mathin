...
3. **正式管理员（身份与登录完成，恢复待登记）**：唯一非固定 Gmail 已完成 verified MFA；一次性受控事务已把该账号 `staff→admin`、把原固定开发 admin `admin→staff`，提交后唯一 active admin 恰好为 1。本人已退出并重新登录，完成 MFA challenge 后成功打开 admin 路由，新会话 AAL2 和应用授权通过。下一步登记一名不同于管理员本人的实际第二恢复联系人；`BUG-R1-LIVE-001` 继续单列修复并在隔离库回归，恢复日常 `admin_set_identity`/profile 角色管理合同。
...
Gate 1 的环境写入需要产品负责人逐项明确授权。正式管理员的 `student→staff` 引导、本人 MFA、`staff/admin` 原子交接和新会话登录验收均已完成；当前只登记恢复责任，不授权其他数据库写入。manifest、真实教师和班级数据仍需后续逐项授权。
