...
产品负责人于 2026-08-31 依次确认 Step 0、Step 1、Step 2 和 Step 2A，并于 2026-09-01 确认 Step 3A 与 Step 3B。Step 3B 最终把文字／图片／形状节点分发、公共透明度／层级／显隐和可折叠图层列表收敛为唯一共享组件；正式课与微课只提供各自文档 adapter。产品负责人要求课件工作区整体重构完成后再双端回看并定稿组件表现，因此该回看仍列为 Step 7 完成后、Step 8 扩量前的人工门。`DEV-CW-1` 现为 **STEP 4A SHARED 4:3 PREVIEW / DEV READY / AWAITING USER AUDIT**：正式 PageDoc 与爱学习 source runtime 已接入同一份策略状态、分类面板、对照画布和撤销／重置逻辑。2026-09-01 截图审计后，A～F／自定义退出当前模型；爱学习和魔法校共用“等宽顶部、等宽居中、等高左侧、等高居中”四种整页视口变换，PageDoc 额外支持“背景等高·内容等宽”的分层页面模式。四种通用模式始终整体变换既有 renderer，避免背景与内容分裂；粗调面板只在对照视图出现，单独 16:9／4:3 保留页面微调。后续复审把缺独立 release 但可生成默认版式的页面从“4:3 不存在”改为直接会话预览，并让爱学习 source Viewer 在固定逻辑 viewport 内渲染、由共享宿主整体缩放，退出单轨二次拟合和白色审阅余边。此批不持久化、不新增 schema/RPC、不改 binding/release、不扩到整讲、批量或生产，不改变 R1-Live Gate 2。
