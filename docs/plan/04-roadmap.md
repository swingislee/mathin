...
> **规划状态**：`active`
>
> **当前施工阶段**：`R1-8 · 财务正式闭环或安全关闭`
>
> **当前未结项**：R1-7 已关闭；学生 CSV、初始化 manifest、版本化质量扫描、领域修复计划、用户权利 artifact 与运营导出分流均通过开发库验证。当前进入 R1-8，先按默认关闭合同核对财务路由、导航、work item、通知、指标和 job；只有完整账务门全部通过才允许启用。
>
> **核对日期**：2026-08-01；依据代码、迁移、内容目录、CI、Git、R1-6 教学成果链路、R1-7 导出链路与 doc 00～26。
...
| **R1-8 财务正式闭环或安全关闭（当前）** | R1-1～4、R1-7 | 默认关闭。若启用，完成订单、收款、退款/冲正、学生账户、台账、对账、异常、权限和导出；若未达门，关闭页面入口、work item、通知、指标和 job | 启用：账务 E2E、幂等、金额平衡和权限全部通过；关闭：路由/导航/任务/指标无可操作入口 |
