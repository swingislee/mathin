...
产品负责人于 2026-08-31 依次确认 Step 0、Step 1、Step 2 和 Step 2A，并于 2026-09-01 确认 Step 3A。Step 3A 的最终结果是一个共享工作台直接承载预览／正式课／微课三种模式，共用目录、顶栏、画布、翻页、文字编辑和保存反馈；正式课只额外开启 `adapt4x3`。固定第 5 页隐藏门与无写入旧样机已删除，正式 PageDoc 按当前页／轨道进入真实编辑链，来源 runtime 无 patch 协议时明确只读，标题保存不再依赖文档类型。产品负责人要求课件工作区整体重构完成后再双端回看并定稿组件表现，因此该回看仍列为 Step 7 完成后、Step 8 扩量前的人工门。`DEV-CW-1` 现为 **STEP 3B SHARED IMAGE / READY FOR USER AUDIT**：正式课与微课已直接复用唯一图片元素面板，统一填充方式、位置尺寸、透明度、层级和显隐；正式课继续沿用 PageDoc 单页草稿自动保存，未接文件上传、binding 改写或共享资源替换。4:3 A～F、爱学习写入、整讲回填、跨页替换、schema/RPC、批量任务和生产动作仍禁止，不改变 R1-Live Gate 2。
