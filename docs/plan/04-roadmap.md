...
产品负责人于 2026-08-31 依次确认 Step 0、Step 1、Step 2 和 Step 2A。Step 2A 最终确认一个共享工作台直接渲染页面目录、插入顶栏、画布/翻页、右栏标题和右栏正文，两条 adapter 共用 `CoursewareInsertionToolbar`，微课是共同能力基线，正式课只额外开启 `adapt4x3`。产品负责人要求课件工作区整体重构完成后再双端回看并定稿组件表现，因此该回看列为 Step 7 完成后、Step 8 扩量前的人工门。`DEV-CW-1` 现为 **STEP 3A UNIFIED HOST / IN PROGRESS**：预览、正式课编辑和微课编辑统一为 `CoursewareWorkbench` 三种模式，共用 renderer、资源加载和完整适配画布；预览不加载编辑顶栏/右栏，微课默认 4:3，正式课只多 4:3 适配。首轮审计发现模式 dispatcher 加共享画布仍不足，正式课与微课各自装配控件导致保存入口和位置回退；当前改为两种编辑 adapter 共用唯一编辑表面，插入与保存由同一顶栏承载。四年级秋季标题偏移已确认为源站特例并退出本轮。爱学习写入、整讲回填、跨页替换、schema/RPC、批量任务和生产动作仍禁止，不改变 R1-Live Gate 2。
