...
机构资料、校区目录和教学日历采用线性设置区或表格，不再默认堆叠卡片。班级无显式 scope 时先选择本人任教班级，其次本人负责班级，仅在两者均为空时回退到全部班级并提示；个人班级保留信息卡，`all` 与 `test` 使用表格，列表按 20 条分页。课件资源库首屏和后续页每次只读取 11 条判断下一页、最多展示并生成 10 条预览签名，避免为整库对象生成临时 URL。
