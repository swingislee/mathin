...
| 当前全量 Vitest 共 90 个测试文件、595 项通过、1 项因未提供爱学习生成包根而条件跳过；其中非 spatial 为 53 个文件、316 项通过、1 项条件跳过，空间数学/SML-0 专项为 37 个文件、279/279，`pnpm r1:test` 为 21 个文件、156/156 | 已执行的单元/合同保持通过；条件跳过不能作为发布证据。正式 Playwright 框架已有本地基线，仍须补发布目标、写态、zh/en、跨浏览器、连续无 flaky、并发、文件和竞争矩阵 |
...
| 阶段 | 依赖 | 动作与产物 | 退出证据 |
| --- | --- | --- | --- |
| **R1-9 Terms 与内容发布链/P6-9** | R1-1～3、R1-7 | 当前推进 P6-9：验证 E 系列 1135 讲和爱学习 G+/X+/A+ 秋季 170 讲的两轨文档、binding、对象和不可变 release；v4 只读来源 manifest 已同步 90+12 门 roster、1305 讲/2610 条 release-1，绑定 current track head 选中的 immutable release、raw snapshot 与含 H5 `launchQuery` 的规范投影。固定 `REPEATABLE READ READ ONLY` SQL、纯导出组装、普通对象流式 hash、H5 精确文件集与逐文件字节校验、受控数据库/Storage runner 和原子 artifact 输出已落地；E adapted 4:3 审批、受控 artifact 根和路径隔离合同已冻结。Notebook 所需共享发布合同可继续；71 篇中文 Terms 的 slug、公式、依赖、引用、搜索、SEO 巡检暂缓 | P6-AIX-2 见[多难度证据](../evidence/r1/r1-9-aixuexi-courseware.md)，来源 manifest 见[合同证据](../evidence/r1/r1-9-courseware-source-manifest.md)。批准副本配置、经审核的 E 系列 provenance、1305 行真实运行、`cw-objects`/`cw-h5` 字节结果与非执行者复核仍 pending；Terms 坏链/悬空关系、英文回退和全量资源门未全部通过，R1-9 不关闭 |
