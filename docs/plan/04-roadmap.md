...
产品负责人于 2026-08-31～09-02 依次确认 Step 0～6。Step 3B 把文字／图片／形状节点与图层能力收敛为共享组件；Step 4A/4B 打通共享 4:3 控制器、旧 A～F 映射和单页草稿；Step 5A/5B 打通页面上下文影响预览、单样本替换与回滚；Step 6 让爱学习来源 Viewer 通过共享编辑桥获得选择、文字、几何、图层、网格与历史能力。`DEV-CW-1` 现为 **STEP 7A REVIEW OBJECT SEPARATION / READY FOR USER AUDIT**：正式课适配审核与教师微课目录审核已拆成独立工作区和侧栏入口，旧 URL 保留兼容跳转；不删除研发任务、适配校对、公共资源或其他旧入口，不合并两套审核状态机，不开放新的游戏/H5/工具能力，也没有数据库、Storage、release 或生产写入。整体重构完成后仍在 Step 7 后、Step 8 前双端回看定稿组件表现；本项不改变 R1-Live Gate 2。
