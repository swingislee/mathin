...
> **当前施工阶段**：`R1-5 · 学生与家庭门户`
>
> **当前未结项**：学生/家庭非课堂主链已闭合考勤、请假/跨班补课、已发布成果、任务驱动视频、多子女/未绑定切换、草稿与内部备注隔离、跨家庭/跨学生拒绝及 zh/en 特殊状态；等待课堂侧并行增量合并后重跑 R1-5 总门并汇总最终证据。正式 Playwright 基础设施与 artifact 仍按阶段顺序归 R1-14。
>
> **核对日期**：2026-07-31；依据代码、迁移、内容目录、CI、Git 与 doc 00～26。
...
| 阶段 | 依赖 | 动作与产物 | 退出证据 |
| --- | --- | --- | --- |
| **R1-5 学生与家庭门户（当前）** | R1-1～4 | 学生从独立班级主页进入课次，分别接收知识总结、正式作业和视频任务，并查看课表、考勤、请假/补课、提交、成果和通知；家长查看已绑定子女并切换多子女；隐藏草稿、内部备注和其他家庭数据 | 学生/家长 E2E 通过；跨家庭、跨学生负向查询 100% 拒绝；未绑定/待审核/撤回/财务关闭状态有 zh/en UI |
