...
产品负责人于 2026-08-31 依次确认 Step 0、Step 1 和 Step 2；在 Step 2 通过后进一步明确正式课件与教师微课“本质上应该是同一个组件”，只授权 Step 2A 共享编辑框架迁移。首轮 Step 2A 审计发现只读微课仍自建预览、正式工作区底部留白过大，且 composition UI 被 `0356de5` 恢复成早于 `487b60d` 的旧布局；第二轮要求正式课件的插入与微课共用顶栏，并把微调／4:3／替换切换器并入右栏标题；第三轮指出仅共享按钮原语和挂载槽仍是两套编辑器，要求以微课完整编辑能力为共同基线，正式课只多 4:3 适配。开发端现由一个 `CoursewareEditorWorkbench` 直接渲染页面目录、插入顶栏、画布/翻页、右栏标题和右栏正文，两条 adapter 共用 `CoursewareInsertionToolbar`，正式轨道内重复标题行已删除；`capabilities.adapt4x3` 只对正式 adapter 开启。composition 后续工具组件、800ms 自动保存和实际 action 不变，正式 PageDoc/source-runtime 仍是无写入样机。开发端状态为 **STEP 2A REVISION READY FOR USER AUDIT**；本轮增量 TypeScript、受影响 ESLint和定向 Vitest 12/12 通过，但不构成视觉或微课回归确认。当前停止在 Step 2A；未授权 Step 3、新 editor action、schema、开发库/Storage 写入、批量任务或生产动作，不改变 R1-Live Gate 2。
