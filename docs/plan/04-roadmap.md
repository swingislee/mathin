...
>
> **当前未结项**：R1-5 已关闭。R1-6 需要把课堂记录、评价、视频/复盘和阶段报告统一为 `draft → review → published → withdrawn/revised` 生命周期，固定指标版本、数据截止时间与机构时区，并完成教师发布→学生/家长读取→撤回→修订的可重放旅程。
>
...
| **R1-6 教学成果与阶段报告（当前）** | R1-5 | 建立课堂记录、评价、视频/复盘、阶段报告的 draft→review→published→withdrawn/revised 生命周期；报告固定指标版本、数据截止时间和时区 | 教师发布→学生/家长读取→撤回→修订旅程可重放；历史版本和审计不变；通知失败进入 job |
