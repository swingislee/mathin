...
> **规划状态**：`active`
>
> **当前施工阶段**：`R1-6 · 教学成果与阶段报告`
>
> **当前未结项**：R1-5 已关闭。R1-6 首轮完整链路已证明阶段报告审校状态机和作业发布可用；仍须拆开知识总结与逐生课评的发布边界，补齐 BlockNote 模板/复制与三类草稿自动保存，建立带日期范围学情证据的阶段报告双栏工作台，并让审核和学生/家长通知带具体标题、可点击入口与正确对象路由。
>
> **核对日期**：2026-08-01；依据代码、迁移、内容目录、CI、Git、R1-6 人工完整链路与 doc 00～26。
...
| **R1-6 教学成果与阶段报告（当前）** | R1-5 | 将知识总结、逐生课评、视频复盘和阶段报告建立为互不阻塞的 draft→review（适用时）→published→withdrawn/revised 生命周期；知识总结使用 BlockNote、模板/复制和自动保存；阶段报告以可选日期范围的课评、作业与视频证据作为双栏写作依据，并固定指标版本、数据截止时间和时区 | 三类草稿自动保存；教师审核及发布→学生/家长读取→撤回→修订旅程可重放；每条通知有具体标题和有效 deep link；历史版本和审计不变；通知失败进入 job |
