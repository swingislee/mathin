...
| 阶段 | 依赖 | 动作与产物 | 退出证据 |
| --- | --- | --- | --- |
| **R1-9 Terms 与内容发布链/P6-9** | R1-1～3、R1-7 | 当前推进 P6-9：验证 E 系列 1135 讲和爱学习 G+/X+/A+ 秋季 170 讲的两轨文档、binding、对象和不可变 release；v3 只读来源 manifest 已同步 90+12 门 roster、1305 讲/2610 条 release-1，并把每轨显式页面绑定到 current track head 选中的 immutable source release；本地对象清单 hash、E adapted 4:3 审批、受控 artifact 根和路径隔离合同已冻结。Notebook 所需共享发布合同可继续；71 篇中文 Terms 的 slug、公式、依赖、引用、搜索、SEO 巡检暂缓 | P6-AIX-2 见[多难度证据](../evidence/r1/r1-9-aixuexi-courseware.md)，来源 manifest 见[合同证据](../evidence/r1/r1-9-courseware-source-manifest.md)。真实只读 exporter、批准副本的 1305 行 inventory、`cw-objects`/`cw-h5` 字节审计与非执行者复核仍 pending；Terms 坏链/悬空关系、英文回退和全量资源门未全部通过，R1-9 不关闭 |
