...
>
> **当前未结项**：2026-08-12 用户正式决定启动空间数学专题，SML-0 成为唯一当前施工阶段。release/template 权威、lecture capability、docVersion-aware track/layout 与首个可信模板的真实 Studio 创建/重排入口均已关闭；当前继续冻结 20 道代表题的教研金标、跨端 hash 向量，以及 create→publish→prep→freeze→live 的 4:3 浏览器空壳验收。该切换不等于 R1-9 或 `v1.0.0` 已完成，不授权清理真实生产数据，也不豁免恢复 R1 后仍须完成的原发布门。
>
...
| 当前全量 Vitest 共 88 个测试文件、567 项；其中非 spatial 基线为 51 个文件、290/290，空间数学/SML-0 专项为 37 个文件、277/277，`pnpm r1:test` 既有基线为 18 个文件、121/121 | SML-0 与 R1-14 均保持全量 100% 通过；正式 Playwright 框架已有本地基线，仍须补发布目标、写态、zh/en、跨浏览器、连续无 flaky、并发、文件和竞争矩阵 |
...
- **已完成的正式增量**：`spatial-page-delivery-v1` 已冻结 4:3 内容与兼容 track 的映射；migrations `20260812000400`～`20260812000600` 已统一 lecture/release capability；migration `20260813000100` 让 immutable release 保存有序 `courseware_pages`，并让备课、冻结、直播、学习检查、资产解析和回退以 selected/frozen release 为权威；migration `20260813000200` 增加显式 docVersion/layout 元数据、空间页 create/copy/save/revert、一次审核原子双 release、成对 rollback 与 freeze/reconnect 断言；migration `20260813000300` 让软删除页保留历史页码但不再占用活动顺序位置，并继续保留事务内可延迟重排。生产 `CoursewareDoc`、Studio/课堂分发按文档 layout 读取，普通空间页在两条兼容轨中仍保持原生 4:3。首个可信分层计数模板已能在空讲次、普通页、导入页和空间页 Studio 状态中创建，并支持跨类型重排与删除；浏览器已证明 native/adapted 兼容选择均读取同一 4:3 文档。legacy `courseware_template` 只保留为 native current release 的受控兼容投影。三个结构阻断项与 Studio 创建入口均已关闭。
- **当前 SML-0 剩余项**：20 道代表题的教研签名与跨领域复核、跨端 canonical hash 向量，以及 create→publish→prep→freeze→live 的真实浏览器 4:3 空壳验收；这些未完成前不得关闭 SML-0。
