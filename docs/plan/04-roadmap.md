...
4. **正式教师（注册完成，岗位待分配）**：首名教师已按现有邮箱绑定一次性邀请注册；去标识化只读核查确认邀请唯一消费、邮箱匹配、账号为 active `staff`、teacher 岗位尚未分配。管理员下一步在 `/dashboard/staff` 分配 teacher staff role。邮箱/手机号通用 password 接口、重复密码、login-only OTP 与微信/QQ 绑定边界已冻结在 [`r1-live-auth-identities.md`](r1-live-auth-identities.md)；手机号生产启用需先完成邀请迁移、非生产验证和 Gate 1/3 保险丝，不作为首名教师开始点名的前置条件。
...
正式管理员的 `student→staff` 引导、本人 MFA、`staff/admin` 原子交接、新会话登录验收、两个 R1-Live migration 的生产部署、首份 active protected-only manifest，以及首名真实教师的邀请注册均已完成。下一步由正式管理员在 `/dashboard/staff` 为该账号分配 teacher 岗位；完成后按 §1.1 的 standing execution direction 直接核查并把教师 auth/profile/岗位纳入 immutable replacement manifest，直到需要真实班级/花名册输入、人工登录验收或出现计划外差异。

2026-08-14 初次目标匿名快照有 12 个 email/password auth user、6 个 `purpose=production` 班级、61 个未删除课次、3 条 active enrollment 和 5 条点名记录；同日 19:59（Asia/Shanghai），仓库精确 migration `20260814000300_p6_six_classroom_cleanup` 已清除这 6 个验收班级及其课堂数据。2026-08-15 migration/首份 manifest 基线为 auth/profile=12、学生=4、班级/课次/报名/点名=0；首名真实教师注册后为 auth/profile=13、学生仍为 4、班级/课次/报名/点名仍为 0。正式管理员和产品负责人明确选择的首名教师以外，其他对象仍不能仅按角色、邮箱后缀或历史 `purpose` 推断为正式或可清理对象。
