...
| 全量 Vitest 共 60 个测试文件、363 项，commit `cbb2a0f` 已将本轮修复前 19 项失败清零；加入 R1-15 只读生产基线合同后为 363/363 全绿，其中 R1 非 spatial 基线 256/256 | R1-14 在后续实现后保持全量 100% 通过，并完成正式 Playwright、并发、文件和竞争矩阵 |
...
| **R1-14 幂等、并发、事务、文件与 E2E** | R1-2～13 | 修复关键写操作双击/重试/竞争；领域事实和审计同事务，外部调用走 job/outbox；使用 Playwright 建立角色/浏览器 E2E；commit `cbb2a0f` 已将本轮修复前 19 项 Vitest 失败清零 | 全量 Vitest 363/363 或等量更新后 100%；关键 E2E 连续 3 次无 flaky failure；无未接受数据竞争和大文件阻断项 |
