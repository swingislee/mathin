...
> **规划状态**：`active`
>
> **当前施工阶段**：`R1-9 · Terms 与内容发布链/P6-9`
>
> **当前未结项**：R1-8 已按安全关闭路径关闭；财务发布门固定关闭，路由/导航、数据、work item/审批、通知、指标和 job 均无可操作入口。当前进入 R1-9，校验 71 篇中文 Terms 与公共内容发布链，并验证 E 系列 1135 讲及爱学习 G+ 秋季 52 讲的 16:9/4:3 双轨 release 资源。爱学习 52 讲已于 2026-08-05 按 4:3 母版清库重导入（首批画布语义有误，876/1525 页底部内容被裁），见 doc 16 §12。
>
> **核对日期**：2026-08-11；依据代码、迁移、内容目录、CI、Git、R1-6 教学成果链路、R1-7 导出链路、R1-8 财务安全关闭证据、R1-9 爱学习课程支持证据与 doc 00～28。
...
| 全量 Vitest 251 项中 19 项失败（2026-08-05 实测；旧记的「109 项中 16 项」已过期） | R1-14 逐项判定为过期断言或产品回归，并清零 |
...
| 阶段 | 依赖 | 动作与产物 | 退出证据 |
| --- | --- | --- | --- |
| **R1-9 Terms 与内容发布链/P6-9** | R1-1～3、R1-7 | 校验 71 篇中文 Terms 的 slug、公式、依赖、引用、搜索、SEO；建立公共内容 draft/review/publish/withdraw/revise；验证 E 系列 1135 讲和爱学习 G+ 秋季 52 讲的两轨文档、binding、对象和不可变 release | Terms 坏链/悬空关系=0；英文缺失回退明确；E 系列 1135×2 与爱学习 52×2 资源 manifest 完整且重复生成 hash 相同。爱学习子门已于 2026-08-05 重导入并通过（见 [`docs/evidence/r1/r1-9-aixuexi-courseware.md`](../evidence/r1/r1-9-aixuexi-courseware.md)） |
...
| **R1-15 生产清理与 release-1 隔离演练** | R1-14 | 在生产快照副本执行 manifest、dry-run、备份、账号/运营数据清理和 1135×2 release-1 重建；第二次运行必须 no-op | 1 次全量演练成功；计数满足 doc 25 不变量；备份可恢复；本阶段未修改真实生产 |
