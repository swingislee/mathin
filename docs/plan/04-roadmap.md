...
>
> **当前未结项**：2026-08-12 用户正式决定启动空间数学专题，SML-0 成为唯一当前施工阶段。先关闭 release/template 权威、lecture capability 与 track/layout 解耦三个阻断项，冻结 4:3 空间文档、金标题、hash、ownership 和纵向空壳合同。该切换不等于 R1-9 或 `v1.0.0` 已完成，不授权清理真实生产数据，也不豁免恢复 R1 后仍须完成的原发布门。
>
...
| 当前全量 Vitest 共 84 个测试文件、544 项，commit `cbb2a0f` 已将本轮修复前 19 项失败清零；其中非 spatial 基线为 51 个文件、290/290，空间数学/SML-0 专项为 33 个文件、254/254，`pnpm r1:test` 为 18 个文件、121/121 | SML-0 与 R1-14 均保持全量 100% 通过；正式 Playwright 框架已有本地基线，仍须补发布目标、写态、zh/en、跨浏览器、连续无 flaky、并发、文件和竞争矩阵 |
...
- **已完成的正式增量**：`spatial-page-delivery-v1` 已冻结 4:3 内容与兼容 track 的映射；migration `20260812000400` 建立统一讲次 capability resolver；migration `20260812000500` 把 8 个页面编辑 RPC 与 5 个提交/评审 RPC 接入 resolver；migration `20260812000600` 继续收口直接发布、评审发布、紧急发布、双轨/旧签名回滚和批量 4:3 发布共 6 个入口。课程研发写入现统一验证 permission、有效责任与 lifecycle；课堂 freeze 保留独立的 `is_session_teacher` 边界，并已证明无研发 assignment 的任课教师仍可冻结精确 release。lecture capability 阻断项已关闭；release/template 权威与 docVersion-aware track/layout 双 head/release 原子映射仍未关闭。
