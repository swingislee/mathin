...
验收：`lint`/`typecheck`/`build`/`messages:check` 与 doc21/22/23/24 四条 audit 全过；26 页 × 7 档宽度（含 80%/125%/150% 缩放等价宽度）× 亮暗双主题共 364 次检查、0 处失败；返回动线 13 条（含四类攻击载荷）全过；主操作层级、表单可访问名称与弹层可用性全过。剩余人工项：亮/暗逐页视觉签收；遗留项见 doc 24 §13.9（告警语义色 token 化需先报批，`features/` 下约 70 处手搓卡片随后续改动迁移）。
