...
>
> **当前未结项**：R1-8 已按安全关闭路径关闭；财务发布门固定关闭，路由/导航、数据、work item/审批、通知、指标和 job 均无可操作入口。当前进入 R1-9，校验 71 篇中文 Terms 与公共内容发布链，并验证 E 系列 1135 讲及爱学习 G+ 秋季 52 讲的 16:9/4:3 双轨 release 资源。
>
...
| E 系列开发数据有 1135 讲双轨资源；爱学习 G+ 秋季有三至六年级 52 讲双轨资源 | R1-9 分别验证两套文档合同与资源；R1-15 演练；R1-18 重建 2374 条正式 release-1 |
...
| 阶段 | 依赖 | 动作与产物 | 退出证据 |
| --- | --- | --- | --- |
| **R1-9 Terms 与内容发布链/P6-9** | R1-1～3、R1-7 | 校验 71 篇中文 Terms 的 slug、公式、依赖、引用、搜索、SEO；建立公共内容 draft/review/publish/withdraw/revise；验证 E 系列 1135 讲和爱学习 G+ 秋季 52 讲的两轨文档、binding、对象和不可变 release | Terms 坏链/悬空关系=0；英文缺失回退明确；E 系列 1135×2 与爱学习 52×2 资源 manifest 完整且重复生成 hash 相同 |
