...
| 全量 Vitest 共 59 个测试文件、358 项，commit `cbb2a0f` 已将本轮修复前 19 项失败清零：358/358 全绿，其中 R1 非 spatial 基线 251/251 | R1-14 在后续实现后保持全量 100% 通过，并完成正式 Playwright、并发、文件和竞争矩阵 |
...
| **R1-11 Games、Minds、Tools、Notebook** | R1-9，可与 R1-10 并行 | 当前继续 Notebook 的私有写作、审核、发布/撤回、公开阅读和互动；3 个游戏及服务端排名、2 个工具的独立/嵌入、2 篇 Minds 与 Terms 关系巡检暂缓 | 四模块关键旅程、越权、失败态和英文回退通过；公开内容无临时/私有资源 URL |
...
| **R1-14 幂等、并发、事务、文件与 E2E** | R1-2～13 | 修复关键写操作双击/重试/竞争；领域事实和审计同事务，外部调用走 job/outbox；使用 Playwright 建立角色/浏览器 E2E；commit `cbb2a0f` 已将本轮修复前 19 项 Vitest 失败清零 | 全量 Vitest 358/358 或等量更新后 100%；关键 E2E 连续 3 次无 flaky failure；无未接受数据竞争和大文件阻断项 |
| **R1-15 生产清理与 release-1 隔离演练** | R1-14 | 在生产快照副本执行 manifest、dry-run、备份、账号/运营数据清理，并为 E 系列 1135×2 与爱学习 52×2 重建共 2374 条 release-1；第二次运行必须 no-op | 1 次全量演练成功；计数满足 doc 25 不变量；备份可恢复；本阶段未修改真实生产 |
