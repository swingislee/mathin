...
> **2026-09-19 授权开发增量**：[仪表盘工作入口重组](dashboard-work-entry-restructure.md)已接入真实页面；按事项组织一级入口，学生使用已建档身份的密集名录卡片，按组织形式找人并选人进入沟通；沟通保留五阶段表格与首联登记，班级整合原分班与教学工作台。状态为开发端已交付、待人工视觉与业务验收，不关闭当前生产阶段或 Gate。
...
2026-09-21 产品负责人要求沿“写入时整理统计字段、数据库按权限聚合、明细按页读取”的方向升级，并将主要数据可操作目标收紧为 1 秒。实施合同见 [Dashboard 统计与列表读取升级设计](dashboard-statistics-read-model-upgrade.md)：首批数据基线、沟通/获客统计内核与教师协作权限批量读取已应用本机，真实数据、身份与回滚对照通过；当前为 **首批内核机器检查通过 / 页面待接入**。其余总览指标、明细分页和列表残余读取继续按 C+D/E 推进，整页 1 秒尚未验收，生产尚未切换；沿用当前 R1-Live-2 阶段及既有生产晋级流程。
