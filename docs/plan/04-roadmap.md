...
>
> **当前阻塞**：`mathin.club` / `supabase.mathin.club` 的运行指纹已完成登记；仓库级 target policy 已把 Xiaomi 固定为当前生产目标，并覆盖 fixture、离线夹具、CI 重建和课程导入/适配入口。迁移 `20260815000100_r1_live_object_protection_manifest.sql` 已在仓库实现数据库指纹、protected/准删条目、hash/计数漂移和删除闭包保护，但尚未部署到 Xiaomi，也没有真实 active manifest。本机隔离开发目标已建立并完成固定账号验证。生产 current 已经用户明确授权更新为 `20260814-221135` / `023f5167…`，previous 为 `20260724-051318` / `b833c4d…`，服务、内外健康和 MFA 路由匿名鉴权探针通过；该次未执行 migration、账号或业务数据写入。随后用户指定唯一非固定 Gmail 为正式管理员候选，并授权将生产 profile 从 student 引导为无岗位 active staff；MFA 仍为 0，现有唯一 admin/MFA 不变。正式管理员交接、正式教师、真实班级/课次/花名册仍未完成，因此 Gate 1 仍为 `BLOCKED`；现有 `admin_set_identity` 与新版 profile 保护触发器不兼容，必须在 admin 交接前修复。目标机没有可核验的数据库/Storage 最近备份，production 账本仍缺 manifest migration，previous 尚未验证兼容回退，Gate 3 保持 `BLOCKED`。本阶段后续不授权创建其他真实账号、写入真实业务数据、部署其他变更、激活 manifest 或清理。
>
...
| **Gate 1 · 正式身份与真实数据** | **BLOCKED** | 目标组合指纹 `799d…63e39`、仓库写入口 target policy、部署 commit、Storage 匿名摘要和现有身份/业务对象计数已登记；测试造数/重建拒绝 Xiaomi；本机隔离 Supabase 和固定账号登录已通过；唯一非固定 Gmail 已被指定为正式管理员候选并从 student 引导为无岗位 active staff，匿名 hash、零业务关联和现有 admin/MFA 不变证据已登记；数据库 purge 的目标指纹、protected/准删清单、hash/计数漂移和闭包拒绝合同已在 migration + 一次性库断言实现；员工邀请、staff 角色/RLS、production/test purpose、学生/分班/课次 UI/RPC 已实现 | 候选账号先绑定 verified MFA；修复 `admin_set_identity`/profile 触发器合同并以 admin 总数始终为 1 完成原子交接和恢复登记；经另行授权部署 manifest migration，分类现有对象并激活 `purge_entry_count=0` 的正式保护 manifest；建立 1 个正式教师、1 个真实 production 班级、至少 1 个真实课次和真实花名册；登记课次 release/snapshot/object hash；教师只见授权范围 |
...
3. **正式管理员（进行中）**：唯一非固定 Gmail 已由产品负责人指定为候选并引导为无岗位 active staff；下一步由本人绑定 MFA。验证 factor 后先修复并回归 `admin_set_identity`/profile 保护触发器合同，再用单事务把候选提升为 admin、把固定开发 admin 降为非 admin，保证任一提交态 admin 角色人数恰好为 1，并登记恢复联系人。
...
Gate 1 的环境写入需要产品负责人逐项明确授权。正式管理员候选的 `student→staff` 单行写入已获授权并完成；MFA 由候选本人操作，admin 原子交接、manifest、真实教师和班级数据仍需后续授权。
