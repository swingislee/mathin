...
状态：开发端已交付，待人工验收。当前施工阶段仍为 R1-Live-2 / DEV-SCHOOL-OPS-1；本次仅应用到本机开发库。
...
- 跨班拖动或选中学生后点击另一班空位，先选择「完全调班」或「临时调班」并确认。同班换位和退回待分班沿用原入口。
- 完全调班继续使用同课程、同学期的正式调班流程；临时调班只调整选定讲次的名单。原班长期座位和报名继续保留，其他讲次仍在原班。
- 临时讲次先按课程讲次 ID 对应原班；目标课次缺少讲次 ID 时，使用讲次序号对应。对应缺失、重复、已开课、已有该学生考勤或名单锁定时显示具体原因。
...
在开发分班页验证圆圈位置、跨班拖动确认、不同讲次的选择与「临」标记，以及选中学生后的退课流程。机器检查仅证明其覆盖的合同，视觉与业务意图以产品负责人实际操作为准。
