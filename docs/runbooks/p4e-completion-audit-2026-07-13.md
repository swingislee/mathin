...
| P4E-V3 可观测/课堂降级 | **完成** | 服务端错误已端到端证明经 `onRequestError` 写入只读内建看板；课堂离线持久化/重启续号/双窗传输有自动化门禁。2026-07-13 在真实课堂页面完成 10 分钟受控服务端断连演练：刷新后 9 条 outbox 均保留，恢复普通模式后 UI 清零，远端 `session_events` 精确计数 9。演练 fixture 可重复创建、核验和清理。 |
...
| P4E-C2 未成年人合规 | **当前范围完成；备案办理由用户暂缓** | 同意记录、视频/数据 scope、注销/导出申请、隐私与儿童规则页、平台 UGC 审核均已实现。ICP/公安备案接口与页面保留；运营主体信息和办理结果由用户明确暂缓，不作为本轮阻断项。 |
| P4E-C3 手机验证码登录 | **当前范围完成；短信供应商由用户暂缓** | 手机 OTP 双轨界面、教师代建手机号账号及 SMS hook 接口已实现。供应商、凭据、签名和模板由用户明确暂缓，不作为本轮阻断项。 |
...
- `pnpm p4e:audit`：25 项控制及 71 个内容 UID 通过
- 10 分钟课堂离线演练：9 条本地事件刷新后保留，恢复在线后远端精确计数 9
...
2. 运营主体的 ICP/公安备案信息与实际办理结果（用户明确暂缓，只保留接口与页面）。
3. 异机备份目标及定时任务（用户明确暂不考虑；本机恢复、备份脚本和磁盘告警已验收）。

除上述用户决策项外，计划 13 的代码、数据库、运行手册和可重复验收均已闭环。
