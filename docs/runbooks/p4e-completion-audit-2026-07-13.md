...
| 任务 | 状态 | 核验依据 |
| --- | --- | --- |
| P4E-S1 备份灾备/恢复/磁盘告警 | **部分完成，生产阻断** | 2026-07-13 隔离恢复成功并核对关键计数；数据库+Storage 原子备份、校验和、保留策略、磁盘/陈旧告警脚本已在 `xiaomi` 实跑通过。尚无真实异机挂载点和告警接收端，timer 未启用。 |
...
| P4E-V3 可观测/课堂降级 | **部分完成，生产阻断** | 服务端错误已有结构化日志；课堂已有 IndexedDB outbox、BroadcastChannel、WebRTC 和恢复回传。尚无可验证错误看板 DSN/采集器，也未完成有登录夹具的 10 分钟拔网现场演练。 |
...
- `pnpm p4e:audit`：12 项控制及 71 个内容 UID 通过
- `pnpm db:types:check`：通过
- 真实远端 `p4e_security_assertions.sql`：通过
- `pnpm build`：Next.js 16.2.10 生产构建通过，279 个页面完成生成；仅有既有 KaTeX 中文数学模式 warning

## 完成计划 13 仍需的外部输入

1. 异机/对象存储备份目标及访问方式，和磁盘告警接收端。
2. 国内短信供应商或 SMS hook 的选择、凭据、签名与模板。
3. Sentry/GlitchTip（或现有日志平台）的 DSN/接入信息，以及可登录的课堂演练账号/时段。
4. 运营主体的 ICP/公安备案信息与实际办理结果。
