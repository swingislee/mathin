...
- [ ] `CLASS-06` 名称、用途、容量、主讲、学辅、学期、教室、时长、开始日、星期和时间校验正确。
      `SKIP · 已填写字段并验证编辑班级容量为 0 时提示“输入有误，请检查后重试”；未覆盖所有字段的非法边界`
...
- [ ] `PREP-07` BlockNote 标准教案按 `mathin-teaching-plan-v1` 模板创建，自动保存图标和 revision 正确。 SKIP · 观察到 contenteditable、模板内容和自动保存提示，但未核对模板标识与 revision 持久值。
