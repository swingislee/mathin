...
- 6.3/6.4 验证了教师/学服/主管范围、花名册入口、周课表切换、教师筛选、课次快速抽屉、QA 课次取消并恢复；QA 课次 3 已恢复到原“未开始”状态。
...
- 8.2 通过班级列表的等价 GET 筛选路由覆盖关键词、年级、主讲、学服、学期、运营状态、用途和准备状态；结果数依次为 `2/3/5/3/5/1/2/3`，并与既有清空筛选验证合并记录。
...
- [x] `DATA-06` sales：学服/前台，可验证线索、家庭、通知、请假、补课和跟进。
...
- [x] `DATA-13` 一个可只读抽样的正式用途课程产品、版本、讲次和 native/adapted release。
  产品族 `E 系列小学数学`（`xueersi-e-primary-math-cn`），版本 `E系列数学1年级暑期A[全国版]`（product code `MFHK00621`），第 1 讲「数的组成和比较」；native-16x9 与 adapted-4x3 各有 `release_no=1`，native track head 已指向对应 release。脚本只读校验，不写入。
- [x] `DATA-14` 一个本轮可修改的测试课程产品，至少包含一个版本和两个讲次。
  `QA-20260803-人工验收课程`（purpose=test）→ 版本 `QA-20260803-人工验收课程·1年级暑期A`（product code `QA-20260803-A`，purpose=test）→ 讲次 `QA-20260803-第1讲 数与运算基础`、`QA-20260803-第2讲 图形认识入门`。两讲**均无 release**，用于 `CLASS-04`／`CLASS-21`／`CLASS-22` 的课件风险分支。责任人：教研 owner、教师 editor、主管 reviewer。
- [x] `DATA-15` 一个 planning 测试班和一个 active 测试班；均有主讲、学服、学期、教室和课次。
  `QA-20260803-筹备测试班`（planning，教室 `QA-A101`，2 节课次）与 `QA-20260803-在读测试班`（active，教室 `QA-A102`，6 节课次）；两班均 purpose=test、绑定 QA 课程版本与当前学期「2026 春季学期」，主讲 `test-teacher`、学服 `test-sales`（`is_primary`）。
...
- [x] `DATA-17` 课次状态可覆盖 scheduled、ready、live/ended、post_pending、completed、cancelled 或 voided；不足状态通过同一测试课次线性推进，不伪造数据库行。
  脚本只写 `scheduled`：在读测试班 6 节课次按用途预留——`课次1·生命周期主线`（备课→行课→课后→完成）、`课次2·调课代课`、`课次3·取消恢复`、`课次4·作废`、`课次5·请假补课`、`课次6·备用`；筹备测试班 2 节。`started_at`／`ended_at`／`cancelled_by`／`voided_at`／`postwork_completed_at` 全部为空，ready 及之后的状态由 §6.4、§7 的真实动作推进。既有 `E系列数学1年级暑期S班[全国版]` 另有 6 节已结束课次可作只读对照。
...
- [x] `DATA-19` 一个视频任务和一个待审视频；附件仅用无敏感信息的小型测试文件。
  视频任务 `QA-20260803-视频任务·待审样本` 挂在已结束课次「数方」（`E系列数学1年级暑期S班[全国版]`，`测试学生` 在读、`test-teacher` 主讲），选已结束课次是为了当轮就能走通「待审→审核」。待审视频 `note=QA-20260803-待审视频样本`，`reviewed_at` 为空；字节由既有 1.1 MB 测试上传在 `session-videos` 桶内复制，不引入新素材、不含真实人物。
...
- [x] `WORK-02` “我的工作”包含本人 owner/editor/校对人、主讲/助教/学服、学生跟进人和审批责任项。
...
- [x] `STU-02` “我的学生”与“全部学生”范围符合 assigned/all 权限；教师/学服不能通过 query 切到全量。
...
- [ ] `FOLLOW-06` 教师只能看关系范围，学服只能看分配范围，主管可看管理范围。
...
- [x] `COURSE-04` product code、年级×季节×班型重复时明确拒绝，不生成半成品。
      对既有1年级暑期 A 版本尝试创建重复 code `QA-20260803-A`，页面提示“这个年级、季节与班型的组合已存在版本。”，未生成新版本。
...
- [x] `COURSE-07` 创建新版本的年级、季节、班型、标题和 code 校验正确。
      在新产品中创建1年级秋季 A 版本，名称和 code `QA-20260803-M02-AUT` 均按输入落库；版本 ID 为 `8df90a37-8b82-400d-94eb-44ef5e7f6b85`。
...
- [x] `CLASS-02` 关键词、用途、学期、年级、主讲、学服、准备度、运营状态筛选和清空正确。
      通过班级列表等价 GET 筛选路由覆盖全部 8 项筛选：关键词 `q=QA-20260803` 共 2、年级 `grade=1` 共 3、主讲 `teacherId=b2c20fa8-cefd-42e0-a47c-8b493ad270d0` 共 5、学服 `supportId=b892e3e4-9db1-487e-85cb-04f2f5033148` 共 3、当前学期 `schoolTermId=65fa0084-f7f3-470c-a941-bf1006f7897b` 共 5、运营状态 `planning` 共 1、用途 `test` 共 2、准备状态 `ready` 共 3；每次直接加载均只展示匹配班级，清空筛选沿用前轮已验证结果。
- [x] `CLASS-03` 建班向导按课程→信息→排课→确认推进，返回上一步不丢失已填数据。
      主管在未提交创建的测试班向导中填写临时班名、测试-教师、测试-学服、容量 10、QA-A102、当前学期和周二排课；从排课返回班级信息、从班级信息返回课程步骤后，课程、测试用途、教师、学服、容量、教室和排课规则均保留；再次推进到确认页，课程、教师、2 条课次、测试用途和冲突结果一致。
...
- [ ] `CLASS-06` 名称、用途、容量、主讲、学服、学期、教室、时长、开始日、星期和时间校验正确。
      `SKIP · 前轮编辑班级容量为 0 时提示“输入有误，请检查后重试”；本轮在未提交的测试班向导中使用 QA 测试课程、临时班名、测试-教师、教室 QA-A102、当前学期和周二排课，容量为 0 可推进到确认页，但最终创建动作被服务端校验拒绝并显示“创建失败，请检查填写内容后重试”；名称回退、学服、时长、开始日、时间及其余非法边界未完成全量覆盖`
...
- [ ] `CLASS-09` 重复点击创建只生成一个班和一组课次；失败不留下半个班。
      `SKIP · 本轮没有提交创建动作`
- [ ] `CLASS-10` 新建后进入班级对象页，课程、教师、学服、学期、课次数量一致。
      `SKIP · 未提交新建班级`
...
- [x] `CLASS-13` 顶部显示班名、生命周期、课程版本、主讲、学服、人数、下一课和唯一主动作。
      P4 班对象页显示班名“测试班-P4”、进行中、课程版本、主讲/学服、在读人数 `2/10`、下一课和“设置”主动作。
...
- [ ] `CLASS-19` 分配/移除主讲、助教、学服后班级范围、今日工作和通知同步更新。
      `SKIP · 责任分工弹窗显示主讲/助教/学服及添加/移除控件；未提交责任变更`
...
- [x] `ROSTER-06` 教师只见所教班，学服只见责任班，主管/教务按权限见全量。
...
- [x] `LEAVE-02` 教务/学服收到具体通知和 work item，可批准、拒绝并填写意见。
...
- [ ] `LIVE-02` 教师开课后学生端可进入；非成员、主管、教务、学服不能因此加入实时频道。 SKIP · 教师未开课，未执行开课后跨角色频道边界。
...
- [ ] `POST-09` 跟进记录可提交或按允许规则跳过，正确指派给教师/学服。 SKIP · 仅观察到逐生跟进输入、“提交跟进”和“本课无需跟进”，未保存。
...
- [ ] `JOURNEY-03` 教务：创建/选择课程→建 planning 班→分配教师/学服→排课冲突检查→入班→激活→调课/代课→历史可查。
- [ ] `JOURNEY-04` 教师：今日工作→下一课→三步备课→产物审核→检查项→完成并冻结→点名→行课→结束→课后闭环→完成课次。
- [ ] `JOURNEY-05` 学服：学生/家庭上下文→课前通知→请假处理→跨班补课→缺勤/服务任务→回访记录→领域动作关闭。
