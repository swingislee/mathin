...
>
> **核查时间**：2026-08-14 初次只读核查；2026-08-15 应用发布、正式管理员身份引导、MFA 核验、原子交接、数据库 migration 部署、首份 protected-only manifest 激活、首名真实教师注册与独立 postflight；2026-08-17 教师双岗位核查与 replacement manifest 原子替换/postflight；2026-08-22 运行时门禁 migration 与对应应用同步发布/postflight、当前 PostgreSQL+Storage 同批次备份及独立复核、报名状态窄修复 migration 回滚演练/正式部署/独立 postflight；2026-08-23 建班字段校验修复应用发布，以及学年四周期 migration/应用的生产 preflight、回滚演练、正式部署与独立 postflight；2026-08-25 手机号/password P0 migration、Auth-only 重建、应用发布与独立 postflight；2026-08-26 课堂 Stage B3 修复底座 migration、应用原子发布与独立 postflight
>
...
| 当前应用 release | `20260822-181331`；commit `b8999422a217ecf83064bd9f02521a751d23f692`；构建时间 `2026-08-22T18:14:43Z` |
| previous 应用 release | `20260822-162416`；commit `6dfb3af96cc81ca09be9b662d7cb047025546019`；构建时间 `2026-08-22T16:25:29Z` |
| 数据库迁移账本 | 181 条；head `20260823000100_r1_live_school_year_periods`，checksum `31441a83…9d67`。仓库 180 个非 snapshot migration 全部存在；生产另保留 1 个已知历史短名 `20260726000100`，无仓库 migration 缺失 |
...
| --- | --- | --- | --- |
| current | `20260826-125052` | `964ca5e6975d94b75591201aa13698a288f4ae70` | `2026-08-26T12:52:09Z` |
| previous | `20260825-085754` | `8c303a21574899728826984181028971dfc77742` | `2026-08-25T08:59:10Z` |
...
#### 4.2.3 2026-08-23 学年四周期 migration 与应用同步发布

产品负责人确认 `2025–2026` 学年春季实际结束于 `2026-06-29`，并采用“学年头部 + 暑/秋/寒/春四周期 + 日期临近后补 + 显式预览后升年级”的方案。生产 preflight 精确核对数据库指纹、180 条账本、旧春季、1 个班级/15 个课次/1 条报名、五年级、active manifest 与 Storage；首次完整回滚演练发现唯一旧函数 owner 为 `supabase_admin`，PostgreSQL 在替换处拒绝并自动回滚。新连接确认零漂移后，修正事务先以 `supabase_admin` 把该函数 owner 原子归一到 `postgres`，再以 `postgres` 执行迁移；相同事务完整跑通并回滚一次、独立核查恢复后，第二次才正式提交。

独立 postflight 确认 2025 学年 active、春季 current 且结束于 `2026-06-29`；2026 学年 planning，四周期日期为空。现有 production 班级、15 个课次和 1 条报名均归到 2026 秋季，其他行摘要不变；学生仍为五年级，promotion 与学年激活事件均为 0。应用提交 `b899942…` 完成本地和 Xiaomi 两次 production build、不可变 release、原子切换与健康门；公网 health/zh/en login 为 200，匿名 zh/en schedule 为精确 307 登录跳转，新 release 后 `operational_errors` 增量为 0。

| 证据字段 | 值 |
| --- | --- |
| `gate_id`, `domain`, `result` | `R1-Live Gate 2`；学年结构、既有业务归属和生产应用；该子步骤 `PASS`，Gate 2 整体仍 `BLOCKED` |
| `measured_value`, `threshold` | ledger `180→181`，head=`20260823000100_r1_live_school_year_periods`，checksum=`31441a83…9d67`；2025 active/春季 current=`2026-02-01～2026-06-29`，2026 planning/四周期日期待定，promotion=0。班级/课次/报名/点名=`1/15/1/0`，秋季引用=`1/15/1`，学生五年级=1；manifest/entry/protected/purge=`1/8/8/0` 且 hash 有效，Storage=`8/123602`。current=`20260822-181331` / `b899942…`，previous=`20260822-162416` / `6dfb3af…`；服务及 HTTP postflight 全通过，release 后错误增量=0 |
| `commit_sha`, `migration_head`, `environment` | `b8999422a217ecf83064bd9f02521a751d23f692`；`20260823000100_r1_live_school_year_periods`；Xiaomi / production；数据库指纹 `10e3…1a0c` |
| `dataset_manifest` | 写前/后对班级、课次、报名除 `term_id` 外的 SHA-256 和全部学生行 SHA-256 做事务内等值断言；班级状态、教师、课次时间/时长、报名状态和学生年级不变。无账号、岗位、manifest、点名或 Storage 写入，无清理，无学年启用或升年级 |
| `started_at`, `finished_at`, `actor`, `approver` | 2026-08-23 生产 preflight；`2026-08-23T02:16:29+08:00` 前完成最终 postflight；Codex；`swingislee`（明确日期并采用该方案） |
| `command_or_runbook` | 生产只读 preflight → serializable 完整回滚演练 → 新连接零漂移核查 → 相同事务正式提交与账本登记 → 独立只读 schema/RPC/grant/业务/年级/manifest/Storage postflight → `publish-mathin-xiaomi.ps1 -Action Publish/Status` → 公网 health/zh/en login/匿名 schedule 探针 |
...
#### 4.2.8 2026-08-25 未授权课堂 migration 提前进入生产

执行者在排查本地 M4a RPC schema cache 时误把 SSH alias `xiaomi` 判断为开发库，未读取本文件与 `r1-write-target-policy.md` 已登记的生产目标事实，也未取得本次生产迁移授权，先后向 Xiaomi 应用 `20260824000200_classroom_input_v2_flag`、`20260825000100_classroom_h5_pointer_v1_flag`、`20260825000200_classroom_roster_star_v2`、`20260825000300_classroom_star_v2_readers`、`20260825000400_classroom_open_roster_backfill` 和 `20260825000500_classroom_roster_rpc_schema_reload`。产品负责人随后明确更正：Xiaomi 是生产库，`192.168.5.213` 上的 Windows Docker/`127.0.0.1:35421` 才是本地开发库。此后所有 Xiaomi 操作立即降为只读，未发布 M3/M4 应用，也未执行回滚。

只读 containment 审计确认 ledger `184→190`、head=`20260825000500_classroom_roster_rpc_schema_reload`；三项新开关 `teaching.classroom_input_v2`、`teaching.classroom_h5_pointer_v1`、`teaching.classroom_layout_v2` 均为当前 version 1 / false。生产共有 1 个 production 班级、15 个课次和 1 条 active 报名；production 课次 roster revision=0，v2 星星事件=0，全部星星/撤销事件也为 0。backfill 只命中 1 个 `purpose=test` 的已开始未结束课次，固定为 roster revision 1 / star schema 1，snapshot revision=1、entry=0；回滚断言标题残留=0。`operational_errors` 仍为 1949。该结果证明尚未观察到 production 业务行回填或新 writer 启用，不证明提前迁移获得批准，也不自动关闭潜在兼容风险。

| 证据字段 | 值 |
| --- | --- |
| `gate_id`, `domain`, `result` | `R1-Live environment boundary`；生产数据库目标治理；`INCIDENT CONTAINED / OWNER DISPOSITION PENDING` |
| `measured_value`, `threshold` | ledger=`190`、head=`20260825000500`；新增 flag 三项均 false；production roster revision=0；v2/全部 star events=`0/0`；仅 `purpose=test` roster revision=`1`、entries=`0`；目标阈值本应是“未经授权零生产写入”，因此事件本身不通过 |
| `commit_sha`, `migration_head`, `environment` | 数据库 migration 来自开发增量 `1c99684`、`aeea93b`；生产应用仍为 `20260823-123746` / `9bc9ff3…`；数据库 `20260825000500_classroom_roster_rpc_schema_reload`；Xiaomi / production |
| `dataset_manifest` | 未创建账号、班级、production 课次、报名、点名、课件、Storage 对象或 v2 event；DDL、三条默认 false feature flag 与一条 test 课次 roster snapshot 为持久变化 |
| `started_at`, `finished_at`, `actor`, `approver` | 2026-08-25（Asia/Shanghai）；用户更正环境后立即停止写入并完成只读审计；Codex；`not_applicable`（本次生产迁移未获批准） |
| `command_or_runbook` | 误操作为 SSH/`docker exec supabase-db psql` migration + schema reload；containment 为 migration/flag/roster/event/业务计数与错误计数只读查询；随后更新 `AGENTS.md`、自托管拓扑与写入目标 runbook |
...
#### 4.2.10 2026-08-25 统一账号中心第一阶段授权发布

产品负责人先后修正应用候选，最终逐字授权 migration `20260825000800_account_center_profile` 与 commit `72d8127`，并允许新增默认中文偏好、空头像 bucket/RLS 和文件治理规则。生产 preflight 发现早期候选 `9b2cf53` 相对 current 会夹带未获准的课堂 migration/应用增量，因此没有写入并改为从 `8ec0ba0` 创建只含 11 个账号中心文件的 release。migration 第一次以 `postgres` runner 替换 `supabase_admin` 所有的既有函数时失败并自动回滚；独立零残留核查通过后，以 `supabase_admin` 完成全量回滚演练、再次零残留核查与正式提交。

| 证据字段 | 值 |
| --- | --- |
| `gate_id`, `domain`, `result` | `POST-LIVE-AUTH-01` 第一阶段；账号资料与治理；`DEPLOYED / PENDING USER ACCEPTANCE` |
| `measured_value`, `threshold` | ledger `191→192`，head/checksum=`20260825000800_account_center_profile` / `564b2909…c37`；`preferred_locale=zh` 为 14/14；头像 bucket/object=`1/0`、Storage policy=`3`、文件策略=`1`。current/previous=`20260825-072801` / `72d8127…` 与 `20260825-041101` / `8ec0ba0…`；三层 health 正常、zh/en login=200、匿名账号中心=307 精确登录跳转、错误仍为 1949 |
...
| `dataset_manifest` | pre/post auth user/profile/phone user=`14/14/0`、staff-role/invitation/identifier assurance=`11/1/0`、学生/班级/课次/报名/点名=`5/3/16/1/0`、Storage object/managed file=`123602/5`、active manifest/entry/protected/purge=`1/8/8/0`；仅授权的空 bucket 使 bucket 总数 `8→9` |
| `started_at`, `finished_at`, `actor`, `approver` | 2026-08-25（Asia/Shanghai）；release `builtAt=2026-08-25T07:29:16Z`；Codex；`swingislee`（逐字授权 migration、空 bucket/RLS/文件治理和 commit `72d8127`） |
| `command_or_runbook` | 只读 preflight/scope diff → 首次失败自动回滚与独立零残留核查 → owner 对齐后的完整回滚演练/独立核查 → 正式 migration/ledger → 精确 archive 的 Xiaomi build、immutable release 与原子切换 → HTTP/数据库/Storage/manifest/错误只读 postflight；未运行 Playwright、全量回归或真实账号写态 |
...
#### 4.2.11 2026-08-25～26 课堂 M5 Stage A、Stage B1、Stage B2 与 Stage B3
...
| `dataset_manifest` | auth/profile/学生/班级/课次/报名/点名=`14/14/5/3/16/1/0`、Storage object=`123602`；Stage B1～B3 启用与回退合计只新增五条 feature flag version 与五条 feature flag 审计事件 |
| `started_at`, `finished_at`, `actor`, `approver` | 2026-08-25～26（Asia/Shanghai）；release `builtAt=2026-08-25T08:59:10Z`；Codex；`swingislee`（“直接推进到Stage B”；“B1通过”；“B2通过”） |
| `command_or_runbook` | 只读目标/备份/release/账本/flags/计数 preflight → PostgreSQL-only 备份 → migration rollback/零残留/formal/schema cache → `publish-mathin-xiaomi.ps1 -Action Publish` → Stage A postflight → board/input 版本化启用 → B1 人工验收/只读写入复核 → layout 版本化启用 → B2 人工验收/只读写入复核 → H5 pointer 版本化启用 → production H5 renderer 全量只读核对 → H5 pointer 版本化关闭；每段均独立 postflight，未造数、未改账号/业务/Storage、未重启 Supabase |
...
| `retention`, `access_roles`, `failure_ticket` | Git、migration、immutable release 与新备份按既有策略保留，本轮无 prune；仓库维护者/Xiaomi 运维角色；B3 返回开发环境补齐 Aixuexi H5 bridge 后重新验收 |

#### 4.2.12 2026-08-26 课堂 Stage B3 修复底座授权发布

产品负责人要求“将对应改动推送到生产”。`main` 在课堂候选之后包含教师微课提交和未提交工作树，执行者未直接发布 `HEAD`，而是从 production current `8c303a2` 隔离拣选四个课堂代码提交，生成 `964ca5e6975d94b75591201aa13698a288f4ae70`。preflight 发现一个 started 但未 ended 的历史课次；它开始约 46 小时、最后事件约 8 小时，最近 60 分钟事件为 0，因此没有活动课堂受服务重启影响，且未修改该课次。

| 证据字段 | 值 |
| --- | --- |
| `gate_id`, `domain`, `result` | `CLASSROOM-M5-B3-REMEDIATION`；课堂输入/H5 能力底座；`DEPLOYED / PENDING USER ACCEPTANCE`，H5 flag 保持 false |
| `measured_value`, `threshold` | backup=`249650397 bytes` / TOC `3778` / SHA-256 `aa0b5888…36bf`；ledger `193→194`、checksum=`8709c55c…80e1`；current/previous=`20260826-125052` / `964ca5e…` 与 `20260825-085754` / `8c303a2…`。双层 health=`ok`，zh/en login=`200/200`，匿名课堂=`307`，journal error=`0`；profile rows=`0`、RLS=true、PostgREST=`200/[]`，checkpoint=`2/2/2`，错误=`1949` |
| `commit_sha`, `migration_head`, `environment` | `964ca5e6975d94b75591201aa13698a288f4ae70`；`20260826000100_classroom_h5_input_profiles`；Xiaomi / production，数据库指纹 `10e3…1a0c` |
| `dataset_manifest` | auth/profile/学生/班级/课次/报名/点名=`14/14/5/3/16/1/0`、Storage object=`123602`；只新增空 registry 表和 ledger，四个 feature flag 未变化 |
...
| `command_or_runbook` | scope 隔离 → 只读 preflight → PostgreSQL-only backup → migration rollback/零残留/formal/schema cache → 本地/远端 production build → immutable release 原子切换 → HTTP/编译产物/数据库/Storage/错误/PostgREST 独立 postflight；未运行生产账号自动化、未造数、未写 profile/flag/业务/Storage |
...
这份证据证明 2026-08-14 的 Xiaomi 初次只读观察，2026-08-15 的本机隔离目标、应用-only 生产发布、发布后健康、正式管理员身份引导/MFA/原子交接、新会话 AAL2/admin 路由验收、两个早期 R1-Live migration、首份 protected-only manifest 和首名真实教师邀请注册，2026-08-17 的 `research`/`teacher` 双岗位核查与 replacement manifest 原子替换，2026-08-22 两条运行时 migration/对应应用的同步生产发布、当前 PostgreSQL+Storage 同批次备份的独立可读性/SHA 复核和 `20260822000300` 报名状态窄修复，以及 2026-08-23 首个正式班级/15 个课次、建班字段校验修复和学年四周期方案的生产同步发布。2026-08-25 的未授权 classroom migration 提前应用事件已如实登记并完成 containment；后续手机号/password P0 取得独立明确授权并以 migration `20260825000600`、Auth-only 重建和热修 `8ec0ba0` 发布，统一账号中心第一阶段也以 migration `20260825000800` 与账号专用提交 `72d8127` 发布；两批均未创建账号、身份、岗位或业务数据，后者只新增空头像 bucket 与既定治理对象。Gate 1 已通过；Gate 2 已关闭建班、课次、报名与学年归属子路径，但仍不证明手机号真实邀请注册/login、正式教师点名持久再读及权限对照已经完成。账号中心仍待人工验收；previous 兼容回退、恢复演练、异机/静态加密备份和 release 错误标签另属 Production 1.0 证据。
