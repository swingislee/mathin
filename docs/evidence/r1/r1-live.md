...
| LIVE-P1-02 | 核心 P1 | 正式管理员已完成 verified MFA、唯一 admin 原子交接、新会话 MFA challenge 和 admin 路由验收，但尚缺恢复责任登记；`admin_set_identity` 与新 profile 保护触发器仍不兼容。目标其余身份、6 个 production 班级等对象尚未分正式/测试，也没有经 manifest 确认的正式教师和最小真实业务数据 | 先登记一名不同于管理员本人的实际第二恢复联系人；增加 migration 修复日常身份变更合同并在隔离库回归；随后分类保护现有对象并走员工邀请、角色分配、学生、production 班级、课次和分班正式 UI/RPC | 本轮只要求提供第二恢复联系人的非敏感稳定代号；migration、真实教师/班级/课次/花名册仍需逐项授权 | admin 恰好 1 且 MFA=100%；新会话达到 AAL2 并可进入 admin 路由；双人恢复责任可追溯；教师只见自己的真实班级；现有对象无误认/误清 |
