...
- [2026-09-19 Dashboard Supabase 查询核查与优化](dashboard-query-audit-20260919.md)：覆盖 70 个页面、3 个读取路由与共享布局，改善请求内重复读取、批次等待、学生学习串行、课件统计截断、资源聚合与课堂事件权限查询。44 项定向测试、数据等价与权限回滚通过；三条迁移已在本机开发库应用，生产本轮仅只读诊断，页面体验待验收。
