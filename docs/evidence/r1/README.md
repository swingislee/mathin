...
- [2026-09-22 微信 AppID 历史清理](wechat-appid-history-cleanup-20260922.md)：受影响公开分支已改写；独立远端镜像扫描 0 命中，17 项隐私检查通过；GitHub 旧对象及缓存清理工单 4780302 已提交、待平台处理。

- [2026-09-22 数据总览汇总优化生产发布](overview-summary-production-20260922.md)：`176600ec` 已上线，管理员汇总 SQL 1.53–2.66 s 降至 0.74–0.84 s；生产 12 组结果、权限、业务/Storage、回退和应用 postflight 通过。受限员工重启后首次 HTTP 1.55 s，后续约 0.90–1.03 s；管理员浏览器 1 秒目标待验收。

- [2026-09-22 连接脚本与小程序配置隐私保护](script-privacy-guards-20260922.md)：实际 AppID 转本机私有配置，增加暂存对象扫描及提交/推送 hooks；连接脚本、认证 URL 和私有取证文件受检查。后续历史清理与服务器缓存状态见上方记录。

- [2026-09-22 数据总览汇总接入](overview-summary-integration-20260922.md)：开发对照中页面内部 33 次/8.62 MB 降至 27 次/3.21 MB，读取 421–469 ms，六角色 SQL 及 16 组完整页面数据一致；后续已随 `176600ec` 上线，生产测量见发布记录，整页 1 秒待验收。

- [2026-09-22 统计分类性能回退修复](statistics-batch-repair-20260922.md)：`d40f694f` 已部署，12 个视图及获客出口改为批量分类；开发完整响应约 3.8 秒降至 1.09–1.32 秒，生产沟通 SQL 5.21 秒降至 0.40 秒。六角色 156 组、生产 52 组结果一致，备份、回退及 postflight 通过；整页 1 秒可操作待浏览器验收。


- [2026-09-22 学生名录数据库批量读取](student-directory-database-reads-20260922.md)：名录复用批量权限、当前页卡片投影和历史来源集合；真实 HTTP 管理员约 0.95–1.01 秒降至 0.22–0.33 秒，教师约 4.75–4.80 秒降至 0.10–0.15 秒，管理员 JSON 减少 74%。全 37 页、角色、历史边界和回退检查通过，开发已应用；保留首次开发 HTML 2.85 秒样本，生产未发布、整页 1 秒未验收。

- [2026-09-21 学生名录剩余耗时诊断](student-directory-read-diagnosis-20260921.md)：管理员 100 人 SQL 约 850 ms，其中完整档案补齐约 624 ms；教师默认 5 人仍逐条检查数千名学生，SQL 约 4.25 秒。临时复用批量权限后教师约 205–214 ms，完整结果一致；仅为本机回滚对照，卡片批量投影与正式修复待实现。
