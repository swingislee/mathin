...
| 阶段 | 状态 | 日期 | 证据 |
| --- | --- | --- | --- |
| DEV-TMC-1 · 普通教师短期微课 | development machine checks passed；product acceptance unknown；production not deployed | 2026-08-26 | [本机实现、SQL/Vitest/Playwright/CI 证据与生产边界](teacher-microcourse-dev.md) |
...
| R1-Live | Gate 1 passed；Gate 2 single-teacher production trial/blocking；课堂 B3 remediation、phone/password P0 与账号中心第一阶段 pending acceptance | 2026-08-26 | [真实教师点名闭环差距表、手机号 P0、账号中心与课堂发布证据](r1-live.md)；[目标核查、应用/数据库发布、正式管理员交接、manifest 激活、真实教师注册/岗位保护、当前生产备份及 2026-08-25 环境误判事件](r1-live-target-audit.md)；[仓库写入目标保险丝](../../runbooks/r1-write-target-policy.md)；[正式对象保护 manifest](../../runbooks/r1-live-object-protection-manifest.md)。生产 current/previous=`20260826-125052` / `964ca5e…` 与 `20260825-085754` / `8c303a2…`，ledger=`194`、head=`20260826000100_classroom_h5_input_profiles`；课堂 B1/B2 已人工通过，B3 修复底座已部署但 H5 flag 仍 false。账号、业务、Storage object 和错误无漂移。课堂 B3 生产验收、账号中心页面、真实手机号邀请注册/login 以及正式教师点名保存/再读、管理员可见和无权限拒绝仍待人工闭环，因此 Gate 2 未通过 |
