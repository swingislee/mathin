...
| DEV-TMC-1 · 普通教师短期微课 | development checks passed；production deployed / flag v2 true；pending product acceptance | 2026-08-27 | [实现、SQL/Vitest/Playwright/CI、生产备份/迁移/release/flag/postflight 证据](teacher-microcourse-dev.md) |
...
| R1-Live | Gate 1 passed；Gate 2 single-teacher production trial/blocking；课堂 B3 remediation、phone/password P0、账号中心第一阶段、教师微课与来源课件运行时 pending acceptance | 2026-08-28 | [真实教师点名闭环差距表、手机号 P0、账号中心与课堂发布证据](r1-live.md)；[目标核查、应用/数据库发布、正式管理员交接、manifest 激活、真实教师注册/岗位保护、当前生产备份及 2026-08-25 环境误判事件](r1-live-target-audit.md)；[来源课件运行时与暑期 A+ 生产证据](courseware-source-runtime-production.md)；[仓库写入目标保险丝](../../runbooks/r1-write-target-policy.md)；[正式对象保护 manifest](../../runbooks/r1-live-object-protection-manifest.md)。生产 current/previous=`20260828-044105` / `b4ad4a9…` 与 `20260828-040041` / `b4ad4a9…`，ledger=`208`、head=`20260828000200_courseware_summer_a_plus_catalog`；课堂 B1/B2 已人工通过，B3 修复底座已部署但 H5 flag 仍 false。教师微课与来源课件运行时是独立开发轨，已发布但不关闭 R1-Live Gate 2。正式账号与既有班课对象无漂移；课堂 B3、账号中心、教师微课生产页面、来源课件产品验收、真实手机号邀请注册/login 及正式教师点名保存/再读与权限对照仍待人工闭环 |
