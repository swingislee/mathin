...
| 阶段 | 状态 | 日期 | 证据 |
| --- | --- | --- | --- |
| R1-Live | Gate 0 passed；Gate 1 current/blocking；Gate 3 blocked | 2026-08-15 | [真实教师点名闭环差距表](r1-live.md)；[目标核查、应用/数据库发布、正式管理员交接与 manifest 激活](r1-live-target-audit.md)；[仓库写入目标保险丝](../../runbooks/r1-write-target-policy.md)；[正式对象保护 manifest](../../runbooks/r1-live-object-protection-manifest.md)。Xiaomi 已固定为当前生产目标；仓库危险写入口与 purge 数据库合同均 fail-closed；本机隔离 Supabase 与固定账号登录已验证。生产 current 已发布为 `20260814-221135` / `023f5167…`，服务及内外健康探针通过。唯一非固定 Gmail 已完成 verified MFA、正式 admin 原子交接、新会话 MFA challenge 和 admin 路由验收。两个 R1-Live migration 已部署；首份 active protected-only manifest 已在回滚式演练后激活，保护 admin auth/profile 与 2 个 production 课程族根，4 个 protected、0 个 purge 条目，候选列表为空且数据零漂移。首名真实教师/业务数据、可恢复备份和 previous rollback 仍缺，Gate 1/3 未通过 |
