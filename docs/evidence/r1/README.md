...
- [2026-09-21 最近两周任务统一生产发布](two-week-session-release-20260921.md)：核查 60 个任务，将教具、工作入口重组、班级连续登记、白板与 Smart、Dashboard 分页等未上线实现统一发布为 `234b5a35`；17 条迁移、72 组权限对照、回滚与 45 项 postflight 检查通过，8 个任务标题已更新。全量 CI 仍有历史断言和测试适配等失败，人工验收及 Gate 2 保持待完成；下方同批开发证据的发布状态以本项为准。
