...
- [2026-09-15 大屏与学科运营服务端传输优化](dashboard-server-transport-20260915.md)：独立候选 `fcc355ed` 让服务端可走已核对的同机 Supabase 网关，并减少测评逐批等待；930 条记录与顺序等价、双语和权限闸门、候选构建通过。等待本次应用与配置发布授权；新进程首次大屏仍测到 3.44 秒，生产整页目标待验收。
