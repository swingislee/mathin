...
const browserSubscription = { endpoint: "https://push.example.test/device/private-capability",
...
beforeEach(() => {
  ...
  vi.stubEnv("MATHIN_WEB_PUSH_ALLOWED_ORIGINS", "https://push.example.test");
...
describe("actual Web Push sender with isolated provider", () => {
  ...
  it("passes only the explicitly configured push proxy to the provider", async () => {
    vi.stubEnv("MATHIN_WEB_PUSH_PROXY", "http://127.0.0.1:18080");
    await run();
    expect(mock.send.mock.calls[0][2]).toMatchObject({ proxy: "http://127.0.0.1:18080" });
  });
