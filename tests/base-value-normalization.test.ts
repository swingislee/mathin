...
describe('Base values are normalized within their business meaning', () => {
  ...
  it('keeps school stages, historical transitions and several children distinct', () => {
    ...
    expect(field('年级', '二升三 年级')).toMatchObject({ value: { transition: { from: 2, to: 3 } }, review: ['transition'] });
    expect(field('年级', '一年级 四年级')).toMatchObject({ value: { grades: [1, 4] }, review: ['multiple_grades'] });
    expect(field('年级', '五六年级年级')).toMatchObject({ value: null, status: 'unparsed', review: ['ambiguous'] });
    expect(field('年级', '小区北门')).toMatchObject({ value: null, display: '小区北门' });
    expect(field('年级', '未知')).toMatchObject({ value: null, review: ['missing'] });
  });
  ...
  it('resolves explicit afternoon times, retains unspecified periods and validates minutes', () => {
    expect(field('到访时段', '下午3点半')).toMatchObject({ display: '15:30', value: { time: '15:30', clock: '24h' } });
    expect(field('到访时段', '3：30')).toMatchObject({ display: '03:30', review: ['time_period'], value: { time: '03:30', clock: 'unspecified' } });
    ...
    expect(field('到访时段', '9:30后')).toMatchObject({ display: '9:30后', status: 'unparsed' });
  });
  it('unifies exact channel aliases while preserving named people, school distinctions and payment methods', () => {
    ...
    expect(field('就读学校', '数独')).toMatchObject({ value: null, review: ['field_mismatch'] });
    expect(field('缴费方式', '微信支付').value).toBe('微信');
    expect(field('缴费方式', '魔法校H5').value).not.toEqual(field('缴费方式', '魔法校二维码').value);
  });
  it('compares multi-select sets independently of ordering and preserves contradictory options', () => {
    ...
    expect(field('获客加V情况', '未加V与群、已加V')).toMatchObject({ value: ['已加微信', '未加微信及群'], review: ['conflicting_options'] });
  });
  it('keeps grade bands, class labels, rating scales and original link references meaningful', () => {
    ...
    expect(field('班型', '慎思—')).toMatchObject({ value: { label: '慎思—', band: null }, review: ['class_label'] });
    ...
    expect(field('第3天', 'recv4NnhBdHKJG')).toMatchObject({ status: 'reference', review: ['reference'] });
  });
  ...
  it('audits actual value changes, alias groups and unresolved source issues without altering sources', () => {
    const originals = [field('年级', '一'), field('年级', '一年级'), field('年级', '二？年级')];
    ...
    expect(audit.issues[0]).toMatchObject({ sourceId: '2', originalText: '二？年级', reasons: ['ambiguous'] });
