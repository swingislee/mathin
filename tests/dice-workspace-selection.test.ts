...
describe("dice selection lifecycle", () => {
  it("deselects and reselects dice without returning moved faces or changing the scene", async () => {
    ...
    try {
      ...
      const clickAction = async (action: string) => act(async () => host.querySelector<HTMLButtonElement>(`button[data-spatial-action="${action}"]`)!.click());
      expect(canvas.props!.transformMode).toBeNull();
      await clickAction("move"); expect(canvas.props!.transformMode).toBe("move");
      await clickAction("rotate"); expect(canvas.props!.transformMode).toBe("rotate");
      await clickAction("rotate"); expect(canvas.props!.transformMode).toBeNull();
      for (const action of ["faceColor", "opacity", "pips", "opposite"]) {
        await clickAction("move"); await clickAction(action); expect(canvas.props!.transformMode).toBeNull();
      }
      expect(canvas.props!.dice[0].offsets).toEqual({ "y-": 0.9 });
      await clickAction("move");
