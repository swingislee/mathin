...
describe("R1 classroom continuity contracts", () => {
  ...
  it("prioritizes 4:3 courseware and gives M4b its right stack and full-width control row", () => {
    ...
    expect(controlBar).toContain('data-classroom-control-background="untinted-optical-glass"');
    ...
    expect(controlBar).not.toContain("bg-paper/30");
    ...
    expect(liveShell).not.toContain("bg-card/70");
    expect(controlMenus).not.toContain("rounded-xl bg-card/70 p-0.5");
    expect(toolbar).toContain('!isRail && "rounded-xl bg-card/70 p-0.5"');
    expect(toolbar).toContain('!isRail && "rounded-xl bg-moon/20 p-0.5"');
