...
function harness() {
  ...
  const change = vi.fn(), end = vi.fn(), cancelInk = vi.fn();
  // 测试用 React 生命周期桩挂载真实输入监听器。
  // eslint-disable-next-line react-hooks/rules-of-hooks
  useClassroomViewportGestures({ enabled: true, stageRef: { current: stage as unknown as HTMLDivElement }, gestureKey: "page-1", percent: 75, min: 40, max: 100,
    onChange: change, onEnd: end, onCancelInk: cancelInk });
  const pointer = (type: string, id: number, x = id * 200, y = 540, pointerType = "touch") => {
    const event = new FakePointerEvent(type, { pointerId: id, screenX: x, screenY: y, clientY: y, pointerType });
  ...
  return { stage, frame, token, pointer, receive, flush, change, end, cancelInk };
}
