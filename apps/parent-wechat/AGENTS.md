...
- 真实 AppID 写入已忽略的 `project.private.config.json`，共享 `project.config.json` 的 `appid` 保持 `touristappid`；使用用户选择且有开发权限的值。AppSecret 和访问令牌放在服务端或用户私有配置中，开发者工具令牌可使用会话环境 `MATHIN_WECHATIDE_TOKEN`；共享代码与日志不记录密钥或令牌。
