    ...
    <main className="grid min-h-screen place-items-center p-6">
      <div className="flex w-full max-w-sm flex-col items-center">
        ...
        <form action={action} className="w-full rounded-[2rem] border bg-card p-8 shadow-sm">
          ...
          <input className="mb-5 w-full rounded-full border bg-transparent px-4 py-3 outline-none transition duration-200 focus:ring-2 focus:ring-moon" id="email" name="email" type="email" required autoComplete="email" />
          <label className="mb-2 block text-sm" htmlFor="password">{t("password")}</label>
          <input className="w-full rounded-full border bg-transparent px-4 py-3 outline-none transition duration-200 focus:ring-2 focus:ring-moon" id="password" name="password" type="password" minLength={6} required autoComplete={mode === "login" ? "current-password" : "new-password"} />
