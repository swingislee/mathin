...
export function Table({ className, containerClassName, ...props }: TableProps) { return <div data-slot="table-container" className={cn("relative w-full overflow-x-auto",containerClassName)}><table data-slot="table" className={cn("w-full bg-card caption-bottom text-sm",className)} {...props}/></div>; }
...
export function TableRow({ className,...props }:React.ComponentProps<"tr">){return <tr className={cn("bg-card transition-colors hover:bg-table-hover",className)} {...props}/>;}
