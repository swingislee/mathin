...
async function ReviewContent({ locale, searchParams }: { locale: string; searchParams: Promise<ReviewQuery> }) {
  ...
  return <DashboardPage title={t("title")} density="compact" commandPanel={<TeachingRecordsCommandPanel groupBy={groupBy} grain={grain} window={window} baseHref={groupedHref()} terms={snapshot.terms} today={calendarDayKey(new Date(), snapshot.timeZone)} selection={selection} />}>
