...
import { DashboardCommandActions, DashboardCommandFilters, DashboardCommandState, DashboardEmptyCard, DashboardPage } from "@/features/school/dashboard-page";
...
export default async function LeadsPage({ params, searchParams }: {
  ...
  return <LeadPoolSelectionProvider key={`${filters.scope}:${filters.assignment ?? ""}:${filters.status ?? ""}:${filters.q ?? ""}:${filters.page}:${filters.pageSize}`} assignableIds={leads.filter((lead) => lead.status !== "invalid" && lead.status !== "converted").map((lead) => lead.id)}>
    ...
    </FollowupCommandPanel>} footer={count > 0 ? <LeadPoolPagination currentPage={filters.page} totalPages={Math.max(1, Math.ceil(count / pageSize))} totalCount={count} pageSize={pageSize} scope={filters.scope} status={filters.status} q={filters.q} extraQuery={filters.assignment ? { assignment: filters.assignment } : {}} /> : null}>
