...
import { TEACHING_REPLAY_ID, teachingReplayAllowed, teachingReplayDetail } from "@/features/school/teaching-workbench/teaching-replay-contract";
...
export async function POST(request: Request, { params }: { params: Promise<{ locale: string }> }) {
  ...
  try {
    const { sessionId, contactPage, pageSize, replayId } = input.data;
    const snapshot = replayId ? await readTeachingReplay(locale) : null;
