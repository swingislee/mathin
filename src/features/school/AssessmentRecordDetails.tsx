...
import { AssessmentClassificationPanel, AssessmentFeedbackPanel, AssessmentWorkflowHistory } from "./AssessmentWorkflowPanels";
...
export function AssessmentRecordDetails({
  ...
  const assessmentSaved = async (saved: AssessmentWorkbenchRow) => {
    ...
    onSaved(saved);
    if (assessmentWorkbenchHasFinalResult(saved)) show("feedback");
  };
  ...
  return <div className="min-w-0 space-y-4" data-assessment-workbench-detail={row.id} aria-busy={pending}>
    <div data-assessment-detail-header className="flex min-w-0 flex-wrap items-center justify-between gap-x-6 gap-y-3">
      <nav aria-label={t("progressLabel")}><ol data-assessment-progress className="flex min-w-0 flex-wrap items-center gap-y-2">
        {ASSESSMENT_STAGES.map((value, index) => {
          ...
          return <li key={value} className="flex items-center gap-1.5 text-xs">
            <button type="button" disabled={pending} aria-current={selected ? "step" : undefined} aria-controls={panelId(value)}
      ...
      {finalized && canWrite ? <Button type="button" size="sm" variant="ghost" onClick={() => setRevising((value) => !value)}><PencilLine className="size-3.5" />{w("revise")}</Button> : null}
    </div>
    ...
    {visited.has("in_progress") ? <section id={panelId("in_progress")} hidden={section !== "in_progress"} className="space-y-4" data-assessment-panel="in_progress">
      ...
      {mayReassign ? <div data-assessor-reassignment={row.id} className="flex max-w-lg flex-wrap items-center gap-2"><span className="text-xs text-muted">{t("assignedAssessor")}</span>
        <FollowupChoice value={assessorId} disabled={reassigning || pending} label={t("changeAssessorFor", { name: row.name })} className="w-40" onValueChange={setAssessorId}
          options={assessors.map((assessor) => ({ value: assessor.userId, label: assessor.displayName, tone: "healthy" }))} />
        <Button type="button" size="sm" variant="secondary" disabled={pending || reassigning || !assessorId || assessorId === row.assessorId} onClick={() => onReassign(assessorId)}>{t("confirmAssessor")}</Button></div> : null}
      {row.assessmentKind === "one_to_one" ? <AssessmentQuickEntry row={row} disabled={!canQuickEntry || finalized || pending} canRoute={false} onSaved={(saved) => { void assessmentSaved(saved); }} onSaveAndNext={onSaveAndNext} />
        : <ActivityAssessmentDetails row={row} disabled={!canAssess || finalized || pending} onSaved={(saved) => { void assessmentSaved(saved); }} onSaveAndNext={onSaveAndNext} />}
    ...
    {visited.has("feedback") ? <section id={panelId("feedback")} hidden={section !== "feedback"} className="space-y-4" data-assessment-panel="feedback">
      <AssessmentFeedbackPanel registrationId={row.registrationId} workflow={row.workflow} hasResult={completed} canWrite={canWrite} pending={pending} onPrepare={() => { void save({ command: "report", values: {} }); }} />
      {completed ? evidence : null}
    </section> : null}
    {visited.has("handled") ? <section id={panelId("handled")} hidden={section !== "handled"} className="space-y-4" data-assessment-panel="handled">
      ...
      <AssessmentClassificationPanel key={row.id + ":" + (row.workflow?.finalizedAt ?? "draft")} workflow={row.workflow} disabled={!canWrite || finalized} pending={pending} locale={locale}
        onSave={(values) => { void save({ command: "classify", values }); }} />
