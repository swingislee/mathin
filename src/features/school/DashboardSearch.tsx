...
export function DashboardSearch({ className, value, defaultValue, onChange, name, onSearch, ...props }: ComponentProps<typeof Input> & { onSearch?: () => void }) {
  ...
  return <span data-dashboard-search className="inline-flex shrink-0">
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild><Button type="button" variant="ghost" size="sm" disabled={props.disabled} aria-label={label} title={String(label)} className={cn("relative size-9 rounded-full p-0", current && "bg-blue/10 text-blue")}>
      ...
      <PopoverContent align="end" className="w-[min(24rem,calc(100vw-2rem))] p-3">
        <div className="flex items-center gap-2"><Input {...props} type="search" value={current} className={cn("h-9 min-w-0", className)}
          ...
          <Button type="button" size="sm" variant="secondary" disabled={props.disabled} aria-label={label} onClick={apply} className="size-9 shrink-0 p-0"><Search className="size-4" aria-hidden /></Button>
        </div>
      </PopoverContent>
    </Popover>
  </span>;
