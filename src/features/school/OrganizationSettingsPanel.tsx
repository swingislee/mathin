    ...
    <Tabs defaultValue="profile" className="space-y-5">
      ...
      <TabsContent value="features">
        <SectionCard title={t("featuresTitle")} description={t("featuresIntro")} icon={<Flag size={19} />}>
          ...
          <div className="mt-4 flex items-center gap-3 rounded-xl border border-line p-4"><Checkbox checked={flagEnabled} onCheckedChange={(checked) => setFlagEnabled(checked === true)} /><div className="min-w-0 flex-1"><p className="font-medium">{flagEnabled ? t("enabled") : t("disabled")}</p><p className="mt-1 text-xs text-muted">{t(`flagHelp_${flagKey.replaceAll(".", "_")}`)}</p></div><Badge variant={flagEnabled ? "default" : "outline"}>{flagEnabled ? t("on") : t("off")}</Badge></div>
          ...
          <div className="mt-4 flex justify-end"><Button disabled={pending || !flagEffectiveAt || !flagReason.trim()} onClick={() => flagRun.run({ flagKey, campusId: flagCampusId, enabled: flagEnabled, effectiveAt: toIso(flagEffectiveAt), reason: flagReason })}>{t("createVersion")}</Button></div>
