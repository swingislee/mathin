...
const COMPACT_CONTROL_CLASS = "h-9 rounded-md px-2.5 py-1.5 shadow-none";
...
export function OrganizationSettingsPanel({ initial }: { initial: OrganizationSettingsSnapshot }) {
  ...
  return (
    <Tabs
      ...
      className="[&_input[data-slot=input]]:h-9 [&_input[data-slot=input]]:rounded-md [&_input[data-slot=input]]:px-2.5 [&_input[data-slot=input]]:py-1.5 [&_input[data-slot=input]]:shadow-none [&_[role=combobox]]:h-9 [&_[role=combobox]]:rounded-md [&_[role=combobox]]:px-2.5 [&_[role=combobox]]:py-1.5 [&_[role=combobox]]:shadow-none"
    >
      ...
      <TabsContent value="profile" className="mt-0">
        <SettingsSection title={t("profileTitle")} description={t("profileIntro")} icon={<Building2 size={19} />}>
          <div className="grid max-w-5xl gap-4 @2xl/page:grid-cols-2 @5xl/page:grid-cols-3">
          ...
          <div className="mt-5 flex max-w-5xl justify-end"><Button disabled={pending} onClick={() => profileRun.run({ name: organizationName, timezone: organizationTimezone, defaultLocale })}>{profileRun.pending ? <LoaderCircle className="size-4 animate-spin" /> : <Save className="size-4" />}{t("save")}</Button></div>
        </SettingsSection>
      </TabsContent>

      <TabsContent value="locations" className="mt-0">
        <SettingsSection title={t("campusTitle")} description={t("campusIntro")} icon={<MapPin size={19} />}>
          <div className="grid max-w-6xl gap-8 @3xl/page:grid-cols-2 @3xl/page:divide-x @3xl/page:divide-line">
        ...
        <SettingsSection title={t("roomTitle")} description={t("roomIntro")} icon={<Building2 size={19} />}>
          {!selectedCampus ? <p className="text-sm text-muted">{t("noCampus")}</p> : <>
            ...
            <div className="mt-4 grid max-w-5xl gap-3 @2xl/page:grid-cols-2 @5xl/page:grid-cols-4">
            ...
            <div className="mt-3 flex max-w-5xl justify-end"><Button variant="secondary" disabled={pending || !roomCode.trim() || !roomName.trim()} onClick={() => roomRun.run({ campusId: selectedCampus.id, code: roomCode, name: roomName, capacity: roomCapacity ? Number(roomCapacity) : null })}>{t("createRoom")}</Button></div>
          </>}
        </SettingsSection>
      ...
      <TabsContent value="rules" className="mt-0">
        <SettingsSection title={t("rulesTitle")} description={t("rulesIntro")} icon={<Settings2 size={19} />}>
          <div className="grid max-w-5xl gap-4 @2xl/page:grid-cols-2 @5xl/page:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.2fr)]">
            ...
            <Label className="grid gap-2">{t("effectiveAt")}<DateTimePicker className={COMPACT_CONTROL_CLASS} mode="datetime" value={ruleEffectiveAt} onValueChange={setRuleEffectiveAt} /></Label>
          </div>
          <Label className="mt-4 grid max-w-5xl gap-2">{t("ruleValue")}<Textarea rows={10} value={ruleValue} onChange={(event) => setRuleValue(event.target.value)} className="rounded-md font-mono text-xs shadow-none" /></Label>
          <Label className="mt-4 grid max-w-5xl gap-2">{t("changeReason")}<Input value={ruleReason} maxLength={200} onChange={(event) => setRuleReason(event.target.value)} /></Label>
          <div className="mt-4 flex max-w-5xl justify-end"><Button disabled={pending || !ruleEffectiveAt || !ruleReason.trim()} onClick={() => ruleRun.run({ domain: ruleDomain, campusId: ruleCampusId, valueText: ruleValue, effectiveAt: toIso(ruleEffectiveAt), reason: ruleReason })}>{t("createVersion")}</Button></div>
      ...
      <TabsContent value="features" className="mt-0">
        <SettingsSection title={t("featuresTitle")} description={t("featuresIntro")} icon={<Flag size={19} />}>
          ...
          <div className="mt-4 grid max-w-5xl gap-4 @2xl/page:grid-cols-2 @5xl/page:grid-cols-3">
            ...
            <Label className="grid gap-2">{t("effectiveAt")}<DateTimePicker className={COMPACT_CONTROL_CLASS} mode="datetime" value={flagEffectiveAt} onValueChange={setFlagEffectiveAt} /></Label>
          </div>
          ...
          <Label className="mt-4 grid max-w-5xl gap-2">{t("changeReason")}<Input value={flagReason} maxLength={200} onChange={(event) => setFlagReason(event.target.value)} /></Label>
          <div className="mt-4 flex max-w-5xl justify-end"><Button disabled={financeReleaseClosed || pending || !flagEffectiveAt || !flagReason.trim()} onClick={() => flagRun.run({ flagKey, campusId: flagCampusId, enabled: flagEnabled, effectiveAt: toIso(flagEffectiveAt), reason: flagReason })}>{t("createVersion")}</Button></div>
