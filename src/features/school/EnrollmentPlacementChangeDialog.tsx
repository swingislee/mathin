...
export function EnrollmentPlacementChangeDialog({student,target,mismatch,blocked,withdraw=false,transfers=[],onClose,onSaved}:{student:PlacementStudent;target?:{classroom:PlacementClassroom;seat:number};mismatch?:{source:string;destination:string};blocked?:string;withdraw?:boolean;transfers?:SessionTransfer[];onClose:()=>void;onSaved:(board:EnrollmentPlacementBoard)=>void}){
  const en=useLocale()==='en';
  const canTransferTemporarily=Boolean(student.membershipId&&student.classroomId);
  ...
  useEffect(()=>{
    ...
    let active=true;
    previewEnrollmentSessionTransferAction({membershipId:student.membershipId,classroomId:target.classroom.id,seat:target.seat,...(mismatch?{allowMismatch:true}:{})}).then(result=>{
      if(!active)return;
      if(result.ok)setOptions(result.data);else setLoadError(placementChangeError(result.code,en));
    });
    return()=>{active=false;};
  },[mode,target,student.membershipId,en,retry,mismatch]);
  ...
  const save=(cancel?:SessionTransfer)=>start(async()=>{
    const result=await changeEnrollmentPlacementAction({requestId,mode:cancel?'cancel_temporary':withdraw?'withdraw':mode,
      ...
      seat:target?.seat??null,expectedSeat:student.seat,reason,allowMismatch:Boolean(mismatch)&&!cancel&&!withdraw,transferId:cancel?.id??null,
  ...
  return <Dialog open onOpenChange={open=>{if(!open&&!pending)onClose();}}><DialogContent className="sm:max-w-xl"><DialogHeader>
    <DialogTitle>{withdraw?(en?'Withdraw enrollment':'确认退课'):mismatch?(en?'Review class mismatch':'班级不匹配，请确认插班'):target?(en?'Confirm class transfer':'确认调班'):(en?'Temporary transfers':'临时调班安排')}</DialogTitle>
    <DialogDescription>{student.name}{target?` → ${target.classroom.name}`:''}</DialogDescription>
  </DialogHeader>
    {mismatch?<div role="alert" className="space-y-1 rounded-md bg-moon/30 p-3 text-sm leading-6"><p className="font-medium">{en?'The course, grade or term differs. Confirm that this class is suitable before continuing.':'课程、年级或学期与目标班不匹配，请老师核对是否适合插班。'}</p><p>{en?'Current: ':'当前：'}{mismatch.source}</p><p>{en?'Target: ':'目标：'}{mismatch.destination}</p></div>:null}
    {blocked?<p role="alert" className="text-sm text-rose">{blocked}</p>:null}
    {withdraw?<p className="text-sm leading-6">{en?'End this enrollment and release its seat. Attendance and course history remain available. Future temporary transfers will be cancelled. Refunds are handled in the payment workflow.':'结束本次报名并释放座位，保留已有考勤和课程记录，同时取消尚未上课的临时安排。退款在收付款流程中办理。'}</p>:target?<>
      {canTransferTemporarily?<fieldset className="grid grid-cols-2 gap-2" disabled={pending}><legend className="sr-only">{en?'Transfer type':'调班类型'}</legend>{(['permanent','temporary'] as const).map(value=><label key={value} className="flex cursor-pointer items-center gap-2 rounded-md border border-line p-3 text-sm"><input type="radio" name="placement-transfer-mode" value={value} checked={mode===value} onChange={()=>{setMode(value);change();}}/>{value==='permanent'?(en?'Permanent transfer':'完全调班'):(en?'Temporary transfer':'临时调班')}</label>)}</fieldset>:null}
    ...
    {withdraw||target?<label className="space-y-1 text-sm"><span>{withdraw?(en?'Withdrawal reason':'退课原因'):mismatch?(en?'Placement reason':'插班原因'):(en?'Note (optional)':'备注（可选）')}</span><Textarea maxLength={2000} value={reason} disabled={pending} onChange={e=>{setReason(e.target.value);change();}}/></label>:null}
    <DialogFooter><Button variant="secondary" disabled={pending} onClick={onClose}>{en?'Close':'取消'}</Button>{withdraw||target?<Button disabled={pending||Boolean(blocked)||(mismatch&&!reason.trim())||(withdraw?!reason.trim():mode==='temporary'&&!selected.length)} onClick={()=>save()}>{pending?<LoaderCircle className="size-4 animate-spin"/>:null}{withdraw?(en?'Confirm withdrawal':'确认退课'):mismatch?(en?'Confirm placement':'确认插班'):(en?'Confirm transfer':'确认调班')}</Button>:null}</DialogFooter>
