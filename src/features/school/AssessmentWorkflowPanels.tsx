...
export function AssessmentClassificationPanel({ id, workflow, disabled, pending, onSave, locale, render }: {
  id: string;
  ...
  const submit = () => {
    ...
    onSave({ classification, parentResponse: response, reasons, nextContactAt: nextAt });
  };
  const entry: AssessmentEntryParts & { nextContactAt: string | null } = {
    dirty: classification !== (workflow?.classification ?? "") || response !== (workflow?.parentResponse ?? "")
    ...
    followup: {
      ...
      disabled, readOnly: disabled, pending, saveDisabled: !classification, onSave: submit, saveLabel: t("saveClassification"),
      reminderContent: <AssessmentNextContactField id={`${id}-next`} value={nextAt} onChange={setNextAt} disabled={locked} />,
      hint: workflow?.finalizedAt ? t("classifiedAt", { name: workflow.updatedByName,
        time: new Intl.DateTimeFormat(locale, { dateStyle: "short", timeStyle: "short", timeZone: "Asia/Shanghai" }).format(new Date(workflow.finalizedAt)) }) : undefined,
    },
    tags: <div data-assessment-field="classification" className="flex min-h-8 min-w-0 max-w-full items-center gap-2">
