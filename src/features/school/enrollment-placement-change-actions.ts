...
export async function previewEnrollmentSessionTransferAction(input:{membershipId:string;classroomId:string;seat:number}):Promise<ActionResult<SessionTransferOption[]>>{
  try{
    const v=parse(z.object({membershipId:z.string().uuid(),classroomId:z.string().uuid(),seat:z.number().int().min(1).max(60)}).strict(),input);
    await authorizedClient('enrollment.manage');
    return {ok:true,data:z.array(sessionTransferOptionSchema).parse(await enrollmentWorkflowRpc('preview_enrollment_session_transfer',{p_membership_id:v.membershipId,p_to_classroom_id:v.classroomId,p_seat:v.seat}))};
