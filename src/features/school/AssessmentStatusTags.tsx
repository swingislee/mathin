...
export function AssessmentStatusTags({ row, locale }: { row: AssessmentWorkbenchRow; locale: string }) {
  ...
  return <div className="flex flex-wrap items-center gap-1" data-assessment-status-tags>
    {assessmentDetailStatuses(row).map((status, index) => <Badge key={status} variant="outline" data-assessment-status={status}
