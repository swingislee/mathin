...
export function SourceCompletionNotice({summary,locale,compact=false}:{summary:SourceCompletionSummary|null|undefined;locale:string;compact?:boolean}) {
  ...
  return <div data-source-completion className="min-w-0 text-xs">
    <div className="flex flex-wrap items-center gap-1">
      {(summary.enrolled?(compact?[m.enrolled]:[m.contacted,m.assessed,m.enrolled]):[m.assessed]).map(label=><Badge key={label} variant="outline" className="border-leaf-deep/35 bg-leaf/20 px-1.5 text-[11px] text-leaf-deep">{label}</Badge>)}
    ...
    {!compact?<>
      <p className="mt-2 text-[11px] leading-5 text-muted">{summary.enrolled?m.basis:m.assessmentBasis}</p>
