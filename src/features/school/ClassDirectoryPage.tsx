...
async function ClassroomCommandPanel({ locale, searchParams }: { locale: string; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <DashboardCommandPanel>
      ...
      {perms.has("class.create") || perms.has("enrollment.manage") ? (
        <DashboardCommandActions>
          {perms.has("enrollment.manage") ? <Link href="/dashboard/classes/import/roster" className={cn(buttonVariants({ variant: "secondary", size: "sm" }))}>{t("importRoster")}</Link> : null}
          {perms.has("class.create") ? <Link href="/dashboard/classes/import" className={cn(buttonVariants({ variant: "secondary", size: "sm" }))}>{t("importClasses")}</Link> : null}
          {perms.has("class.create") ? <Link href="/dashboard/classes/new" className={cn(buttonVariants({ size: "sm" }))}>{t("newClass")}</Link> : null}
        </DashboardCommandActions>
      ) : null}
    </DashboardCommandPanel>
  );
...
async function ClassroomLibrary({ locale, searchParams }: { locale: string; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const resetHref = `/dashboard/classes?view=directory&scope=${scope.scope}`;
...
function classroomPageHref(scope: ClassroomScope, filters: ClassroomListFilters, page: number) {
  const query = new URLSearchParams({ scope, view: "directory" });
