...
type StoredRecord = {
  id: string; source_sha256: string; source_data: { filename: string };
...
function hydrate(stored: StoredRecord): HistoryArchiveRow {
  ...
  return { id: stored.id, label: record.label, sourceName: stored.source_data.filename, tableName: record.tableName,
    ...
    excerpt: cells.map(cell => `${cell.fieldName}：${cell.text}`).join('\n').slice(0, 260), matchStatus: stored.match_status,
    matchReason: stored.match_data.reason, entity: stored.entity_data, candidateCount: stored.candidate_data.length, warnings: record.warnings ?? [] };
}
...
export async function loadHistoryArchivePage(filters: HistoryArchiveFilters): Promise<HistoryArchivePageData> {
  ...
  let query = client.from('history_import_records').select('*,history_import_batch_records!inner(batch_id)', { count:'exact' })
    .eq('history_import_batch_records.batch_id', batch.id).eq('record_data->>hasContent', 'true');
  if (filters.status !== 'all') query = query.eq('match_status', filters.status);
...
export async function loadHistoryArchiveDetail(id: string, page = 1): Promise<HistoryArchiveDetail | null> {
  const client = await archiveClient();
  const batch = await completeBatch(client);
  const { data, error } = await client.from('history_import_records').select('*').eq('id', id).maybeSingle();
  ...
  if (stored.entity_data && batch) {
    const start = (page - 1) * relatedPageSize;
    const result = await client.from('history_import_records').select('*,history_import_batch_records!inner(batch_id)', { count:'exact' })
      .eq('history_import_batch_records.batch_id', batch.id).eq('entity_data->>key', stored.entity_data.key).neq('id', id)
      .eq('record_data->>hasContent','true').order('id').range(start, start + relatedPageSize - 1);
