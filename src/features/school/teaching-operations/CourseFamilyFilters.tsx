...
export async function CourseFamilyFilters({ filters, scope }: { filters: Filters; scope: CourseScope }) {
  ...
  return <form className="mt-5 rounded-2xl border border-line bg-card p-4">
    ...
    <div className="grid gap-3 md:grid-cols-[minmax(14rem,1fr)_9rem_9rem_9rem_auto] md:items-end">
      <Input name="q" defaultValue={filters.q} maxLength={80} placeholder={t("searchFamilies")} aria-label={t("searchFamilies")} />
    ...
    <div className="mt-3 flex justify-end"><Link href={`/dashboard/courses?scope=${scope}`} className={cn(buttonVariants({ variant: "ghost", size: "sm" }), "h-9")}>{t("clearFilters")}</Link></div>
  </form>;
}
