...
export function TeacherMicrocourseQuickPreview({ familyId, course, canCreateBranch }: {
  ...
  if (!course) return <section className="h-full" data-teacher-microcourse-quick-preview><header className="border-b border-line/70 px-3 py-2.5"><h2 className="text-sm font-medium">{t("quickPreview")}</h2></header><p className="px-3 py-8 text-sm leading-6 text-muted">{t("selectCourseHint")}</p></section>;
  ...
  return <section className="h-full min-w-0" data-teacher-microcourse-quick-preview>
    <header className="border-b border-line/70 px-3 py-2.5"><div className="flex items-center justify-between gap-2"><h2 className="text-sm font-medium">{t("quickPreview")}</h2><div className="flex items-center gap-1"><Badge variant="outline">{t("currentDefault")}</Badge><Badge variant={course.ready ? "secondary" : "outline"}>{course.ready ? t("ready") : t("incomplete")}</Badge></div></div></header>
    <div className="min-w-0 px-3 py-3">
      ...
      <dl className="mt-3 grid grid-cols-3 border-y border-line/70 py-2 text-center"><div><dt className="text-[11px] text-muted">{t("lectures")}</dt><dd className="font-medium tabular-nums">{course.lectureCount}</dd></div><div className="border-x border-line/60"><dt className="text-[11px] text-muted">{t("released")}</dt><dd className="font-medium tabular-nums">{course.releasedLectureCount}</dd></div><div><dt className="text-[11px] text-muted">{t("branches")}</dt><dd className="font-medium tabular-nums">{course.branchCount}</dd></div></dl>
      ...
      <div className="mt-3 grid gap-1 border-t border-line/70 pt-3">
