...
function previewHref(lectureId: string) {
  return `/dashboard/curriculum/lectures/${lectureId}`;
}
...
export async function TeachingPlan({
  ...
  const previewLink = (lecture: CourseFamilyDetail["teachingPlan"][number]) => <Link href={previewHref(lecture.id)} className={cn(buttonVariants({ variant: "secondary", size: "sm" }), "shrink-0")}>{t("openLecture")}</Link>;
