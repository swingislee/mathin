...
export function CourseCreateDialog() {
  ...
  const { run: submit, pending } = useAction(createCourseAction, {
    successMessage: t("courseCreated"),
    errorMessage: { default: t("actionFailed") },
    onSuccess: (id) => { setOpen(false); setForm(EMPTY); router.push(`/dashboard/courses/${id}`); },
  });
  return <>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-w-lg"><DialogHeader><DialogTitle>{t("newCourse")}</DialogTitle></DialogHeader>
      ...
      <DialogFooter><Button variant="secondary" size="sm" onClick={() => setOpen(false)}>{t("cancel")}</Button><Button size="sm" disabled={pending || !form.title.trim()} onClick={() => submit(form)}>{pending && <LoaderCircle size={15} className="animate-spin"/>}{t("create")}</Button></DialogFooter>
...
export function CourseCrudPanel({ course, canEditTemplate }: { course: CourseDetail; canEditTemplate: boolean }) {
  ...
  const run = (job: () => Promise<ActionResult>, successMessage: string, errorMessage?: (code: string) => string) => startTransition(async () => {
    const result = await job();
    if (result.ok) { toast.success(successMessage); router.refresh(); }
    else toast.error(errorMessage?.(result.code) ?? t("actionFailed"));
  });
  const move = (index: number, delta: number) => {
    const ids = course.lectures.map((x) => x.id); const target=index+delta; [ids[index],ids[target]]=[ids[target],ids[index]];
    run(() => reorderLecturesAction(course.id, ids), t("lecturesReordered"));
  };
  return <div className="mt-6 space-y-6">
    <section className="rounded-xl border border-line bg-card p-5"><div className="flex items-center justify-between"><h2 className="font-medium">{t("courseInfo")}</h2><Button size="sm" disabled={pending || !form.title.trim()} onClick={() => run(() => updateCourseAction(course.id, form), t("courseSaved"))} className="gap-1.5"><Save size={15}/>{t("save")}</Button></div><div className="mt-4"><CourseFields form={form} setForm={setForm}/></div></section>
    <section className="rounded-xl border border-line bg-card p-5"><h2 className="font-medium">{t("lectures")}</h2>
      ...
      <div className="mt-5 grid gap-3 rounded-lg bg-line/40 p-3 sm:grid-cols-[1fr_2fr_auto]"><Input value={name} onChange={e=>setName(e.target.value)} placeholder={t("lectureName")}/><Input value={objectives} onChange={e=>setObjectives(e.target.value)} placeholder={t("objectives")}/><Button size="sm" disabled={pending||!name.trim()} onClick={()=>startTransition(async()=>{const result=await createLectureAction(course.id,name,objectives);if(result.ok){toast.success(t("lectureAdded"));setName("");setObjectives("");router.refresh();}else toast.error(t("actionFailed"));})} className="gap-1.5"><Plus size={15}/>{t("addLecture")}</Button></div>
    </section>
  </div>;
}
