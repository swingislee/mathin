...
export function CourseCrudPanel({ course, canEditTemplate }: { course: CourseDetail; canEditTemplate: boolean }) {
  ...
  return <div className="mt-6 space-y-6">
    ...
    <section className="rounded-xl border border-line bg-card p-5"><h2 className="font-medium">{t("lectures")}</h2>
      ...
      <div className="mt-5 grid gap-3 rounded-lg bg-background p-3 sm:grid-cols-[1fr_2fr_auto]"><Input value={name} onChange={e=>setName(e.target.value)} placeholder={t("lectureName")} className={inputClass}/><Input value={objectives} onChange={e=>setObjectives(e.target.value)} placeholder={t("objectives")} className={inputClass}/><Button size="sm" disabled={pending||!name.trim()} onClick={()=>run(async()=>{await createLectureAction(course.id,name,objectives);setName("");setObjectives("");})} className="gap-1.5"><Plus size={15}/>{t("addLecture")}</Button></div>
