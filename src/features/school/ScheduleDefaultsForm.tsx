...
export function ScheduleDefaultsForm({ defaults }: { defaults: ScheduleDefaultsV2 }) {
  ...
  return (
    <section id="schedule-defaults" className="scroll-mt-40 border-t border-line/60 pt-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="max-w-3xl"><h2 className="text-base font-medium text-ink">{t("classDefaultsTitle")}</h2><p className="mt-1 text-sm text-muted">{t("classDefaultsIntro")}</p></div>
        <Badge variant="outline">{t("newClassesOnly")}</Badge>
      </div>
      <div className="mt-5 grid gap-6">
        <div className="grid gap-3 sm:grid-cols-[minmax(12rem,16rem)_minmax(0,1fr)] sm:items-center">
        ...
        <div className="grid gap-3 sm:grid-cols-[minmax(12rem,16rem)_minmax(0,1fr)]">
