...
export function FollowupEntryFields({
  ...
  return <div data-followup-entry-fields className="min-w-0 space-y-4" onKeyDown={(event) => {
    ...
    {Children.toArray(children).length ? <div data-followup-business className="min-w-0 max-w-6xl space-y-3">{children}</div> : null}
    <div className="min-w-0 max-w-4xl space-y-3">
      <div className="space-y-1.5">
        ...
        <Textarea id={`${id}-note`} value={note} onChange={(event) => onNoteChange(event.target.value)}
          rows={2} maxLength={2000} disabled={disabled || pending}
          placeholder={placeholder ?? t("notePlaceholder")}
          className="min-h-20 resize-y bg-card text-xs leading-5" />
      </div>
      ...
      <div data-followup-entry-actions className="flex min-w-0 flex-wrap items-center gap-2 border-t border-line pt-3">
        ...
        <Button type="button" size="sm" variant={canAdvance ? "secondary" : "primary"}
          className="h-auto min-h-9 whitespace-normal px-3 py-1.5 text-xs"
        ...
        {canAdvance ? <Button type="button" size="sm" className="h-auto min-h-9 whitespace-normal px-3 py-1.5 text-xs"
