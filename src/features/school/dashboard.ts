...
export async function getActivityToday():Promise<ActivityTodayRow[]> {
  ...
  const {data,error}=await supabase.from("activities").select("id,title,scheduled_at,activity_registrations(status)").is("deleted_at",null).gte("scheduled_at",from.toISOString()).lt("scheduled_at",to.toISOString()).order("scheduled_at",{ascending:true}).returns<Array<{id:string;title:string;scheduled_at:string;activity_registrations:Array<{status:string}>|null}>>();
  if(error)throw new Error(error.message);return(data??[]).map(x=>({id:x.id,title:x.title,scheduledAt:x.scheduled_at,bookedCount:(x.activity_registrations??[]).filter(r=>r.status==="booked"||r.status==="attended").length}));
}
