...
export function SourceEnrollmentPlacementDialog({record,name,options,locale,onClose,onSaved}:{record:HistoricalEnrollment;name:string;options:EnrollmentWorkflowOptions;locale:string;onClose:()=>void;onSaved:(board:EnrollmentPlacementBoard)=>void}) {
  const en=locale==='en';
  const current=record.record_state==='current';
  ...
  async function save() {
    if(!current||!studentId||!classroomId)return;
  ...
  return <Dialog open onOpenChange={value=>{if(!value&&!busy)onClose();}}><DialogContent className="max-h-[85dvh] overflow-y-auto sm:max-w-lg">
    <DialogHeader><DialogTitle>{current?(en?'Continue placement':'继续分班'):(en?'Enrollment history':'历史报名')} · {name}</DialogTitle><DialogDescription>{current?(en?'Confirm the student and select the class for this enrollment.':'确认学生并选择这次报名对应的班级。'):(en?'Review and correct the recorded enrollment details.':'查看并修订当时记录的报名资料。')}</DialogDescription></DialogHeader>
