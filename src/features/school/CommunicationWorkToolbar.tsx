...
import { ChevronLeft, ChevronRight, ListChecks, ListPlus } from "lucide-react";
...
export function CommunicationWorkToolbar({ options, scope, canViewAll, canManage, worklist, worklists, pageKeys, count, today, query = "", hasFieldFilters = false, secondaryFilters }: {
  ...
  pageKeys: string[]; count: number; today: string; query?: string; hasFieldFilters?: boolean;
  ...
  return <>
    <DashboardCommandFilters>
      <FollowupPrimaryFilter value={options.view} label={t("viewLabel")} disabled={pending}
        options={(["unscheduled", "day", "records", "all", ...(worklist ? ["worklist"] : [])] as const).map((value) => ({ value, label: value === "worklist" ? t("view_worklist") : t(`view_${value}`) }))}
        onValueChange={(view) => navigate({ view, fields: null, ...(view !== "all" ? { state: "current" } : {}), worklist: view === "worklist" ? worklist?.id ?? null : null })} />
    ...
    <DashboardCommandActions>
      ...
      {canManage && options.view !== "worklist" && keys.length > 0 ? <Popover open={createOpen} onOpenChange={setCreateOpen}><PopoverTrigger asChild><Button type="button" size="sm" disabled={pending}><ListPlus className="size-4" />{t("saveSelected", { count: keys.length })}</Button></PopoverTrigger>
        <PopoverContent align="end" className="w-80 space-y-3">
          <p className="text-xs text-muted">{t("selectedListHint", { count: keys.length })}</p>
      ...
      {options.view === "unscheduled" && count === 0 ? <Button variant="secondary" size="sm" disabled={pending} onClick={() => navigate({ view: "all", fields: null })}>{t("showAll")}</Button> : null}
    </DashboardCommandActions>
  </>;
}
