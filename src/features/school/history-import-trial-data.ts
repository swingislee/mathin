...
import type { ImportedFamilyHistory } from './imported-family-history-contract';
...
export async function loadImportedFamilyHistory(locale: string, studentId: string): Promise<ImportedFamilyHistory | null> {
  ...
  const supabase = await createClient();
  const { data: batch, error } = await supabase.from('history_import_batches')
    .select('id,manifest,verification,imported_at').eq('work_scope', 'history_only')
    .contains('manifest', { mode: 'local_family_audit', subject: { studentId } })
    .order('imported_at', { ascending: false }).limit(1).maybeSingle();
  if (error) throw new Error(`HISTORY_FAMILY_READ_${error.code}`);
  if (!batch) return null;
  const { data: links, error: linkError } = await supabase.from('history_import_batch_records')
    .select('record_id').eq('batch_id', batch.id).limit(251);
  if (linkError || !links?.length || links.length > 250) throw new Error('HISTORY_FAMILY_LINKS');
  const { data: records, error: recordError } = await supabase.from('history_import_records')
    .select('id,source_data,record_data,match_status,entity_data,candidate_data,student_id,lead_id,search_text')
    .in('id', links.map(link => link.record_id)).order('id').limit(251);
  if (recordError) throw new Error(`HISTORY_FAMILY_RECORDS_${recordError.code}`);
  const manifest = batch.manifest as unknown as ImportedFamilyHistory['manifest'];
  if (manifest.mode !== 'local_family_audit' || manifest.subject.studentId !== studentId
    || records?.length !== manifest.recordCount || links.length !== manifest.recordCount
    || manifest.coverage.length !== manifest.recordCount
    || !manifest.coverage.every(item => records.some(record => record.id === item.recordId))) throw new Error('HISTORY_FAMILY_COVERAGE');
  return { importedAt: batch.imported_at, manifest, verification: batch.verification as unknown as ImportedFamilyHistory['verification'], records: records as unknown as ImportedHistoryRecord[] };
}
