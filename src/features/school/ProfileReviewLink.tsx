...
import { PROFILE_REVIEW_ENABLED } from './profile-review-contract';

/** 开发端按实际分配的读取权限显示入口。 */
export async function ProfileReviewLink({ locale }: { locale: string }) {
  if (!PROFILE_REVIEW_ENABLED || !isLocalHistoryArchiveEnvironment(process.env.NODE_ENV, process.env.NEXT_PUBLIC_SUPABASE_URL)) return null;
