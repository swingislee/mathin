...
import { businessSubjectKey, type BusinessRecordSource, type BusinessHistoryKind, type StudentBusinessHistory } from './student-business-history-contract';
...
export function BusinessHistorySections({ data, locale, kind, showStudent = false, query = '' }: {
  ...
  const visible=<T extends BusinessRecordSource>(rows:T[])=>rows.filter(row=>!needle||`${data.students[businessSubjectKey(row)]??''} ${JSON.stringify(row)}`.toLocaleLowerCase(locale).includes(needle));
  ...
  const studentCell=(row:BusinessRecordSource,currentKind:BusinessHistoryKind)=>showStudent?<TableCell className="align-top">{row.student_id?<Link href={`/dashboard/students/${row.student_id}?tab=history&history=${currentKind}`} className="underline underline-offset-4">{data.students[businessSubjectKey(row)]??m.unknown}</Link>:data.students[businessSubjectKey(row)]??m.unknown}</TableCell>:null;
  ...
  return <div className="min-w-0 space-y-7" id="student-business-history">
    {include('renewal')&&<DashboardSection title={`${m.renewal} · ${renewals.length}`}>
      {!renewals.length?empty:<DashboardTableShell><Table>
        ...
        <TableBody>{renewals.map(row=><TableRow key={row.id} id={row.id}>
          ...
          <TableCell className="align-top">{[row.class_label,row.teacher_label].filter(Boolean).join(' · ')||m.unknown}<BusinessRecordRevisionButton kind="renewal" recordId={row.id} subject={data.students[businessSubjectKey(row)]}/></TableCell>
        </TableRow>)}</TableBody>
    ...
    {include('renewal')&&renewalConversations.map(row=><section key={row.id} id={row.id}>
      ...
      <BusinessRecordRevisionButton kind="communication" recordId={row.id} subject={data.students[businessSubjectKey(row)]}/>
    </section>)}
      ...
      {!activities.length?empty:<DashboardTableShell><Table>
        ...
        <TableBody>{activities.map(row=><TableRow key={row.id} id={row.id}>
          ...
          <TableCell className="align-top">{row.reported_result||m.unknown}{row.reported_result&&<p className="mt-1 text-xs text-muted">{m.reportedResult}{row.result_link_status==='edition_unconfirmed'?` · ${m.editionPending}`:''}</p>}
            <BusinessRecordRevisionButton kind="activity" recordId={row.id} subject={data.students[businessSubjectKey(row)]}/>
    ...
    {include('assessment')&&<DashboardSection title={`${m.assessment} · ${assessments.length}`}>
      {!assessments.length?empty:<DashboardTableShell><Table>
        ...
        <TableBody>{assessments.map(row=><TableRow key={row.id} id={row.id}>
          ...
          <TableCell className="max-w-xl align-top whitespace-pre-wrap leading-6">{historicalAssessmentFeedback(row,data)||m.unknown}<BusinessRecordRevisionButton kind="assessment" recordId={row.id} subject={data.students[businessSubjectKey(row)]}/></TableCell>
        </TableRow>)}</TableBody>
      </Table></DashboardTableShell>}
    </DashboardSection>}
    ...
    {include('enrollment')&&<DashboardSection title={`${m.enrollment} · ${enrollments.length}`}>
      {!enrollments.length?empty:<><DashboardTableShell><Table>
        ...
        <TableBody>{enrollments.map(row=><TableRow key={row.id} id={row.id}>
          ...
          <TableCell className="align-top">{row.schedule_label||m.unknown}{row.room_label&&<p className="mt-1 text-xs text-muted">{m.room} {row.room_label}</p>}<BusinessRecordRevisionButton kind="enrollment" recordId={row.id} subject={data.students[businessSubjectKey(row)]}/></TableCell>
        </TableRow>)}</TableBody>
    ...
    {include('communication')&&<DashboardSection title={`${m.communication} · ${communications.length}`}>
      {!communications.length?empty:<ol className="space-y-6">{communications.map(row=>{
        ...
        {showStudent&&row.student_id&&<Link href={`/dashboard/students/${row.student_id}?tab=history&history=communication`} className="mb-2 inline-block text-sm underline underline-offset-4">{data.students[businessSubjectKey(row)]??m.unknown}</Link>}
        ...
        <BusinessHistoryEvidence data={data} reference={row} locale={locale}/>
        <BusinessRecordRevisionButton kind={assessment?'assessment':'communication'} recordId={assessment?.id??row.id} subject={data.students[businessSubjectKey(row)]}/>
      </li>;})}</ol>}
    </DashboardSection>}
  </div>;
}
