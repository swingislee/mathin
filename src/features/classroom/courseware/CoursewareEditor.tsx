...
import { coursewareKind, uploadCoursewareAsset } from "./upload";
...
export function CoursewareEditor({
  ...
  classroomId: string;
  sessionId: string;
  initialPages: CoursewarePage[];
}) {
  const t = useTranslations("classroom.session");
  const tGames = useTranslations("games");
  const [pages, setPages] = useState<CoursewarePage[]>(initialPages);
  ...
  const persist = useCallback(async () => {
    setSaveState("saving");
    try {
      await saveCourseware(sessionId, pagesRef.current);
      setSaveState("saved");
    } catch {
      setSaveState("error");
    }
  ...
  const mutate = useCallback((updater: (prev: CoursewarePage[]) => CoursewarePage[]) => {
    setPages(updater);
    setSaveState("dirty");
  ...
  const addFiles = async (files: FileList | null) => {
    ...
    try {
      for (const file of Array.from(files)) {
        const kind = coursewareKind(file);
        if (!kind) continue;
        const path = await uploadCoursewareAsset(classroomId, file);
  ...
  return (
    <section className="mt-8">
      <div className="flex flex-wrap items-center gap-3">
        <h3 className="text-sm font-medium text-muted">{t("pagesTitle", { count: pages.length })}</h3>
