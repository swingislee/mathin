...
export function ChangeBell({initialEvents}:{initialEvents:ChangeEvent[]}){
  ...
  return <Popover onOpenChange={openChange}>
    <PopoverTrigger asChild><button type="button" aria-label={t("label",{count:unread})} className="edge-control relative"><Bell size={18}/>{unread>0&&<span className="absolute -right-1 -top-1 grid min-w-5 place-items-center rounded-full bg-rose px-1 text-[10px] leading-5 text-white">{Math.min(unread,99)}</span>}</button></PopoverTrigger>
