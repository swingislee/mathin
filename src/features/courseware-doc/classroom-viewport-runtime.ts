...
(() => {
  ...
  const reserve = (active) => {
    ...
    if (active) {
      cancelling = true;
      for (const [id, point] of points) point.target?.dispatchEvent(new PointerEvent("pointercancel", { pointerId: id, pointerType: "touch", bubbles: true }));
      cancelling = false;
    } else suppressUntil = performance.now() + 600;
  ...
  const pointer = (event) => {
    if (!enabled || cancelling || event.pointerType !== "touch") return;
    ...
    if (phase === "down") {
      ...
      if (points.size > 1 && !reserved) reserve(true);
  ...
  window.addEventListener("message", (event) => {
    ...
    if (!enabled || data.token !== token || data.type !== "touch" || !Number.isSafeInteger(data.id)
      || !["down", "move", "up", "cancel"].includes(data.phase)
      || ![data.x, data.y, data.ny].every((n) => typeof n === "number" && Number.isFinite(n) && Math.abs(n) <= 1e6) || data.ny < 0 || data.ny > 1) return;
    ...
    send({ type: "touch", phase: data.phase, id, x: data.x, y: data.y, ny: Math.max(0, Math.min(1, (rect.top + data.ny * rect.height) / Math.max(1, innerHeight))) });
