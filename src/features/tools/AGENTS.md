...
- 连续对象手势通过 `CubeMoveHandles.bodyGesture` 接入共用平面投影、Arcball 与指针归属；领域提供固定支点、包围半径和准确候选。Soma 使用三标准平面移动及自由轨迹球；按视角自动选面时，方向提示与起拖共用同一个选面器，手势中保持锁定。触屏点旋转后直接拖对象，精确轴保留。需要准确落位时复用 `rotation-snap` 的邻近判断，在松手时自动平滑吸附；开关与视角吸附分开，领域校验落点，准确性融入动作。任意姿态按新版合同保存，其他教具待手感验收后逐一接入。
