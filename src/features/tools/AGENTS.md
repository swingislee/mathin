...
- 所有可移动刚体的本体拖动默认沿 XZ，轴／平面手柄才指定其他约束；接入 `bodyGesture` 后，旧轴控制器仅处理明确命中的轴箭头。未选中也能直接起拖，按实际命中对象解析领域目标，不依赖侧栏当前 ID。平面与支点锁定至松手，不按视角猜轴／换平面；仅保留纵向高度虚线。可选自由轨迹球显式接入 Arcball；只支持离散旋转的适配器关闭该入口。需要准确落位时复用 `rotation-snap`，松手自动平滑吸附；开关与视角吸附分开。自由姿态仅按支持它的新版合同保存，共用手柄不扩大其他教具的合法姿态范围。
