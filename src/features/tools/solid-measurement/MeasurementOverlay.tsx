...
function ActiveMeasurementOverlay({ entity, settings, feature, locale }: MeasurementOverlayProps & { entity: SolidEntity }) {
  ...
  return <group position={tuple(entity.position)} rotation={tuple(entity.rotation)} name={`solid-measurement:${entity.id}`}>
    {dimensionLines.map((line) => <group key={line.id}>
      ...
      <Html center position={tuple(line.label)} zIndexRange={[5, 0]} style={{ pointerEvents: "none" }}>
        <span className="whitespace-nowrap rounded bg-paper/95 px-1 py-0.5 text-xs text-ink">{m.dimensionNames[line.id]} {formatMeasurementValue(line.value, locale)}</span>
      </Html>
    </group>)}
    ...
    {faceArea !== null && <Html center position={tuple(faceAnchor)} occlude zIndexRange={[5, 0]} style={{ pointerEvents: "none" }}>
      <span className="whitespace-nowrap rounded bg-moon/95 px-1.5 py-0.5 text-xs text-ink">{m.selectedArea} {formatMeasurementValue(faceArea, locale, 2)}</span>
    </Html>}
    {settings.totals && <Html center position={[0, upper + 0.62, 0]} zIndexRange={[5, 0]} style={{ pointerEvents: "none" }}>
      <span className="block whitespace-nowrap rounded bg-paper/95 px-2 py-1 text-xs leading-5 text-ink">{formatMeasurementEquation("S", metrics.surfaceArea, locale, 2)}<br />{formatMeasurementEquation("V", metrics.volume, locale, 3)}</span>
    </Html>}
  </group>;
}
