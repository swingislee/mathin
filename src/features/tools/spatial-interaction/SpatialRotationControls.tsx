...
export function SpatialRotationControls({ center, radius = 1, action }: {
  ...
  return <group name="spatial-object-rotation">
    {hint && <><Line points={Array.from({ length: 65 }, (_, i) => point(direction ? direction * i * Math.PI / 128 : i * Math.PI / 32))} color={color} lineWidth={2} depthWrite={false} depthTest={false} raycast={() => null} />
    ...
    <Html position={[center.x, center.y + r + 0.35, center.z]} center calculatePosition={toolbarPosition} zIndexRange={[7, 0]}>
      <div className="flex gap-0.5 rounded-xl border border-line bg-paper/95 p-1 shadow-sm" role="toolbar" aria-label={action.label}
        ...
        {(["x", "y", "z"] as const).map((value) => <CubeIconButton key={value} label={`${action.label} ${value.toUpperCase()}`} active={axis === value} disabled={action.disabled} onClick={() => action.onAxisChange(value)}>
          <span className="text-xs font-bold" style={{ color: CUBE_AXIS_COLORS[value] }}>{value.toUpperCase()}</span>
        </CubeIconButton>)}
        {([-1, 1] as const).map((turn) => <CubeIconButton key={turn} label={`${action.label} ${axis.toUpperCase()} ${turn > 0 ? "+" : "−"}90°`} disabled={action.disabled} onPointerEnter={() => setDirection(turn)} onFocus={() => setDirection(turn)} onClick={() => action.onRotate(axis, turn)}>
