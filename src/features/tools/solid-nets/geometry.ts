export type SolidNetKind = "cuboid" | "triangular-prism";
...
export function solidNetGeometry(kind: SolidNetKind, dimensions: SolidNetDimensions) {
  ...
  const side = kind === "cuboid" ? h : Math.hypot(w / 2, h);
  const sideAngle = kind === "cuboid" ? 90 : Math.acos(-w / 2 / side) * 180 / Math.PI;
  ...
  if (kind === "cuboid") hinges.push({ id: "front-top", parentId: "front", faceId: "top", start: p(x0, z1 + h), end: p(x1, z1 + h), closedDegrees: 90, depth: 2 });
  return { faces, hinges };
}
