...
export function SolidSectionOverlay({ entity, frame, plane }: { entity: SolidEntity; frame: SolidSectionFrame; plane: SolidSectionPlane }) {
  ...
  return <group name={`solid-section:${entity.id}`}>
    ...
    {result.kind === "polygon" && <mesh geometry={cap} raycast={ignoreRaycast} renderOrder={2}>
      <meshBasicMaterial color={CUBE_SELECTION_COLOR} transparent opacity={frame.sectionOpacity} side={DoubleSide} depthWrite={false} polygonOffset polygonOffsetFactor={-2} polygonOffsetUnits={-2} />
    </mesh>}
    {result.points.length >= 2 && <Line points={(result.kind === "polygon" ? [...result.points, result.points[0]] : result.points).map(tuple)} color={CUBE_SELECTION_COLOR} lineWidth={3} transparent opacity={Math.min(1, frame.sectionOpacity * 1.2)} depthWrite={false} raycast={ignoreRaycast} />}
    {result.kind === "point" && <mesh position={tuple(result.points[0])} raycast={ignoreRaycast}><sphereGeometry args={[0.035, 12, 8]} /><meshBasicMaterial color={CUBE_SELECTION_COLOR} transparent opacity={frame.sectionOpacity} /></mesh>}
  </group>;
}
