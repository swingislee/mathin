...
export function ToolSceneLibrary({ version, scene, onOpen, title, pageHeader, children }: {
  ...
  return <div className={cn("relative min-w-0 shrink-0", pageHeader && "border-b")} data-tool-scene-library>
    {/* 固定单行高度，横向滚动条、保存状态与场景选择的显隐均不改变舞台高度。 */}
    <div className={cn("flex h-12 min-w-0 flex-nowrap items-center gap-2 overflow-x-auto overflow-y-hidden [scrollbar-width:thin]", pageHeader ? "px-4" : "px-1")} data-tool-scene-toolbar>
      {pageHeader?.start}
      {title}
      ...
      {pageHeader && <div className="ml-auto flex shrink-0 items-center gap-3 whitespace-nowrap">{pageHeader.end}</div>}
    </div>
