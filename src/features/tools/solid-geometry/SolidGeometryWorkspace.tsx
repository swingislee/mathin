...
import { MeasurementButton, MeasurementAnyPanel as MeasurementPanel } from "../solid-measurement/MeasurementControls";
...
import { arrangeSolidCut, createSolidCut, createSolidGeometryExplorationInitial, replaceExplorationEntity, solidCutPieceId, solidExplorationObjects, solidGeometryExplorationInitial, solidGeometryExplorationInitialSchema, solidGeometryExplorationSnapshot, solidGeometryExplorationSnapshotSchema, solidMeshTopology, type SolidGeometryExplorationInitial, type SolidGeometryExplorationSnapshot } from "./exploration-contract";
import { solidCutPlaneFromSection, splitSolidByPlane } from "../solid-sections/solid-cut-model";
import { SolidCutControls, SolidCutPieceReadout } from "../solid-sections/SolidCutControls";
import type { AnyMeasurementSettings } from "../solid-measurement/measurement-v2-contract";
...
type AnyInitial = SolidGeometryInitial | SolidGeometryExplorationInitial;
type AnySnapshot = SolidGeometrySnapshot | SolidGeometryExplorationSnapshot;
function objectsFor(snapshot: AnyInitial) { return "cuts" in snapshot ? solidExplorationObjects(snapshot) : { entities: snapshot.entities, meshes: undefined, cutColors: undefined }; }
function replaceEntity(snapshot: AnySnapshot, entity: SolidEntity): AnySnapshot { return "cuts" in snapshot ? replaceExplorationEntity(snapshot, entity) : { ...snapshot, entities: snapshot.entities.map((item) => item.id === entity.id ? entity : item) }; }
export interface SolidWorkspaceContext {
  snapshot: AnySnapshot; selected: SolidEntity | null; disabled: boolean; locale: string;
  update: (next: unknown) => boolean; closePanel: () => void;
}
...
export interface SolidGeometryExplorationWorkspaceProps extends Omit<SolidGeometryWorkspaceProps, "initial" | "onSnapshot" | "classroom"> {
  modern: true; initial?: SolidGeometryExplorationInitial; onSnapshot?: (initial: SolidGeometryExplorationInitial | null) => void;
  classroom?: { state?: SolidGeometryExplorationSnapshot; onChange?: (next: SolidGeometryExplorationSnapshot) => Promise<void> };
}
/** 共用备课/课堂宿主只管理离散参数；截面、测量通过三个扩展槽接入同一空间。 */
export function SolidGeometryWorkspace(props: SolidGeometryWorkspaceProps) { return <SolidGeometrySharedWorkspace {...props} />; }
export function SolidGeometrySharedWorkspace(props: SolidGeometryWorkspaceProps | SolidGeometryExplorationWorkspaceProps) {
  const { initial, onSnapshot, classroom, readOnly = false, renderScene, renderToolbar, renderPanel, onToolChange } = props;
  const modern = "modern" in props && props.modern;
  const locale = useLocale() === "en" ? "en" : "zh", m = { ...solidGeometryMessages(locale), ...spatialActionMessages(locale) };
  const origin = useMemo(() => modern ? solidGeometryExplorationSnapshot((initial as SolidGeometryExplorationInitial | undefined) ?? createSolidGeometryExplorationInitial()) : solidGeometrySnapshot((initial as SolidGeometryInitial | undefined) ?? createSolidGeometryInitial()), [initial, modern]);
  const publish = useCallback(async (next: AnySnapshot) => { if (!classroom?.onChange) return; if (modern) await (classroom.onChange as (s: SolidGeometryExplorationSnapshot) => Promise<void>)(solidGeometryExplorationSnapshotSchema.parse(next)); else await (classroom.onChange as (s: SolidGeometrySnapshot) => Promise<void>)(solidGeometrySnapshotSchema.parse(next)); }, [classroom, modern]);
  const runtime = classroom ? { state: classroom.state, onChange: classroom.onChange ? publish : undefined } : undefined;
  const host = useToolSnapshot<AnySnapshot>(origin, runtime), snapshot = host.snapshot;
  ...
  const [frame, setFrame] = useState(() => { const objects = objectsFor(origin); return getSolidsFrame(objects.entities, objects.meshes); });
  ...
  const [sectionGesture, setSectionGesture] = useState<{ source: AnySnapshot; settings: SolidSectionSettings; pending: boolean } | null>(null);
  const [measurementAnimating, setMeasurementAnimating] = useState(false);
  ...
  const displayTargets = useMemo(() => objectsFor(displaySnapshot).entities.map((entity) => entity.id === displaySnapshot.selectedId ? measurementDisplayEntity(entity, displaySnapshot.measurement) : entity), [displaySnapshot]);
  ...
  const selected = objects.entities.find((entity) => entity.id === snapshot.selectedId) ?? null;
  const selectedCut = "cuts" in snapshot ? snapshot.cuts.find((cut) => cut.pieces.some((_, index) => solidCutPieceId(cut, index) === snapshot.selectedId)) : undefined;
  const cutSource = selectedCut ? snapshot.entities.find((entity) => entity.id === selectedCut.entityId) : undefined;
  const rollTargets = useMemo(() => panel === "roll" && selected && !selectedCut ? Object.fromEntries(SPATIAL_ROLL_DIRECTIONS.map((direction) => [direction, solidRollTarget(selected, direction, objects.entities)])) : {}, [panel, selected, selectedCut, objects.entities]);
  const captured = useMemo(() => "cuts" in snapshot ? solidGeometryExplorationInitial(snapshot) : solidGeometryInitial(snapshot), [snapshot]);
  const captureValue = useCallback((value: AnyInitial | null) => { if (!onSnapshot) return; if (modern) (onSnapshot as (s: SolidGeometryExplorationInitial | null) => void)(value ? solidGeometryExplorationInitialSchema.parse(value) : null); else (onSnapshot as (s: SolidGeometryInitial | null) => void)(value as SolidGeometryInitial | null); }, [onSnapshot, modern]);
  const capture = useSceneCapture(disabled || dragging || sectionDragging || !!sectionPreview || presentation.animating || sectionPresentation.animating || measurementAnimating ? null : captured, captureValue);
  const update = useCallback((next: unknown) => {
    const parsed = modern ? solidGeometryExplorationSnapshotSchema.safeParse(next) : solidGeometrySnapshotSchema.safeParse(next); if (!parsed.success) return false;
    setInstantKey(null); directMove.clear(); setSectionInstantKey(null); setSectionGesture(null); return host.update(parsed.data);
  }, [host, directMove, modern]);
  ...
  const updateEntity = (next: SolidEntity) => update(replaceEntity(snapshot, next));
  const pick = (id: string, feature: SolidFeatureSelection | null) => { if (!disabled) {
    activateSelection();
    if (tool === "section" && "cuts" in snapshot && snapshot.cuts.some((cut) => cut.pieces.some((_, index) => solidCutPieceId(cut, index) === id))) setPanel("section", "orbit");
    update({ ...snapshot, selectedId: id, feature });
  } };
  ...
  const fit = () => { setFrame(getSolidsFrame(objects.entities, objects.meshes)); update({ ...snapshot, cameraRevision: snapshot.cameraRevision + 1 }); };
  ...
  const remove = () => { if (!selected || selectedCut) return; const entities = snapshot.entities.filter((entity) => entity.id !== selected.id); update({ ...snapshot, entities, selectedId: objects.entities.filter((entity) => entity.id !== selected.id).at(-1)?.id ?? null, feature: null }); };
  const move = (operation: CubeMoveOperation, direct = false) => { const entities = moveSolidByDrag(objects.entities, operation); if (!entities) return;
    const changed = entities.filter((entity) => operation.ids.includes(entity.id)).reduce(replaceEntity, snapshot);
    const next = { ...changed, selectedId: operation.ids[0], feature: snapshot.selectedId === operation.ids[0] ? snapshot.feature : null };
  ...
  const selectedMesh = selected ? objects.meshes?.get(selected.id) : undefined;
  const topology = selectedMesh ? solidMeshTopology(selectedMesh) : selected ? getSolidTopology(selected) : null;
  const parts = panel === "face" ? topology?.faces : panel === "edge" ? topology?.edges : panel === "vertex" ? topology?.vertices : null;
  const featureLabel = (id: string, index: number) => id === "cut" ? locale === "en" ? "Cut face" : "切面" : id in m.faces ? m.faces[id as keyof typeof m.faces] : id.startsWith("rim-") ? `${m.boundary} ${index + 1}` : `${panel === "edge" ? m.edge : m.vertex} ${index + 1}`;
  const context: SolidWorkspaceContext = { snapshot, selected, disabled, locale, update, closePanel: () => setPanel(null) };
  const applyMeasurement = (measurement: AnyMeasurementSettings) => update({ ...snapshot, measurement, section: measurement.enabled ? { ...snapshot.section, enabled: false } : snapshot.section } as AnySnapshot);
  const cut = () => {
    if (!("cuts" in snapshot) || !selected || selectedCut || disabled) return;
    const cut = createSolidCut(selected, solidCutPlaneFromSection(selected, snapshot.section)); if (!cut) return;
    if (update({ ...snapshot, cuts: [...snapshot.cuts, cut], selectedId: solidCutPieceId(cut, 0), feature: null, section: { ...snapshot.section, enabled: false } })) { activateSelection(); setPanel("section", "orbit"); }
  };
  const canCut = modern && selected && !selectedCut && supportsSolidSection(selected.kind) && !!splitSolidByPlane(selected, solidCutPlaneFromSection(selected, snapshot.section));
  return <section className={styles.workspace} data-workbench-mode="courseware" data-solid-geometry-workspace={modern ? "v2" : "v1"} aria-label={m.title} {...capture} {...controls.bindings}>
    <div className={styles.viewport}><div className={styles.canvas}>
      <Canvas entities={presentation.entities} state={canvasState} meshes={objects.meshes} cutColors={objects.cutColors} selectedId={snapshot.selectedId} feature={snapshot.feature} pickMode={featureMode} section={sectionPresentation.frame}
        ...
        onTransform={(entity) => {
          ...
          activateSelection(); directMove.hold(next); setInstantKey(solidEntitiesKey(objectsFor(next).entities.map((item) => item.id === next.selectedId ? measurementDisplayEntity(item, snapshot.measurement) : item))); return true;
        }}
        rollAction={panel === "roll" ? rollAction : undefined}
        renderScene={(context) => <>{context.selected && (selectedCut && cutSource && "cuts" in snapshot ? <SolidCutPieceReadout entity={context.selected} source={cutSource} cut={selectedCut} index={solidCutPieceId(selectedCut, 0) === context.selected.id ? 0 : 1} locale={locale} settings={snapshot.measurement} /> : <MeasurementOverlay entity={context.selected} settings={snapshot.measurement} feature={snapshot.feature} locale={locale} onAnimating={setMeasurementAnimating} />)}{renderScene?.(context)}</>} />
      ...
      <div className={`${styles.dock} ${styles.meta}`}>
        ...
        <SpatialActionButton action="reset" label={m.reset} disabled={disabled} onClick={() => { const objects = objectsFor(origin); setFrame(getSolidsFrame(objects.entities, objects.meshes)); update({ ...structuredClone(origin), cameraRevision: snapshot.cameraRevision + 1 }); controls.closePanel(); onToolChange?.(); }} />
      </div>
      ...
      <div className={`${styles.dock} ${styles.tools}`} role="toolbar" aria-label={m.tools} data-solid-tools-toolbar>
        ...
        <SpatialActionButton action="section" label={sectionMessages.title} active={panel === "section" || snapshot.section.enabled} disabled={disabled || !selected} onClick={() => {
          open("section", selectedCut ? "orbit" : "section");
          if (!selectedCut && selected && supportsSolidSection(selected.kind) && !snapshot.section.enabled) update({ ...snapshot, section: { ...snapshot.section, enabled: true }, measurement: { ...snapshot.measurement, enabled: false } });
        }} />
        <MeasurementButton locale={locale} active={panel === "measurement" || snapshot.measurement.enabled} disabled={disabled || !selected || !!selectedCut} onClick={() => {
        ...
        <span className={styles.toolSeparator} />
        <SpatialActionButton action="remove" label={m.remove} disabled={disabled || !selected || !!selectedCut} onClick={remove} />
      </div>
      {panel && panel !== "measurement" && <SpatialCanvasPanel title={panel === "section" ? sectionMessages.title : m[panel]} anchor={panel === "settings" ? "meta" : "tool"} closeLabel={m.close} onClose={() => setPanel(null)}>
        <div className="space-y-3 text-xs">
          ...
          {panel === "section" && (selectedCut && cutSource && "cuts" in snapshot ? <SolidCutControls cut={selectedCut} entity={cutSource} locale={locale} settings={snapshot.measurement} disabled={disabled || dragging || presentation.animating}
            onArrange={(apart) => update({ ...snapshot, cuts: snapshot.cuts.map((item) => item === selectedCut ? arrangeSolidCut(item, cutSource, apart) : item) })}
            onRestore={() => update({ ...snapshot, cuts: snapshot.cuts.filter((item) => item !== selectedCut), selectedId: cutSource.id, feature: null })}
            onColor={(cutColor) => update({ ...snapshot, cuts: snapshot.cuts.map((item) => item === selectedCut ? { ...item, cutColor } : item) })}
            onSelect={(index) => pick(solidCutPieceId(selectedCut, index), null)} /> : <>
            <SolidSectionControls entity={selected} settings={snapshot.section} locale={locale} disabled={disabled || sectionDragging} onChange={(section) => update({ ...snapshot, section, measurement: section.enabled ? { ...snapshot.measurement, enabled: false } : snapshot.measurement })} />
            {modern && <Button size="sm" variant="secondary" disabled={disabled || sectionDragging || sectionPresentation.animating || presentation.animating || !canCut} onClick={cut}><SpatialActionIcon action="cut" className="mr-1 size-3.5" />{locale === "en" ? "Cut into two pieces" : "沿当前平面切成两块"}</Button>}
          </>)}
          ...
          {panel === "objects" && <>
            <div className="flex flex-wrap gap-1">{objects.entities.map((entity, index) => <Button key={entity.id} size="sm" variant={selectionActive && entity.id === snapshot.selectedId ? "secondary" : "ghost"} disabled={disabled} onClick={() => pick(entity.id, null)}>{index + 1} · {objects.meshes?.has(entity.id) ? locale === "en" ? "Cut piece" : "切块" : m.kinds[entity.kind]}</Button>)}</div>
            {selected && !selectedCut && solidDimensionKeys(selected.kind).map((key) => <div key={`${selected.id}-${key}`} className="flex items-center gap-2"><Label className="w-12" htmlFor={`solid-${selected.id}-${key}`}>{m.dimensions[key]}</Label>
      ...
      {panel === "measurement" && !selectedCut && <MeasurementPanel locale={locale} settings={snapshot.measurement} selected={selected} feature={snapshot.feature} disabled={disabled}
        onChange={applyMeasurement}
        onUnitExample={modern && selected && (selected.kind === "cube" || selected.kind === "cuboid") ? (measurement) => {
          const resized = { ...selected, dimensions: { width: 10, height: 10, depth: 10, radius: 1 }, position: { ...selected.position, y: 5 } };
          const next = { ...replaceEntity(snapshot, resized), measurement, cameraRevision: snapshot.cameraRevision + 1 } as AnySnapshot;
          if (update(next)) { const objects = objectsFor(next); setFrame(getSolidsFrame(objects.entities, objects.meshes)); }
        } : undefined}
