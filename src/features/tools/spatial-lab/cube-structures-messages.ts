...
const zh = {
  ...
  move: "移动单位块", moveHint: "拖动 XYZ 箭头，或选择轴后拖动方块。单击可选中一块；没有单独选择时拖动当前分组或整体。松手吸附整数格（拆开展示为半格），记录一步。",
  ...
  cutHint: "选择输入方式，悬浮预览并高亮命中位置；点击锁定后，在截面旁选择切割或取消。",
  cutInput: "截面选择方式", cutEdge: "选层间线", cutFace: "选面朝向", cutEdgeHint: "靠近方块之间的棱线，选择穿过该线的 XYZ 截面。外轮廓和面中心不作为层间线；交点处沿想选的线稍微移开。", cutFaceHint: "指向面中心，选择平行于该面的内侧最近层界。锁定后可在参数里微调层界。",
  cutConfirm: "切割",
...
const en = {
  ...
  move: "Move cubes", moveHint: "Drag an XYZ arrow, or choose an axis and drag cubes. Click to select one cube; otherwise drag the active group or whole structure. Release to snap to whole units (half units for display separation) and record one step.",
  ...
  cutHint: "Choose an input mode, hover to preview the section and its picked location, then click to lock. Choose Cut or Cancel beside the section.",
  cutInput: "Section selection", cutEdge: "Pick layer edge", cutFace: "Pick face direction", cutEdgeHint: "Move near an edge between cubes to select the XYZ plane through it. Outer outlines and face centers are not layer edges. At a crossing, move slightly along the intended edge.", cutFaceHint: "Point at a face center to select the nearest inward layer boundary parallel to it. After locking, fine-tune the layer in the panel.",
  cutConfirm: "Cut",
