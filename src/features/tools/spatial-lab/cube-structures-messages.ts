...
const zh = {
  ...
  cutHint: "靠近棱线选线，移入面内选面，自动识别。两条不同的共面线，或一个可见层界面，可以确定截面；锁定后点击切割。可用视图按钮，或切到观察旋转后继续，已选线会保留。",
  cutGeometryHint: "选面使用点击面所在的平面。外边界无法分开两侧时，请用内部层界上的两条线。悬浮面只高亮小面；0% 填充面允许穿透，可见棱边仍可选。",
...
const en = {
  ...
  cutHint: "Edges and faces are detected automatically: move near an edge to pick a line, or inside a face to pick it. Two distinct coplanar lines or one exposed layer face define a section. Lock it, then click Cut. Use view buttons or Observe to rotate; selected lines are retained.",
  cutGeometryHint: "Face selection uses the clicked face's actual plane. If it is an outer boundary, use two lines on an interior layer boundary. Face hover highlights only that small face. 0% fills allow picking through; visible edges remain selectable.",
