...
const zh = {
  ...
  move: "移动单位块", moveHint: "观察状态下直接拖动相连结构或已编组部分，拖动空白旋转视角。移动工具可单选单位块；多选后操作选中范围。起拖方向选择轴，XYZ 箭头可指定轴。开启磁铁实时对齐；关闭时连续预览，松手落在整格（拆开展示为半格）。",
...
const en = {
  ...
  move: "Move cubes", moveHint: "In Observe, drag a connected structure or group; drag empty space to orbit. Use Move to pick a single cube, or multi-select a range. Initial drag direction chooses an axis; XYZ arrows specify one. The magnet snaps live; otherwise preview continuously and release to whole units (half units for display separation).",
