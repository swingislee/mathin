...
const zh = {
  ...
  move: "移动单位块", moveHint: "直接拖结构沿 XZ 平面移动，拖空白旋转视角。精确移动时可单选单位块，XYZ 箭头沿轴移动，轴间小方片在对应平面移动；多选后操作选中范围。再次点击精确移动或关闭面板收起手柄。开启磁铁实时对齐，关闭时连续预览、松手落格。",
...
const en = {
  ...
  move: "Move cubes", moveHint: "Drag a structure across XZ; drag blank space to orbit. Precise move lets you select individual cubes or a range, then use arrows for one axis or squares for a plane. Tap Precise move again or close the panel to hide handles. The magnet snaps live; otherwise release to the grid.",
