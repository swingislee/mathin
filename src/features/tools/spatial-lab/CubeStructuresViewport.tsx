...
import { cubeDisplayPosition, cubePaintGroups, type CubeStructureState } from "./cube-structures-contract";
...
export function CubeStructuresViewport({ scene, sceneKey, opacityPreview, moveInteraction, cutInteraction, onMovingChange, renderSceneOverlay, ...props }: VoxelModelCanvasProps & {
  ...
  return <VoxelModelCanvas {...props} model={model} paintedFaceGroups={paints} readOnly={props.readOnly || moving}
    preserveSelectedColors sceneOverlay={<>{renderSceneOverlay?.(presentation)}<CubeStructuresScene {...scene} state={presentation} tool={moving ? "orbit" : scene.tool} />
